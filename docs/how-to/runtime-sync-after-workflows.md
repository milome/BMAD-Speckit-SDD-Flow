...
> 说明：本页的 `runtime sync` 只负责 context / registry / run bootstrap。审计通过后的 scoring 写入、auditIndex 更新与其它 post-audit close-out，当前统一由 invoking host/runner 通过 `runAuditorHost` 承接，而不是理解为分散的手工命令。
...
7. **实施后审计（post-audit）**  
   ...
   说明：这里的 `ensure-run-runtime-context` 只负责为 post-audit 建立和持久化 run context；真正的 post-audit 自动收口仍由 `runAuditorHost` 承接。
