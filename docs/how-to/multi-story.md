...
> **Current path**: 多个 Story 共存时，消费用户仍通过 `$bmad-speckit`、`/bmad-speckit`、`bmad-speckit` 或宿主等价 skill 入口激活主控；主 Agent 内部执行 `inspect / dispatch-plan`，并从 `requirement-record.json`、`currentMentalModel` 和六个心智模型链路决定下一步。子代理只执行 `bounded packet`，`runAuditorHost` 仅负责 post-audit close-out 证据收口。
...
BMAD 的多 Story 支持，重点不是“同时开几个 state 文件”，而是让每个 Story 都走同一条受控 RequirementRecord 和六个心智模型链路：

1. 选择要继续的 Story
2. 主 Agent 内部执行或等价消费 `inspect`
3. 只有受控记录要求 materialize packet 时才内部执行或等价消费 `dispatch-plan`
4. 子代理消费 `bounded packet`
5. 审计通过后由 `runAuditorHost` 做 close-out 证据收口
6. close-out 完成后，主 Agent 再次回读受控 RequirementRecord、当前 hash、当前 attempt 和六个心智模型状态
...
普通消费用户只需要在当前 AI 宿主会话中使用 `$bmad-speckit`、`/bmad-speckit`、`bmad-speckit` 或 Story skill 入口表达要继续的 Story。`inspect / dispatch-plan` 是主控内部动作；npm / npx CLI 只允许用于安装验证、CI、debug 或 no-skill fallback。

然后按受控记录返回的 packet 生命周期推进：

- 主 Agent claim / dispatch / complete / invalidate packet
- 子代理只执行 `bounded packet`
- 审计通过后，`runAuditorHost` 做 post-audit close-out
- close-out 结束后，主 Agent 重新读取受控 RequirementRecord 和六个心智模型状态

### 多 Story 时的实践建议

- 每次继续某个 Story 前，都重新读取一次受控 RequirementRecord 和六个心智模型状态
...
上面的请求只负责告诉主 Agent“你想继续哪一个 Story”。真正是否可继续、是否要派发 packet、是否需要 close-out，仍以受控 RequirementRecord、当前 hash、当前 attempt 和六个心智模型状态为准。
