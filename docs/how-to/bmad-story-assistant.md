...
`--continue` 在 handoff 满足 `next_action + ready=true` 时允许系统自动推进下一阶段。默认不启用自动续跑。
...
- 未通过审计 = 阶段未完成
- 通过审计后才允许推进 state
- `next_action` 只是推荐，是否自动继续取决于 `auto_continue` 配置
- 默认不隐式连跑
...
2. 默认不要打开 auto-continue，除非明确想让流程根据 handoff 自动串跑
