...
- **Scoring 系统**：`bmad-speckit score`（原 `parse-and-write-score.ts`）和 `bmad-speckit check-score`（原 `check-story-score-written.ts`）是共享基础设施，Claude 适配版必须产出与 Cursor 版格式一致的审计报告，否则 scoring 写入会失败
...
- `docs/explanation/path-conventions.md` 需更新以反映三目录架构
...
- FR17: 审计产出的报告格式可以被 `bmad-speckit score`（原 `parse-and-write-score.ts`）正确解析并写入 scoring 存储
...
- NFR4: Claude 适配版产出的审计报告必须可被 `bmad-speckit score`（原 `parse-and-write-score.ts`）正确解析（与 Cursor 版产出格式一致）
- NFR5: `bmad-speckit check-score`（原 `check-story-score-written.ts`）对 Claude 适配版产出的 scoring 数据验证通过率 = 100%
