...
- **审计后自动化系统**：`runAuditorHost` 是统一宿主入口，内部承接评分写入与 audit index 更新。Claude 适配版必须产出与 Cursor 版格式一致的审计报告，否则宿主无法完成审计后自动动作
...
- FR17: 审计产出的报告格式可以被统一 auditor host runner 正确消费，并完成评分写入与 registry 更新
...
- NFR4: Claude 适配版产出的审计报告必须可被统一 auditor host runner 正确消费（与 Cursor 版产出格式一致）
- NFR5: 统一 auditor host runner 的完成态校验对 Claude 适配版产出的审计后自动动作验证通过率 = 100%
