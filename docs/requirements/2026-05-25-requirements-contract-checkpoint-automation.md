...
7. Mandatory whole-document global consistency gate after checkpoint completion, because checkpoint splitting removes the implicit full-document consistency pass that a single generation step used to provide.
8. Integration with the existing deterministic definition drilldown, previous report filtering, resolution ledger, changed-only mode, and decision packet.
9. Final cleanup that removes temporary requirements contract documents from Git index and places them under `.gitignore` when the final delivery strategy requires it.
...
```text
_bmad-output/runtime/requirement-records/<recordId>/authoring/semantic-checkpoint-progress.json
```
...
The pre-render gate must also run the AI-TDD contract completeness check as the unified manifest standard for contract execution coverage. `ContractExecutionManifest` is the only accepted machine-readable authority for `FAIL-*` / `EDGE-*`, `currentTargetMap`, command target, trace closure, canonical surface, legacy denial, closeout proof, and evidence trust coverage at this stage; renderer-local checklists, reverse-audit-local checklists, or broad acceptance rows cannot replace it.

Pre-render must fail closed when any `FAIL-*` or `EDGE-*` lacks explicit NEG binding, EVD binding, TRACE coverage, ACC or E2E mapping, or human-readable view coverage. A generic acceptance row that covers a broad `MUST-*` or `NEG-*` set does not satisfy error-case coverage unless it names the concrete failure or edge ID through `failurePathRefs[]` or `edgeCaseRefs[]`.
...
The workflow must block HTML render until checkpoint completion, whole-document global consistency, and definition checks pass.

Acceptance:

1. HTML render is blocked when required checkpoints are incomplete.
2. HTML render is blocked when the source lacks a parseable `implementationConfirmation` block.
3. HTML render is blocked when any required core array is missing, any ID is duplicated, any `MUST` or `NEG` lacks reciprocal `traceRows` coverage, any trace row references missing evidence, any evidence or trace command reference is undefined, or any failure/edge/view reference is unresolved.
4. HTML render is blocked when `pre_render_definition_drilldown.js` reports unresolved blockers.
5. HTML render may proceed only when checkpoint completion, global consistency, and definition drilldown pass, or definition blockers are explicitly resolved, waived with current hashes, or converted to explicit open questions or out-of-scope boundaries.

### FR-006A Global Consistency Gate

The checkpoint runner must not treat completion of all checkpoint logs as equivalent to source readiness.
...
1. The gate writes `pre-render-global-consistency-report.json`.
2. The progress record records `validation.globalConsistency` as `pass` or `fail`.
3. `run` and `resume` return `pre_render_ready` only when this gate passes.
4. A document with 18 `traceRows[]` items still fails when those trace rows reference missing evidence or undefined commands.
5. A checkpoint execution log, checkpoint count, or trace row count is never accepted as proof of global trace/evidence consistency.
...
### FR-009 Reverse Audit Stage Separation

Reverse audit must no longer expose a generic `PASS` that can be reused across confirmation, readiness, delivery, and closeout stages.

Required stages:

1. `contract_authoring_audit`: verifies the generated source document is written to final product-grade contract standards.
2. `contract_confirmability_audit`: verifies the confirmation HTML/report can be shown to the user for scope confirmation.
3. `implementation_readiness_audit`: verifies the confirmed contract is specific enough to guide implementation.
4. `delivery_verification_audit`: verifies the actual implementation satisfies the confirmed contract with current evidence.
5. `closeout_integrity_gate`: verifies closeout may be recorded only from a current delivery verification result.

Acceptance:

1. Each audit output includes `stage`, stage-specific `verdict`, `allowedNextActions`, and `blockedNextActions`.
2. A `contract_confirmable` result is explicitly marked `notValidForCloseout: true`.
3. `deliveryReadiness.ready=false` may be reported during confirmation, but it must block `implementation_readiness`, `delivery_verification`, and `closeout` unless the relevant later stage proves readiness.
4. Any caller that attempts to use a generic `PASS` for closeout must fail closed.

### FR-010 Product-Grade Evidence And Trace Authoring

Generated EVD/TRACE matrices must be authored to final product-grade standards even before implementation exists.

Acceptance:

1. Every `MUST`, `NEG`, and relevant `OUT` is linked to `TRACE`, `EVD`, `CMD`, artifact, failure path, edge case, and view coverage.
2. Every `EVD` declares a real gate command, independent oracle, target control flow, state/file/event assertions, negative assertion expectation, and current-attempt binding requirement.
3. Every `TRACE` separates `contractValidationCommandRefs[]` from `deliveryEvidenceCommandRefs[]`; legacy `commandRefs[]` alone is invalid.
4. Smoke-only evidence such as exit code, stdout, HTTP 200, page render, or mock call assertions cannot satisfy acceptance evidence.
5. Generated matrices may declare planned proof only. They must not mark delivery proof as passed before delivery verification audit writes current evidence.

### FR-011 Current Target Map Strictness

When current/target comparison is applicable, the source document must describe the current implementation state and target product state with enough precision for reverse audit and delivery verification audit.

If `implementationConfirmation.applicability.currentTargetMap.applies=true`, the current/target comparison is a first-class HTML confirmation hard gate. The confirmation page must not be `confirmable` unless the detailed current/target section is rendered and the render report proves non-zero structured coverage.

Acceptance:

1. The map includes current and target canonical fields, schemas, reducers, writer registries, events, scripts, tests, evidence artifacts, and user-visible behavior.
2. The map explicitly lists legacy surfaces that must not remain authoritative after implementation.
3. The map binds each migration assertion to `MUST`, `NEG`, `EVD`, `TRACE`, and command IDs.
4. The map distinguishes consumer/business behavior from governance/control mechanics.
5. The map is audited as planned proof during contract authoring and as executed proof during delivery verification.
6. When `currentTargetMap.applies=true`, `requiredViewPacks[]` must include `currentTargetMap`; optional view packs are not enough for confirmation.
7. When `currentTargetMap.applies=true`, `currentTargetMap.schemaVersion` must be `current-target-map/v1` and `currentTargetMap.displayProfile` must be `closed_loop_current_target_map`.
8. When `currentTargetMap.applies=true`, HTML confirmation is blocked if `currentTargetCoverage.currentSummary < 1`, `targetSummary < 1`, `diffRows < 3`, `process < 1`, or `artifactPaths < 1`.
9. When governance, runtime, scoring, scripts, hooks, delivery verification, or closeout semantics apply, HTML confirmation is also blocked if `currentTargetCoverage.canonicalArtifacts < 1`.
10. The pre-render gate must fail before HTML render for the same missing view pack, missing display profile, or insufficient coverage conditions.
11. Acceptance tests must reproduce the regression where `currentTargetMap.applies=true` and source rows exist, but the view pack/display profile is missing and the renderer previously produced a confirmable page without a current/target comparison.

### FR-012 Delivery Verification And Closeout Fail-Closed Semantics

Delivery completion and closeout must be proven by a delivery verification audit, not by contract confirmability or readiness pass results.

Acceptance:

1. `delivery_verification_audit` fails when `deliveryReadiness.ready !== true`.
2. `delivery_verification_audit` fails when `currentPassTraceRows < totalTraceRows`.
3. `delivery_verification_audit` fails when `missingEvidenceCount > 0`.
4. `delivery_verification_audit` fails when any required command points to a missing test/script file.
5. `delivery_verification_audit` fails when tests do not exercise the target control flow declared by the contract.
6. `delivery_verification_audit` fails when canonical fields, schemas, reducers, writer registries, events, tests, or closeout evidence do not match the confirmed contract.
7. `closeout_integrity_gate` accepts only `delivery_verification=verified`; all other PASS-like results are non-closeout evidence.

### FR-013 Command Target Collection Gate

Every required command must prove that the files and targets it claims to execute actually exist and were collected by the runner before its exit code can be used as evidence.

Acceptance:

1. A command that names a missing test file, script file, config file, or evidence input fails before execution evidence is accepted.
2. A test runner command must emit or be accompanied by a collected-target manifest that lists the exact test files collected for the current run.
3. A glob that expands to zero files is a blocker even if the command process exits with code 0.
4. A mixed test command cannot pass a requirement when any explicitly named target file is missing or uncollected.
5. `exitCode=0` is non-evidence until target existence, target collection, current command run identity, and artifact binding are verified.

### FR-014 Trace Closure Integrity Gate

Each `traceRows[]` closure must be tied to target-state assertions, not only to command success or evidence file existence.

Acceptance:

1. Every trace row that can become `current_pass` declares closure assertions for target schema fields, reducer behavior, writer registry ownership, event types, state transitions, test files, artifacts, negative legacy behavior, and current-attempt binding.
2. A trace row cannot be closed by a runtime packet, completion packet, summary file, or implementation-authored report unless an independent gate verifies the declared target assertions.
3. Trace closure fails when any declared target assertion is missing, stale, uncollected, or proven only by smoke-level evidence.
4. Trace closure fails when evidence proves a different control flow than the target control flow declared by the confirmed contract.

### FR-015 Canonical Surface Reconciliation Gate

Delivery verification must reconcile the confirmed target state against actual implementation surfaces before it can verify delivery.

Acceptance:

1. The gate verifies target canonical fields exist in schema or equivalent source of truth.
2. The gate verifies reducers or state transition code consume and preserve the target canonical fields.
3. The gate verifies writer registries authorize the intended writer and reject unowned event/control writes.
4. The gate verifies target canonical event types are emitted and consumed as authoritative delivery evidence.
5. The gate verifies tests cover the target schema, reducer, writer registry, event, state, artifact, and negative legacy assertions.

### FR-016 Legacy Substitution Denial Gate

Legacy control flow must never substitute for the target canonical implementation when proving delivery or closeout.

Acceptance:

1. A legacy event cannot satisfy a target canonical event assertion unless the confirmed contract explicitly declares it as the canonical target.
2. A legacy field cannot be upcast, adapted, or summarized into a target field and then treated as target-state evidence unless the target map explicitly allows that migration and verifies it.
3. Legacy reducers, old gate checks, old closeout branches, and diagnostic-only checks cannot write delivery verification or closeout state.
4. If a target event is absent but a legacy event is present, delivery verification fails with a legacy-substitution blocker.

### FR-017 Controlled `record_closed` Write

`record_closed` is a controlled state change and may be written only after closeout integrity accepts a current delivery verification result.

Acceptance:

1. Implementation runners, tests, summary writers, completion packet writers, and delivery evidence writers cannot directly write `record_closed`.
2. The `record_closed` payload includes current source document hash, implementationConfirmation hash, delivery verification report hash, closeout integrity report hash, requirement record hash, current attempt id, verified trace row IDs, command run refs, and evidence artifact hashes.
3. `record_closed` write fails when any hash, trace row, command run ref, artifact hash, or attempt id is missing or stale.
4. The write receipt is append-only, requirement-scoped, and idempotent for the same verified delivery and closeout attempt.

### FR-018 Evidence Trust State Model

Evidence must move through explicit trust states so planned proof cannot masquerade as executed proof.

Acceptance:

1. Evidence starts as `planned` when authored in the source document.
2. Command output can move evidence to `observed` only when the command target collection gate passes.
3. Independent assertion gates can move evidence to `assertion_validated` only when target-state assertions match the confirmed contract.
4. Delivery verification can move evidence to `delivery_verified` only when current hashes, current command runs, artifacts, trace closure, canonical surfaces, and closeout attempt binding all match.
5. Closeout consumes only `delivery_verified` evidence; `planned`, `observed`, and `assertion_validated` are non-closeout evidence.

### FR-018A Target Modification Paths Confirmation Surface

The confirmation HTML must render a first-class target modification path list before users confirm scope.

Acceptance:

1. `implementationConfirmation.targetModificationPaths[]` lists every expected code, script, hook, test, config, schema, source document, or runtime artifact path the agent may modify, add, or generate after confirmation.
2. Each target modification row declares `id`, `path`, `changeType`, `intent`, `ownerModel`, `requirementRefs`, `traceRefs`, `evidenceRefs`, `artifactRefs`, and `requiresReconfirmationOnChange`.
3. The HTML renderer shows a dedicated `目标修改路径清单` section and includes the same rows in `confirmation-render-report.json`.
4. When `scriptsAndHooks.applies=true`, `currentTargetMap.applies=true`, `governanceEvents.applies=true`, runtime recovery applies, or implementation/code artifacts exist, confirmability is blocked unless explicit `targetModificationPaths[]` rows are present.
5. Rows derived from `artifactAutomationPlan[]` are display hints only; they cannot satisfy the explicit target modification path requirement.
6. Any change outside the confirmed target modification paths requires reconfirmation or architecture confirmation before implementation continues.
7. When reverse audit is split into stage-specific CLIs, `targetModificationPaths[]` must explicitly list the new audit CLIs, legacy wrapper, shared audit library, skill reference docs, stage-boundary tests, downstream readiness/closeout gates, schema/control-store updates, and stage report artifacts before the user confirms scope.

### FR-019 Semantic Kernel First Large-Document Authoring

Large requirements must use semantic-kernel-first generation so checkpointing remains a persistence and recovery mechanism, not a fragmented reasoning mechanism.

Acceptance:

1. Scale assessment returns one of `single_pass`, `kernel_then_checkpoint`, or `kernel_then_checkpoint_with_amendment`.
2. Large documents default to `kernel_then_checkpoint` unless the source is small enough for `single_pass`.
3. Before checkpoint materialization starts, the workflow creates `semantic-kernel.json` with global goal, non-goals, business requirements, governance requirements, ID registries, current/target summary, open questions, blocker risks, and applicability of delivery/closeout gates.
4. `semantic-kernel.json` is intentionally compact and optimized for global reasoning completeness, not for final prose length.
5. Checkpoints may only materialize semantic kernel content into the source document; they must not perform independent requirement discovery unless an amendment path is triggered.

### FR-020 Kernel Audit, ID Freeze, And Drift Control

The semantic kernel must be audited and core IDs must be frozen before checkpoints write long-form source content.

Acceptance:

1. The kernel completeness gate verifies source requirement coverage, business/governance separation, `MUST/NEG/OUT/EVD/TRACE/CMD/ART` registries, open questions, and blocker assumptions.
2. The ID registry freeze gate prevents checkpoint steps from renaming, deleting, or reinterpreting frozen core IDs.
3. Each checkpoint runs kernel drift checks against the current source document.
4. Drift checks fail when a checkpoint introduces new core semantics not present in the kernel, omits kernel IDs that should be materialized, changes frozen ID meaning, converts planned proof to executed proof, or confuses confirmability with delivery readiness.

### FR-021 Full Contract Reconciliation And Kernel Amendment

After checkpoint materialization, the workflow must prove the final source document is a complete and non-drifting projection of the semantic kernel before HTML render.

Acceptance:

1. Full-contract reconciliation compares human-readable prose, `implementationConfirmation`, Mermaid views, tables, reports, `EVD/TRACE/CMD/ART`, and requirement boundaries against the semantic kernel.
2. HTML render is blocked when the source document contains semantics not present in the kernel or omits required kernel semantics.
3. If checkpoint execution discovers a real requirement definition issue, the workflow must create a kernel amendment, re-audit the kernel, re-materialize impacted checkpoints, and rerun global reconciliation.
4. Directly patching long-form prose to change core requirements without a kernel amendment is a blocker.
5. Critical delivery, closeout, `record_closed`, canonical runtime governance, and legacy retirement gates are expanded only when the semantic kernel marks them applicable.

### FR-022 Single-Responsibility Reverse Audit CLIs

Reverse audit must be split into stage-specific CLI entry points so one generic script cannot expose a cross-stage `PASS`.

Acceptance:

1. `audit_contract_confirmability.js` owns only confirmation render authority, render report shape, `blockingIssues`, `confirmability`, hash freshness, confirmation phrase, and confirmation-page readiness.
2. `audit_implementation_readiness.js` owns only confirmed-source readiness for implementation, including controlled confirmation record, blocking open questions, current/target sufficiency, planned evidence sufficiency, and explicit `notValidForCloseout: true`.
3. `audit_delivery_verification.js` owns only actual implementation verification, including deliveryReadiness, current-pass trace closure, command target collection, target control-flow coverage, canonical surface reconciliation, evidence freshness, and legacy substitution denial.
4. `audit_closeout_integrity.js` owns only terminal closeout acceptance, consumes only current `delivery_verification=verified`, rejects all non-verification verdicts, and controls `record_closed`.
5. `pre_render_definition_drilldown.js` remains the only pre-render definition-only audit gate; `reverse_audit_contract.js --definition-only` remains a compatibility alias only.
6. Shared validation standards may live in a library, but stage verdicts and CLI exit semantics must remain single-responsibility.

### FR-023 Reverse Audit Compatibility Wrapper Deprecation

The existing `reverse_audit_contract.js` may remain as a compatibility wrapper, but it must not be a generic global audit authority.

Acceptance:

1. Legacy wrapper output includes `deprecatedGenericPass: true` whenever it emits legacy `PASS`.
2. Legacy wrapper output includes `notValidForCloseout: true` unless it delegates to `audit_delivery_verification.js` and receives `stage=delivery_verification` with `verdict=verified`.
3. `reverse_audit_contract.js --mode implementation` delegates to confirmability/readiness-compatible behavior and cannot produce closeout-valid evidence.
4. `reverse_audit_contract.js --mode readiness` delegates to implementation readiness behavior and cannot produce closeout-valid evidence.
5. Callers that attempt to feed legacy wrapper `PASS`, `contract_confirmable`, or `ready_for_implementation` into closeout fail closed.

### FR-024 Stage Boundary Regression Tests

Strict regression tests must lock every audit stage boundary so future standardization work cannot collapse stage semantics again.

Acceptance:

1. Contract confirmability pass cannot satisfy implementation readiness, delivery verification, or closeout.
2. Implementation readiness pass cannot satisfy delivery verification or closeout.
3. `deliveryReadiness.ready=false` fails delivery verification and closeout.
4. Missing required test files or uncollected command targets fail delivery verification even when a mixed test command exits 0.
5. Incomplete current-pass trace closure, `missingEvidenceCount > 0`, stale evidence, or stale attempt binding fails delivery verification.
6. Legacy events, fields, reducers, gate checks, closeout branches, or diagnostic-only checks cannot satisfy target canonical assertions.
7. Completion packets, summary files, and implementation-authored reports cannot close traces or write `record_closed` without independent delivery verification and closeout integrity.
8. Generic `PASS` from any wrapper or legacy script is rejected by closeout integrity.

### FR-025 AI-TDD ContractExecutionManifest As Unified Gate Schema

Implementation readiness, delivery verification, and closeout gates must use AI-TDD `ContractExecutionManifest` as the shared schema for contract completeness decisions. Stage-specific gates may add runtime evidence checks, but they must not redefine contract completeness with parallel local checklists.

Acceptance:

1. `ContractExecutionManifest` is mandatory in the AI-TDD gate report and is consumed by implementation readiness, delivery verification, closeout integrity, HTML render report stage checks, and reverse audit stage checks.
2. `ContractExecutionManifest.currentTargetMap` is mandatory and must expose the source-driven current/target comparison sections, not a renderer-local hidden fallback.
3. Every `FAIL-*` row has explicit `linkedNegIds[]`, `linkedEvidenceIds[]`, trace coverage, view coverage, and at least one ACC or E2E row that names the failure through `failurePathRefs[]`.
4. Every `EDGE-*` row has explicit `linkedFailurePathIds[]` or direct NEG linkage, `linkedEvidenceIds[]`, trace coverage, view coverage, and at least one ACC or E2E row that names the edge through `edgeCaseRefs[]`.
5. Broad acceptance rows, smoke commands, generic MUST/NEG coverage, renderer-only visibility, or reverse-audit-only assertions cannot substitute for concrete `FAIL-*` / `EDGE-*` references.
6. Missing error-case mappings set `contractCompletenessReport.ready=false`, append a blocking issue with the missing ID and missing dimension, and block implementation readiness and closeout.
7. `ContractExecutionManifest.commandTargetCollection` is mandatory and lists each required command, named target files, TRACE bindings, and EVD bindings; missing target files or missing bindings block readiness and closeout before any command exit code can count.
8. `ContractExecutionManifest.traceClosureAssertions` is mandatory and lists each trace row's requirement, evidence, command, acceptance, and artifact bindings; incomplete trace closure blocks readiness and closeout.
9. `ContractExecutionManifest.canonicalSurfaceReconciliation` is mandatory and lists target canonical schema, reducer, writer, event, script, test, artifact, or record surfaces derived from the confirmed current/target map; missing canonical surfaces or missing refs block readiness and closeout.
10. `ContractExecutionManifest.legacyDenial` is mandatory and lists legacy events, fields, reducers, old closeout branches, diagnostic-only checks, and old artifacts that cannot satisfy target proof; missing denial oracle or refs block readiness and closeout.
11. `ContractExecutionManifest.closeoutProof` is mandatory and lists required commands, target refs, current-attempt policy, orphan policy, and record_closed policy; missing closeout proof policy blocks readiness and closeout.
12. `ContractExecutionManifest.evidenceTrustStates` is mandatory and declares planned -> observed -> assertion_validated -> delivery_verified promotion, with closeout consuming only delivery_verified evidence.
13. `contractCompletenessReport.ready=false` is the single contract-completeness blocker consumed by implementation readiness and closeout. Readiness/closeout gates must first require the AI-TDD gate report to be present, current, and complete; then they may add stage-specific runtime evidence checks.
14. Existing user-visible confirmation content must not shrink when adopting the AI-TDD manifest. `targetModificationPaths`, current/target map, failure/edge views, command targets, trace closure assertions, canonical surfaces, legacy denial, closeout proof, and evidence trust states remain visible confirmation surfaces when applicable.
...
6. Splitting a single generation pass into semantic checkpoints can create false positives unless an explicit whole-document consistency gate revalidates all ID, trace, evidence, command, failure, edge, and view references after checkpoint completion.
7. Reusing generic reverse-audit `PASS` across stages can turn `contract_confirmable` into a false closeout approval.
8. Low-standard generated EVD/TRACE matrices can hide delivery gaps until after implementation and encourage half-finished acceptance.
9. Current/target maps that omit canonical fields, schemas, reducers, events, or tests can let delivery evidence prove the wrong control flow.
10. Treating planned proof as executed proof can produce false positive delivery and closeout claims.
11. Accepting `exitCode=0` without collected-target verification can hide missing test files and unexecuted target coverage.
12. Allowing runtime packets or completion packets to close traces can let implementation self-certify without independent target-state proof.
13. Allowing legacy events, fields, reducers, or closeout branches to satisfy target assertions can make old behavior appear compliant.
14. Allowing `record_closed` outside closeout integrity can turn evidence packaging into a false terminal state.
15. Allowing checkpoints to continue doing requirement discovery can fragment global reasoning and produce locally complete but semantically inconsistent sections.
16. Allowing checkpoints to add or reinterpret IDs outside a frozen kernel can reintroduce ID drift and false completeness.
17. Applying critical delivery or closeout governance to every large document can pollute authoring documents and reduce human confirmability.
18. Keeping all reverse-audit stages under one CLI can make external runners consume the wrong stage verdict.
19. Preserving a legacy generic `PASS` without deprecation metadata can reintroduce false closeout.
20. Standardizing audit checks without stage-boundary regression tests can silently recreate the previous functional regression.
21. Treating `FAIL-*` and `EDGE-*` rows as descriptive text instead of first-class acceptance surfaces can hide missing negative assertions and produce false positive readiness.
22. Letting renderer, reverse audit, and readiness gates each define their own error-case checklist can drift the standard and make one stage pass while another would fail.
...
5. Pre-render automation blocks HTML render until checkpoint completion, whole-document global consistency, and definition drilldown are clean or explicitly resolved.
6. Final cleanup flow is documented and tested.
7. Reverse audit outputs are stage-specific and cannot be reused as generic closeout PASS.
8. Generated EVD/TRACE matrices satisfy final product-grade proof-plan standards while preserving planned-vs-executed proof separation.
9. Current/target maps, when applicable, are strict enough to audit canonical fields, schemas, reducers, events, scripts, tests, and legacy surfaces.
10. Delivery verification and closeout integrity gates fail closed on delivery_not_ready, missing evidence, incomplete trace closure, missing tests, stale hashes, or wrong control-flow coverage.
11. Required commands cannot contribute evidence until named files exist and runner collection manifests prove the target files were collected.
12. Trace rows cannot become `current_pass` without target-state closure assertions verified by an independent gate.
13. Delivery verification reconciles canonical schema, reducer, writer registry, event, tests, artifacts, and negative legacy assertions.
14. Legacy control flow cannot substitute for target canonical control flow.
15. `record_closed` is written only by closeout integrity after current `delivery_verification=verified`.
16. Evidence trust states distinguish `planned`, `observed`, `assertion_validated`, and `delivery_verified`, and closeout consumes only `delivery_verified`.
17. Large-document authoring creates and audits `semantic-kernel.json` before checkpoint materialization.
18. Core IDs are frozen after kernel audit, and checkpoint output cannot drift from the kernel.
19. Final source documents pass full-contract reconciliation against the semantic kernel before HTML render.
20. Requirement changes discovered during checkpointing use kernel amendment, not direct long-form prose mutation.
21. Reverse audit has stage-specific CLI entry points for confirmability, implementation readiness, delivery verification, and closeout integrity.
22. `reverse_audit_contract.js` is retained only as a compatibility wrapper with deprecated generic PASS metadata and non-closeout semantics.
23. Regression tests prove confirmability/readiness/generic PASS cannot satisfy delivery verification or closeout.
24. HTML confirmation renders a first-class `targetModificationPaths[]` section so users can confirm the planned modification surface before implementation.
25. AI-TDD `ContractExecutionManifest` exposes first-class `errorCaseCoverage`, `currentTargetMap`, `commandTargetCollection`, `traceClosureAssertions`, `canonicalSurfaceReconciliation`, `legacyDenial`, `closeoutProof`, and `evidenceTrustStates`.
26. Implementation readiness, delivery verification, renderer stage checks, reverse-audit stage checks, and closeout consume `contractCompletenessReport.ready` from the AI-TDD gate report before evaluating their own stage-specific runtime evidence.
27. Renderer, reverse audit, implementation readiness, delivery verification, and closeout do not maintain independent contract-completeness standards for error cases, command targets, trace closure, canonical surfaces, legacy denial, closeout proof, or evidence trust states.
...
sequenceDiagram
  ...
  Runner->>Runner: Run whole-document global consistency gate [MUST-007][NEG-004]
  Runner->>Drilldown: Run deterministic pre-render drilldown [MUST-005][EVD-005]
  alt checkpoint, global consistency, or definition blockers remain [NEG-001][NEG-004]
    Drilldown-->>Runner: Decision packet [NEG-001][EVD-005]
    Runner-->>Renderer: Do not render [NEG-001][NEG-004]
  else no blockers [MUST-005][MUST-007]
    Runner->>Renderer: Render confirmation HTML [MUST-005][MUST-007]
  end
```

### Reverse Audit Stage Separation

This remediation separates document confirmability from implementation readiness, delivery verification, and closeout. The core rule is that each stage emits a stage-specific verdict and the next allowed action; no caller may consume a generic `PASS`.

```mermaid
sequenceDiagram
  participant Authoring as Contract Authoring Audit
  participant Confirm as Contract Confirmability Audit
  participant Ready as Implementation Readiness Audit
  participant Verify as Delivery Verification Audit
  participant Closeout as Closeout Integrity Gate
  Authoring->>Confirm: product-grade proof plan only [MUST-009][EVD-009]
  Confirm-->>Ready: contract_confirmable, notValidForCloseout=true [MUST-008][NEG-005]
  Ready-->>Verify: ready_for_implementation only, not closeout [MUST-008]
  Verify-->>Closeout: delivery_verification=verified only after current evidence [MUST-011]
  Closeout-->>Closeout: reject generic PASS, readiness PASS, and planned proof [NEG-005][NEG-006]
```

### Product-Grade Proof Plan And Current-Target Verification

The generated EVD/TRACE matrix is required to be strict before implementation starts, but it remains planned proof. Only delivery verification may promote rows to `current_pass`, and only when current implementation surfaces match the confirmed current/target map.

```mermaid
flowchart TD
  A[Author final-standard EVD and TRACE plan MUST-009] --> B[Audit proof plan: no smoke-only, no missing oracle EVD-009]
  B --> C[Map current and target canonical surfaces MUST-010]
  C --> D[Run delivery verification against real code, tests, events, artifacts MUST-011]
  D -->|deliveryReadiness=false or missing evidence| E[Block delivery verification and closeout EVD-011]
  D -->|all trace rows current_pass and evidence current MUST-011| F[delivery_verification=verified]
  F --> G[Closeout integrity accepts only delivery_verified current attempt MUST-011]
```

### Command Target Collection And Trace Closure

Command output is not trusted until target files exist and runner collection proves the intended tests ran. Trace rows are not trusted until independent closure assertions prove target-state behavior, not just command success.

```mermaid
sequenceDiagram
  participant Command as Required Command
  participant Collector as Command Target Collection Gate
  participant Trace as Trace Closure Integrity Gate
  participant Verify as Delivery Verification Audit
  Command->>Collector: command, named files, globs, commandRunRef [MUST-012]
  alt missing or uncollected target [NEG-007][EVD-012]
    Collector-->>Verify: block evidence binding [NEG-007][EVD-012]
  else collected target manifest present [MUST-012]
    Collector->>Trace: observed command output with target manifest [MUST-012][EVD-012][ART-012]
    Trace->>Trace: verify closureAssertions [MUST-013]
    Trace-->>Verify: verified trace closure only when target-state assertions match [EVD-013][MUST-013]
  end
```

### Canonical Surface And Legacy Denial

Delivery verification must compare the confirmed target map against actual implementation surfaces. Legacy fields, events, reducers, old closeout branches, and diagnostic-only checks are blockers when they substitute for target canonical behavior.

```mermaid
flowchart TD
  A[Confirmed currentTargetMap MUST-010] --> B[Canonical surface reconciliation MUST-014]
  B -->|canonical match required MUST-014| C{schema, reducer, writer, event, test, artifact match?}
  C -->|no| D[Block delivery verification EVD-014]
  C -->|yes MUST-014| E[Legacy substitution denial MUST-015]
  E -->|legacy denial required MUST-015| F{legacy surface used as target proof?}
  F -->|yes| G[Block as legacy substitution NEG-009]
  F -->|no NEG-009| H[Allow delivery verification to evaluate current evidence MUST-011]
```

### Controlled record_closed And Evidence Trust States

`record_closed` is not a packaging event. It is a terminal controlled write that can occur only after closeout integrity consumes current delivery_verified evidence.

```mermaid
stateDiagram-v2
  [*] --> planned: source-authored EVD/TRACE [MUST-017]
  planned --> observed: command target collection passes [MUST-012]
  observed --> assertion_validated: trace closure and canonical surface gates pass [MUST-013][MUST-014]
  assertion_validated --> delivery_verified: delivery_verification=verified for current attempt [MUST-011]
  delivery_verified --> record_closed: closeout_integrity_gate writes controlled event [MUST-016]
  planned --> blocked: closeout attempted too early [NEG-006]
  observed --> blocked: closeout attempted before verification [NEG-006]
  assertion_validated --> blocked: closeout attempted before delivery verification [NEG-006]
```

### Semantic Kernel First Materialization

Large documents must perform one compact global semantic pass before checkpoint materialization. Checkpoints persist and expand the kernel; they do not become local requirement discovery agents.

```mermaid
sequenceDiagram
  participant Assessor
  participant Kernel as Semantic Kernel
  participant Freeze as Kernel Audit and ID Freeze
  participant Checkpoint
  participant Reconcile as Full Contract Reconciliation
  Assessor->>Kernel: route kernel_then_checkpoint [MUST-018]
  Kernel->>Freeze: semantic-kernel.json with global registries [EVD-018]
  Freeze-->>Checkpoint: frozen ID registry and kernel hash [MUST-019]
  Checkpoint->>Checkpoint: materialize kernel section only [NEG-011][MUST-019]
  Checkpoint->>Freeze: kernel drift check per checkpoint [EVD-019]
  Freeze-->>Reconcile: all checkpoints materialized without drift [EVD-019]
  Reconcile-->>Reconcile: compare prose, YAML, Mermaid, EVD, TRACE, CMD, ART [MUST-020]
  Reconcile-->>Renderer: allow HTML render only after PASS [EVD-020]
```

### Kernel Amendment Path

If a real requirement issue is found during checkpointing, the workflow changes the kernel first, not the long-form prose.

```mermaid
flowchart TD
  A[Checkpoint discovers definition issue EDGE-012] -->|amendment control NEG-012| B{Core requirement change?}
  B -->|no NEG-012| C[Correct materialization and rerun drift gate EVD-019]
  B -->|yes NEG-012| D[Create kernel amendment NEG-012]
  D --> E[Re-audit kernel and ID registry MUST-019]
  E --> F[Re-materialize impacted checkpoints MUST-020]
  F --> G[Run full-contract reconciliation EVD-020]
  G --> H[HTML render may proceed only after PASS MUST-020]
```

### Single-Responsibility Reverse Audit CLI Split

The productized audit interface is multiple stage-specific commands, not one global reverse-audit command with a reusable `PASS`.

```mermaid
sequenceDiagram
  actor Runner
  participant Confirm as audit_contract_confirmability.js
  participant Ready as audit_implementation_readiness.js
  participant Verify as audit_delivery_verification.js
  participant Close as audit_closeout_integrity.js
  participant Wrapper as reverse_audit_contract.js
  Runner->>Confirm: confirmability request [MUST-021]
  Confirm-->>Runner: stage=contract_confirmability, notValidForCloseout=true [EVD-021]
  Runner->>Ready: implementation readiness request [MUST-021]
  Ready-->>Runner: stage=implementation_readiness, notValidForCloseout=true [EVD-021]
  Runner->>Verify: delivery verification request with current evidence [MUST-021]
  Verify-->>Runner: stage=delivery_verification, verdict=verified|not_verified [EVD-021]
  Runner->>Close: closeout request with delivery verification report [MUST-021]
  Close-->>Runner: stage=closeout_integrity, verdict=closeout_accepted|blocked [EVD-021]
  Runner->>Wrapper: legacy wrapper request [MUST-022]
  Wrapper-->>Runner: deprecatedGenericPass and notValidForCloseout unless delegated delivery verification [EVD-022]
```

### Stage-Boundary Regression Gate

Regression tests must lock the lifecycle boundaries so future attempts to unify audit standards cannot collapse stage semantics again.

```mermaid
flowchart TD
  A[Audit result enters downstream gate MUST-023] --> B{stage-specific verdict? MUST-021}
  B -->|no generic PASS NEG-013| C[Reject NEG-013]
  B -->|confirmability or readiness MUST-021| D[Allow only declared next action MUST-021]
  D --> E{closeout requested? NEG-014}
  E -->|yes NEG-014| F[Reject notValidForCloseout NEG-014]
  E -->|no MUST-021| G[Continue within declared stage MUST-021]
  B -->|delivery_verification MUST-021| H{delivery_verification=verified and current evidence? EVD-023}
  H -->|no EVD-023| I[Block delivery or closeout EVD-023]
  H -->|yes MUST-023| J[Closeout integrity may evaluate MUST-023]
  J --> K{closeout_accepted? MUST-023}
  K -->|yes MUST-023| L[Controlled record_closed may be written later MUST-016]
  K -->|no EVD-023| M[Keep requirement open EVD-023]
```

### Target Modification Paths Confirmation Gate

The confirmation page must show the planned modification surface before the user confirms scope.
...
sequenceDiagram
  ...
  participant User
  Source->>Renderer: targetModificationPaths[] [MUST-024][EVD-024]
  Renderer-->>User: Target modification path section [MUST-024][EVD-024][TRACE-024]
  alt explicit targetModificationPaths missing [MUST-024][EVD-024][EDGE-015]
    Renderer-->>User: confirmability=blocked target_modification_paths_missing [MUST-024][EVD-024][FAIL-015]
  else explicit paths present [MUST-024]
    Renderer-->>User: show paths, refs, artifact links, reconfirmation rule [EVD-024]
  end
```

### AI-TDD Error-Case Coverage Manifest Gate

Every failure path and edge case is a first-class acceptance surface. The gate does not accept "covered by the general happy path" as proof; it requires the exact `FAIL-*` or `EDGE-*` ID to be named in NEG, EVD, TRACE, ACC or E2E, and view coverage.

```mermaid
flowchart TD
  A[Source implementationConfirmation FAIL and EDGE rows MUST-025] --> B[AI-TDD ContractExecutionManifest.errorCaseCoverage EVD-025]
  B --> C{Every FAIL has NEG, EVD, TRACE, ACC/E2E, view coverage? MUST-025}
  C -->|no FAIL-016| D[contractCompletenessReport.ready=false NEG-015]
  C -->|yes MUST-025| E{Every EDGE has failure/NEG linkage, EVD, TRACE, ACC/E2E, view coverage? MUST-025}
  E -->|no EDGE-016 NEG-015 EVD-025| D
  E -->|yes EVD-025| F[Implementation readiness may evaluate next stage TRACE-025]
  D --> G[Block readiness, delivery verification, closeout, and HTML confirmability if rendered as complete EVD-025]
```
...
Artifacts are requirement-scoped and classified as assessment, plan, receipt, progress, decision packet, cleanup plan, stage decision report, proof-matrix audit report, current-target strictness report, delivery verification report, command target collection report, trace closure integrity report, canonical surface reconciliation report, legacy substitution denial report, record-closed write receipt, evidence trust state report, semantic kernel, kernel audit/drift report, full-contract reconciliation report, stage CLI capability report, wrapper compatibility report, or stage-boundary regression report. `ART-008` through `ART-023` are control-affecting evidence and must fail closed when stale, malformed, self-certified, drifting, wrapper-derived without deprecation metadata, or used outside their declared stage.

### traceRows Findings

Trace rows are pending until implementation evidence exists. `TRACE-008` through `TRACE-023` explicitly cover stage separation, proof-plan strictness, current/target mapping, delivery verification, command target collection, trace closure integrity, canonical surface reconciliation, legacy substitution denial, controlled `record_closed`, evidence trust states, semantic kernel first authoring, kernel drift control, full reconciliation, kernel amendment, stage-specific audit CLIs, wrapper deprecation, and stage-boundary regression tests. They are not allowed to become `current_pass` from planned authoring evidence, command exit code, runtime packet, completion packet, checkpoint-local semantic invention, generic reverse-audit `PASS`, or compatibility wrapper output.

### Row Quality Findings

Evidence commands are declared as planned validation commands. Product-grade rows require split contract validation and delivery evidence commands, independent oracles, target control-flow assertions, negative assertion expectations, artifact references, and current-attempt binding.

### AI-TDD ContractExecutionManifest Findings

Contract completeness is audited through AI-TDD `ContractExecutionManifest`, not through independent renderer, reverse-audit, readiness, or closeout checklists. The manifest must expose first-class `errorCaseCoverage`, `currentTargetMap`, `commandTargetCollection`, `traceClosureAssertions`, `canonicalSurfaceReconciliation`, `legacyDenial`, `closeoutProof`, and `evidenceTrustStates`. Missing mappings are blockers for implementation readiness, delivery verification, and closeout; broad MUST/NEG coverage, smoke-level acceptance, command exit code, renderer visibility, or stage-local assertions cannot substitute for the AI-TDD manifest standard.

### E2E Anti-Smoke Findings

Acceptance evidence requires fixture assertions and independent oracle checks, not smoke-only proof. Exit code success, stdout success, HTTP 200, page render, mock calls, checkpoint count, and trace row count are explicitly non-delivery evidence unless a delivery verification report binds them to current command runs, target control flow, artifacts, hashes, and attempt id.

### Stage Separation Findings

The prior regression came from mixing contract confirmability, implementation readiness, delivery verification, and closeout semantics under a generic reverse-audit `PASS`. This document now requires stage-specific outputs with `stage`, `verdict`, `allowedNextActions`, `blockedNextActions`, current hashes, and `notValidForCloseout` when a result is not valid for closeout.

### Delivery Verification Findings

Delivery and closeout cannot be inferred from HTML confirmability, renderer success, readiness success, or planned proof. `delivery_verification_audit` must fail on `deliveryReadiness.ready=false`, incomplete current-pass trace closure, missing evidence, missing required files, wrong control-flow coverage, stale hashes, canonical surface mismatch, or closeout attempt mismatch. `closeout_integrity_gate` accepts only current `delivery_verification=verified`.

### Command Target Collection Findings

Required command outputs are not evidence until the command target collection gate proves every named target exists, every glob expands to at least one target, the runner collected the target files, and the commandRunRef belongs to the current attempt. A mixed test command cannot hide a missing target test behind unrelated passing tests.

### Trace Closure Integrity Findings

Trace closure is no longer a statement that a packet may self-report. Each current-pass trace row requires target-state closure assertions and an independent report proving schema fields, reducers, writer registries, events, state transitions, tests, artifacts, negative legacy behavior, and current-attempt binding.

### Canonical Surface And Legacy Findings

Delivery verification must reconcile actual implementation surfaces with confirmed canonical target surfaces. Legacy events, fields, reducers, old gate checks, old closeout branches, and diagnostic-only checks are explicitly non-authoritative unless the confirmed currentTargetMap declares and verifies them as the target migration.

### record_closed Findings

`record_closed` can be written only by closeout integrity after current `delivery_verification=verified`. The event payload must bind current source hash, implementationConfirmation hash, delivery verification report hash, closeout report hash, requirement record hash, attempt id, verified trace row IDs, command run refs, and evidence artifact hashes.

### Evidence Trust Findings

Evidence trust states are `planned`, `observed`, `assertion_validated`, and `delivery_verified`. Closeout consumes only `delivery_verified`; planned proof, observed command output, and assertion_validated-but-not-delivery_verified evidence remain non-closeout evidence.

### Semantic Kernel Findings

Large-document checkpointing is a materialization and recovery mechanism, not a reasoning strategy. `semantic-kernel.json` is the compact global semantic source; checkpoints may only expand kernel content and must pass kernel drift checks. Core requirement changes after freeze require kernel amendment, re-audit, impacted checkpoint re-materialization, and full reconciliation.

### Single-Responsibility Audit CLI Findings

The target audit interface is `audit_contract_confirmability.js`, `audit_implementation_readiness.js`, `audit_delivery_verification.js`, and `audit_closeout_integrity.js`. Each entry point must own one lifecycle verdict vocabulary and one exit semantic. Shared validation logic may exist underneath, but a stage CLI must not emit a generic cross-stage `PASS`.

### Compatibility Wrapper Findings

`reverse_audit_contract.js` is retained only for compatibility. Wrapper output is non-closeout evidence by default and must include `deprecatedGenericPass` when legacy `PASS` is emitted. It must include `notValidForCloseout` unless it delegates to delivery verification and receives a current `stage=delivery_verification`, `verdict=verified` result.

### Stage-Boundary Regression Findings

Regression coverage must prove that contract confirmability, implementation readiness, generic `PASS`, not-ready delivery, missing command targets, incomplete trace closure, missing evidence, legacy substitution, and implementation self-certification cannot satisfy delivery verification or closeout. Closeout consumes only current delivery_verified evidence and then emits a separate closeout integrity verdict.
...
Validated scope: `sequenceViews`, `flowViews`, `edgeCaseViews`, and `boundaryViews` are present and preserve business/governance separation for the renderer, including command target, trace closure, canonical surface, legacy denial, record_closed, evidence trust, semantic kernel, kernel drift, reconciliation, and amendment views.

### cp-06-artifacts-commands-closeout

Status: passed

Validated scope: `artifactAutomationPlan`, `requiredCommands`, `suggestedCommands`, and `closeoutReadinessPreview` are present before render, including command target collection, trace closure integrity, canonical surface reconciliation, legacy substitution denial, controlled record_closed, evidence trust, semantic kernel, kernel drift, full reconciliation, and amendment gates.

### cp-07-conditional-modules

Status: passed

Validated scope: conditional module declarations are present for scripts, hooks, and current-target mapping, including current and target surfaces for command collection, trace closure, canonical reconciliation, legacy denial, record_closed, evidence trust, semantic kernel, kernel drift, and reconciliation; non-applicable governance/runtime/scoring domains remain explicitly bounded.

### cp-08-human-readable-views-dod-reverse-audit

Status: passed

Validated scope: human-readable Mermaid views, evidence overview, E2E overview, Definition of Done, reverse-audit stage separation, delivery verification semantics, command target collection, trace closure integrity, canonical surface reconciliation, legacy denial, controlled record_closed, evidence trust states, semantic kernel first materialization, kernel amendment, and the draft reverse audit report are present. This completes checkpoint execution up to the pre-render gate.
