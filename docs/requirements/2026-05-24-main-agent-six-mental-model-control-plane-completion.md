...
Audit basis: current code, schema, control-store, main-agent orchestration entry points, and acceptance tests.
Overall status: `partially_implemented_control_plane_foundation`

当前实现不能声明为本需求完成。现有代码已经实现若干局部 lane、foundation control-store 能力、readiness remediation、compiled prompt dispatch、post-close intake 和 closeout/confirmation 相关门禁，但尚未实现完整 `currentMentalModel + sixModelResults + controlled blocker intake + reconfirmation rollback + canonical model result events` 驱动的六心智模型控制面。

### 19.1 已实现或大部分实现的局部能力

| Capability | Status | Boundary |
|---|---|---|
| `requirement_confirmation.pre_confirmation_drilldown` lane | implemented | 只属于需求确认模型增强 lane，不能证明六模型控制面完成。 |
| `implementation_readiness.readiness_auto_remediation` lane | implemented_partial | 能处理 readiness blocker 自动修复，但尚未写入目标态 `implementation_readiness_result_recorded`。 |
| compiled prompt dispatch integration | implemented_partial | 能接入 req-trace compiler、execution strategy、profile 和 SDD artifact manifest，但这是实施调度扩展，不是模型推进权威。 |
| append control event and replay foundation | implemented_partial | `appendControlEventAndReplay` 存在，但目标态所有 canonical control writes 尚未统一迁移到该语义。 |
| canonical foundation fields | implemented_partial | Schema/reducer 已覆盖 `executionStrategySelections`、`requirementClosures`、`gateChecks`、`contractChecks`、`failureRecords`、`rcaRecords`、`rerunLoops`、`readinessBaselineMetadata`、`artifactIndex`、`deliveryEvidence`、`hookReconciliation`、`aiTddContractGate`、`eventChainHead` 和 `controlStore`，但这些 foundation 字段不能替代六模型目标态字段。 |
| post-close defect intake | implemented_partial | 已有 `post-close-defect-intake` action 和分类逻辑，但状态术语、carrier event 和六模型边界仍需收紧。 |
| delivery closeout gate foundation | implemented_partial | pass 路径能写 `record_closed`，但 fail/blocked 路径仍会写目标态禁止的 `closeout_recorded`。 |

### 19.2 MUST 实现状态

| MUST | Current status | Required update |
|---|---|---|
| MUST-001 | partial | Inspect 能解析 active record 和 next action，但必须改为以 `currentMentalModel`、当前模型结果和 `sixModelResults` 顺序作为编排依据。 |
| MUST-002 | not_implemented | 必须新增 canonical `currentMentalModel` 写入、`mentalModelTransitions` 持久化和 before/after hash 记录。 |
| MUST-003 | not_implemented | 必须实现 `main-agent-mental-model-gate` 并写入六个模型的 `sixModelResults`。 |
| MUST-004 | partial | 现有 adaptive intake 和 diagnostics 不能替代 canonical `controlled-blocker-intake` 与 `NormalizedBlockerSignal`。 |
| MUST-005 | partial | 局部门禁能阻断，但必须在模型迁移和 closeout 前检查 pending/blocking blocker intake。 |
| MUST-006 | partial | 现有 confirmation drift route 不能替代 `reconfirmationRequests` 和 `mental_model_rollback_recorded`。 |
| MUST-007 | conflict | `architecture_confirmation_state_checked` 仍在 schema、control-store、implementation 和 tests 中出现，必须迁出 canonical authority。 |
| MUST-008 | partial_with_conflict | Readiness gate 和 auto remediation 已存在，但仍写入 `implementation_readiness_check_recorded` 与 `readiness_baseline_activation_requested` 等目标态冲突语义，必须落到目标态 `implementation_readiness_result_recorded`。 |
| MUST-009 | partial | Execution packet、iteration 和 evidence 已存在，但必须新增 `execution_closure_result_recorded`。 |
| MUST-010 | partial | Audit/scoring convergence 已存在，但必须新增 `audit_review_result_recorded` 并同步 unresolved findings 到 blocker intake。 |
| MUST-011 | conflict | Failed/blocked delivery closeout 仍可写 `closeout_recorded`，必须改为 `delivery_confirmation_result_recorded` 加 blocker records。 |
| MUST-012 | partial | Inspect 输出已较完整，但必须显示由 canonical six model result 派生的 current model status 和 summary。 |
| MUST-013 | implemented_partial_with_conflict | Post-close defect intake 已有主体实现，但 tests/host surface 仍把 `delivery_closeout` 当作 `currentMentalModel` 值；必须迁移为 `delivery_confirmation` 并补受控 carrier/link 事件。 |
| MUST-014 | not_implemented | 必须新增 canonical `bmadAssociation`、association decision replay、external board outbox/receipt evidence-only 边界。 |
| MUST-015 | partial_with_schema_gap | Schema、reducer 和 normalizer 已覆盖多项 foundation 字段，但尚未覆盖 `currentMentalModel`、`mentalModelTransitions`、`sixModelResults`、`pendingBlockerIntake`、`blockerIntakeRuns`、`reconfirmationRequests`、`bmadAssociation`、`sprintStatusUpdateAuthorizations` 和 `externalBoardSyncReceipts` 等目标态 canonical 字段；`readinessBaselineMetadata.status` schema 也尚未接受目标态 `not_established`。 |
| MUST-016 | partial | append/replay 基础存在，但普通控制状态写入尚未全部统一为 `appendControlEventAndReplay`。 |

### 19.3 当前目标态冲突

以下冲突必须先清理，否则后续实现会在旧语义上继续堆叠：

1. `architecture_confirmation_state_checked` 仍作为 schema/control-store/test 中的目标态事件出现；目标态只能保留诊断 receipt，不得写 canonical control event、`lastEventType` 或 replay authority。
2. `implementation_readiness_check_recorded` 仍作为 readiness gate 写入事件或 `lastEventType` 出现；目标态应由 `implementation_readiness_result_recorded` 表达。
3. `readiness_baseline_activation_requested` 仍作为 schema 字段和 readiness gate 写入语义出现；目标态应由 `implementation_readiness_result_recorded` payload 与 `readinessBaselineMetadata` 表达。
4. `readinessBaselineActivation.status=audit_required` 仍作为 main-agent 自动审计触发状态出现；目标态必须把它迁出 baseline status，降级为 evidence/outbox/diagnostic workflow state。
5. `closeout_recorded` 仍作为 failed/blocked closeout 事件出现；目标态 failed/blocked closeout 必须写 `delivery_confirmation_result_recorded`，必要时追加 `controlled_blocker_recorded`。
6. `delivery_closeout` 仍作为 `currentMentalModel` fixture 值或 host/agent output option 出现；目标态 `currentMentalModel` 只能使用 `delivery_confirmation`，`delivery_closeout` 只能保留为 gate/stage/report 命名或 legacy migration source。
7. 上述 legacy 语义还存在于 tests、renderer fixture、confirmation rendering/ingest surface、strict closeout proof gate tests、requirements skill contract、host/agent surfaces 和 migration/upcaster fixture 候选中；P0 清理必须同步更新这些 surface，否则测试契约仍会把 legacy event 或 legacy model value 当作目标态。

### 19.4 MUST-008 冲突消解方案

`implementation_readiness_check_recorded` 和 `readiness_baseline_activation_requested` 的最优修复不是简单改名，而是把 readiness 的 foundation check、baseline metadata 和六模型评估结果分层。

目标态规则如下：

1. `implementation-readiness-report.json` 继续作为 deterministic report 输出。
2. `gate_check_recorded` 可继续作为 foundation check/evidence event。
3. `implementation_readiness_check_recorded` 只能作为 legacy diagnostic input 或 migration source，不得作为 target-state canonical control event、`lastEventType` 或 replay authority。
4. `readiness_baseline_activation_requested` 只能作为 legacy migration source，不得作为 target-state requested event、`lastEventType` 或 replay authority。
5. `implementation_readiness_result_recorded` 是唯一能表达 `implementation_readiness` 模型评估结果的 target-state control event。
6. Baseline activation 不再是 request event；baseline 状态必须作为 `implementation_readiness_result_recorded` payload 的一部分，并 replay 到 `readinessBaselineMetadata`。
7. `readinessBaselineMetadata.status` 的目标态枚举固定为 `current | stale | blocked | not_established`，不得写入其它值。
8. `audit_required` 和 `audit_requested` 只能作为 external audit workflow 的 evidence/outbox/diagnostic 状态写入 outbox、artifact index 或 audit evidence receipt，不得写入 `readinessBaselineMetadata.status`，不得驱动 `sixModelResults.implementation_readiness`，也不得作为 mental model progression 条件。
9. Legacy `readinessBaselineActivation.status` 可作为 migration source 被读取，但不得作为目标态 baseline status、model result status、`lastEventType` 或 replay authority。

`implementation_readiness_result_recorded` payload 必须至少包含：

1. `payloadKind: model_result`
2. `model: implementation_readiness`
3. `recordId`
4. `requirementSetId`
5. `sourceDocumentHash`
6. `implementationConfirmationHash`
7. `status: pass | blocked | fail | stale`
8. `resultRecordedAt`
9. `resultRecordedBy`
10. `blockingReasons`
11. `sourceRefs`
12. `currentHashes`
13. `readinessReportRef`
14. `readinessBaselineMetadata` when baseline status is `current`, `stale`, `blocked`, or `not_established`; newly established baseline must use `status: current` plus `establishedAt` and `baselineCreatedByEventId`

Reducer 规则如下：

1. Replaying `implementation_readiness_result_recorded` updates `sixModelResults.implementation_readiness`.
2. Replaying `implementation_readiness_result_recorded` updates `readinessBaselineMetadata` only from the event payload.
3. Replaying `gate_check_recorded` alone never updates `currentMentalModel`, `sixModelResults`, `sixModelResults.implementation_readiness`, `readinessBaselineMetadata`, `record_closed`, or model progression.
4. Replaying legacy `implementation_readiness_check_recorded` or `readiness_baseline_activation_requested` must upcast to diagnostic artifact or `implementation_readiness_result_recorded`; legacy events must not remain replay authority.

Required implementation sequence:

1. Add failing tests proving target-state readiness does not write `implementation_readiness_check_recorded` or `readiness_baseline_activation_requested`.
2. Add a negative reducer test proving `gate_check_recorded` alone cannot write `sixModelResults.implementation_readiness`.
3. Add schema definitions for `implementation_readiness_result_recorded`, `model_result` payload, `sixModelResults.implementation_readiness`, and target-state `readinessBaselineMetadata`.
4. Update control-store reducer to replay `implementation_readiness_result_recorded`.
5. Update `scripts/main-agent-implementation-readiness-gate.ts` to write `implementation_readiness_result_recorded` after evaluating readiness.
6. Remove target-state use of `readinessBaselineActivationEventType`.
7. Add migration/upcaster coverage for historical `implementation_readiness_check_recorded` and `readiness_baseline_activation_requested`.
8. Update renderer, confirmation, skill surface, readiness, schema, closeout, and target-artifact fixtures that still treat the legacy events as target-state pass paths.

MUST-008 remains `partial_with_conflict` until all rules in this subsection pass.

### 19.5 当前文档更新决策

本节只记录现有实现审计状态，不降低 `MUST-*`、`AC-*`、`TRACE-*` 或 rollout gate 标准。任何实现完成声明必须以本节状态清零为前提，并且必须通过第 24 节的 P0/P1/P2 分层门禁。

### 19.6 Six-model target-state implementation checklist

本清单是第 24 节 rollout gate 的实施级入口。实施时必须按文件逐项修改、补测试并回查，不得只改文档状态。

#### MUST-013 `delivery_closeout` currentMentalModel 冲突

目标：`delivery_closeout` 只能作为 closeout gate、stage、report、metric 或 legacy migration source 名称保留；不得作为 `currentMentalModel` 值、host 心智模型枚举、RequirementRecord example 或 target-state test fixture。

必须修改：

1. `tests/acceptance/main-agent-post-close-defect-intake.test.ts`
   - Replace `currentMentalModel: 'delivery_closeout'` fixtures with `currentMentalModel: 'delivery_confirmation'`.
   - Keep closeout attempt ids, closeout reports, and `delivery_closeout_blocked` failure type unchanged unless the value is used as `currentMentalModel`.
   - Add or update assertions proving post-close defect intake keeps the original closed record at `currentMentalModel='delivery_confirmation'`.

2. `.claude/agents/bmad-master.md`
   - Replace the output option line `requirement_confirmation | architecture_confirmation | implementation_readiness | execution_closure | audit_review | delivery_closeout` with `requirement_confirmation | architecture_confirmation | implementation_readiness | execution_closure | audit_review | delivery_confirmation`.
   - Preserve references where `delivery_closeout` means gate/stage/report only, and label them explicitly as non-mental-model operational stage names.

3. `_bmad/claude/agents/bmad-master.md`
   - Apply the same host surface correction as `.claude/agents/bmad-master.md`.
   - Ensure packaged Claude surface and local Claude surface do not diverge.

4. Host and skill generated surfaces under `.codex/`, `.cursor/`, `.claude/`, and `_bmad/*/skills/**`
   - Search for `currentMentalModel` lines or “当前心智模型” menus that list `delivery_closeout`.
   - Replace only mental-model enumerations with `delivery_confirmation`.
   - Do not rename `delivery_closeout_gate`, `delivery_closeout_report`, `delivery_closeout_passed`, `delivery_closeout_blocked`, or metrics such as `delivery_closeout_gate_latency`.

5. `scripts/target-artifact-realization-gate.ts`
   - Keep `record_closed` as lifecycle terminal shortcut only.
   - Do not treat `record.lastEventType === 'record_closed'` as a substitute for canonical `delivery_confirmation_result_recorded` in six-model completion semantics.
   - Add or update assertions that target artifact realization compatibility logic is terminal-lifecycle aware but not six-model authority.

6. `tests/acceptance/render-requirements-confirmation-html.test.ts`
   - If `ownerModel: delivery_closeout` is intended to mean six-model owner, replace it with `ownerModel: delivery_confirmation`.
   - If it is intentionally a closeout gate/stage owner, make the fixture explicit with a non-mental-model field such as `ownerStage` or add an assertion that it is not treated as `currentMentalModel`.

7. Migration/upcaster fixtures
   - Add a fixture proving historical records with `currentMentalModel='delivery_closeout'` are upcast to `currentMentalModel='delivery_confirmation'` or preserved only as diagnostic legacy source.
   - Add a negative replay assertion that legacy `delivery_closeout` cannot remain replay authority for target-state `currentMentalModel`.

Required verification:

1. `rg -n -e 'currentMentalModel.*delivery_closeout|delivery_closeout.*currentMentalModel|当前心智模型:.*delivery_closeout' -- tests scripts _bmad .codex .cursor .claude`
   must return no target-state usage.
2. `rg -n -F 'delivery_closeout' -- tests scripts _bmad .codex .cursor .claude`
   may still return gate/stage/report/metric names, but each remaining occurrence must be classified as non-mental-model usage.
3. `npx vitest run tests/acceptance/main-agent-post-close-defect-intake.test.ts`
   must pass.
4. `npx vitest run tests/acceptance/target-artifact-realization-gate.test.ts`
   must pass with delivery confirmation compatibility assertions.

#### MUST-015 `readinessBaselineMetadata.status` schema gap

目标：target-state schema、normalizer、reducer 和 tests must accept `readinessBaselineMetadata.status=current|stale|blocked|not_established`, and must reject `audit_required` and `audit_requested` for `readinessBaselineMetadata.status`.

必须修改：

1. `_bmad/_schemas/requirement-record.schema.json`
   - Update `readinessBaselineMetadata.properties.status.enum` from `["current", "stale", "blocked"]` to `["current", "stale", "blocked", "not_established"]`.
   - Do not add `audit_required` or `audit_requested` to `readinessBaselineMetadata.status`.
   - Keep `readinessBaselineActivation.status` as legacy/evidence input only until P0 migration removes it from target-state authority.

2. `scripts/requirement-record-control-store.ts`
   - Ensure canonicalization preserves `readinessBaselineMetadata.status='not_established'`.
   - Ensure no reducer path maps `readinessBaselineActivation.status='audit_required'` or `audit_requested` into `readinessBaselineMetadata.status`.

3. `scripts/main-agent-implementation-readiness-gate.ts`
   - Stop creating target-state baseline authority through `readinessBaselineActivationEventType`.
   - Emit or prepare `implementation_readiness_result_recorded` payload with `readinessBaselineMetadata.status='not_established'` when readiness is not baseline-established.

4. `scripts/controlled-readiness-audit-bridge.ts`
   - Treat `readiness_audit_requested`, `readiness_audit_completed`, `readiness_scoring_record_written`, and `readiness_baseline_current_recorded` as readiness audit evidence workflow events, not six-model result authority.
   - Ensure completed audit may produce `readinessBaselineMetadata.status='current'`, but audit request states never become `readinessBaselineMetadata.status`.

5. `scripts/main-agent-orchestration.ts`
   - Replace run-loop or diagnostic routing that keys directly on `readinessBaselineActivation.status='audit_required'` with a compatibility branch that is explicitly marked legacy/evidence-only.
   - Prefer target-state routing from canonical `implementation_readiness_result_recorded` plus `readinessBaselineMetadata.status`, with `not_established` treated as “baseline missing / not yet materialized”.
   - Ensure task report evidence and diagnostic category names do not imply that `readinessBaselineActivation.status` is target-state authority.

6. `tests/acceptance/requirement-record-schema.test.ts`
   - Add positive schema test for `readinessBaselineMetadata.status='not_established'`.
   - Add negative schema tests rejecting `readinessBaselineMetadata.status='audit_required'` and `readinessBaselineMetadata.status='audit_requested'`.

7. `tests/acceptance/main-agent-orchestration-consumer.test.ts`
   - Add or update tests proving readiness audit request/completion evidence does not write `sixModelResults.implementation_readiness`.
   - Add or update tests proving `readinessBaselineActivation.status='audit_required'` can trigger diagnostic remediation but cannot satisfy `implementation_readiness`.
   - Add or update tests proving `readinessBaselineMetadata.status='not_established'` is recognized as target-state “baseline missing”, not coerced back to `audit_required`.

Required verification:

1. `npx vitest run tests/acceptance/requirement-record-schema.test.ts`
   must pass.
2. `npx vitest run tests/acceptance/main-agent-orchestration-consumer.test.ts`
   must pass for readiness baseline scenarios.
3. `rg -n -e 'readinessBaselineMetadata.*audit_required|readinessBaselineMetadata.*audit_requested' -- scripts tests _bmad/_schemas`
   must return no target-state schema, reducer, or positive fixture usage.
4. `rg -n -e 'activationStatus === ''audit_required''|repairable_readiness_audit_required|trigger_controlled_readiness_audit' -- scripts/main-agent-orchestration.ts tests/acceptance/main-agent-orchestration-consumer.test.ts`
   must only show legacy/evidence compatibility handling, not target-state readiness authority.
...
52. Negative reducer test proves `gate_check_recorded` alone cannot write `sixModelResults.implementation_readiness`.
53. BMAD-internal `reconfirmation_requested` 必须有 requestId、targetModel、blocking、sourceRefs、requestedAt、requestedBy 和 closure condition，并且只写 `reconfirmationRequests`。
54. 外部 outbox requested 事件 `association_external_publish_requested` 和 `external_board_patch_back_requested` 必须有 idempotency key、externalSyncKey、payloadRef、receipt condition 和 failure handling，但只能写 `artifactIndex` 或 projection/outbox evidence；`externalBoardSyncReceipts` 只能由 receipt/validation 类 evidence event 追加，且不得成为 BMAD 控制权威字段。
55. 没有 durable lifecycle 的 requested 动作标记必须被拒绝为 canonical control event，并迁移为 `*_result_recorded` payload、metadata、diagnostic artifact 或 fact-like `*_recorded`。
56. `reconfirmation_requested` 不得直接写 `currentMentalModel` 或 `mentalModelTransitions`。
57. reconfirmation、blocker 或 drift 导致的确认模型回退必须写 `mental_model_rollback_recorded`，且不要求 current model pass。
58. 普通前进迁移仍只能由 `mental_model_transition_recorded` 表达，并必须要求 current model pass。
59. `sprint_status_update_authorization_recorded` 必须 replay 到 `sprintStatusUpdateAuthorizations`，并保留 current、consumed、superseded 或 revoked 授权历史。
60. event-only、artifactIndex-only、`bmadAssociation.sprintStatusUpdateCandidate=true` 或 non-current `sprintStatusUpdateAuthorizations` 不得更新 `_bmad-output/implementation-artifacts/sprint-status.yaml`。
61. encoding integrity scan pass。
...
### 24.1 P0 legacy event conflict cleanup gate

P0 目标是先清理目标态冲突的 legacy event，不允许在旧控制语义上继续实现六模型控制面。P0 必须满足以下条件：

1. `architecture_confirmation_state_checked` 不再作为 target-state canonical control event 写入 schema、control-store reducer、`events/control-events.jsonl`、`lastEventType` 或 replay authority。
2. `architecture-state-check` 只能产出 diagnostic report 或 receipt；stale、mismatch 或 rejected 必须转为 `controlled_blocker_recorded` 或 `reconfirmation_requested`。
3. `implementation_readiness_check_recorded` 不再作为 target-state canonical control event 写入 schema、control-store reducer、`events/control-events.jsonl`、`lastEventType` 或 replay authority。
4. `readiness_baseline_activation_requested` 不再作为 target-state requested event 写入 schema、control-store reducer、`events/control-events.jsonl`、`lastEventType` 或 replay authority。
5. `readinessBaselineActivation.status=audit_required|audit_requested` 不得作为 target-state baseline status、model result status、`lastEventType` 或 replay authority；审计触发只能表达为 evidence/outbox/diagnostic workflow state。
6. Readiness baseline activation 必须由 `implementation_readiness_result_recorded` payload 和 `readinessBaselineMetadata` 表达。
7. Failed 或 blocked delivery closeout 不得写 target-state `closeout_recorded`；必须写 `delivery_confirmation_result_recorded`，并在需要阻断时写 `controlled_blocker_recorded`。
8. `delivery_closeout` 不得作为 target-state `currentMentalModel` 值写入 RequirementRecord、test fixture、host surface 或 agent output；目标态必须使用 `delivery_confirmation`。
9. Legacy `architecture_confirmation_state_checked`、`implementation_readiness_check_recorded`、`readiness_baseline_activation_requested`、`readinessBaselineActivation.status=audit_required|audit_requested`、`delivery_closeout` as currentMentalModel 和 `closeout_recorded` 必须有 migration/upcaster 或 diagnostic artifact 策略，不得继续作为 replay authority。
10. Tests、renderer、skill surface、confirmation surface、strict closeout proof surface、host/agent surface 和 migration/upcaster fixture 必须同步迁移到目标态事件语义；不得保留把 legacy event、legacy baseline status 或 legacy model value 当作 target-state pass path 的断言。
11. P0 通过前，不得声明 `MUST-007`、`MUST-008`、`MUST-011` 或 `MUST-013` 完成。

### 24.2 P1 canonical schema, reducer, and writer gate

P1 目标是补齐 canonical RequirementRecord 控制字段和事件写入路径。P1 必须满足以下条件：

1. RequirementRecord schema 接受并约束 `currentMentalModel`、`mentalModelTransitions`、`sixModelResults`、`pendingBlockerIntake`、`reconfirmationRequests`、`blockerIntakeRuns`、`bmadAssociation`、`sprintStatusUpdateAuthorizations` 和 `externalBoardSyncReceipts`。
2. Control-store reducer 保留并 replay `currentMentalModel`、`mentalModelTransitions`、`sixModelResults`、`pendingBlockerIntake`、`reconfirmationRequests`、`blockerIntakeRuns`、`bmadAssociation`、`sprintStatusUpdateAuthorizations` 和 `externalBoardSyncReceipts`。
3. RequirementRecord schema accepts target-state `readinessBaselineMetadata.status=current|stale|blocked|not_established`; `audit_required` and `audit_requested` remain rejected for this field.
4. Writer registry 或等价控制写入入口只能通过 `appendControlEventAndReplay` 写入 canonical control state。
5. `mental_model_transition_recorded` 是普通前进迁移唯一事件，并且必须要求当前模型 result status 为 `pass`。
6. `mental_model_rollback_recorded` 是 reconfirmation、blocker 或 drift 导致的模型回退事件，并且必须要求存在 open blocking reconfirmation 或 blocker。
7. `reconfirmation_requested` 只能写 `reconfirmationRequests`，不得直接写 `currentMentalModel` 或 `mentalModelTransitions`。
8. P1 通过前，不得声明 `MUST-001`、`MUST-002`、`MUST-003`、`MUST-005`、`MUST-006`、`MUST-015` 或 `MUST-016` 完成。

### 24.3 P2 full six-model gate, association, and external board boundary gate

P2 目标是补齐完整模型评估链、BMAD association 和外部看板 evidence-only 边界。P2 必须满足以下条件：

1. `main-agent-mental-model-gate` 输出 requirement_confirmation、architecture_confirmation、implementation_readiness、execution_closure、audit_review 和 delivery_confirmation 六个模型评估结果。
2. `implementation_readiness_result_recorded`、`execution_closure_result_recorded`、`audit_review_result_recorded` 和 `delivery_confirmation_result_recorded` 均写入 canonical model result payload，并包含 `resultRecordedAt`、`resultRecordedBy`、`sourceRefs`、`blockingReasons` 和 current hash refs。
3. Inspect 输出必须从 canonical `currentMentalModel`、当前模型 result 和 `sixModelResults` 派生 current model status、six model summary、blocking source 和 next safe action。
4. `controlled-blocker-intake` 必须覆盖 TaskReport、SubagentEvidenceEnvelope、inspect diagnostics、resolver projection、gate failures、drift results、audit/scoring failures 和 closeout blockers。
5. `bmadAssociation` 必须由 canonical association decision replay 得出，并支持 orphan、linked、adopted_by_story、promoted_to_story 和 native story。
6. `sprintStatusUpdateAuthorizations` 必须成为 `_bmad-output/implementation-artifacts/sprint-status.yaml` 写入的唯一 BMAD authority。
7. GitHub/GitLab outbox、webhook validation、accepted receipt 和 patch-back receipt 必须保持 evidence-only，不得写 `currentMentalModel`、`bmadAssociation`、`record_closed` 或 sprint-status。
8. Compiled prompt dispatch、`model_packet.json`、execution strategy selection、ExecutionDisciplineProfile、audit scoring convergence policy 和 SDD artifact manifest 只能作为 `implementation_readiness` 之后的 dispatch/evidence integration；它们不得写 `currentMentalModel`、不得满足 `sixModelResults`、不得替代 `implementation_readiness_result_recorded`，也不得作为六模型完成证据。
9. P2 通过前，不得声明 `MUST-004`、`MUST-009`、`MUST-010`、`MUST-012`、`MUST-013` 或 `MUST-014` 完成。

### 24.4 Final completion evidence gate

P0、P1 和 P2 全部通过后，仍必须满足以下完整 evidence 清单，才能宣称本需求完成：
...
13. Acceptance tests proving post-close defect fixtures, host/agent surfaces, and RequirementRecord examples use `currentMentalModel=delivery_confirmation`, never `delivery_closeout`.
14. Acceptance tests for BMAD association dedupe, adoption, promotion, orphan sprint-status denial, and multi same-type projection aggregation。
15. Acceptance tests proving GitHub/GitLab external board sync writes outbox/receipts only and cannot affect BMAD control flow。
16. Acceptance tests proving GitHub/GitLab webhook drag events are validated against BMAD control state and illegal transitions are patched back.
17. Acceptance tests proving patch-back receipts are evidence only and cannot mutate RequirementRecord control fields or `_bmad-output/implementation-artifacts/sprint-status.yaml`.
18. Acceptance tests proving target-state implementation readiness writes `implementation_readiness_result_recorded`, not `implementation_readiness_check_recorded`.
19. Acceptance tests proving readiness baseline activation is captured in `implementation_readiness_result_recorded` / `readinessBaselineMetadata`, not target-state `readiness_baseline_activation_requested`.
20. Migration/upcaster tests proving legacy `implementation_readiness_check_recorded` and `readiness_baseline_activation_requested` are removed from replay authority.
21. Schema/reducer compatibility tests proving foundation record events remain accepted and preserved as non-authoritative sub-record inputs.
22. Negative control-flow tests proving foundation record events alone cannot advance `currentMentalModel`, write `sixModelResults`, write `record_closed`, or bypass canonical blocker/model result events.
23. Durable reconfirmation request tests proving `reconfirmation_requested` has requestId, targetModel, blocking state, sourceRefs, requestedAt/requestedBy, closure condition, and writes only `reconfirmationRequests`.
24. External outbox request tests proving `association_external_publish_requested` and `external_board_patch_back_requested` have idempotency key, externalSyncKey, payloadRef, receipt condition, and failure handling while remaining evidence-only.
25. Negative registry tests proving requested-named events without durable lifecycle are rejected or migrated out of canonical replay authority.
26. Tests proving `reconfirmation_requested` cannot directly mutate `currentMentalModel` or `mentalModelTransitions`.
27. Tests proving `mental_model_rollback_recorded` writes rollback transitions without requiring current model pass, but only with an open blocking reconfirmation or blocker.
28. Tests proving `mental_model_transition_recorded` remains the only forward transition event and still requires current model pass.
29. Tests proving `sprint_status_update_authorization_recorded` uses `sprint_status_update_authorization` payload kind and includes target path, target Epic/Story, authorized transition, association decision, delivery confirmation result, scope, author, and before/after hashes.
30. Schema/reducer tests proving `sprint_status_update_authorization_recorded` replays into canonical `sprintStatusUpdateAuthorizations` and preserves current, consumed, superseded, or revoked authorization history.
31. Negative tests proving `bmadAssociation.sprintStatusUpdateCandidate=true` alone cannot update `_bmad-output/implementation-artifacts/sprint-status.yaml`.
32. Negative tests proving event-only, artifactIndex-only, or non-current `sprintStatusUpdateAuthorizations` entries cannot update `_bmad-output/implementation-artifacts/sprint-status.yaml`.
33. Required-command preflight tests proving every named test file exists, is collected by the runner, and appears in command output before an exitCode=0 can satisfy closeout.
34. Reverse-audit delivery readiness tests proving `deliveryReadiness.ready=false`, `missing_controlled_record`, non-current trace rows, or missing evidence force implementation closeout failure.
35. Closeout integrity tests proving an open `closure_integrity_incident` or open RCA blocks `record_closed` and prevents previous completion evidence packets from being reused as seal authority.
36. Independent seal audit tests proving completion packets cannot self-certify seal; they must be checked against schema/reducer/event/test coverage in a separate audit step.
37. One-record six-model routing E2E matrix proving canonical happy path, every-model fail-closed behavior, rerun-loop recovery, semantic-drift reconfirmation return, and post-close defect evidence isolation。
38. Encoding integrity scan pass。
39. No unrelated source changes in implementation path。

The six-model routing E2E matrix must be implemented by `tests/acceptance/six-mental-model-routing-e2e.test.ts` and must pass with this command:

```powershell
npx vitest run tests/acceptance/six-mental-model-routing-e2e.test.ts
```

The final grouped routing evidence command must also pass:

```powershell
npx vitest run tests/acceptance/six-mental-model-routing-e2e.test.ts tests/acceptance/main-agent-mental-model-gate.test.ts tests/acceptance/main-agent-post-close-defect-intake.test.ts
```

## 25. Must Not Count As Completion
