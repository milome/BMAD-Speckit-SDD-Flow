...
Checkpoint: 1 - Header, background, scope, non-goals, and frozen decisions
Checkpoint status: committed draft baseline pending confirmation-contract expansion
...
Checkpoint 2 必须从本边界继续，新增 `implementationConfirmation` core fields 和完整 `applicability.*` 声明。Checkpoint 2 不应重写本 checkpoint 的范围、非目标或冻结决策，除非同步标记为 scope change。

## 3. 背景与当前遗漏

### 3.1 缺少 `currentMentalModel` 的真实读写闭环
...
### 3.3 缺少统一 `controlled-blocker-intake`
...
### 3.5 Architecture Confirmation 没有完全接入 main-agent action switch
...
### 3.7 Execution Closure 与 closeout 之间缺少模型级汇总
...
### 3.8 Dashboard 的 6 模型只是 read model
...
## 6. controlled-blocker-intake 详细机制
...
## 7. main-agent-mental-model-gate
...
## 8. controlled-reconfirmation-router
...
## 9. Architecture Confirmation 接入主控
...
## 21. Must Not Count As Completion
