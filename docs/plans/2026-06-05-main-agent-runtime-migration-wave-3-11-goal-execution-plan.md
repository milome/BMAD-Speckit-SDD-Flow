...
- Every Wave 3.11 registry entry MUST use these exact common field values when G009 writes it: `refinesWaveId: main-agent-runtime-migration-wave-3.10`, `originalPathStatus: retained`, `migrationStatus: validated`, `validationStatus: passed`, `evidenceRefs: ["repo-governance/script-migrations/main-agent-runtime-migration-wave-3.11/registry-evidence.json"]`, `deletionAllowed: false`, and `deletionApprovalRef: null`.
- Registry `status: validated`, entry `migrationStatus: validated`, and entry `validationStatus: passed` mean the registry record has registry-compatible evidence for the thirteen Wave 3.11 entries. They do not by themselves prove G011/G012 final closeout; final closeout remains proven only by `evidence.json`, `install-matrix.json`, `summary.md`, `final-evidence-packet.json`, the final acceptance command row, the final validator command row, and the final human response.
- G009 MUST NOT leave the Wave 3.11 registry wave at `status: in_progress`, and MUST NOT leave any Wave 3.11 entry at `validationStatus: partial`, `migrationStatus: migrated`, or `migrationStatus: caller_switched`; those are execution-phase descriptions and will fail the Wave 3.11 validator.
...
- Set the Wave 3.11 registry wave `status: validated`, and set every Wave 3.11 entry `migrationStatus: validated` and `validationStatus: passed` after registry-compatible evidence is promoted.
...
- The Wave 3.11 registry wave MUST validate as `status: validated`; every Wave 3.11 entry MUST validate as `migrationStatus: validated` and `validationStatus: passed`.
...
- [ ] ACC009: `repo-governance/script-migration-registry.yaml` contains a Wave 3.11 registry wave with exactly thirteen entries, `refinesWaveId: main-agent-runtime-migration-wave-3.10`, wave `status: validated`, entry `migrationStatus: validated`, entry `validationStatus: passed`, `deletionAllowed: false`, and `deletionApprovalRef: null` on every entry; every Wave 3.11 entry references `registry-evidence.json`; `registry-evidence.json` satisfies the legacy registry evidence schema with the same thirteen entries; and `classification-evidence.json` records the same thirteen entries with D010 schema fields.
...
| ACC009 | G009 | `node tools/script-migration/validate-registry.cjs`; `repo-governance/script-migration-registry.yaml`; `repo-governance/script-migrations/main-agent-runtime-migration-wave-3.11/registry-evidence.json`; `repo-governance/script-migrations/main-agent-runtime-migration-wave-3.11/classification-evidence.json` | Command exits `0`, Wave 3.11 has exactly thirteen entries with wave `status: validated`, entry `migrationStatus: validated`, and entry `validationStatus: passed`, every Wave 3.11 entry references registry-compatible `registry-evidence.json`, registry evidence satisfies the legacy validator schema for the same thirteen entries, and classification evidence satisfies D010 schema for the same thirteen entries. |
