...
| **技能路径** | 全局skills：`{SKILLS_ROOT}\`（**GAP-042 修复**：Windows 示例路径；非 Windows 使用 `$HOME/.cursor/skills/` 或环境变量 `CURSOR_SKILLS_PATH`） |
...
**路径解析（GAP-042 修复）**：技能根路径 = `os.environ.get("CURSOR_SKILLS_PATH")` 或 `$HOME/.cursor/skills/`（Linux/macOS）或 `%USERPROFILE%\.cursor\skills\`（Windows）；文档中 `{SKILLS_ROOT}\` 为 Windows 示例，实施时替换为上述解析结果。
