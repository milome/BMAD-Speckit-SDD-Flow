...
若 registry、`activeScope` 或对应 context 缺失，则直接 fail loud；**不允许再回退到** 任何旧共享 context 或已移除的 speckit fallback 输入。
...
- **生成 context file**：维护方仍可通过 `node scripts/write-runtime-context.js <root> <flow> <stage> [templateId] [epicId] [storyId] [storySlug] [runId] [artifactRoot] [contextScope]` 生成一个 runtime context 文件；随后必须通过 `BMAD_RUNTIME_CONTEXT_FILE` 显式传入给 emit / hooks。
