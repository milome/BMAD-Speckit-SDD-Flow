...
> **Current path**: Runtime Governance 输入统一求值，post-audit automation 由 `runAuditorHost` 统一收口
> **Legacy path**: 各 YAML 或脚本直接决定 score / trigger / 后置动作
...
当 `enabled: false` 时，审计通过后即使仍由 `runAuditorHost` 统一收口，也不会执行 scoring 写入，最终表现为 `scoringEnabled=false`。
...
### bmad-help 状态感知推荐（Phase 1）

`bmad-help.csv` 仍然是帮助命令的数据源，但从 Phase 1 开始，`bmad-help` 的语义合同不再只由目录项解释，而要同时遵守状态感知推荐参考：

- [`bmad-help-routing-model.md`](./bmad-help-routing-model.md)

这里需要特别固定三条约束：

1. `sourceMode` 保留 runtime 内部来源语义，不在配置层被重新定义成用户可见成熟度字段。
2. 用户可见成熟度字段是派生出来的 `contextMaturity`，不是 `sourceMode` 的别名直出。
3. `story`、`bugfix`、`standalone_tasks` 对外统一只使用 `implementation-readiness` 作为实施前 gate 名称；`bugfix-readiness`、`tasks-readiness` 等名字不进入外部配置口径。
...
- [bmad-help 路由模型参考](./bmad-help-routing-model.md) — `contextMaturity`、`implementationReadinessStatus`、统一 gate 命名与输出合同
