...
- **正式主控消费面**：消费用户通过 `$bmad-speckit`、`/bmad-speckit` 或 `bmad-speckit` 在当前 AI 宿主会话中激活主控；主 Agent 在决定下一步前，必须读取受控 `requirement-record.json`、`currentMentalModel` 和六个心智模型链路，而不是只读取 handoff 中的 `mainAgentNextAction / mainAgentReady` 或旧 runtime projection。
