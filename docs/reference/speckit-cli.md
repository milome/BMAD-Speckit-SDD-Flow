...
| 命令                                                 | 用途                                                                 | 关键输出                                                                                                                                                 |
| ---------------------------------------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bmad-speckit dashboard --json --output-json <path>` | 生成 dashboard markdown，并落一份 runtime snapshot JSON              | `_bmad-output/dashboard/runtime-dashboard.json`，目标态顶层含 `selection` / `overview` / `resolvedRuntimeContext` / `stage_timeline` / `score_detail` / `sft_summary`；历史 `runtime_context` 字段不得作为控制输入 |
...
| `bmad-speckit dashboard-live`                        | 启动本地 live dashboard web server                                   | stdout 打印 URL；提供 `/health`、`/api/snapshot`、`/api/resolved-runtime-context`、`/api/stage-timeline`、`/api/score-detail`、`/api/sft-summary`                 |
