...
帮助层读取 `implementationReadinessStatus` 时，固定按以下优先级消费证据：

1. 最新 readiness report
2. 最新 remediation artifact
3. execution record / rerun gate 结果
4. deferred gaps tracking
5. runtime context / activeScope
...
| 文档存在，但 `auditor-bugfix` / `auditor-tasks-doc` 未通过或尚未执行         | `blocked`                     |
| 文档审计通过，但 readiness 证据缺失                                          | `missing`                     |
