...
4. 把 `recommended / blocked` 与 `rerouteRequired` 的实现边界输出固定下来

本文件定义的是 **帮助层推荐语义**。若 runtime kernel 已产出统一 `ImplementationEntryGate`，则本文中的帮助层标签退回为 compatibility / presentation 语义，不得再形成第二套放行器。
...
- `blocked`
- 或 `blocked + rerouteRequired=true`
...
6. `standalone_tasks` 命中 `missing_readiness_evidence` 时，优先进入 **auto-remediation + rerun gate loop**：若 authoritative `auditor-tasks-doc` closeout 已存在且缺口仅是 unified gate 所需的 machine-readable evidence，则 host / runtime 必须先补齐该 evidence，再重新计算 `implementation-readiness`，而不是直接把用户卡在实现入口。
...
2. 推荐标签
   - `recommended`
   - `blocked`
   - `rerouteRequired`（兼容字段，用于表达“当前 flow 被阻断且必须升轨”）
3. 推荐路径
4. 阻断说明 / 缺失证据
...
- `blocked + rerouteRequired=true`: 当前 `standalone_tasks` 不得继续作为直接实施路径
...
- `blocked + rerouteRequired=true`: 当前 flow 不允许直接实现，必须先切换到推荐 flow
...
1. `/bmad-help` 不是纯命令目录
2. 它会先识别状态，再给路径
3. 它会把路径标记为 `recommended / blocked`，并在需要升轨时显式给出 `rerouteRequired`
