...
> **Current path**: interactive 主链先读 `main-agent-orchestration inspect`，必要时才 `dispatch-plan`，子代理只执行 `bounded packet`，`runAuditorHost` 只负责 post-audit close-out
> **Legacy path**: 把 handoff summary、`runAuditorHost` 或 `bmad-speckit score` 直接写成上层主控入口
...
当前 interactive runtime 口径：

1. 主 Agent 先读取 `main-agent-orchestration inspect`
2. 只有 surface 需要 materialize packet 时才运行 `dispatch-plan`
3. 子代理只消费 `bounded packet`
4. 审计通过后的 close-out 才交给 `runAuditorHost`
...
- **职责**：定义每个阶段的必须步骤、需求映射、审计闭环、TDD 红绿灯模式；interactive 分支决策仍回到 `main-agent-orchestration`
...
- **职责**：五层架构的 Layer 3-5 编排，Party-Mode 触发，handoff/state 管理；主 Agent 先读 `inspect`，必要时 `dispatch-plan`
...
- **职责**：编排各 stage 的审计流程，并统一通过 `runAuditorHost` 承接审计后的 scoring 写入、auditIndex 更新与其它自动动作；它负责 close-out，不负责选择下一条全局分支
...
`bmad-standalone-tasks` 的“独立”只表示文档作用域独立，不表示它绕过主 Agent。interactive 模式下仍然先读 `main-agent-orchestration inspect`，必要时才 `dispatch-plan`。
