...
export function bundleEntry(relative: string) {
  return { path: relative, rootClass: 'source_bundle', proposedAuthorityClass: 'source_authority',
    bodySchemaVersion: BUNDLE_SCHEMA };
}

export function typedChild(id: string, role: string, text: string, extra: Json = {}) {
  return { sourceRootId: id, rootClass: 'typed_source_node', proposedAuthorityClass: 'source_authority',
    bodySchemaVersion: 'requirements-contract-source-node/v2', semanticBody: {
      schemaVersion: 'requirements-contract-source-node/v2', executionRole: role, text,
      polarity: 'required', normativeStrength: 'must', conditions: [], scope: { kind: 'source_section' },
      declaredIds: [],
      ...extra,
    } };
}
...
export function createSmallBundle(count = 129) {
  ...
  const sourceRoots = Array.from({ length: count }, (_, index) => typedChild(`REQ-CHILD-${index + 1}`,
    index === 0 ? 'action' : 'boundary', `Exact source declaration ${index + 1}.`, {
  ...
  writeJson(root, 'inputs/authority.json', { schemaVersion: BUNDLE_SCHEMA, sourceRoots, sourceRelations: [],
    sourceArtifact: { artifactId: 'raw-source', path: 'inputs/raw-source.md', bytes: raw.length, sha256: hash(raw) } });
...
function clauseRole(block: Json, clause: Json): string {
  ...
  if (['association', 'scope_declaration', 'input_declaration', 'command_declaration'].includes(block.disposition)) return 'binding';
...
function separatePhysical(value: unknown, fieldRef: string, bindings: Json[]): unknown {
  ...
  for (const [key, child] of Object.entries(value)) {
    ...
    if (['source', 'sourceLine', 'lineStart', 'lineEnd', 'byteStart', 'byteEnd'].includes(key) ||
...
export function createFullSourceBundle() {
  const prefix = path.resolve('packages/bmad-speckit/tests/fixtures/standalone-goal/real-source-plan-20260904');
  const raw = readFileSync(`${prefix}.md`);
  const expectedBytes = readFileSync(`${prefix}.expected.json`);
  ...
  for (const [sectionIndex, section] of expected.sections.entries()) {
    ...
    for (const work of section.works) {
      ...
      const child = typedChild(work.id, 'action', heading.text, { declaredIds: [work.id],
        sourceBlockId: heading.id, sourceDisposition: 'work_declaration', scope: { kind: 'work', owner: work.id } }) as Json;
    ...
    for (const block of section.blocks) {
      if (block.definedId) {
        ...
        roots.push(typedChild(block.definedId, /^WORK-/u.test(block.definedId) ? 'action' : 'definition', block.text, {
          declaredIds: [block.definedId], sourceBlockId: block.id, sourceDisposition: block.disposition,
          scope: block.scope,
          ...(/^WORK-/u.test(block.definedId) ? {} : { polarity: 'descriptive', normativeStrength: 'descriptive' }),
        }));
      ...
      for (const clause of block.semantics) {
        ...
        roots.push(typedChild(`SOURCE-${clause.id.replace(':', '-')}`, clauseRole(block, clause), clause.text, {
          sourceClauseId: clause.id, sourceBlockId: block.id, declaredIds: block.definedId ? [block.definedId] : [],
          sourceDisposition: clause.disposition, polarity: clause.polarity,
          normativeStrength: clause.polarity === 'permitted' ? 'may' : clause.polarity === 'descriptive' ? 'descriptive' : 'must',
          conditions: clause.conditions, scope: clause.scope, expectedOutcome: clause.expectedOutcome,
        }));
        roots.at(-1)!.sourceBinding = { sourceArtifactRef: 'raw-source',
          byteStart: clause.byteStart, byteEnd: clause.byteEnd };
      }
    }
    ...
    const context = {
      ...
      commandDeclarations: section.commands, workDeclarations: section.works,
      scenarioDeclarations: section.scenarios, fixDeclarations: section.fixes,
      sections: [{ id: section.id, scope: section.scope, orderedPosition: sectionIndex }],
    };
    const payload = { sourceRoots: roots, sourceRelations: section.relations,
      ...separatePhysical(context, '', sourceContextBindings), sourceContextBindings };
    fileBytes.push(writeJson(root, relative, { schemaVersion: BUNDLE_SCHEMA,
      evidenceClass: 'test-only-independent-expected-not-confirmation',
      sourceArtifact: { artifactId: 'raw-source', path: 'inputs/raw-source.md', bytes: raw.length, sha256: SOURCE_HASH },
  ...
  writeJson(root, 'intake.json', { evidenceClass: 'test-only-not-confirmation', authoritySources });
  return { root, intakeSource: path.join(root, 'intake.json'), authoritySources,
    sourceRoots, sourceRelations, declaredIds, fileBytes, expected };
}
