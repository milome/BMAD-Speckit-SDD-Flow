...
describe('runtime-policy dev-story tasks run-first consumption', () => {
  it('resolves dev_story tasks run through registry-first consumption with workflow-stage semantics', () => {
    const root = mkdtempSync(path.join(os.tmpdir(), 'dev-story-tasks-'));
    try {
      ...
      const runContext = buildRunContext(root, {
        ...
        runId: 'run-dev-tasks-001',
        lifecycleStage: 'dev_story',
        workflowStage: 'tasks',
      });
      registry.runContexts['run-dev-tasks-001'] = runContext;
      ...
      expect(output).toContain('"runId":"run-dev-tasks-001"');
      expect(output).toContain('"stage":"tasks"');
