...
import { mainDecisionFieldCheck } from '../../packages/bmad-speckit/src/main-agent/source-authority/scripts/main-agent-decision-field-check';

function writeRecord(
  root: string,
  values: { gateChecks?: unknown[]; contractChecks?: unknown[] }
): string {
...
describe('main-agent decision field check', () => {
  it('passes gateChecks that use decision only', () => {
    ...
    try {
      const recordPath = writeRecord(root, {
        gateChecks: [{ gate: 'delivery_closeout', decision: 'pass' }],
      });
  ...
  it('blocks canonical gateChecks that retain result', () => {
    ...
    try {
      ...
      const report = JSON.parse(
        fs.readFileSync(path.join(path.dirname(recordPath), 'decision-field-check.json'), 'utf8')
      );
  ...
  it('blocks canonical contractChecks that retain result', () => {
    ...
    try {
      const recordPath = writeRecord(root, {
        contractChecks: [
          { contract: 'requirement_record_schema', decision: 'pass', result: 'pass' },
        ],
      });
      expect(
        mainDecisionFieldCheck([
      ...
      expect(report.blockingReasons).toContain(
        'contract_check_result_forbidden:requirement_record_schema:0'
      );
    } finally {
      fs.rmSync(root, { recursive: true, force: true });
    }
