...
function packageFixture(): string {
  ...
  for (const fileName of [
    ...
    'audit-review-final-acceptance-judge.prompt.md',
  ]) {
  ...
  for (const fileName of [
    'requirements-contract-critical-auditor-judge-request.schema.json',
    'requirements-contract-final-acceptance-judge-request.schema.json',
  ]) {
    writeFileSync(
...
describe('requirements contract Judge prompt loader', () => {
  it('loads both role assets from the package-owned prompt and schema roots', () => {
    ...
    expect(loaded.map((asset) => asset.judgeRole)).toEqual([
  ...
  it('rejects caller path overrides, repository-root fallback, stale hashes, role mismatch, and mutable includes', () => {
    ...
    writeFileSync(
      mutablePrompt,
      `${readFileSync(mutablePrompt, 'utf8')}\n@include ./audit-review-final-acceptance-judge.prompt.md.include.md\n`,
      'utf8'
    );
    expect(() =>
      loadRequirementsContractJudgePromptAsset({
