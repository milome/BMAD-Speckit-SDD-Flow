...
describe('hook presenter resolution', () => {
  it('uses fallback presenter resolution instead of hard-coded runtime/hooks requires in claude and cursor hook entrypoints', () => {
    const files = [
      path.join(repoRoot, '.claude', 'hooks', 'stop.cjs'),
      path.join(repoRoot, '.claude', 'hooks', 'post-tool-use.cjs'),
      path.join(repoRoot, '.cursor', 'hooks', 'post-tool-use.cjs'),
      path.join(repoRoot, '_bmad', 'claude', 'hooks', 'stop.cjs'),
      path.join(repoRoot, '_bmad', 'claude', 'hooks', 'post-tool-use.cjs'),
      path.join(repoRoot, '_bmad', 'cursor', 'hooks', 'post-tool-use.cjs'),
    ];
