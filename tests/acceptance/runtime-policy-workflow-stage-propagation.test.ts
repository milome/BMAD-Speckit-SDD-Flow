...
describe('runtime-policy workflow-stage propagation', () => {
  it('resolves dev_story implement run through registry-first consumption with run identity', () => {
    const root = mkdtempSync(path.join(os.tmpdir(), 'workflow-stage-'));
    try {
      ...
      registry.runContexts['run-dev-002'] = runContext;
      ...
      expect(output).toContain('"runId":"run-dev-002"');
      expect(output).toContain('"stage":"implement"');
      expect(output).toContain('"triggerStage":"speckit_5_2"');
    } finally {
