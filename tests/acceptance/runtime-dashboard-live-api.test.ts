...
describe('runtime dashboard live api', () => {
  ...
  beforeAll(() => {
    if (skipUiDependentSuite) {
      console.warn(
        '[runtime-dashboard-live-api] skipping in restricted Windows session to avoid flaky esbuild/vite spawn EPERM during test bootstrap'
      );
    }
  });
  ...
  it.skipIf(skipUiDependentSuite)(
    'serves overview, runtime, timeline, score detail, and sft summary from the shared query core',
    { timeout: 60000 },
    async () => {
      ...
      try {
        ...
        try {
          ...
          const runtimeContext = (await (
            await fetch(`${server.url}/api/runtime-context`)
          ).json()) as {
            run_id: string;
            current_stage: string;
          };
          const timeline = (await (
            await fetch(`${server.url}/api/stage-timeline`)
          ).json()) as Array<{
            stage: string;
