...
describe('runtime dashboard selection api', () => {
  ...
  beforeAll(() => {
    if (skipUiDependentSuite) {
      console.warn(
        '[runtime-dashboard-selection-api] skipping in restricted Windows session to avoid flaky esbuild/vite spawn EPERM during test bootstrap'
      );
    }
  });
  ...
  it.skipIf(skipUiDependentSuite)(
    'defaults snapshot selection to the selected run board group and honors board/work item query params',
    { timeout: 60000 },
    async () => {
      ...
      try {
        ...
        expect(defaultSnapshot.selection.work_item_id).toBe(
          'bugfix:orphan:fix-runtime-dashboard-findings-duplication'
        );
        expect(defaultSnapshot.workboard.active_work_item_id).toBe(
          'bugfix:orphan:fix-runtime-dashboard-findings-duplication'
        );

        const standaloneSnapshot = (await (
          await fetch(`${server.url}/api/snapshot?board_group_id=queue%3Astandalone-ops`)
        ).json()) as {
          selection: { board_group_id: string | null; work_item_id: string | null };
          workboard: { active_board_group_id: string | null; active_work_item_id: string | null };
        };
        ...
        expect(standaloneSnapshot.workboard.active_work_item_id).toBe(
          'standalone_task:orphan:scores'
        );

        const storyWorkItemSnapshot = (await (
          await fetch(
        ...
        expect(storyWorkItemSnapshot.selection.work_item_id).toBe(
          'story:15-1-runtime-dashboard-sft'
        );
        expect(storyWorkItemSnapshot.workboard.active_work_item_id).toBe(
          'story:15-1-runtime-dashboard-sft'
        );
        expect(storyWorkItemSnapshot.runtime_context.work_item?.board_group_id).toBe(
          'epic:epic-15'
        );
        expect(storyWorkItemSnapshot.runtime_context.work_item?.work_item_id).toBe(
          'story:15-1-runtime-dashboard-sft'
        );

        const workboardSelection = (await (
          await fetch(
