...
function writeReceipt(base: string, eventId: string, eventHash: string): void {
  writeJson(path.join(base, 'events', 'receipts', `${eventId.replace(/[^a-z0-9_.-]/giu, '_')}.json`), {
    eventId,
    eventHash,
  });
}
...
function datasetArtifacts(base: string) {
  ...
  [quality, redaction, contamination, lineage, evalReport, training].forEach((file) => writeJson(file, { ok: true }));
  return { train, validation, test, openai, hf, quality, redaction, contamination, lineage, evalReport, training };
}

function writeFixture(root: string, overrides: { omitFailureCommand?: boolean; omitHfProjection?: boolean } = {}) {
  ...
  writeJson(failurePath, {
    resumeFailureCaseRegistryCoverage: {
      caseEvidence: Array.from({ length: 27 }, (_, index) => ({
        caseId: `case-${index + 1}`,
        sourceRefs: [{ sourceType: 'functionalResumeFailureCaseRegistry.failureCases', id: `case-${index + 1}` }],
  ...
  writeJson(manifestPath, {
    ...
    projections: {
      ...
      ...(overrides.omitHfProjection ? {} : { huggingface: { path: dataset.hf, hash: sha256File(dataset.hf) } }),
    },
    reports: {
  ...
  const event = {
    ...
    payload: { packet: { closeoutAttemptId: ATTEMPT, artifactRefs: refs.slice(1), extensionRefs: [refs[0]] } },
  };
  ...
  const commandRunRefs = [
    { commandId: 'CMD-PRODUCTION-SUBSYSTEM-ACCEPTANCE', closeoutAttemptId: ATTEMPT, runId: 'run-prod', exitCode: 0 },
    ...(!overrides.omitFailureCommand ? [{ commandId: 'CMD-FULL-FAILURE-CASE-COVERAGE', closeoutAttemptId: ATTEMPT, runId: 'run-failure', exitCode: 0 }] : []),
    { commandId: 'CMD-STRICT-CLOSEOUT-PROOF-GATE', closeoutAttemptId: ATTEMPT, runId: 'run-strict', exitCode: 0 },
  ];
  const record = {
...
function addControlledRebaselineFixture(recordPath: string, base: string) {
  ...
  const historical = { eventId: 'historical:clean', eventType: 'implementation_evidence_ingested', writerId: 'implementation-evidence-ingest', previousEventHash: ZERO_HASH, eventHash: HISTORICAL_HASH, payload: {} };
  const broken = { eventId: 'historical:broken', eventType: 'closeout_recorded', writerId: 'delivery-closeout-gate-writer', previousEventHash: EVENT_HASH, eventHash: BROKEN_HASH, payload: {} };
  const rebaseline = { eventId: 'control_log_rebaseline_recorded:strict', eventType: 'control_log_rebaseline_recorded', writerId: 'control-log-rebaseline-writer', previousEventHash: BROKEN_HASH, eventHash: REBASELINE_HASH, payload: { rebaseline: { controlled: true, mode: 'event-log-chain-rebaseline', priorEventHash: BROKEN_HASH } } };
  original.previousEventHash = REBASELINE_HASH;
  writeFileSync(eventLogPath, [historical, broken, rebaseline, original].map((event) => JSON.stringify(event)).join('\n') + '\n', 'utf8');
  for (const event of [historical, broken, rebaseline]) writeReceipt(base, event.eventId, event.eventHash);
  const record = JSON.parse(readFileSync(recordPath, 'utf8'));
  writeJson(recordPath, { ...record, eventCount: 4, eventChainHead: EVENT_HASH, lastAppliedEventHash: EVENT_HASH });
}
...
describe('strict closeout proof gate', () => {
  ...
  it('fails closed when a current-attempt failure case command or HF projection is missing', () => {
    ...
    try {
      const { recordPath, base } = writeFixture(root, { omitFailureCommand: true, omitHfProjection: true });
      const reportPath = path.join(base, 'strict-report.json');
      const code = mainStrictCloseoutProofGate([
        '--requirement-record',
        recordPath,
