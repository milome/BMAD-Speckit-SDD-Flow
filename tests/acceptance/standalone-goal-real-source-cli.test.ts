...
const {
  buildSourceSnapshot,
} = require('../../packages/bmad-speckit/dist/utils/goal-contract/dual-view-derivation.js');
const {
  extractSourceObligations,
} = require('../../packages/bmad-speckit/dist/utils/goal-contract/source-obligation-extractor.js');
const fixtureTools = require('../../packages/bmad-speckit/tests/fixtures/standalone-goal/canonical-full-fixture.cjs');

const ROOT = process.cwd();
const materializedFixture = fixtureTools.materializeFullFixture({ copyOracleHelpers: true });
process.on('exit', () => {
  try {
    rmSync(materializedFixture.root, {
      recursive: true,
      force: true,
      maxRetries: 5,
      retryDelay: 100,
...
const FIXTURE = materializedFixture.legacySourcePath;
const ORACLE = materializedFixture.expectedOraclePath;
...
const without = (row: any, field: string) =>
...
describe('standalone Goal contract full frozen Source Plan CLI', () => {
  it('publishes one complete sparse hash-closed authority without an authoring Judge', () => {
    ...
    const expectedWorks = [
      ...new Set(
        oracleBlocks
          .map((block: any) => block.context?.work)
          .filter((id: unknown) => typeof id === 'string' && /^WORK-\d{2}$/u.test(id as string))
      ),
    ].sort();
    const source = extractSourceObligations({
    ...
    expect(oracle.verification).toMatchObject({
      clauses: 2241,
      works: 16,
      unexplainedBlocks: [],
      uncoveredByteRanges: [],
    ...
    const sourceClauseSpans = source.sourceClauseCoverage.map((row: any) => ({
      startByte: row.startByte,
      endByteExclusive: row.endByteExclusive,
      exactTextHash: row.exactTextHash,
    }));
    const oracleClauseSpans = oracleClauses.map((row: any) => ({
      startByte: row.byteStart,
      endByteExclusive: row.byteEnd,
      exactTextHash: `sha256:${sha256(Buffer.from(row.text, 'utf8'))}`,
    }));
    expect(sha256Stable(sourceClauseSpans)).toBe(sha256Stable(oracleClauseSpans));
    expect(expectedWorks).toEqual(
    ...
    try {
      ...
      expect(
        coverage.sourceClauseCoverage.reduce((sum: number, row: any) => sum + row.clauseCount, 0)
      ).toBe(2241);
      expect(coverage.sourceClauseCoverage[0].clauseSetHash).toBe(CLAUSE_SET_HASH);
      expect(json(generation.generationReceiptPath).goalExecutionIRHash).toBe(
        generation.goalExecutionIRHash
      );
      ...
      expect(semantic.standaloneGoalSemanticIRHash).toBe(
        generation.standaloneGoalSemanticIrRef.hash
      );
      ...
      expect(
        ids(
          semanticPayload.obligations.filter((row: any) => row.executionRole === 'action'),
          'obligationId'
        )
      ).toEqual(expectedWorks);
      expect(ids(semanticPayload.atoms, 'requirementRef')).toEqual(expectedWorks);
      expect(
        ids(
          goalIr.obligations.filter((row: any) => row.executionRole === 'action'),
          'obligationId'
        )
      ).toEqual(expectedWorks);
      expect(goalIr.atomicTasks.every((row: any) => refs(row, 'obligationRefs').length === 1)).toBe(
        true
      );
      expect(
        ids(
          goalIr.atomicTasks
            .flatMap((row: any) => refs(row, 'obligationRefs'))
      ...
      expect(goalIr.standaloneLineage).toMatchObject({
        sourcePlanHash: generation.sourcePlanHash,
        standaloneGoalSemanticIRHash: semantic.standaloneGoalSemanticIRHash,
      ...
      expect(closure).toMatchObject({
        decision: 'pass',
        goalExecutionIRHash: generation.goalExecutionIRHash,
      });
      expect(closure.goalExecutionClosureHash).toBe(generation.closureRef.hash);
      expect(sha256Stable(without(closure, 'goalExecutionClosureHash'))).toBe(
        closure.goalExecutionClosureHash
      );
      ...
      const nonGlobal = semanticPayload.executionConstraints.filter(
        (row: any) => row.scope !== 'global'
      );
      expect(
        nonGlobal.some((row: any) => refs(row, 'applicableMustRefs').length === universe)
      ).toBe(false);
      expect(
        semanticPayload.executionConstraints.some(
          (row: any) =>
      ...
      expect(
        nonGlobal.some((row: any) => {
          const actionRefs = [
            ...new Set(refs(row, 'applicableMustRefs').filter((ref) => actionSet.has(ref))),
          ];
