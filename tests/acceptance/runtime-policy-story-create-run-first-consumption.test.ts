...
describe('runtime-policy story-create run-first consumption', () => {
  it('resolves story_create run through registry-first consumption with the correct lifecycle semantics', () => {
    const root = mkdtempSync(path.join(os.tmpdir(), 'story-create-run-'));
    try {
      ...
      const runContext = buildRunContext(root, {
        ...
        lifecycleStage: 'story_create',
      });
      registry.runContexts['run-story-create-001'] = runContext;
      ...
      expect(output).toContain('"runId":"run-story-create-001"');
      expect(output).toContain('"stage":"story_create"');
