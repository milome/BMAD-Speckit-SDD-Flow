...
describe('runtime language english post-audit output', () => {
  it('keeps post-audit registry-first output english-oriented and machine-safe', () => {
    const root = mkdtempSync(path.join(os.tmpdir(), 'en-post-audit-'));
    try {
      ...
      const runContext = buildRunContext(root, {
        ...
        runId: 'run-en-post-audit-003',
        lifecycleStage: 'post_audit',
      });
      registry.runContexts['run-en-post-audit-003'] = runContext;
      ...
      expect(output).toContain('"runId":"run-en-post-audit-003"');
      expect(output).toContain('"triggerStage":"bmad_story_stage4"');
      expect(output).toContain('"strictness":"strict"');
      expect(output).not.toContain('缺少');
      expect(output).not.toContain('未找到');
