...
const PROJECTION_FILE_NAME = 'requirements-contract-lint-profile-registry.json';
...
describe('requirements contract lint-profile registry surface parity', () => {
  it('exports one source owner and deterministic projection factory', () => {
    ...
    expect(createRequirementsContractLintProfileRegistryProjection).toBeTypeOf('function');
    expect(
      existsSync(path.resolve(ROOT, REQUIREMENTS_CONTRACT_LINT_PROFILE_REGISTRY_OWNER_PATH))
    ).toBe(true);
  });
  ...
  it('publishes a schema-valid projection derived from the lint owner', () => {
    ...
    expect(projection).toEqual(createRequirementsContractLintProfileRegistryProjection(ownerHash));
  });
