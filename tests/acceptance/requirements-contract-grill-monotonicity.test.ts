...
describe('requirements contract Grill monotonicity', () => {
  it('documents the batch frontier and canonical decision receipt owner', () => {
    const skill = readFileSync(
      path.resolve('_bmad/skills/requirements-contract-grill/SKILL.md'),
      'utf8'
    );
    ...
    expect(skill).toContain('Locator-only source binding changes never reopen Grill');
    expect(skill).not.toContain('before and after semantic model hashes');
    ...
    const graph = createRequirementsGrillQuestionGraph({
      ...
      questions: [
        ...
        },
        {
          questionId: 'q-c',
          questionVersion: 'v1',
          dependencies: ['q-a', 'q-b'],
          affectedFieldIds: ['FIELD-C'],
          authorityPremiseHashes: [premise],
          affectedNodeIds: ['NODE-C'],
        },
      ],
    });
        ...
        questions: [
          {
            questionId: 'q-cycle',
            questionVersion: 'v1',
            dependencies: ['q-cycle'],
            affectedFieldIds: ['FIELD-CYCLE'],
            authorityPremiseHashes: [premise],
            affectedNodeIds: ['NODE-CYCLE'],
          },
        ],
      })
  ...
  it('supports a zero-to-many ready frontier in stable dependency order', () => {
    ...
    expect(computeRequirementsGrillReadyFrontier({ questions, resolvedQuestionIds: [] })).toEqual([
      'q-a',
      'q-b',
    ]);
    expect(
  ...
  it('rejects a linked grill session path without writing through to the external target', () => {
    ...
    try {
      ...
      symlinkSync(
        external,
        path.join(sessionsRoot, 'SESSION-LINK'),
      ...
      expect(() => publishRequirementsContractDecisionReceipt({ recordRoot, receipt })).toThrow(
        'requirements_grill_session_path_reparse_forbidden'
      );
