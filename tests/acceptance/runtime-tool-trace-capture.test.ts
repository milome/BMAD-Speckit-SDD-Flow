...
describe('runtime tool trace automatic capture', () => {
  it('post-tool-use hook appends canonical tool trace sidecar and runtime artifact events', () => {
    ...
    try {
      ...
      const hookScript = path.join(repoRoot, '_bmad', 'claude', 'hooks', 'post-tool-use.js');
      ...
      const firstRun = spawnSync(process.execPath, [hookScript], {
        cwd: repoRoot,
        input: firstPayload,
        encoding: 'utf8',
        env: {
      ...
      const secondRun = spawnSync(process.execPath, [hookScript], {
        cwd: repoRoot,
        input: secondPayload,
        encoding: 'utf8',
        env: {
      ...
      expect(trace.messages[3]).toMatchObject({
        role: 'tool',
      });
  ...
  it('replays a real cursor postToolUse stdin fixture into the canonical tool trace sidecar', () => {
    ...
    try {
      ...
      const hookScript = path.join(repoRoot, '_bmad', 'cursor', 'hooks', 'post-tool-use.js');
      const payload = fs.readFileSync(cursorRealFixturePath, 'utf8');
