...
import { evaluateRequiredCommandFileExistence } from '../../packages/bmad-speckit/src/main-agent/source-authority/scripts/target-artifact-realization-gate';
...
describe('required command file existence gate', () => {
  ...
  it.each(['.cursor', '.claude'])(
    'resolves skill-dir command file refs from %s skill installs',
    (surface) => {
      const root = mkdtempSync(
        path.join(os.tmpdir(), `required-command-file-${surface.slice(1)}-`)
      );
      ...
      try {
        ...
        writeText(
          path.join(root, surface, 'skills', 'requirements-contract-authoring', 'SKILL.md'),
        ...
        const report = evaluateRequiredCommandFileExistence({ sourcePath });
        expect(report.decision).toBe('pass');
        const checkedFiles = report.checkedFiles as Array<{ absolutePath: string }>;
        expect(checkedFiles[0].absolutePath).toContain(
