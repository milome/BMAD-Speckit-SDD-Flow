...
function materializePublicationFixture(
  workspaceRoot: string,
  entry: 'req_trace_direct' | 'main_agent_compile'
) {
  return materializeAiTddManifestCloseoutRunnerFixture({
    root: workspaceRoot,
    authorityMode: entry === 'req_trace_direct' ? 'legacy' : 'canonical',
  });
}
afterEach(() => {
...
function pendingJournal(outDir: string): string {
  return JSON.parse(fs.readFileSync(path.join(outDir, '.compiler-publication.lock'), 'utf8'))
    .journalPath;
}
...
describe.each(['req_trace_direct', 'main_agent_compile'])('%s publication budget', (entry) => {
  ...
  it.each([
    ...
    const fixture = materializePublicationFixture(path.join(root, 'test-only-workspace'), entry);
    const outDir = path.join(root, 'out');
    const label = `${entry}-${crashPoint.replace(':', '-')}`;
    expect(runGenerator({ label: `${label}-crash-baseline`, entry, outDir, fixture }).status).toBe(
      0
    );
    growSyntheticConfirmation(fixture, 80);
    const crashed = runGenerator({
      label: `${label}-crash-exit`,
      entry,
      outDir,
      fixture,
      crashPoint,
    ...
    expect(JSON.parse(fs.readFileSync(journalPath, 'utf8')).state).toBe(
      committed ? 'completed' : 'rolled_back'
    );
    expectPublished(outDir, retry.stdout);
  });

  it('recovers a second process exit across an interrupted publication', () => {
    const fixture = materializePublicationFixture(path.join(root, 'test-only-workspace'), entry);
    const outDir = path.join(root, 'out');
    expect(
      runGenerator({ label: `${entry}-repeated-baseline`, entry, outDir, fixture }).status
    ).toBe(0);
    growSyntheticConfirmation(fixture, 80);
    const firstCrashPoint =
      entry === 'main_agent_compile' ? 'audit_receipt.json' : 'model_packet.json';
    expect(
      runGenerator({
        label: `${entry}-repeated-first-exit`,
        entry,
        outDir,
        fixture,
        crashPoint: firstCrashPoint,
      }).status
    ).toBe(86);
    const journalPath = pendingJournal(outDir);
    ...
    expect(
      runGenerator({
        label: `${entry}-repeated-recovery-exit`,
        entry,
        outDir,
        fixture,
        crashPoint: changed.name,
      }).status
    ).toBe(86);
    expect(fs.existsSync(journalPath)).toBe(true);
    const recoveredJournalPath = pendingJournal(outDir);
    const recoveryClaims = fs
      .readdirSync(path.dirname(journalPath))
      .filter((name) => /^\.recovery-[a-f0-9]{64}\.json$/u.test(name));
    const publicationJournals = fs
      .readdirSync(outDir, { withFileTypes: true })
      .filter((entry) => entry.isDirectory() && entry.name.startsWith('.compiler-publication-'));
    if (entry === 'main_agent_compile') {
      // Canonical output can be fully written before the journal marker; recovery settles it as completed.
      expect(recoveredJournalPath).not.toBe(journalPath);
      expect(recoveryClaims).toHaveLength(0);
      expect(publicationJournals.length).toBeGreaterThanOrEqual(2);
    } else {
      // Legacy output leaves a mixed quartet, so the second process exits during rollback.
      expect(recoveredJournalPath).toBe(journalPath);
      expect(recoveryClaims.length).toBeGreaterThanOrEqual(1);
      expect(publicationJournals.length).toBeGreaterThanOrEqual(1);
    }
    const retry = runGenerator({ label: `${entry}-repeated-recovered`, entry, outDir, fixture });
    expect(retry.status, retry.stdout).toBe(0);
    expect(
      fs.readdirSync(path.dirname(journalPath)).some((name) => name.startsWith('.recovery-'))
    ).toBe(false);
    expectPublished(outDir, retry.stdout);
  });
  ...
  it('recovers a fresh quartet without inventing previous artifacts', () => {
    ...
    expect(
      runGenerator({
        label: `${entry}-fresh-partial-exit`,
        entry,
        outDir,
        fixture,
        crashPoint: 'human_prompt.txt',
      }).status
    ).toBe(86);
    const journalPath = pendingJournal(outDir);
    const retry = runGenerator({
      label: `${entry}-fresh-partial-recovered`,
      entry,
      outDir,
      fixture,
    });
    ...
    expect(
      recovered.artifacts.every(
        (item: { previousHash: string | null }) => item.previousHash === null
      )
    ).toBe(true);
    expectPublished(outDir, retry.stdout);
  });
  ...
  it('does not publish a canonical rejection into a fresh damaged transaction', () => {
    ...
    const outDir = path.join(root, 'out');
    const crashed = runGenerator({
      label: `${entry}-fresh-journal-exit`,
      entry,
      outDir,
      fixture,
    ...
    const transaction = fs
      .readdirSync(outDir)
      .find((name) => name.startsWith('.compiler-publication-'))!;
  ...
  it.each(['backup', 'journal-json', 'missing-intent', 'unknown-write'] as const)(
    'preserves the interrupted artifact bytes across repeated %s recovery rejection',
    (damage) => {
      const fixture = materializePublicationFixture(path.join(root, 'test-only-workspace'), entry);
      const outDir = path.join(root, 'out');
      ...
      const journal = JSON.parse(fs.readFileSync(journalPath, 'utf8'));
      if (damage === 'backup')
        fs.appendFileSync(journal.artifacts[0].backupPath, 'TEST-ONLY-CORRUPTION');
  ...
  it('does not overwrite a newer complete quartet during stale recovery', () => {
    ...
    expect(runGenerator({ label: `${entry}-newer-baseline`, entry, outDir, fixture }).status).toBe(
      0
    );
    growSyntheticConfirmation(fixture, 80);
    expect(
      runGenerator({
        label: `${entry}-newer-exit`,
        entry,
        outDir,
        fixture,
        crashPoint: 'human_prompt.txt',
    ...
    expect(
      runGenerator({ label: `${entry}-newer-independent`, entry, outDir: newerDir, fixture }).status
    ).toBe(0);
  ...
  it('excludes a real second process while the publication owner is alive', () => {
    ...
    const outDir = path.join(root, 'out');
    expect(
      runGenerator({ label: `${entry}-contended-baseline`, entry, outDir, fixture }).status
    ).toBe(0);
    growSyntheticConfirmation(fixture, 80);
    const owner = runGenerator({
      label: `${entry}-contended-exit`,
      entry,
      outDir,
      fixture,
      crashPoint: 'human_prompt.txt',
  ...
  it.each(['human_prompt.txt', 'model_packet.json', 'audit_receipt.json'])(
    'rolls back the entire prior quartet if publishing %s fails',
    (failedName) => {
      const fixture = materializePublicationFixture(path.join(root, 'test-only-workspace'), entry);
      const outDir = path.join(root, 'out');
      expect(
        runGenerator({ label: `${entry}-io-${failedName}-baseline`, entry, outDir, fixture }).status
      ).toBe(0);
      const before = artifactHashes(outDir);
      growSyntheticConfirmation(fixture, 80);
      if (failedName === 'audit_receipt.json') {
        expect(
          runGenerator({
            label: `${entry}-io-audit-recovery-exit`,
            entry,
            outDir,
            fixture,
            crashPoint: 'model_packet.json',
          }).status
        ).toBe(86);
      }
      ...
      const result = runGenerator({
        label: `${entry}-io-${failedName}-rejected`,
        entry,
        outDir,
        fixture,
        preload,
      ...
      expect(artifactHashes(outDir)).toEqual(before);
      const retried = runGenerator({
        label: `${entry}-io-${failedName}-retry`,
        entry,
        outDir,
        fixture,
      });
  ...
  it('allows oversized sparse derived artifacts when no provider capacity is declared', () => {
    ...
    expect(
      summary.payloadBudgets.every((metric: { limit: number | null }) => metric.limit === null)
    ).toBe(true);
  });
