...
describe('governance post-tool-use hook', () => {
  it.each([
    ...
    'enqueues governance rerun events and keeps %s background trigger on the unified adapter contract',
    (_, hook) => {
      ...
      const queued = JSON.parse(
        readFileSync(
          governancePendingQueueFilePath(root, pendingFiles[0].replace(/\.json$/i, '')),
          'utf8'
        )
      ) as {
      ...
      expect(
        queued.payload?.journeyContractHints?.map((item: { signal: string }) => item.signal)
  ...
  it.each([
    ...
    'drains stage-emitted rerun-result events before handling %s post-tool-use payload',
    (_, hook) => {
      const root = mkdtempSync(path.join(os.tmpdir(), 'gov-post-tool-use-stage-event-'));
      tempRoots.push(root);
      mkdirSync(
        path.join(root, '_bmad-output', 'runtime', 'governance', 'queue', 'pending-events'),
        {
          recursive: true,
        }
      );
      ...
      require('node:fs').writeFileSync(
        stagedEventPath,
        JSON.stringify(stagedEvent, null, 2),
        'utf8'
      );
      ...
      const queuedPayloads = pendingFiles.map(
        (file) =>
      ...
      expect(
        queuedPayloads.some(
          (item) => item.payload?.runnerInput?.rerunGate === 'architecture-contract-gate'
        )
      ).toBe(true);
  ...
  it.each([
    ...
    're-overwrites handwritten readiness packet files after %s writes a packet path',
    (_, hook) => {
      ...
      expect(result).toBeNull();
      expect(readFileSync(cursorPacketPath, 'utf8')).toContain(
        '# Governance Remediation Executor Packet'
      );
      expect(readFileSync(claudePacketPath, 'utf8')).toContain(
        '# Governance Remediation Executor Packet'
      );
    }
  );
