...
const PROJECTION_FILE_NAME = 'requirements-contract-artifact-role-registry.json';
...
describe('requirements contract artifact-role registry surface parity', () => {
  it('exports one source owner and deterministic projection factory', () => {
    ...
    expect(createRequirementsContractArtifactRoleRegistryProjection).toBeTypeOf('function');
    expect(
      existsSync(path.resolve(ROOT, REQUIREMENTS_CONTRACT_ARTIFACT_ROLE_REGISTRY_OWNER_PATH))
    ).toBe(true);
  });
  ...
  it('publishes a schema-valid projection derived from the classifier owner', () => {
    ...
    expect(projection).toEqual(createRequirementsContractArtifactRoleRegistryProjection(ownerHash));
  });
