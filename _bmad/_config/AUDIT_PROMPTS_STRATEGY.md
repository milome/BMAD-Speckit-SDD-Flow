...
- **`_bmad/i18n/manifests/*.yaml`**：已有 `zh` / `en` 键，由 `packages/bmad-speckit/src/main-agent/source-authority/scripts/i18n/render-template.ts` 渲染，**与 `references/audit-prompts*.md` 不是同一条管线**。
...
1. **解析器**：`packages/bmad-speckit/src/main-agent/source-authority/scripts/i18n/resolve-audit-prompt-path.ts`（单测 `tests/i18n/resolve-audit-prompt-path.test.ts`）。
...
- **实现**：`packages/bmad-speckit/src/main-agent/source-authority/scripts/i18n/print-resolved-audit-prompt.ts` — 向 stdout 打印 JSON：`locale`、`projectRoot`、`resolvedPath`、`usedFallback`、`variant`、`exists`。
