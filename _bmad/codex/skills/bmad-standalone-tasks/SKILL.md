...
description: |
  ...
  涓?Agent 绂佹鐩存帴淇敼鐢熶骇浠ｇ爜锛屽疄鏂介』閫氳繃 Agent tool 瀛愪唬鐞嗭紙subagent_type: general-purpose锛夈€?  `auditor-tasks-doc` 灞炰簬 TASKS/BUGFIX 鏂囨。鍓嶇疆瀹¤锛屽繀椤诲厛浜庡疄鏂芥墽琛岄€氳繃锛涘湪鐪熸杩涘叆瀹炴柦鍓嶏紝涓?Agent 浠嶅繀椤绘墽琛岀粺涓€ `implementation-readiness` gate 鏂█锛屽彧鏈?`decision=pass` 鍙户缁紱瀹炴柦鍚庡璁′紭鍏?`.codex/agents/auditors/auditor-implement`锛屾寜 Fallback 閾鹃檷绾с€?  閬靛惊 ralph-method锛坧rd.{stem}.json / progress.{stem}.txt锛夈€乀DD 绾㈢豢鐏€乻peckit-workflow銆?  閫傜敤鍦烘櫙锛氱敤鎴锋彁渚?TASKS/BUGFIX 鏂囨。骞惰姹傛墽琛屾湭瀹屾垚浠诲姟銆傚叏绋嬩腑鏂囥€?when_to_use: |
  Use when: 鐢ㄦ埛璇淬€屾寜 TASKS_xxx.md 涓殑鏈畬鎴愪换鍔″疄鏂姐€嶃€屾寜 BUGFIX_xxx.md 瀹炴柦銆嶆垨鎻愪緵 TASKS/BUGFIX 鏂囨。璺緞瑕佹眰鎵ц銆?references:
...
> **Orphan standalone closeout contract**锛氬綋 TASKS / BUGFIX 鏂囨。浣嶄簬 `_orphan/` 璺緞鏃讹紝缁撴瀯鍖栧璁℃姤鍛婂繀椤绘樉寮忔彁渚?`stage=standalone_tasks`銆乣artifactDocPath`銆乣reportPath`锛涗笉寰楃户缁娇鐢?`stage=document` 浣滀负 orphan closeout 杩斿洖鍊笺€傜己澶变换涓€瀛楁鎴栦粎鏈?PASS 鏂囨湰鏃讹紝涓?Agent 涓嶅緱杩涘叆瀹炵幇鎵ц锛宧ost closeout 蹇呴』 fail-closed銆?
...
鍦?interactive main-agent 妯″紡涓嬶紝涓?Agent 鍦ㄥ彂璧枫€佺户缁垨鏀跺彛 `standalone_tasks` 閾捐矾鍓嶏紝蹇呴』浼樺厛璇诲彇锛?
```bash
...
鑻ュ綋鍓嶅疄闄呭涓绘槸 **Cursor IDE**锛屾垨璋冪敤涓婁笅鏂囨槑鏄句娇鐢?Cursor 渚т换鍔¤涔夛紙渚嬪 Cursor 鍘熺敓浠诲姟杞借嵎銆丆ursor 涓撶敤鎵ц鍣ㄣ€佹垨璋冪敤鏂规槑纭鈥滃湪 Cursor 瀹夸富涓墽琛屸€濓級锛屽垯锛?
...
**瀹炴柦鍓嶇殑 `auditor-tasks-doc` 灞炰簬 TASKS/BUGFIX 鏂囨。鍓嶇疆瀹¤锛屼笖蹇呴』鍏堜簬浠讳綍瀹炴柦鎵ц銆?* 涓嶅厑璁告妸鍓嶇疆瀹¤闄嶇骇鎴愬缓璁」銆佸彲閫夋楠ゆ垨瀹炴柦鍚庤ˉ鍋氶」銆?
### When to use
...
6. **TASKS/BUGFIX 鏂囨。鍓嶇疆瀹¤鏄疄鏂藉墠纭棬妲?*
   `auditor-tasks-doc` 鐨勮亴璐ｆ槸 **TASKS/BUGFIX 鏂囨。鍓嶇疆瀹¤**銆傚彧瑕佽瀹¤灏氭湭閫氳繃銆佸皻鏈墽琛屾垨缁撹涓嶆槑锛?*绂佹**杩涘叆浠讳綍瀹炴柦鎵ц銆佷唬鐮佷慨鏀广€佹祴璇曞疄鐜版垨鈥滃厛鍋氬啀琛ュ璁♀€濈殑璺緞銆?7. **Implementation Entry Gate 鏄疄鏂藉墠绗簩纭棬妲?*
   `auditor-tasks-doc` 閫氳繃鍚庯紝涓?Agent 浠嶅繀椤绘墽琛岀粺涓€ `implementation-readiness` gate 鏂█銆備粎 `decision=pass` 鍙繘鍏ュ疄鏂斤紱`decision=block` 琛ㄧず蹇呴』鍏堣繘鍏?*鑷姩淇 + rerun gate loop**锛屼慨澶?facts 鍚庣珛鍗抽噸绠?gate锛屼笉寰楁妸甯歌 fact repair 鍥為€€鎴愮敤鎴风‘璁ょ偣锛沗decision=reroute` 琛ㄧず涓嶅緱缁х画 standalone 杞昏矾寰勮€屽繀椤诲崌杞ㄣ€?
8. **Standalone gate UX contract**
   瀵?`standalone_tasks`锛孖mplementation Entry Gate 鐨勯粯璁よ涓哄繀椤荤被浼?rerun gate loop锛?   - 鑻ラ樆鏂師鍥犲睘浜庡綋鍓嶉」鐩簨瀹炲彲鑷姩淇鐨勮寖鍥达紙渚嬪 authoritative `auditor-tasks-doc` closeout 宸插瓨鍦紝浣嗙粺涓€ gate 浠嶇己灏戣冻澶熺殑 machine-readable evidence锛夛紝涓?Agent / host 蹇呴』鑷姩琛ラ綈浜嬪疄骞剁珛鍗?rerun gate锛?   - gate 淇鍚庡繀椤荤户缁師鎵ц閾撅紝鎭㈠ Ralph / implement / post-audit 闂幆锛屼笉寰楁妸鐢ㄦ埛閲嶆柊鎷夊洖涓棿纭锛?   - 浠呭綋闃绘柇灞炰簬鐪熷疄涓嶅彲鑷姩淇 blocker锛屾垨 gate 杩斿洖 `reroute` 鏃讹紝鎵嶅厑璁稿悜鐢ㄦ埛鏄惧紡鍗囩骇銆?
...
- [ ] DOC_PATH 宸插～鍏ワ紙缁濆璺緞鎴栫浉瀵归」鐩牴锛?- [ ] 宸叉槑纭鎵ц鏂囨。鏄?TASKS 杩樻槸 BUGFIX
- [ ] 宸茶緭鍑?CLI Calling Summary
- [ ] 宸插噯澶囧畬鏁村璁?prompt 鎴栧畬鏁?auditor-tasks-doc 鎵ц浣撴鏂?
### CLI Calling Summary 绀轰緥

```yaml
=== CLI Calling Summary ===
Input: DOC_PATH={鏂囨。璺緞}
Template: Step 0 Pre-Implementation Document Audit
Output: TASKS/BUGFIX 鏂囨。鍓嶇疆瀹¤鎶ュ憡
Fallback: L2 閫氱敤 Agent tool + 瀹¤ prompt 鈫?L3 涓?Agent 鐩存帴鎵ц
Acceptance: auditor-tasks-doc 缁撹涓?PASS锛屾柟鍙繘鍏?Step 1
```

### Mandatory rules

1. `auditor-tasks-doc` 鐨勮涔夋槸 **TASKS/BUGFIX 鏂囨。鍓嶇疆瀹¤**銆?2. 鍙 Step 0 鏈€氳繃銆佹湭鎵ц鎴栫粨璁轰笉鏄庯紝**绂佹**杩涘叆 Step 1銆?3. Step 0 澶辫触鏃跺繀椤诲厛淇鏂囨。鎴栧洖鍒扮敤鎴疯ˉ鍏呬俊鎭樁娈碉紝涓嶅緱鈥滃厛瀹炴柦鍚庤ˉ瀹¤鈥濄€?
---
...
- [ ] `auditor-tasks-doc` 宸插畬鎴?**TASKS/BUGFIX 鏂囨。鍓嶇疆瀹¤** 涓旂粨璁轰负 PASS锛涜嫢鏈€氳繃锛屽凡鍏堣繑鍥炴枃妗ｄ慨姝ｆ祦绋?- [ ] DOC_PATH 宸插～鍏ワ紙缁濆璺緞鎴栫浉瀵归」鐩牴锛?- [ ] TASK_LIST 宸蹭粠鏂囨。瑙ｆ瀽骞跺～鍏?- [ ] 宸茶緭鍑?CLI Calling Summary
- [ ] 妯℃澘宸叉暣娈靛鍒讹紙闈炴憳瑕侊級
...
### Scoring / AuditIndex 闆嗘垚

- 瀹¤鎶ュ憡椤诲寘鍚彲瑙ｆ瀽璇勫垎鍧楋紙渚涚粺涓€ auditor host runner 鑷姩瑙﹀彂 score 鍐欏叆锛?- `reportPath`銆乣artifactDocPath`銆乣stage` 蹇呴』瀹屾暣锛屼互渚跨粺涓€ auditor host runner 鑷姩瑙﹀彂璇勫垎鍐欏叆涓?registry auditIndex 鏇存柊
- 涓?Agent **涓嶅緱**鍦?standalone 璺緞閲屾墜宸ュ垎鍒皟鐢?`bmad-speckit score` 鎴?`update-runtime-audit-index`锛涚粺涓€閫氳繃 `runAuditorHost` / 缁熶竴 auditor host runner 鎵挎帴
