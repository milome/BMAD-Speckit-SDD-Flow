...
5. **读取审计配置**: 调用 `packages/bmad-speckit/src/main-agent/source-authority/scripts/bmad-config.ts` 的`shouldAudit('gaps')` 确定是否执行审计
...
npx --no-install bmad-speckit run-auditor-host \
  ...
  --stage gaps \
  --artifactPath specs/epic-{epic}-{epic-slug}/story-{story}-{story-slug}/IMPLEMENTATION_GAPS-E{epic}-S{story}.md \
