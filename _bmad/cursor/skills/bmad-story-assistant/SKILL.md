...
>   - `runAuditorHost 杩斿洖缁撴灉`
>   - `璇勫垎鍐欏叆缁撴灉锛堟垚鍔?澶辫触鐮侊級`
>   - `closeout 鐘舵€侊紙approved/鏈猘pproved锛塦

# BMAD Story 鍔╂墜

## Main Agent Orchestration Surface

鍦?interactive main-agent 妯″紡涓嬶紝涓?Agent 鍦ㄥ彂璧枫€佺户缁垨鏀跺彛 `story` 閾捐矾鍓嶏紝蹇呴』浼樺厛璇诲彇锛?
```bash
...
`mainAgentNextAction / mainAgentReady` 浠嶄繚鐣欎负 handoff compatibility 瀛楁锛屼絾涓嶅啀鏄敮涓€ runtime truth銆?
...
- Session Bootstrap JSON 鐢卞涓诲湪 `SubagentStart` 娉ㄥ叆锛屼富 Agent 涓嶅緱鐪佺暐璇ユ墽琛岄摼銆?- Cursor 鍒嗘敮涓笉鍋氫腑閫旀殏鍋滐紝涔熶笉鍦?`20 / 40 / ...` 杞浜よ繕涓?Agent锛涘瓙浠ｇ悊涓€鏃﹀惎鍔紝蹇呴』鍦ㄥ悓涓€浼氳瘽鍐呰繛缁繍琛屽埌鐢ㄦ埛閫夋嫨鐨勬€昏疆娆°€?- 鑻?party-mode 瀛愪唬鐞嗗湪 `22/50`銆乣10/50` 绛変腑閫旇疆娆℃彁鍓嶇粨鏉燂紝涓?Agent **涓嶅緱**鑷缁啓璁ㄨ鎴栦粠 Round 1 閲嶆柊寮€濮嬫€荤粨銆侰ursor 鐜板凡鏀寔 `subagentStop` hook锛屽涓讳細鍦?`subagentStop` 鑷姩瑙﹀彂 party-mode 杩斿洖鏀跺彛涓庢憳瑕佸埛鏂帮紱浣嗚繑鍥炲悗蹇呴』**鍏堣鍙?* `_bmad-output/party-mode/runtime/current-session.json`锛屽苟浠ヨ鏂囦欢浣滀负鍞竴妫€鏌ュ叆鍙ｏ紱**绂佹**鎸変慨鏀规椂闂寸寽娴嬧€滄渶鏂?`pm-*` 浼氳瘽鈥濄€傛鏌ラ『搴忓浐瀹氫负锛氣憼 `validation_status`銆乣status`銆乣session_key`銆乣target_rounds_total`锛涒憽 **浼樺厛璇诲彇** `visible_output_summary` 涓?`visible_fragment_record_present`锛屽厛鐪?`observed_visible_round_count`銆乣first_visible_round`銆乣last_visible_round`銆乣progress_current_round`銆乣progress_target_round`銆乣final_gate_present`銆乣final_gate_profile`銆乣final_gate_total_rounds`銆乣excerpt`锛涒憿 **浠呭湪闇€瑕佹繁鎸栨椂**鍐嶈鍙?`session_log_path`銆乣snapshot_path`銆乣audit_verdict_path`銆乣visible_output_capture_path`銆?*绂佹**鍦ㄨ鍙?`visible_output_summary` 涔嬪墠鍏堢炕 `session log` 鎴?`tool-result.md`銆傝嫢 `validation_status != PASS` 鎴栨湭杈惧埌鐢ㄦ埛閫夋嫨鐨勬€昏疆娆★紝蹇呴』娌跨敤鍚屼竴杞涓庡悓涓€ gate profile 绔嬪嵆閲嶅彂 facilitator銆?- 鍦ㄥ綋鍓?Cursor IDE 涓紝party-mode-facilitator 鍏佽閫氳繃 `generalPurpose-compatible` 鎵ц璺緞鎵胯浇锛沗.cursor/agents/party-mode-facilitator.md` 浠嶆槸 canonical prompt/source asset锛屽涓诲繀椤诲湪 `SubagentStart` 娉ㄥ叆 `Party Mode Session Bootstrap (JSON)`銆?
## 蹇€熷喅绛栨寚寮?
### 浜斿眰鏋舵瀯姒傝
```
...
3. 瀹¤閫氳繃鍚庯紝**蹇呴』鍏堟墽琛岀粺涓€ Implementation Entry Gate 鏂█**锛坄implementation-readiness`锛涘缓璁懡浠わ細`node scripts/assert-implementation-entry.ts --cwd {project-root}` 鎴栫瓑浠?`bmad-speckit assert-implementation-entry`锛夈€備粎褰撶粨鏋滀负 `decision=pass` 鏃讹紝鏂瑰彲鍙戣捣 Dev Story 瀹炴柦瀛愪换鍔★紱鑻ョ粨鏋滀负 `block` 鎴?`reroute`锛屼富 Agent 涓嶅緱杩涘叆闃舵涓?4. 瀹炴柦瀹屾垚鍚庯紝**蹇呴』**鍙戣捣瀹炴柦鍚庡璁★紙audit-prompts.md 搂5锛夛紙鏈楠や负蹇呴』锛岄潪鍙€夛級
5. 瀹¤閫氳繃鍗虫祦绋嬬粨鏉?
...
**璺宠繃鍒ゆ柇**锛氫粎褰撶敤鎴?*鏄庣‘**璇村嚭銆屽凡閫氳繃 party-mode 涓斿璁￠€氳繃銆嶃€岃烦杩?Create Story銆嶆椂锛屼富 Agent 鏂瑰彲璺宠繃闃舵涓€銆佷簩銆傝嫢鐢ㄦ埛浠呮彁渚?Epic/Story 缂栧彿鎴栬銆孲tory 宸插瓨鍦ㄣ€嶈€屾湭鏄庣‘涓婅堪琛ㄨ堪锛?*蹇呴』**鎵ц Create Story锛堜富 Agent 鍏堝睍绀?`20 / 50 / 100` 寮哄害閫夐」锛涜嫢娑夊強璁捐瀹氱鍒欓粯璁?`final_solution_task_list_100`锛夈€?
### 1.1 鍙戣捣瀛愪换鍔?
...
涓?Agent 鍦ㄦ敹鍒伴樁娈典簩閫氳繃缁撹鍚庛€佽繘鍏ラ樁娈典笁涔嬪墠锛?*蹇呴』鍏?*纭缁熶竴 auditor host runner 宸插畬鎴?post-audit automation銆傝嫢 host 灏氭湭鎵ц涓旀姤鍛婃枃浠?`_bmad-output/implementation-artifacts/epic-{epic}-*/story-{story}-*/AUDIT_Story_{epic}-{story}_stage2.md` 瀛樺湪锛屽垯涓?Agent 鎵ц 姝ラ 2.2 琛ヨ窇 `runAuditorHost`銆傛湭鑾峰緱 `closeout approved` 鍓嶄笉寰楄繘鍏ラ樁娈典笁銆?
...
```
琛ヨ窇鍚庡啀娆＄‘璁?host 宸插畬鎴愭敹鍙ｏ紱鑻ユ姤鍛婁笉瀛樺湪鍒欏仠姝㈠苟鏄庣‘缂哄け鍘熷洜銆傝嫢璋冪敤澶辫触锛屽繀椤昏嚜鍔ㄩ噸璇曘€佽褰曟瘡娆″け璐ュ師鍥犱笌淇鍔ㄤ綔锛岀洿鍒拌幏寰?`closeout approved` 鎴栫‘璁ゅ瓨鍦ㄧ湡瀹?blocker銆?
...
- story 闃舵鐨勮瘎鍒嗗啓鍏ャ€乤uditIndex 鏇存柊涓庡叾瀹?post-audit automation 缁熶竴鐢?`runAuditorHost` 鎵挎帴锛涗富 Agent 閫氳繃 姝ラ 2.2/2.3 鍋氬畬鎴愭€佹鏌ヤ笌琛ヨ窇锛?*蹇呴』鍚?`--iteration-count {绱鍊紏`**锛泂tage=story銆傝嫢澶辫触锛屽繀椤昏嚜鍔ㄩ噸璇曞苟璁板綍 `resultCode`銆佸け璐ュ師鍥犮€佷慨澶嶅姩浣滐紱鏈幏寰?`closeout approved` 鍓嶄笉寰楃户缁€?
---
...
瀹¤閫氳繃鍚庯紝**蹇呴』鍏?*鎵ц缁熶竴 Implementation Entry Gate 鏂█锛坄implementation-readiness`锛夈€備粎褰撶粨鏋滀负 `decision=pass` 鏃讹紝鎵嶈兘鎵ц **/bmad-bmm-dev-story** 绛変环宸ヤ綔娴侊紝瀵?Story `{epic_num}-{story_num}` 杩涜寮€鍙戝疄鏂斤紱鑻ョ粨鏋滀负 `block` 鎴?`reroute`锛屽繀椤诲厛琛ラ綈 readiness 浜嬪疄鎴栨寜鎺ㄨ崘 flow 璋冩暣锛岀姝㈢洿鎺ュ惎鍔?Dev Story銆?
### 鍓嶇疆妫€鏌?
鍦ㄥ紑濮嬪疄鏂藉墠锛屽繀椤荤‘璁や互涓嬫鏌ラ」锛?- [ ] 宸叉墽琛岀粺涓€ Implementation Entry Gate 鏂█锛屼笖缁撴灉涓?`decision=pass`
- [ ] PRD闇€姹傝拷婧珷鑺傚凡琛ュ厖锛堝垪鍑烘湰Story娑夊強鐨勬墍鏈塒RD闇€姹侷D锛?- [ ] Architecture绾︽潫宸蹭紶閫掑埌Story鏂囨。锛堝垪鍑虹浉鍏崇殑Architecture缁勪欢鍜岀害鏉燂級
- [ ] 澶嶆潅搴﹁瘎浼板凡瀹屾垚锛堢‘璁ゆ湰Story鐨勫鏉傚害鍒嗘暟锛?- [ ] Epic/Story瑙勫垝灞傜殑渚濊禆鍒嗘瀽宸茬‘璁わ紙纭鍓嶇疆Story宸插畬鎴愶級

**閲嶇畻瑙勫垯锛堝己鍒讹級**锛氳嫢 Dev Story 鍏堣繘鍏?`specify / plan / gaps / tasks` 绛夌函鏂囨。瀛愰樁娈碉紝鍒欏湪棣栦釜鐪熸鍐欑敓浜т唬鐮?/ 娴嬭瘯浠ｇ爜鐨?`implement` 瀛愰樁娈靛惎鍔ㄥ墠锛屼富 Agent 鎴栧涓诲繀椤?*鍐嶆閲嶇畻鍚屼竴涓?Implementation Entry Gate**銆傜姝㈡妸绗竴娆￠€氳繃缁撴灉褰撴垚姘镐箙閫氳璇併€?
...
prompt: |
  ...
  **鍚?stage 瀹¤閫氳繃鍚庤惤鐩樹笌缁熶竴 host 鏀跺彛绾︽潫锛堝己鍒讹級**锛?
  ...
  锛?锛夎皟鐢ㄥけ璐ユ椂璁板綍 `resultCode`銆佸け璐ュ師鍥犱笌淇鍔ㄤ綔杩涘璁¤瘉鎹紝骞剁珛鍗宠嚜鍔ㄩ噸璇曪紱鏈幏寰?`closeout approved` 鍓嶄笉寰楃粨鏉熷綋鍓?stage銆佷笉寰椾氦杩樼敤鎴锋墜鍔ㄦ搷浣溿€?
...
**閫氳繃锛圓/B绾э級**锛?- Story鏍囪涓哄畬鎴?- #### 姝ラ 4.3锛歋tory 瀹屾垚鑷锛堝己鍒讹紝鍏堟墽琛岋級
  - 鍦?*鎻愪緵瀹屾垚閫夐」涔嬪墠**锛屼富 Agent **蹇呴』鍏?*纭缁熶竴 auditor host runner 宸插畬鎴?implement 闃舵 post-audit automation銆?  - 鑻?host 宸插畬鎴愶紝鍒欐棤闇€鎵ц 姝ラ 4.2銆?  - 鑻?host 鏈畬鎴愪笖 `AUDIT_Story_{epic}-{story}_stage4.md` 瀛樺湪锛屽垯涓?Agent 鎵ц 姝ラ 4.2 琛ヨ窇銆?  - 鑻ユ姤鍛婂彲瑙ｆ瀽鍧楃淮搴﹂敊璇紝鍒欏厛淇鎶ュ憡锛屽啀閫氳繃 `runAuditorHost` 閲嶆柊鏀跺彛銆?  - 琛ヨ窇澶辫触鏃朵笉寰楃户缁富娴佺▼锛涘繀椤昏嚜鍔ㄩ噸璇曞苟璁板綍姣忔澶辫触鍘熷洜涓庝慨澶嶅姩浣滐紝鐩村埌鑾峰緱 `closeout approved` 鎴栫‘璁ょ湡瀹?blocker銆?- #### 姝ラ 4.2锛氳ˉ璺?runAuditorHost锛堟楠?4.3 鍒ゅ畾 host 鏈畬鎴愭椂鎵ц锛?  - 褰?姝ラ 4.3 鍒ゅ畾 host 鏈畬鎴愪笖鎶ュ憡瀛樺湪鏃讹紝涓?Agent 鎵ц锛歚npx ts-node scripts/run-auditor-host.ts --projectRoot <projectRoot> --stage implement --artifactPath <story 鏂囨。璺緞> --reportPath <鎶ュ憡璺緞> --iterationCount {鏈?stage 绱 fail 杞暟锛? 琛ㄧず涓€娆￠€氳繃}`
  - 鑻ヨ皟鐢ㄥけ璐ワ紝璁板綍 `resultCode`銆佸け璐ュ師鍥犱笌淇鍔ㄤ綔锛屽苟鑷姩閲嶈瘯锛涙湭鑾峰緱 `closeout approved` 鍓嶄笉寰楁彁渚涘畬鎴愰€夐」銆?- #### 瀹¤閫氳繃鍚庣粺涓€ Host 鏀跺彛锛堝己鍒讹級
  - 瀛愪唬鐞嗗湪銆愬璁￠€氳繃鍚庡繀鍋氥€戜腑杩斿洖 host 鎵€闇€瀛楁锛涗富 Agent 閫氳繃 姝ラ 4.3/4.2 鍋氬畬鎴愭€佹鏌ヤ笌琛ヨ窇锛?*蹇呴』鍚?`--iteration-count {绱鍊紏`**锛泂tage=implement銆傝嫢澶辫触锛屽繀椤昏嚜鍔ㄩ噸璇曞苟璁板綍 `resultCode`銆佸け璐ュ師鍥犮€佷慨澶嶅姩浣滐紱鏈幏寰?`closeout approved` 鍓嶄笉寰楃粨鏉熴€?- 鎻愪緵瀹屾垚閫夐」锛堣涓嬫枃锛?
...
- **涓嶄腑鏂墽琛?contract**锛氬疄鏂藉瓙浠ｇ悊蹇呴』杩炵画瀹屾垚褰撳墠浣滅敤鍩熷唴鐨勫叏閮ㄥ墿浣?User Story/浠诲姟锛屼笉寰楀湪 milestone 瀹屾垚鍚庢殏鍋滅瓑寰呬富 Agent 鎵瑰噯銆傛帶鍒舵潈浠呭彲鍦ㄤ互涓嬩笁绉嶆儏鍐典笅杩斿洖涓?Agent锛氣憼 褰撳墠浣滅敤鍩熷伐浣滃叏閮ㄥ畬鎴愪笖鍙繘鍏?post-audit锛涒憽 鍑虹幇鐪熷疄 blocker锛岄渶瑕?reroute / remediation锛涒憿 鏈妧鑳芥樉寮忓畾涔夌殑瀹¤鎴?checkpoint 杈圭晫宸插埌杈俱€?
---
