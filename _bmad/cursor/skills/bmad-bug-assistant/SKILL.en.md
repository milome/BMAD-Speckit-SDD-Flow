...
4. **Cursor-only return validation**: because Cursor currently has no `SubagentStop` hook, after a party-mode sub-agent returns the main Agent must verify that the run has reached the user-selected total rounds and produced the final summary / `## Final Gate Evidence`. If the run stops early at rounds such as `22/50` or `10/50`, the main Agent must **not** continue the debate in the main thread and must **not** restart from Round 1. It must immediately re-issue the facilitator with the same total rounds and same gate profile.
...
Each time a subtask (party-mode or mcp_task) is initiated, the main Agent **must** comply with the following rules, otherwise the subagent will easily deviate from the requirements of this skill due to incomplete prompt words:
...
1. Based on the BUG problem information provided by the user, **priority** is introduced through the Cursor Task scheduling **party-mode-facilitator** (`.cursor/agents/party-mode-facilitator.md`) to introduce the above multiple roles, conduct **at least 100 rounds** of mutual questioning and debate (BUGFIX belongs to the "generate the final solution and final task list" scenario), and meet the convergence conditions (root cause consensus + no new updates in the past 2–3 rounds) gap) and then end; if party-mode-facilitator is unavailable, fall back to **mcp_task** + `generalPurpose`, and explicitly ask in the prompt to simulate multiple roles (Winston architect, Mary analyst, Amelia developer, Quinn tester, John product manager) to debate and reach a root cause consensus.
...
4. Initiate the **audit subtask**:
   - Subagent: Prioritize Cursor Task scheduling **code-reviewer** (`.claude/agents/` or `.cursor/agents/`), if not found, **mcp_task** + `generalPurpose`
...
1. Copy the entire section of the template **BUG-A1-ROOT** (stage root debate complete prompt template) into the `prompt` parameter of mcp_task or party-mode.
...
Audit subtask: Prioritize Cursor Task scheduling **code-reviewer**, if not found, **mcp_task** + `generalPurpose`. **Must** copy the entire paragraph below into the prompt of the audit subtask and cannot be omitted.
```
...
1. Use **party-mode** (or fall back to mcp_task) again, introduce the above multiple roles to conduct **at least 100 rounds** of debate (BUGFIX belongs to the "generating the final plan and final task list" scenario, and ends after meeting the convergence conditions).
...
4. Initiate the audit subtask (same as stage 1):
   - Subagent: **code-reviewer** or **mcp_task generalPurpose**
...
1. Copy the entire section of the template **BUG-A2-UPDATE** (phase two information supplementary debate complete prompt template) into the `prompt` parameter of mcp_task or party-mode.
...
4. **[Required]** Initiate the audit subtask: use the template ID **BUG-A1-AUDIT** (phase one audit complete prompt template) to copy the entire section, and the subagent is **code-reviewer** or **mcp_task generalPurpose**.
5. **[Required]** If the audit conclusion is failed, **the audit sub-agent must directly modify the BUGFIX document** in this round to eliminate the gap; the main Agent will initiate the audit again after receiving the report. It is prohibited to only output modification suggestions without modifying the document. For document audit iteration rules, see [audit-document-iteration-rules.md](../speckit-workflow/references/audit-document-iteration-rules.md). Repeat until the conclusion is "complete coverage, verification passed". **Prohibited** Only one round of audit will be completed if it fails.
...
1. Use **party-mode** (or fall back to mcp_task), introduce the above multiple roles to conduct **at least 100 rounds** of debate (BUGFIX belongs to the "generate final plan and final task list" scenario, and end after meeting the convergence conditions).
...
1. Copy the entire section of the template **BUG-A3-TASKS** (the phase three task list supplements the complete prompt template) into the `prompt` parameter of mcp_task or party-mode, and replace the placeholder: `{Main Agent fills in the BUGFIX document path}` → the actual BUGFIX document full path; `{project-root}` → the project root directory absolute path; if BUGFIX For specific code locations, fill in the file path and line number or positionable fragment in the "Key Code Location Reference".
...
5. **[Required]** After the subtask returns, initiate the audit subtask: use the template **BUG-A3-AUDIT** (Phase 3 §7 Audit complete prompt template) to copy the entire section, and the subagent is **code-reviewer** or **mcp_task generalPurpose**.
6. **[Required]** If the audit conclusion is failed, **the audit sub-agent must directly modify the BUGFIX document** in this round; the main Agent will initiate the audit again after receiving the report. It is prohibited to only output modification suggestions without modifying the document. For document audit iteration rules, see [audit-document-iteration-rules.md](../speckit-workflow/references/audit-document-iteration-rules.md). Repeat until the conclusion is "Passed". **Prohibited** Only one round of audit will be completed if it fails.
...
**Recommended BMAD Agent (display name)**: **Amelia Development** (implementation); use **code-reviewer** (or mcp_task generalPurpose fallback) during the audit phase.

**Mandatory constraints**:
- Implementation must be done via a **subagent** (`mcp_task` calling `generalPurpose` or the dev subagent)
...
| Item | Content |
| -- | ---- |
| Subagent | `generalPurpose` of `mcp_task` (or dev subagent if available) |
...
| Main Agent Responsibilities | Initiate mcp_task, pass in complete prompt, collect subagent output |

#### Main Agent self-check list before launching

Before initiating mcp_task, you must confirm that the prompt contains all of the following content, otherwise the subagent cannot comply with ralph-method and TDD traffic lights:
...
#### Stage 4 implements detailed prompt words (the main Agent must be copied in its entirety to the prompt of mcp_task and cannot be omitted)

**Template ID**: BUG-A4-IMPL.

**Main Agent operation**: Copy the entire paragraph from "Please execute the following BUGFIX document" to "Inline constraint execution." **to the `prompt` parameter of mcp_task; only replace `{main Agent fill in}` with the BUGFIX document path, and replace `{project-root}` with the absolute path of the project root directory; **Do not** delete any mandatory compliance items.
```
...
| Item | Content |
| -- | ---- |
| Subagent | Prioritize **code-reviewer** (Cursor Task), if not found, **mcp_task** + `generalPurpose` |
...
- Disable direct execution of `search_replace` or `write` on production code
- Implementation tasks must be delegated to subagents
- If the subagent returns incomplete, you can initiate a secondary subtask with `resume`. You must not replace the subagent and change the code directly.
...
Main Agent: Execute Phase 1 - Copy the entire "Phase 1 Debate Complete Prompt Template" and fill in the user description before launching mcp_task/party-mode; after the subtask returns, copy the entire "Phase 1 Audit Complete Prompt Template" and launch the audit subtask; iterate until the audit is passed.
