...
- Per-MUST trace closure is mandatory and must not be compressed. Every business `MUST-*` needs either an independent `ACC-*` / `E2E-*` row or an explicit per-MUST assertion/oracle mapping; shared `EVD-*`, `CMD-*`, `targetModificationPaths[]`, `currentTargetMap`, and business visual rows that cover multiple MUST rows must declare per-MUST oracle, assertion, or responsibility mappings. A single generic "source-derived" row covering all MUST rows is a confirmation blocker.
...
node <skill-dir>/scripts/promote-draft-large-doc.js \
  ...
  --scale-assessment <authoring-dir>/scale-assessment-initial.json \
  ...
  --encoding-report <authoring-dir>/encoding-report.json \
  --receipt-out <authoring-dir>/promotion-receipt.json \
  --require implementationConfirmation: \
  --min-bytes <minimum-expected-bytes> \
  --retry-receipt <authoring-dir>/large-doc-write-retry-receipt.json \
  --json
...
`--promotion-stage authoring-draft` allows only `status: draft`, `status: draft_updated_not_confirmation_ready`, or `status: reconfirm_required`. It is persistence only: it is not confirmation-ready, not implementation-ready, and not execution-ready. Its receipt must include `promotionStage`, `allowedStatuses`, `statusValue`, `confirmationReady: false`, `safePromotionAsDraft: true`, `requiresUserConfirmationBeforeExecution: true`, `authoringPromotionGate`, `manifestPath`, `writeReceipt`, `backupPath`, `targetHash`, and `receiptPath`. It must skip confirmation-ready reverse audit because that audit intentionally requires `status: user_confirmed`.

Authoring-draft promotion is guarded. Before target replacement, `promote-draft-large-doc.js` MUST validate all of these artifacts:

- `scale-assessment-initial.json` with `schemaVersion: contract-authoring-scale-assessment/v1`, `phase: initial_assessment`, and visible `assessmentTrace.visibleOutputStream: stderr`.
- `scale-routing-decision.json` with `schemaVersion: contract-authoring-scale-routing-decision/v1`, current `initialAssessmentRef.path`, and current `initialAssessmentRef.hash`.
- `source-mutation-decision.json` with `schemaVersion: requirements-authoring-source-mutation-decision/v1`, `finalDecision: allow_source_materialization`, `sourceMutationAllowed: true`, `sourceDocumentHashBefore` bound to the current target raw document hash, `sourceDocumentHashAfter` bound to the current draft manifest raw hash, and `semanticSourceHashAfter` bound to the current draft semantic `sourceDocumentHash`. If the current target has a parseable inline `implementationConfirmation`, `semanticSourceHashBefore` must also match the current target semantic `sourceDocumentHash`. For new target files, it must also declare `sourceDocumentExistedBefore: false` and promotion must verify that the target still does not exist.
- `encoding-report.json` from `encoding-integrity-guardian` with `checkedFiles > 0` and `findings: []`.
- `promotion-receipt.json` path supplied through `--receipt-out`.
- `checkpoint-persistence-evidence.json` when the routing decision is `checkpoint_required` or `checkpoint_required_with_amendment`; it must prove all semantic checkpoints `cp-00` through `cp-08` and include progress, pre-render MUST gate, pre-render global consistency, and packet/source reconciliation hashes. Any referenced checkpoint receipt must be bound to the current draft semantic `sourceDocumentHash` and `implementationConfirmationHash`.
- The requirement-record `authoring` directory must not contain ad hoc executable helper scripts (`.cjs`, `.js`, `.mjs`, or `.ps1`). If one is present, promotion must fail closed with `authoring_temporary_executable_helper_present`; move or archive the helper before rerunning the skill-local flow.
