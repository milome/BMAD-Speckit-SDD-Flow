# Implementation Confirmation Reference

```text
assetKind: reference
authority: none
canonicalRole: implementation_confirmation_reference
machineSchema: packages/bmad-speckit/src/main-agent/source-authority/schemas/requirements-contract-implementation-confirmation.schema.json
codec: packages/bmad-speckit/src/main-agent/source-authority/scripts/requirements-contract-implementation-confirmation-codec.ts
```

This human reference is the migrated guidance from the transitional `contract-template.md`. It documents the inline `implementationConfirmation` shape and review rules. It must not act as machine schema authority or be parsed as a contract. Structural authority belongs to the JSON Schema; extraction, parsing, normalization, and serialization belong to the Codec.
