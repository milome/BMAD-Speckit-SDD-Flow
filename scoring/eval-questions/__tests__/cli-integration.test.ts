...
function runCli(args: string[], cwd = process.cwd()): string {
  const cmd = `npx ts-node "${CLI_SCRIPT}" ${args.join(' ')}`;
...
describe(
  ...
  beforeEach(() => {
    ...
    fs.mkdirSync(path.join(tmpRoot, 'scoring', 'eval-questions', 'v1'), { recursive: true });
    fs.mkdirSync(path.join(tmpRoot, 'scoring', 'eval-questions', 'v2'), { recursive: true });
    fs.writeFileSync(
      path.join(tmpRoot, 'scoring', 'eval-questions', 'v1', 'manifest.yaml'),
      'questions: []\n',
      'utf-8'
    );
    fs.writeFileSync(
      path.join(tmpRoot, 'scoring', 'eval-questions', 'v2', 'manifest.yaml'),
      'questions: []\n',
      'utf-8'
    );
  ...
  it('add 生成的文件可被 loadManifest 解析', () => {
    ...
    const v1Dir = path.join(tmpRoot, 'scoring', 'eval-questions', 'v1');
  ...
  it('add --version v2 写入 v2 目录', () => {
    ...
    const v2Dir = path.join(tmpRoot, 'scoring', 'eval-questions', 'v2');
  ...
  describe('run (Story 8.3)', () => {
    ...
    it('run 使用 --reportPath 与有效报告时写入评分', () => {
      ...
      fs.cpSync(
        path.join(projRoot, 'scoring', 'rules'),
        path.join(tmpRoot, 'scoring', 'rules'),
        { recursive: true }
      );
      ...
      fs.mkdirSync(path.join(tmpRoot, 'scoring', 'schema'), { recursive: true });
      fs.copyFileSync(
        path.join(projRoot, 'scoring', 'schema', 'run-score-schema.json'),
        path.join(tmpRoot, 'scoring', 'schema', 'run-score-schema.json')
      );
...
`,
      ...
      const dataDir = path.join(tmpRoot, 'scoring', 'data');
