...
```yaml
# 全局 skill 路径（%USERPROFILE% 于加载时展开）
...
- `required_skill_path`：必引全链路 Skill 路径（全局 skill：`%USERPROFILE%\.cursor\skills\bmad-code-reviewer-lifecycle\SKILL.md`；跨平台 `~/.cursor/skills/`）
