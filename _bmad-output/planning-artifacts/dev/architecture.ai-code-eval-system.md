...
**覆盖范围**：§2.1 表 A/B、§2.3、§2.4、§3.6、§3.7、§3.8、§3.9、§3.10、§3.11、§3.12、§3.13。§3.2.1、§3.4.x、§3.5 由 scoring/docs/SCORING_CRITERIA_AUTHORITATIVE.md 承载，E2.2 产出。
...
评测题目执行场景下，须满足以下四条防护措施：

| 措施 | 说明 |
|------|------|
| 题目来源与时间隔离 | 评测题基于自有业务需求或历史项目原创设计，不采用公开题库，避免进入训练数据 |
| 定期迭代题池 | 按周期（如 1–2 个月）更新题池，淘汰旧题，降低被训练数据覆盖风险 |
| 混淆变量校验（可选） | 对题目做逻辑等价改写（变量名、表述替换），若受测模型在改写题上通过率下降 >20%，触发人工复核 |
| 私有闭卷与评测接口分离 | 完整题库不对外公开，仅开放评测接口或脱敏样本，避免被爬取加入训练数据 |

**落位**：操作要点与触发阈值由实施时在 `scoring/` 或项目内单独 checklist 中定义；对应 Story E4.3 eval-scenario-bmad-integration。
...
### 9.4 权威文档与题量表述（§3.9、§3.10）

**权威文档**：`scoring/docs/SCORING_CRITERIA_AUTHORITATIVE.md` 须含需求 §3.10 定义的 **24 项内容**。实施时 E2.2 的 spec/tasks 须含「24 项与需求 §3.10 逐一核对清单」，确保每项可验证、可追溯。

**题量表述**（§3.9）：权威文档及产出须区分「已实现并验证的题数」与「目标题池规模」（如需求拆解目标 20+，全流程题池目标 50+），在文档与产出一致，避免与实现状态不符。
...
*本文档覆盖需求 §2.1 表 A/B、§2.3、§2.4、§3.6、§3.7、§3.8、§3.9、§3.10、§3.11、§3.12、§3.13。*
