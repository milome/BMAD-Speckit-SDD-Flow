...
const runId = 'trace-017-contract-traceability-policy-001';
const reportPath =
  '_bmad-output/runtime/requirement-records/REQ-CLOSED-LOOP-DESIGN/evidence/TRACE-017/trace-017-contract-traceability-policy-001/entry-flow-adaptation-matrix.json';

const commands = [
  {
    commandId: 'CMD-TRACE-017-ENTRYFLOW-TRACEABILITY-CHECK',
    command:
      'npx ts-node --project tsconfig.node.json --transpile-only scripts/main-agent-entryflow-traceability-check.ts --requirement-record _bmad-output/runtime/requirement-records/REQ-CLOSED-LOOP-DESIGN/requirement-record.json --source docs/design/需求实现完整性门禁与重跑闭环设计.md --report-path ' +
      reportPath +
      ' --evaluated-at 2026-05-19T22:30:00.000+08:00 --evaluated-by codex --json',
  },
  {
    commandId: 'CMD-TRACE-017-ENTRYFLOW-TRACEABILITY-TEST',
    command:
      'npx vitest run tests/acceptance/main-agent-entryflow-traceability-check.test.ts tests/acceptance/implementation-evidence-ingest.test.ts tests/acceptance/requirement-record-schema.test.ts tests/acceptance/requirements-confirmation-ingest.test.ts tests/acceptance/resolve-active-requirement.test.ts',
  },
  {
