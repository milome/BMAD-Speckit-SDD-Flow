...
An entry-flow-specific discipline contract that constrains how an implementation is executed without defining implementation scope.
_Avoid_: implementation prompt, task source, command authority, skill orchestrator

**Compiled Prompt Artifact**:
A prompt artifact generated by the req-trace compiler from confirmed implementation confirmation material.
_Avoid_: hand-written worker prompt, legacy skill prompt, control authority

**Goal Execution Document**:
A `/goal` entry document projected from the compiled execution packet as the human execution scope for long-running host execution.
_Avoid_: machine-readable authority, closeout proof, independent requirement source, standalone completion evidence

**TaskReport**:
The packet-bound JSON return artifact that reports execution status, changed files, validations, evidence, and downstream context back to Main Agent orchestration.
_Avoid_: chat summary, stdout, exit code, audit receipt, goal completion marker

**Native Goal TaskReport Handoff**:
The controlled handoff that binds a compiled prompt packet, Goal Execution Document, audit receipt, TaskReport path, and return action so host-native `/goal` execution can return to Main Agent orchestration.
_Avoid_: standalone `/goal` command, worker adapter execution, unbound receipt, ad hoc report path

**BMADS Runtime Console**:
The user-facing runtime view that projects the current RequirementRecord safety route and next safe action for `bmads` and `bmad-speckit`.
_Avoid_: legacy help surface, BMAD workflow catalog, completion authority, source authority

**Governed Orphan Work-Product**:
A non-story SDD work-product that lacks an Epic/Story projection but is still bound to an active requirement record, entry flow, and dispatch packet.
_Avoid_: loose orphan artifact, unowned implementation artifact, closeout proof

**Controlled Integration Extension**:
...
> **Dev:** "Can the worker adapter add execution discipline only outside `goal_execution.md`?"
> **Domain expert:** "No. An **Execution Discipline Profile** must be rendered by the compiler into every relevant **Compiled Prompt Artifact**, including the **Goal Execution Document** when `/goal` is used."

> **Dev:** "Can a standalone or bugfix audit report in `_orphan/` count as closeout evidence?"
> **Domain expert:** "Only if it is a **Governed Orphan Work-Product**: bound to the active requirement record, entry flow, and dispatch packet, then indexed as review evidence. Loose orphan files remain compatibility artifacts."
...
- "goal document" could have meant machine authority or completion proof; resolved: a **Goal Execution Document** is human execution scope, while the model packet remains machine-readable authority and the **TaskReport** carries return evidence.
- "TaskReport" could have meant a chat summary or worker-only artifact; resolved: **TaskReport** is the packet-bound JSON return artifact accepted by Main Agent orchestration.
- "native /goal handoff" could have meant a standalone command; resolved: use **Native Goal TaskReport Handoff** to bind the packet, goal document, audit receipt, TaskReport path, and return action.
- "BMADS help" could have meant the user-facing runtime console; resolved: **BMADS Runtime Console** is the `bmads` and `bmad-speckit` runtime view, while legacy help surfaces remain mirrors.
