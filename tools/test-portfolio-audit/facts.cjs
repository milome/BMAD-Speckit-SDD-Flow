...
function normalizeFilesystemCandidates(repoRoot, candidates) {
  return stableUnique(
    candidates
      ...
      .filter((testPath) => !isFixtureAssetPath(testPath))
      .filter(
...
function excludeFixtureAssets(result) {
  return {
    ...result,
    tests: result.tests.filter((test) => !isFixtureAssetPath(test.testPath)),
    configuredCandidateRefs: (result.configuredCandidateRefs || []).filter(
      (reference) => !isFixtureAssetPath(reference.testPath)
    ),
  };
}
...
function criticalBindingSelectionRefs(packageRecords, criticalAuthorityPackagePaths) {
  const authorityPaths = new Set(criticalAuthorityPackagePaths);
  const selectionRefs = new Map();
  for (const record of packageRecords) {
    if (!authorityPaths.has(record.packagePath)) continue;
    const bindings = record.packageJson?.testPortfolioAudit?.criticalBindings;
    if (!Array.isArray(bindings)) continue;
    for (const [bindingIndex, binding] of bindings.entries()) {
      const scriptName = typeof binding?.scriptName === 'string' ? binding.scriptName.trim() : '';
      if (!scriptName) continue;
      selectionRefs.set(
        `source:${record.packagePath}#testPortfolioAudit.criticalBindings[${bindingIndex}]`,
        `script:${scriptName}`
      );
    }
  }
...
function isNestedFixturePackage(record) {
  return String(record.packagePath || '')
    .replace(/\\/g, '/')
    .startsWith('tests/fixtures/');
}
...
function discoverGenericNodeTests(repoRoot, packageRecords) {
  ...
  const issues = [];
  for (const record of packageRecords) {
    if (!record.packageJson || isNestedFixturePackage(record)) continue;
    for (const [scriptName, command] of scriptEntries(record)) {
...
function discoverConfiguredTests({ repoRoot }) {
  ...
  try {
    ...
    return {
      ...
      criticalBindingSelectionRefs: new Map(),
      runnerResults: [],
      issues: [
  ...
  const criticalAuthorityPackagePaths = selectCriticalAuthorityPackagePaths(packageRecords);
  const runnerResults = [];
  ...
  return {
    failed: false,
    packagePaths,
    criticalAuthorityPackagePaths,
    criticalBindingSelectionRefs: criticalBindingSelectionRefs(
      packageRecords,
      criticalAuthorityPackagePaths
    ),
    runnerResults,
...
function attachCriticalBindingSelectionRefs(analyzerResults, selectionRefs) {
  return analyzerResults.map((result) => {
    if (result.dimension !== 'criticality') return result;
    return {
      ...result,
      findings: (result.findings || []).map((finding) => ({
        ...finding,
        bindings: (finding.bindings || []).map((binding) => {
          const selectionRef = selectionRefs.get(binding.evidenceRef);
          return selectionRef ? { ...binding, selectionRef } : binding;
        }),
      })),
    };
  });
...
async function collectAuditFacts(options) {
  ...
  let analyzerResults;
  try {
    analyzerResults = attachCriticalBindingSelectionRefs(
      await runAnalyzersIndependently({
        repoRoot: options.repoRoot,
        inventory,
        routeGraph,
      ...
      discoveryRun.criticalBindingSelectionRefs
    );
  } finally {
