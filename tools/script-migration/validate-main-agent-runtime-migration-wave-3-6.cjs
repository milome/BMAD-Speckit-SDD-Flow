...
const MANIFEST_PATH = 'repo-governance/script-migrations/main-agent-runtime-migration-wave-3.6/candidate-manifest.json';
const EVIDENCE_PATH = 'repo-governance/script-migrations/main-agent-runtime-migration-wave-3.6/evidence.json';
const SUMMARY_PATH = 'repo-governance/script-migrations/main-agent-runtime-migration-wave-3.6/summary.md';
const PACKAGE_TEST_PATH = 'packages/bmad-speckit/tests/main-agent-wave-3-6-runtime-actions.test.js';
const ACCEPTANCE_TEST_PATH = 'tests/acceptance/main-agent-runtime-migration-wave-3-6-contract.test.ts';
const WAVE_ID = 'main-agent-runtime-migration-wave-3.6';
const REFINES_WAVE_ID = 'main-agent-runtime-migration-wave-3.5';
...
function validateManifest(errors) {
  ...
  const p1Rows = parsePriorityMatrix(matrixText);
  const validated = registryValidatedScripts(registryText, WAVE_ID);
  const expected = p1Rows.filter((row) => !validated.has(row.script));
  const validatedP1 = p1Rows.filter((row) => validated.has(row.script));
  ...
  expectEqual(errors, 'manifest.selection.priority', manifest.selection?.priority, 'P1');
  expectEqual(errors, 'manifest.selection.matrixTotal', manifest.selection?.matrixTotal, 31);
  expectEqual(errors, 'manifest.selection.alreadyValidated', manifest.selection?.alreadyValidated, 9);
  expectEqual(errors, 'manifest.selection.remaining', manifest.selection?.remaining, 22);
  ...
  expectEqual(errors, 'P1 matrix total', p1Rows.length, 31);
  expectEqual(errors, 'P1 validated count', validatedP1.length, 9);
  expectEqual(errors, 'P1 remaining count', expected.length, 22);
  ...
  for (const pathName of expectedPaths) {
    if (!actualPaths.includes(pathName)) errors.push(`manifest missing P1 remaining script ${pathName}`);
  }
  ...
  for (const row of expected) {
    ...
    expectEqual(errors, `${row.script} priority`, entry.priority, 'P1');
...
function validatePackageCliDispatch(entries, errors) {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'main-agent-wave-3-6-cli-'));
  try {
    for (const entry of entries) {
