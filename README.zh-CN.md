# BMAD-SpeCKit-SDD-Flow：需求契约驱动的主 Agent 编排
...
BMAD-SpeCKit-SDD-Flow 是一个需求契约驱动的 AI 辅助软件交付编排层。它结合 BMAD-METHOD 的产品与交付结构，以及 Spec-Kit 的规格驱动开发流程，并在此基础上为 Cursor、Claude Code 和 Codex 增加主 Agent 治理控制面。

它不是要替代 BMAD 或 Spec-Kit，而是让从产品意图到技术实现的完整路径更安全、更可追溯，也更适合由 AI Agent 执行。工作流通过 CLI 安装到消费项目，然后在 Codex、Claude Code CLI 或 Cursor 中通过 `bmads` / `bmad-speckit` 技能激活主控流程。
