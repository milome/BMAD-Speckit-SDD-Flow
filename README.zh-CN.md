...
<p align="center">
  ...
  <em>v1 整合 BMAD + Speckit 全流程；v2 将这条流程升级为 AI-TDD 控制面和主 Agent 编排系统。</em>
</p>
...
它的目标很明确：让 AI 在需求契约、门禁、证据和追溯链内执行。v1 把 BMAD 与 Speckit 串成完整交付流；v2 以 AI-TDD、六个心智模型和双门禁为核心，把交付流升级为需求契约驱动的 Agent 编排系统。
...
如果你是通过 CI artifact 安装而不是直接走 npm registry，推荐的非侵入式路径就是把上面的 `--package` 换成 `./bmad-speckit-sdd-flow-<version>.tgz`。
...
本项目中的 AI-TDD 指 Manifest 级的验收驱动开发。Manifest 是需求契约矩阵，承载 `MUST`、`MUST NOT`、`NOT DONE`、`EVIDENCE`、`TRACE MATRIX` 等可被人和 Agent 共同验证的定义。
...
在 Codex、Claude Code CLI 或 Cursor 中，从宿主技能开始：
...
| 输出 | 含义 |
| --- | --- |
| 当前需求 | 当前活动的 requirement record 或 requirement set。 |
| 当前心智模型 | 六个 AI-TDD 阶段中，哪个阶段拥有下一步判断权。 |
| 阻塞项 | 缺少确认、架构陈旧、readiness 失败、审计 finding 或证据缺口。 |
| 推荐下一步 | 下一步受控动作，而不是泛泛的编码建议。 |
| 需要补齐的证据 | Manifest、Trace Matrix、测试、命令、产物、评分、审计记录或 closeout proof。 |

不要绕过 Implementation Readiness Gate 直接让实现 Agent 开工。不要用 dashboard green、score green、task done 或聊天信心来宣称交付。交付只能通过 Delivery Closeout Gate 和当前证据链关闭。
...
下面的 CLI 截图是公开 npm 接口的证据，不是用户日常主流程。
...
v1 的 BMAD + Speckit 资产仍然是兼容面：Product Brief、PRD、Architecture、Epic/Story、Speckit specify/plan/GAPS/tasks、实现、审计、评分、看板、Coach 和 SFT extraction 仍然有用。

v2 README 不再以旧五层架构图作为主图。当前主架构是 AI-TDD 工具链生态和主 Agent 控制面。v1 产物是控制面里的输入和投影，而不是需求契约权威的替代品。
