...
3. Inspect current project runtime state via `_bmad-output/runtime/` and the main-agent orchestration surface.
4. Route execution through `main-agent-orchestration` / `main-agent-unified-ingress` for the active host.
5. Keep `bmad-help` as a state-aware recommendation helper, not as the root takeover alias.
