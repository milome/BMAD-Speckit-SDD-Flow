...
description: "Execute unfinished tasks from a user-provided TASKS/BUGFIX document via subagents only. Use when the user asks to implement unfinished items from a named TASKS or BUGFIX markdown file. Enforces Codex worker adapter for implementation, ralph-method (prd + progress, TDD), speckit-workflow (no pseudo-implementation, run acceptance commands), and code-reviewer-style audit with Critical Auditor >50% and three consecutive no-gap rounds. The main Agent must NOT edit production code."
---
