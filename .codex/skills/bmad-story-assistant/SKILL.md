...
description: "BMAD Story Assistant: Execute the complete Create Story → Audit → Dev Story → Post-Implementation Audit workflow by Epic/Story number. Phase zero: Automatically detect and patch party-mode display name optimization in new projects/worktree (if _bmad exists and is not optimized). Use subagent to perform tasks; the audit step prioritizes scheduling code-reviewer (.codex/agents/ or .codex/agents/) through Codex worker dispatch, and falls back to Codex worker adapter general-purpose if it fails. Fol"
---
