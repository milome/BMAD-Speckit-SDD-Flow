...
exports.coachDiagnose = coachDiagnose;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const veto = __importStar(require("../veto"));
const path_1 = require("../constants/path");
const cluster_weaknesses_1 = require("../analytics/cluster-weaknesses");
...
const MIN_SAFE_FALLBACK_PERSONA = {
    role: 'AI Code Coach + Iteration Gate Keeper',
    identity: 'Senior-engineer lens: industrial-grade deliverables. Diagnosis uses only existing scoring and audit artifacts.',
...
function isSkillAvailable(skillPath, forceSkillLoadError) {
...
function readManifestAgentRow(manifestPath, agentName) {
...
function parsePersonaYamlBlock(markdownSource) {
...
function loadPersonaFromMarkdownFile(personaPath) {
...
function loadAiCoachPersona(options) {
    const manifestPath = resolveAbsolutePath(options.personaManifestPath ?? '_bmad/_config/agent-manifest.csv');
    ...
    const explicitPersonaPath = resolveAbsolutePath(options.personaPath ?? '_bmad/scoring/agents/ai-coach.md');
...
function buildRecommendations(hasStageVeto, epicTriggered, fallbackMode, persona) {
    const recommendations = [];
    if (fallbackMode) {
        recommendations.push('Fallback mode: end-to-end Skill unavailable; diagnosis uses existing scoring data only.');
    }
    if (hasStageVeto) {
        recommendations.push('Stage veto triggered: fix the corresponding check items before the next iteration.');
    }
    if (epicTriggered) {
        recommendations.push('Epic veto triggered: prioritize high-risk issues that match the trigger conditions.');
    }
    if (!hasStageVeto && !epicTriggered) {
        recommendations.push('No veto this run: continue improving stability and maintainability of lower-scoring stages.');
    }
    const boundaryRule = persona.principles.find((item) => /Reviewer|audit flow|does not replace|Do not run/i.test(item) ||
        item.includes('不替代') ||
        item.includes('不执行') ||
        item.includes('审计')) ?? 'Does not replace Reviewer; does not run new audit flows.';
    if (!recommendations.includes(boundaryRule)) {
        recommendations.push(boundaryRule);
    }
    return recommendations;
}
function buildWeakAreas(phaseScores) {
 ...
 */
async function coachDiagnose(runId, options = {}) {
    ...
    if (options.records != null && options.records.length > 0) {
        ...
        records = (0, loader_1.loadRunRecords)(normalizedRunId, options.dataPath);
    }
    if (records.length === 0) {
        return { error: 'run_not_found', run_id: normalizedRunId };
    }
    const config = (0, config_1.loadCoachConfig)(options.configPath);
    ...
    const rulesDir = options.rulesDir ?? (0, path_1.resolveRulesDir)();
    const scored = records.map((record) => {
        const result = veto.applyTierAndVeto({ ...record, raw_phase_score: record.phase_score }, { rulesDir });
    ...
    const storyRecords = scored.map((item) => ({
    ...
    const passedStoryCount = options.passedStoryCount ??
    ...
    const journeyContractHints = (0, journey_contract_remediation_1.buildJourneyContractRemediationHints)(records);
    ...
    let weaknessClusters;
    try {
        weaknessClusters = (0, cluster_weaknesses_1.clusterWeaknesses)(records);
    }
    catch {
        weaknessClusters = [];
    }
    const stageEvolutionTraces = {};
    for (const item of scored) {
    ...
    const report = {
        ...
        stage_evolution_traces: Object.keys(stageEvolutionTraces).length > 0 ? stageEvolutionTraces : undefined,
    ...
    const forbiddenWords = (0, forbidden_1.loadForbiddenWords)(options.forbiddenWordsPath);
    const validationText = `${report.summary}\n${report.recommendations.join('\n')}`;
    const forbiddenValidation = (0, forbidden_1.validateForbiddenWords)(validationText, forbiddenWords);
    ...
    if (forbiddenValidation.warnings.length > 0) {
        report.recommendations.push(`Wording alert: ambiguous terms detected (${forbiddenValidation.warnings.join(', ')}). Replace with explicit commitments.`);
    }
    return report;
}
