...
exports.extractAuditReportSections = extractAuditReportSections;
exports.extractSpecAssistantTargets = extractSpecAssistantTargets;
exports.extractPlanAssistantTargets = extractPlanAssistantTargets;
exports.extractTasksAssistantTargets = extractTasksAssistantTargets;
exports.extractStoryAssistantTargets = extractStoryAssistantTargets;
exports.extractSpecCoverageAssistantTargets = extractSpecCoverageAssistantTargets;
exports.extractGapsAssistantTargets = extractGapsAssistantTargets;
exports.extractImplementAssistantTargets = extractImplementAssistantTargets;
const CRITIC_HEADING_RE = /##\s*(?:\d+\.\s*)?(?:批判审计员结论|Critical Auditor Conclusion)\s*\n([\s\S]*?)(?=##\s|$)/i;
const ROUND_CONCLUSION_RE = /\*\*(?:本轮结论|Round conclusion)\*\*[：:]\s*([^\n]+(?:\n(?!\*\*)[^\n]*)*)/i;
const NO_GAP_RE = /(?:本轮无新\s*gap|no new gaps?)/i;
const GAP_ITEM_SPLIT_RE = /[；;]\s*|\d+\)\s*|①\s*|②\s*|③\s*|④\s*|⑤\s*/;
const SUGGESTIONS_HEADING_RE = /##\s*(?:修改建议|Suggestions|Modification suggestions)\s*\n([\s\S]*?)(?=##\s|$)/i;
const SUGGESTIONS_BOLD_RE = /\*\*(?:修改建议|Suggestions)\*\*[：:]\s*([^\n]+(?:\n(?!\*\*)[^\n]*)*)/i;
const SUGGESTIONS_TABLE_RE = /\|\s*Gap\s*\|\s*(?:修改建议|Suggestion|Recommendation)\s*\|[\s\S]*?\n\|[-:\s|]+\|\s*\n([\s\S]*?)(?=\n\n|##\s|$)/i;
const REQUIRED_FIX_SECTION_RE = /##\s*(?:§\d+\s*)?(?:发现的问题\s*\(Required Fixes\)|Required Fixes|发现的问题)\s*\n([\s\S]*?)(?=##\s|$)/i;
const REQUIRED_FIX_HEADING_RE = /###\s+(?:Gap[^\n]*)\n[\s\S]*?\*\*建议\*\*[：:]\s*([^\n]+(?:\n(?!\*\*|###|##)[^\n]*)*)/g;
 ...
 */
function extractAuditReportSections(content) {
...
function parseCriticConclusion(content) {
...
function parseGapBlock(block) {
    const trimmed = block.trim();
    if (!trimmed)
        return [];
    const parts = trimmed
        .split(GAP_ITEM_SPLIT_RE)
        .map((s) => s.trim())
        .filter(Boolean);
    return parts;
}
...
function parseRequiredFixes(content) {
...
function extractSpecAssistantTargets(content) {
...
function extractPlanAssistantTargets(content) {
    const sections = extractAuditReportSections(content);
    const lines = content
        .split(/\r?\n/)
        .map((line) => line.trim())
    ...
    const planCoverageRows = content
        ...
        .split('|')
        .map((cell) => cell.trim()))
        .filter((cell) => cell.includes('未覆盖') ||
        cell.includes('缺失') ||
        cell.includes('建议') ||
        cell.includes('模糊') ||
        cell.includes('fallback'));
    const recommendationBlocks = [
        ...content.matchAll(/###\s+(未通过项与修改建议|可补充项|模糊表述与术语歧义)[\s\S]*?(?=###\s+|##\s+|$)/g),
    ]
...
function extractTasksAssistantTargets(content) {
    ...
    const taskBodyLines = lines
        .map((line) => line.trim())
        .filter((line) => /^-\s*\[(?: |x)\]\s*T\d+(?:\.\d+)?/.test(line) ||
        /^-\s*\[(?: |x)\]\s+.+internalYes/.test(line))
    ...
    const quotedModifyLines = lines
        .map((line) => line.trim())
        .filter((line) => line.startsWith('- **修改**：') ||
        line.startsWith('- **内容**：') ||
        line.startsWith('- **验收**：'))
...
function extractStoryAssistantTargets(content) {
    const sections = extractAuditReportSections(content);
    ...
    return [
        ...new Set([...sections.requiredFixes, ...sections.suggestions, ...conclusions].filter(Boolean)),
    ];
}
function extractSpecCoverageAssistantTargets(content) {
    const lines = content.split(/\r?\n/).map((line) => line.trim());
    const coverageRows = lines
        .filter((line) => line.startsWith('|') &&
        (line.includes('❌') || line.includes('⚠️') || line.includes('建议')))
    ...
    const sectionBullets = [
        ...
        .trim()))
        .filter(Boolean);
    const coverageConclusionLines = lines.filter((line) => line.includes('未完全通过') ||
        line.includes('未达到') ||
        line.includes('建议后续动作') ||
        line.includes('覆盖度小结'));
    return [
        ...new Set([
            ...coverageRows,
            ...
            ...issueBullets,
            ...sectionBullets,
            ...coverageConclusionLines,
        ].filter(Boolean)),
    ];
}
function extractGapsAssistantTargets(content) {
    const sections = extractAuditReportSections(content);
...
function extractImplementAssistantTargets(content) {
    const sections = extractAuditReportSections(content);
    ...
    const acceptanceCells = acceptanceRows
        .flatMap((row) => row.split('|').map((cell) => cell.trim()))
        .filter((cell) => cell.includes('AC-') ||
        cell.includes('accept:') ||
        cell.includes('PASS') ||
        cell.includes('parse-and-write'));
    const sectionHeadings = content
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter((line) => /^###\s+/.test(line))
        .map((line) => line.replace(/^###\s+/, '').trim())
        .filter((line) => line.includes('任务') ||
        line.includes('生产代码') ||
        line.includes('验收') ||
        line.includes('批判审计员'));
    const implementPriorityBlocks = [
        ...content.matchAll(/##\s+(审计结论|批判审计员结论|strict 3 轮收敛记录)[\s\S]*?(?=##\s+|$)/g),
    ]
        .flatMap((match) => (match[0] ?? '')
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter((line) => line.length > 0 && !line.startsWith('## ')))
        .filter((line) => line.includes('完全覆盖') ||
        line.includes('验证通过') ||
    ...
    const scopeAndRequirementRows = content
        ...
        .map((line) => line.trim())
        .filter((line) => line.startsWith('|') &&
        (line.includes('Story 需求') ||
            line.includes('需求 | 实现 | 测试/验收') ||
            line.includes('本 Story 范围') ||
            line.includes('验收标准')))
        .flatMap((row) => row
        .replace(/^\|/, '')
        .replace(/\|$/, '')
        .split('|')
        .map((cell) => cell.trim()))
        .filter((cell) => cell.includes('parseAndWriteScore') ||
        cell.includes('除零') ||
        cell.includes('覆盖') ||
        cell.includes('验收'));
    const scopeTableCells = content
        ...
        .map((line) => line.trim())
        .filter((line) => line.startsWith('|') &&
        (line.includes('add 函数') ||
            line.includes('subtract 函数') ||
            line.includes('multiply 函数') ||
            line.includes('divide 函数') ||
            line.includes('parseAndWriteScore')))
        .flatMap((row) => row
        .replace(/^\|/, '')
        .replace(/\|$/, '')
        .split('|')
    ...
    const auditSummaryLines = content
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter((line) => line.includes('复核结论') ||
    ...
    const emphasizedSummaryLines = content
        ...
        .filter((line) => line.includes('TDD 完整性') ||
        line.includes('完全覆盖、验证通过') ||
        line.includes('关键路径与孤岛'));
    return [
        ...new Set([
            ...
            ...acceptanceCells,
            ...sectionHeadings,
            ...implementPriorityBlocks,
            ...scopeAndRequirementRows,
            ...scopeTableCells,
            ...auditSummaryLines,
            ...emphasizedSummaryLines,
            ...sections.suggestions,
            ...sections.requiredFixes,
            ...completionHints,
        ].filter(Boolean)),
