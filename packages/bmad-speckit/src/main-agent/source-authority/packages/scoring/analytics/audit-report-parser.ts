...
const CRITIC_HEADING_RE =
  /##\s*(?:\d+\.\s*)?(?:批判审计员结论|Critical Auditor Conclusion)\s*\n([\s\S]*?)(?=##\s|$)/i;
...
const STORY_COMPLETE_RE =
  /(?:审计结论|批判审计员结论)[\s\S]*?(?:建议主 Agent 将本 Story 标记为完成|完全覆盖、验证通过|本轮无新 gap)/i;
...
export function extractPlanAssistantTargets(content: string): string[] {
  ...
  const recommendationBlocks = [
    ...content.matchAll(
      /###\s+(未通过项与修改建议|可补充项|模糊表述与术语歧义)[\s\S]*?(?=###\s+|##\s+|$)/g
    ),
  ]
    .flatMap((match) =>
      (match[0] ?? '')
...
export function extractStoryAssistantTargets(content: string): string[] {
  ...
  return [
    ...new Set(
      [...sections.requiredFixes, ...sections.suggestions, ...conclusions].filter(Boolean)
    ),
  ];
}
...
export function extractSpecCoverageAssistantTargets(content: string): string[] {
  ...
  const coverageConclusionLines = lines.filter(
    (line) =>
      line.includes('未完全通过') ||
      line.includes('未达到') ||
...
export function extractImplementAssistantTargets(content: string): string[] {
  ...
  const taskRows = content
    ...
    .map((line) => line.trim())
    .filter(
      (line) => line.startsWith('| T') || line.includes('验收命令') || line.includes('建议主 Agent')
    );
  ...
  const scopeAndRequirementRows = content
    ...
    .filter(
      (cell) =>
        cell.includes('parseAndWriteScore') ||
        cell.includes('除零') ||
        cell.includes('覆盖') ||
        cell.includes('验收')
    );
