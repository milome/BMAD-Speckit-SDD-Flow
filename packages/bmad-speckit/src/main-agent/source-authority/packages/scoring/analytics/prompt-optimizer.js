...
exports.formatPromptSuggestionsMarkdown = formatPromptSuggestionsMarkdown;
/**
...
function keywordOverlap(keywords, content) {
...
function priorityFromFrequency(frequency) {
    if (frequency >= 5)
        return 'high';
    if (frequency >= 3)
        return 'medium';
 ...
 */
function generatePromptSuggestions(clusters, skillsDir) {
 ...
 */
function formatPromptSuggestionsMarkdown(suggestions) {
    if (suggestions.length === 0)
        return '# Prompt optimization suggestions\n\n(none)\n';
    const lines = [
