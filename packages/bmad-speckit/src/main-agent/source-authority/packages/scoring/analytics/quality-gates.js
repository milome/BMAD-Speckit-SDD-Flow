...
exports.applyQualityGates = applyQualityGates;
function requiresCodePair(sample, options) {
    if (options.requireCodePair != null) {
        return options.requireCodePair;
    }
    return sample.metadata.sample_kind !== 'documentation';
}
function hasOverTokenLimit(sample, maxTokens) {
    return sample.quality.token_estimate > maxTokens;
}
function hasUserAndAssistantMessages(sample) {
...
function hasAssistantTarget(sample) {
    return sample.messages.some((message) => message.role === 'assistant' &&
        (typeof message.content === 'string'
            ? message.content.trim().length > 0
            : message.content.some((part) => part.text.trim().length > 0)));
}
function usesTooling(sample) {
    return (Boolean(sample.tools && sample.tools.length > 0) ||
        sample.messages.some((message) => message.role === 'tool' || Boolean(message.tool_calls && message.tool_calls.length > 0)));
}
function withCompatibility(compatible, reasons, warnings) {
    return {
        compatible,
        reasons,
        warnings,
...
function hasUnresolvedFinding(sample, matcher) {
    return sample.redaction.findings.some((finding) => matcher(finding.kind) && finding.action !== 'redact');
}
function applyQualityGates(sample, options = {}) {
    ...
    const effectiveTraceCompleteness = sample.redaction.status === 'blocked' ? 'blocked' : sample.quality.trace_completeness;
    ...
    const acceptanceDecision = hardReasons.length > 0 ? 'rejected' : softReasons.length > 0 ? 'downgraded' : 'accepted';
    ...
    return {
        ...
        export_compatibility: {
            ...
            hf_conversational: toolAware
                ? withCompatibility(false, unique([...sharedHardReasons, 'target_incompatible_hf_conversational']), sharedWarnings)
            ...
            hf_tool_calling: sample.tools && sample.tools.length > 0
                ? withCompatibility(sharedHardReasons.length === 0, sharedHardReasons, sharedWarnings)
                : withCompatibility(false, unique([...sharedHardReasons, 'target_incompatible_hf_tool_calling']), sharedWarnings),
        },
    };
}
