...
exports.assignDedupeClusters = assignDedupeClusters;
exports.buildDatasetDuplicateSummary = buildDatasetDuplicateSummary;
exports.buildDatasetBalanceSummary = buildDatasetBalanceSummary;
exports.buildDatasetTrainingViewSummary = buildDatasetTrainingViewSummary;
...
function resolveSourceScope(sample) {
    ...
    const sourcePath = normalizePath(sample.provenance.source_path ?? sample.source.artifact_refs[0]?.path ?? null);
    if (sourcePath.includes('/_orphan/') ||
        /(?:^|\/)bugfix[_-]/i.test(sourcePath) ||
        sourcePath.includes('/standalone_tasks/')) {
        return 'orphan_scoped';
    }
    if (/\/epic-[^/]+\/story-[^/]+/i.test(sourcePath) ||
...
function assignDedupeClusters(samples) {
    const groups = new Map();
    for (const sample of samples) {
        const fingerprint = sampleFingerprint(sample);
        const clusterId = `dup-${(0, hash_1.computeStringHash)(fingerprint).slice(0, 12)}`;
...
function buildDatasetDuplicateSummary(samples) {
...
function buildDatasetBalanceSummary(samples) {
...
function buildDatasetTrainingViewSummary(samples) {
    const readySamples = samples.filter((sample) => sample.quality.acceptance_decision === 'accepted' && sample.quality.training_ready === true);
    return {
        assistant_only_ready: readySamples.filter((sample) => hasAssistantTarget(sample)).length,
        completion_only_ready: readySamples.filter((sample) => hasAssistantTarget(sample) && !usesTooling(sample)).length,
