...
exports.summarizeGovernanceRoutingModeDistribution = summarizeGovernanceRoutingModeDistribution;
exports.summarizeGovernanceSignalHotspots = summarizeGovernanceSignalHotspots;
exports.summarizeGovernanceRerunGateFailureTrend = summarizeGovernanceRerunGateFailureTrend;
function effectiveStage(record) {
    return record.trigger_stage === 'speckit_5_2' ? 'implement' : record.stage;
}
function allGovernanceEvents(records) {
    return records.flatMap((record) => (record.governance_rerun_history ?? []).map((event) => ({ record, event })));
}
 ...
 */
function summarizeGovernanceRoutingModeDistribution(records) {
 ...
 */
function summarizeGovernanceSignalHotspots(records) {
 ...
 */
function summarizeGovernanceRerunGateFailureTrend(records) {
    ...
    return [...byGate.entries()]
        ...
        const trend = !firstFailed && latestFailed
            ? 'worsening'
