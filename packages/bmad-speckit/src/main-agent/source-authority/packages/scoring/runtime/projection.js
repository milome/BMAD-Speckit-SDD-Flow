...
exports.buildRunProjection = buildRunProjection;
function isRecord(value) {
    return value != null && typeof value === 'object' && !Array.isArray(value);
}
function isRuntimeRunStatus(value) {
...
function mergeScope(currentScope, eventScope) {
    if (eventScope == null) {
        return currentScope;
    }
    const normalizedEventScope = Object.fromEntries(Object.entries(eventScope).filter(([, value]) => value !== undefined));
    return {
        ...(currentScope ?? {}),
        ...normalizedEventScope,
    };
}
function upsertStageProjection(stageHistory, stage, patch) {
...
function maybePushScoreRef(scoreRefs, event) {
    if (!isRecord(event.payload)) {
        return;
    }
    const scoreRecordId = typeof event.payload.score_record_id === 'string' ? event.payload.score_record_id : undefined;
...
function maybePushArtifactRef(artifactRefs, event) {
    ...
    artifactRefs.push({
        ...
        content_hash: typeof event.payload.content_hash === 'string' ? event.payload.content_hash : undefined,
    });
}
function maybePushDatasetCandidateRef(candidateRefs, event) {
    if (!isRecord(event.payload) || typeof event.payload.sample_id !== 'string') {
        return;
    }
    candidateRefs.push({
        sample_id: event.payload.sample_id,
        status: event.payload.status === 'accepted' ||
            event.payload.status === 'rejected' ||
            event.payload.status === 'downgraded'
            ? event.payload.status
            : undefined,
        split_assignment: event.payload.split_assignment === 'train' ||
...
function createEmptyProjection(runId) {
    return {
        run_id: runId,
        status: 'pending',
        current_stage: null,
...
function buildRunProjection(events, runId) {
    ...
    for (const event of relevantEvents) {
        ...
        switch (event.event_type) {
            ...
            case 'stage.completed': {
                if (event.stage) {
                    const nextStatus = isRecord(event.payload) && isRuntimeRunStatus(event.payload.status)
                        ? event.payload.status
                        : 'passed';
                    projection.current_stage = event.stage;
