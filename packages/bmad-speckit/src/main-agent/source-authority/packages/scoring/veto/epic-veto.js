...
exports.evaluateEpicVeto = evaluateEpicVeto;
 ...
 */
function evaluateEpicVeto(input, options) {
    const triggered = [];
    ...
    if (vetoCount >= 3)
        triggered.push(CONDITION_IDS[0]);
    // ② 需求交付率 < 80%
    if (passedStoryCount != null && epicStoryCount > 0) {
        if (passedStoryCount / epicStoryCount < 0.8)
            triggered.push(CONDITION_IDS[1]);
    }
    // ③ 高危漏洞 ≥ 2
    const vetoIds = (0, veto_1.buildVetoItemIds)(options);
    let highRiskCount = 0;
    for (const r of storyRecords) {
        for (const c of r.check_items) {
            if (vetoIds.has(c.item_id) && !c.passed)
                highRiskCount++;
        }
    }
    if (highRiskCount >= 2)
        triggered.push(CONDITION_IDS[2]);
    // ④ 测试通过率 < 80%
    if (testStats != null && testStats.total > 0) {
        if (testStats.passed / testStats.total < 0.8)
            triggered.push(CONDITION_IDS[3]);
    }
    // ⑤ 整改≥4 次未通过 Story 数 ≥1
    const iter4Fail = storyRecords.filter((r) => r.iteration_count >= 4 && (r.veto_triggered || r.phase_score === 0));
    if (iter4Fail.length >= 1)
        triggered.push(CONDITION_IDS[4]);
    // ⑥ 一次通过率 < 50%
    ...
    if (iter3Count >= 2)
        triggered.push(CONDITION_IDS[6]);
    // ⑧ 致命问题整改≥3 次 Story 数 ≥1
    const fatalIter3 = storyRecords.filter((r) => {
        const fatalFails = r.iteration_records.filter((rec) => rec.result === 'fail' && rec.severity === 'fatal');
        return fatalFails.length >= 3;
    });
    if (fatalIter3.length >= 1)
        triggered.push(CONDITION_IDS[7]);
    return {
