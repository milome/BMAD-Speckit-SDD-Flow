...
exports.renderDashboardSnapshotMarkdown = renderDashboardSnapshotMarkdown;
exports.writeDashboardSnapshotFiles = writeDashboardSnapshotFiles;
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
function ensureDir(filePath) {
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
}
function renderDashboardSnapshotMarkdown(markdown, snapshot, includeRuntime = false) {
    if (!includeRuntime) {
        return markdown.endsWith('\n') ? markdown : `${markdown}\n`;
    }
    const lines = [markdown.trimEnd(), '', '## Six Mental Models', ''];
    lines.push(`- Control Authority: ${snapshot.six_model_projection.controlAuthority.allowedControlSource}`);
    lines.push(`- Dashboard Can Close Requirement: ${snapshot.six_model_projection.controlAuthority.dashboardCanCloseRequirement ? 'yes' : 'no'}`);
    lines.push(`- Can Affect Control Flow: ${snapshot.six_model_projection.canAffectControlFlow ? 'yes' : 'no'}`);
    lines.push(`- Forbidden Completion Sources: ${snapshot.six_model_projection.controlAuthority.forbiddenCompletionSources.join(', ')}`);
    lines.push('');
    ...
    for (const model of snapshot.six_model_projection.models) {
        lines.push(`| ${model.displayName} (${model.id}) | ${model.status} | ${model.coreQuestion} | ${model.evidenceSignals.join('<br>')} | ${model.drillDownSliceRefs.length > 0 ? model.drillDownSliceRefs.join('<br>') : 'N/A'} |`);
    }
    lines.push('');
    ...
    lines.push('|------------|-------|-----------|');
    lines.push(`| stories | ${snapshot.six_model_projection.entryFlowSlices.stories.length} | ${snapshot.six_model_projection.entryFlowSlices.stories
        .map((item) => item.sliceId)
        .join('<br>') || 'N/A'} |`);
    lines.push(`| bugfixes | ${snapshot.six_model_projection.entryFlowSlices.bugfixes.length} | ${snapshot.six_model_projection.entryFlowSlices.bugfixes
        .map((item) => item.sliceId)
        .join('<br>') || 'N/A'} |`);
    lines.push(`| standaloneTasks | ${snapshot.six_model_projection.entryFlowSlices.standaloneTasks.length} | ${snapshot.six_model_projection.entryFlowSlices.standaloneTasks
        .map((item) => item.sliceId)
        .join('<br>') || 'N/A'} |`);
    lines.push(`| epics | ${snapshot.six_model_projection.entryFlowSlices.epics.length} | ${snapshot.six_model_projection.entryFlowSlices.epics
        .map((item) => item.sliceId)
        .join('<br>') || 'N/A'} |`);
    lines.push('');
    ...
    if (snapshot.runtime_context.reviewer_contract) {
        ...
        lines.push(`- Shared Core: ${snapshot.runtime_context.reviewer_contract.sharedCore
            ...
            : 'N/A'}`);
        lines.push(`- Active Reviewer Consumer: ${snapshot.runtime_context.reviewer_contract.activeAuditConsumer
        ...
        const executionHost = snapshot.execution_state.dispatched_host ??
            snapshot.execution_state.configured_authoritative_host ??
            null;
        if (executionHost && reviewerRoute?.hosts?.[executionHost]) {
            const currentCarrier = reviewerRoute.hosts[executionHost];
            lines.push(`- Current Carrier: ${executionHost} -> ${currentCarrier.carrierSourcePath} -> ${currentCarrier.runtimeTargetPath}`);
        }
        lines.push(`- Cursor Route: ${reviewerRoute
            ? `${reviewerRoute.hosts.cursor.preferredRoute.tool}/${reviewerRoute.hosts.cursor.preferredRoute.subtypeOrExecutor}`
            : 'N/A'}`);
        lines.push(`- Claude Route: ${reviewerRoute
        ...
        lines.push(`- Rollout Gate: ${snapshot.runtime_context.reviewer_contract.rolloutGate
    ...
    if (snapshot.runtime_context.latest_reviewer_closeout) {
        ...
        lines.push(`- Required Fixes: ${closeout.required_fixes.length > 0 ? closeout.required_fixes.join(', ') : 'N/A'}`);
    ...
    for (const entry of snapshot.stage_timeline) {
        lines.push(`| ${entry.stage} | ${entry.status} | ${entry.phase_score ?? 'N/A'} | ${entry.veto_triggered ? 'yes' : 'no'} | ${entry.iteration_count ?? 'N/A'} |`);
    }
    if (snapshot.readiness_projection) {
        lines.push('');
        lines.push('## Readiness Drift Projection');
        lines.push('');
        lines.push(`- Readiness Baseline Run ID: ${snapshot.readiness_projection.readiness_baseline_run_id ?? 'N/A'}`);
        lines.push(`- Readiness Score: ${snapshot.readiness_projection.readiness_score ?? 'N/A'}`);
        lines.push(`- Readiness Raw Score: ${snapshot.readiness_projection.readiness_raw_score ?? 'N/A'}`);
        ...
        lines.push(`- Re-Readiness Required: ${snapshot.readiness_projection.re_readiness_required ? 'yes' : 'no'}`);
        lines.push(`- Drift Signals: ${snapshot.readiness_projection.drift_signals.length > 0
            ? snapshot.readiness_projection.drift_signals.join(', ')
            : 'N/A'}`);
        lines.push(`- Drifted Dimensions: ${snapshot.readiness_projection.drifted_dimensions.length > 0
    ...
    if (snapshot.sft_summary.redaction_applied_rules.length > 0) {
        lines.push(`- Redaction Rules: ${snapshot.sft_summary.redaction_applied_rules
            .map((entry) => `${entry.rule}(${entry.count})`)
            .join(', ')}`);
    }
    return lines.join('\n') + '\n';
}
function writeDashboardSnapshotFiles(snapshot, options) {
    const markdownBody = renderDashboardSnapshotMarkdown(options.markdown, snapshot, options.includeRuntime ?? false);
