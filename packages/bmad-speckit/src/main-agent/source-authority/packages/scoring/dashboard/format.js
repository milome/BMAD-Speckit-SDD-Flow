...
exports.formatDashboardMarkdown = formatDashboardMarkdown;
/**
...
function formatGovernanceHistorySummary(governanceRoutingModeDistribution, governanceSignalHotspots, governanceGateFailureTrend) {
    const lines = [];
    ...
    if (dominantMode) {
        lines.push(`- Dominant Routing Mode: ${dominantMode.mode} (${dominantMode.count}/${totalModeEvents})`);
    }
    const topSignal = governanceSignalHotspots[0];
    if (topSignal) {
        lines.push(`- Top Signal: ${topSignal.signal} (${topSignal.count} 次；阶段 ${topSignal.affected_stages.join('、') || '-'}；gates ${topSignal.rerun_gates.join('、') || '-'})`);
    }
    const worseningGates = governanceGateFailureTrend
        .filter((item) => item.trend === 'worsening')
        .map((item) => item.rerun_gate);
    lines.push(`- Worsening Gates: ${worseningGates.length > 0 ? worseningGates.join('、') : 'none'}`);
    lines.push('');
    return lines;
}
/**
 ...
 */
function formatDashboardMarkdown(data, options) {
    const lines = [];
    ...
    lines.push('');
    lines.push(...formatGovernanceHistorySummary(governanceRoutingModeDistribution, governanceSignalHotspots, governanceGateFailureTrend));
    lines.push('## 四维雷达图');
    lines.push('');
    lines.push('| 维度 | 分数 |');
    lines.push('|------|------|');
    ...
    if (!governanceRoutingSummary) {
        ...
        lines.push(`- Affected Stages: ${governanceRoutingSummary.affectedStages.join('、') || '(none)'}`);
    ...
    lines.push('');
    lines.push(...buildGovernanceLatestRawEventSectionLines(governanceRoutingSummary?.runnerSummaryLines ?? []));
    ...
    if (governanceSignalHotspots.length === 0) {
        ...
        for (const item of governanceSignalHotspots) {
            lines.push(`- ${item.signal}: ${item.count} 次；影响阶段 ${item.affected_stages.join('、') || '-'}；gates ${item.rerun_gates.join('、') || '-'}`);
        }
    }
    lines.push('');
    ...
    if (governanceGateFailureTrend.length === 0) {
        ...
        for (const item of governanceGateFailureTrend) {
            lines.push(`- ${item.rerun_gate}: ${item.trend}；失败 ${item.failure_count}/${item.total_events}；latest ${item.latest_outcome} @ ${item.latest_timestamp}`);
        }
    }
    lines.push('');
