...
exports.extractTriggeredJourneySignals = extractTriggeredJourneySignals;
exports.deriveReadinessDriftSeverity = deriveReadinessDriftSeverity;
exports.deriveReadinessDriftedDimensions = deriveReadinessDriftedDimensions;
exports.findLatestImplementationReadinessBaseline = findLatestImplementationReadinessBaseline;
exports.evaluateReadinessDrift = evaluateReadinessDrift;
exports.buildReadinessDriftProjection = buildReadinessDriftProjection;
const DRIFT_SIGNAL_DIMENSION_MAP = {
...
function extractTriggeredJourneySignals(signals) {
    if (!signals)
        return [];
    return Object.keys(DRIFT_SIGNAL_DIMENSION_MAP).filter((key) => signals[key] === true);
}
...
function projectionFromRequirementBaseline(baseline) {
    return {
...
function projectionFromRequirementScopedScore(baseline) {
    return {
        readiness_baseline_run_id: baseline.scoringRunId ?? null,
        readiness_score: typeof baseline.score === 'number' ? baseline.score : null,
        readiness_raw_score: typeof baseline.rawScore === 'number' ? baseline.rawScore : (baseline.score ?? null),
...
function evaluateReadinessDrift(input) {
    ...
    if ((input.stage === 'implement' || input.stage === 'post_impl') &&
...
function buildReadinessDriftProjection(input) {
    ...
    if (currentBaseline) {
        ...
        if (staleFields.length > 0) {
            return {
                ...
                readiness_raw_score: typeof currentBaseline.rawScore === 'number'
    ...
    const scopedCandidates = (requirementBaseline?.scopedScoring ?? []).filter((record) => record.stage === 'implementation_readiness');
    const scoped = scopedCandidates[scopedCandidates.length - 1];
    if (scoped) {
        return projectionFromRequirementScopedScore(scoped);
    }
    const baseline = findLatestImplementationReadinessBaseline(input.allRecords);
    const currentRecord = input.currentRecord ??
        input.allRecords
            .filter((record) => record.stage === 'implement' || record.stage === 'tasks' || record.stage === 'post_impl')
    ...
    if (currentRecord &&
        ...
        return {
            ...
            readiness_raw_score: baseline?.raw_phase_score ??
                baseline?.phase_score ??
                null,
            readiness_dimensions: baseline?.dimension_scores && baseline.dimension_scores.length > 0
