...
exports.LONG_RUN_RUNTIME_POLICY = void 0;
exports.hashLongRunPolicy = hashLongRunPolicy;
exports.validateLongRunPolicy = validateLongRunPolicy;
exports.attachLeaseToPendingPacket = attachLeaseToPendingPacket;
exports.applyLongRunPolicyToState = applyLongRunPolicyToState;
exports.main = main;
const node_crypto_1 = require("node:crypto");
exports.LONG_RUN_RUNTIME_POLICY = {
...
function hashLongRunPolicy(policy = exports.LONG_RUN_RUNTIME_POLICY) {
    return (0, node_crypto_1.createHash)('sha256').update(JSON.stringify(policy)).digest('hex');
}
function validateLongRunPolicy(policy) {
...
function attachLeaseToPendingPacket(packet, input) {
    const policy = input.policy ?? exports.LONG_RUN_RUNTIME_POLICY;
...
function applyLongRunPolicyToState(state, input) {
    const policy = input.policy ?? exports.LONG_RUN_RUNTIME_POLICY;
...
function main(argv) {
    if (argv.includes('--json')) {
        process.stdout.write(`${JSON.stringify(exports.LONG_RUN_RUNTIME_POLICY, null, 2)}\n`);
        return 0;
    }
    process.stdout.write(`${JSON.stringify({
        policyVersion: exports.LONG_RUN_RUNTIME_POLICY.version,
        policyHash: hashLongRunPolicy(),
        requiredKeys: REQUIRED_KEYS,
    }, null, 2)}\n`);
    return 0;
}
function isDirectLongRunRuntimePolicyCli(entry) {
