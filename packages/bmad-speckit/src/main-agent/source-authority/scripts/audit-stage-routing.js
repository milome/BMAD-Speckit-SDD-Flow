...
exports.auditStageRouting = auditStageRouting;
exports.generateMarkdownReport = generateMarkdownReport;
const fs = require("fs");
const path = require("path");
function auditStageRouting() {
    const results = [];
    const routingRules = [];
    ...
    results.push({
        ...
        status: storyTypeDetectionFound && hasCodeFlow && hasDocumentFlow
            ? 'PASS'
    ...
    results.push({
        ...
        status: hasEpicRoutingSection && epicRoutingFound
            ? 'PASS'
    ...
    results.push({
        ...
        status: autoContinueFound && hasAutoContinueCheck && hasActionToStageMap
            ? 'PASS'
            : autoContinueFound
                ? 'WARNING'
                : 'FAIL',
