...
exports.mainExecutionClosureGate = mainExecutionClosureGate;
...
function resolveTraceExecutionDir(recordPath, attemptId) {
    return path.join(path.dirname(recordPath), 'trace-execution', attemptId);
}
function defaultModelPacketPath(recordPath, attemptId) {
    return path.join(resolveTraceExecutionDir(recordPath, attemptId), 'model_packet.json');
}
function defaultImplementationEvidencePath(recordPath, attemptId) {
    return path.join(resolveTraceExecutionDir(recordPath, attemptId), 'implementation-evidence', 'implementation-evidence-packet.json');
}
function defaultCommandSummaryPath(recordPath, attemptId) {
...
function defaultReportPath(recordPath, attemptId) {
    return path.join(resolveTraceExecutionDir(recordPath, attemptId), 'execution-closure-report.json');
}
...
function commandSummaryCheck(summaryPath, modelPacket) {
...
function implementationEvidenceCheck(evidencePath, input) {
...
function executionEvidenceInRecordCheck(record, attemptId) {
    const iterations = objects(record.executionIterations).filter((iteration) => text(iteration.runId) === attemptId || text(iteration.executionIterationId) === attemptId);
    const latestIteration = iterations.at(-1);
    const closures = objects(record.requirementClosures).filter((closure) => text(closure.status) === 'pass');
    const blockingReasons = uniqueStrings([
...
function evaluate(input) {
    ...
    checks.push({
        ...
        passed: text(modelPacket.sourceDocumentHash) === text(input.record.sourceDocumentHash) &&
    ...
    if (text(modelPacket.implementationConfirmationHash) !==
        text(input.record.implementationConfirmationHash)) {
...
function mainExecutionClosureGate(argv) {
    const args = parseArgs(argv);
    if (args.help) {
        console.log('Usage: main-agent-execution-closure-gate --requirement-record <json> [--attempt-id <id>] [--model-packet <json>] [--implementation-evidence <json>] [--command-summary <json>] [--json]');
        return 0;
    }
    ...
    const evidencePath = path.resolve(args.implementationEvidence ?? defaultImplementationEvidencePath(recordPath, attemptId));
    const commandSummaryPath = path.resolve(args.commandSummary ?? defaultCommandSummaryPath(recordPath, attemptId));
    ...
    const commit = (0, requirement_record_control_store_1.appendControlEventAndReplay)({
        ...
        payload: {
            ...
            evaluatedAt,
            evaluatedBy,
            recordHashBeforeClosure: (0, requirement_record_control_store_1.sha256Text)(JSON.stringify(record)),
        },
