...
function resolveHooksDir() {
  return [
    path.join(path.resolve(__dirname, '..', '..', '..', '..'), '_bmad', 'runtime', 'hooks'),
    path.join(__dirname, '..', '_bmad', 'runtime', 'hooks'),
  ].find(candidate => fs.existsSync(candidate)) || path.join(__dirname, '..', '_bmad', 'runtime', 'hooks');
}

function displayHookPath(hooksDir, file) {
  return path.join('_bmad', 'runtime', 'hooks', path.relative(hooksDir, file));
}
...
function verify() {
  ...
  for (const file of files) {
    const relativePath = displayHookPath(hooksDir, file);
