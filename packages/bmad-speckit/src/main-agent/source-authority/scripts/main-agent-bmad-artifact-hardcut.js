...
exports.mainBmadArtifactHardcut = mainBmadArtifactHardcut;
...
function relativeToRoot(projectRoot, file) {
    return normalizePathForRecord(path.relative(projectRoot, file));
}
function resolveFirstExisting(projectRoot, candidates) {
...
function checkBmadCatalog(projectRoot) {
    ...
    for (const row of rows) {
        const searchable = [
            ...
            .join(' ')
            .toLowerCase();
        for (const [key, tokens] of Object.entries(requiredAuthoring)) {
            if (tokens.some((token) => new RegExp(`(^|[^a-z])${token}([^a-z]|$)`, 'u').test(searchable))) {
                foundOutputs.add(key);
            }
        }
    }
    const missing = Object.keys(requiredAuthoring).filter((item) => !foundOutputs.has(item));
    return {
...
function checkBmadWorkflow(projectRoot) {
...
function checkBmadAuthoringPaths(projectRoot) {
    ...
    const missing = [];
    if (!planning.includes('_bmad-output/planning-artifacts'))
        missing.push('planning_artifacts');
...
function checkFiveLayerMapping(projectRoot) {
    ...
    const layerToStages = mapping.layer_to_stages;
...
function collectRecordArtifacts(record) {
    ...
    const commandArtifacts = arrayOfObjects(deliveryEvidence?.requiredCommands).flatMap((command) => arrayOfObjects(command.artifactRefs));
    return [...direct, ...commandArtifacts];
}
function checkArtifactBoundary(record) {
    if (!record) {
        return {
    ...
    const docsReferenceViolations = [];
    const requirementScopedArtifacts = [];
    ...
    for (const artifact of collectRecordArtifacts(record)) {
        ...
        if (LEGACY_RUNTIME_PREFIXES.some((prefix) => artifactPath.startsWith(prefix)) &&
...
function buildReport(input) {
    ...
    return {
        ...
        architectureConfirmationHash: text(record?.architectureConfirmationState
            ?.currentArchitectureConfirmationHash) || null,
        inputs: {
...
function defaultOutPath(projectRoot, requirementRecordPath) {
    if (requirementRecordPath) {
        return path.join(path.dirname(requirementRecordPath), 'evidence', 'TRACE-011', 'bmad-artifact-hardcut-report.json');
    }
    return path.join(projectRoot, '_bmad-output', 'runtime', 'requirement-records', 'artifact-hardcut-report.json');
}
function mainBmadArtifactHardcut(argv) {
