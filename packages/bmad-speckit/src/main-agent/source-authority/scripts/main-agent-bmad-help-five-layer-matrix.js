...
exports.writeLayer1PrdCompletionMarker = exports.validateLayer1PrdCompletionMarker = exports.buildLayer1PrdCompletionMarker = void 0;
exports.markBmadHelpFiveLayerStageComplete = markBmadHelpFiveLayerStageComplete;
exports.resolveBmadHelpFiveLayerProgressState = resolveBmadHelpFiveLayerProgressState;
exports.runBmadHelpFiveLayerMatrix = runBmadHelpFiveLayerMatrix;
exports.main = main;
...
var bmad_help_five_layer_progress_marker_2 = require("./bmad-help-five-layer-progress-marker");
Object.defineProperty(exports, "buildLayer1PrdCompletionMarker", { enumerable: true, get: function () { return bmad_help_five_layer_progress_marker_2.buildLayer1PrdCompletionMarker; } });
Object.defineProperty(exports, "validateLayer1PrdCompletionMarker", { enumerable: true, get: function () { return bmad_help_five_layer_progress_marker_2.validateLayer1PrdCompletionMarker; } });
Object.defineProperty(exports, "writeLayer1PrdCompletionMarker", { enumerable: true, get: function () { return bmad_help_five_layer_progress_marker_2.writeLayer1PrdCompletionMarker; } });
...
function loadLayerDefinitions(projectRoot) {
    ...
    return ['layer_1', 'layer_2', 'layer_3', 'layer_4', 'layer_5'].map((id) => {
        ...
        const executableStages = Array.from(new Set(stages.map((stage) => {
...
function resolveStageEvidence(projectRoot, layer, stage) {
    ...
    if (stage === 'release_gate') {
        const completed = fs.existsSync(root) &&
            gateReportPassed(path.join(root, 'main-agent-release-gate-report.json'), (value) => value.critical_failures === 0 && value.blocked_sprint_status_update === false);
        return { completed, evidenceKind: completed ? 'gate_report' : 'missing' };
    }
    if (stage === 'delivery_truth_gate') {
        const completed = fs.existsSync(root) &&
            gateReportPassed(path.join(root, 'main-agent-delivery-truth-gate-report.json'), (value) => value.completionAllowed === true);
        return { completed, evidenceKind: completed ? 'gate_report' : 'missing' };
    }
    const explicitStageMarker = stageEvidencePath(projectRoot, layer, stage);
    if (layer.id === 'layer_1' && stage === 'prd') {
        const strictMarkerComplete = (0, bmad_help_five_layer_progress_marker_1.validateLayer1PrdCompletionMarker)({
            projectRoot,
            markerPath: explicitStageMarker,
        });
        if (strictMarkerComplete)
...
function markBmadHelpFiveLayerStageComplete(input) {
    ...
    if (input.layer === 'layer_1' && input.stage === 'prd') {
        return (0, bmad_help_five_layer_progress_marker_1.writeLayer1PrdCompletionMarker)({ projectRoot: input.projectRoot });
    }
...
function resolveBmadHelpFiveLayerProgressState(input) {
    ...
    const stageStatuses = layers.flatMap((layer) => layer.stages.map((stage) => {
    ...
    const completedLayers = layers
        ...
        layer.stages.every((stage) => stageStatuses.some((item) => item.layer === layer.id && item.stage === stage && item.completed)))
        .map((layer) => layer.id);
    return {
...
function validateBmadHelpEntry(projectRoot) {
    ...
    return {
        ...
        docsExposeCodex: readme.includes('codex') &&
        ...
        bmadsRuntimeContract: bmadsRuntime.includes('schemaVersion: bmads-runtime/v1') &&
        ...
        bmadHelpIsUpstreamOnly: !command.includes('RUN OR EMULATE') &&
...
function prepareStageRoot(root, layer, stage) {
    const stageRoot = path.join(root, '_bmad-output', 'runtime', 'bmad-help-five-layer', `${Date.now()}-${process.pid}`, layer.id, stage);
    fs.mkdirSync(stageRoot, { recursive: true });
    (0, runtime_context_registry_1.writeRuntimeContextRegistry)(stageRoot, (0, runtime_context_registry_1.defaultRuntimeContextRegistry)(stageRoot));
    (0, runtime_context_1.writeRuntimeContext)(stageRoot, (0, runtime_context_1.defaultRuntimeContextFile)({
        flow: layer.flow,
        stage,
...
function runLayerStage(input) {
    ...
    const helpPolicy = (0, bmad_config_1.resolveBmadHelpRuntimePolicy)({
        ...
        runtimeContext: {
            ...
            stage: input.stage,
            sourceMode: 'full_bmad',
            contextScope: input.stage === 'prd' || input.stage === 'arch' || input.stage === 'epics'
                ? 'project'
                : 'story',
    ...
    const initialSurface = (0, main_agent_orchestration_1.resolveMainAgentOrchestrationSurface)({
        projectRoot: stageRoot,
        flow: input.layer.flow,
        stage: input.stage,
        implementationEntryGate: helpPolicy.implementationEntryGate,
    });
    const instruction = (0, main_agent_orchestration_1.buildMainAgentDispatchInstruction)({
        projectRoot: stageRoot,
        flow: input.layer.flow,
        stage: input.stage,
        implementationEntryGate: helpPolicy.implementationEntryGate,
    ...
    const runLoop = (0, main_agent_orchestration_1.runMainAgentAutomaticLoop)({
        ...
        executor: ({ projectRoot, instruction, args }) => {
            const reportPath = (0, main_agent_orchestration_1.writeMainAgentRunLoopTaskReport)(projectRoot, instruction, args);
    ...
    const passed = helpPolicy.mainAgentOrchestration.source !== 'none' &&
        helpPolicy.mainAgentNextAction != null &&
        instruction != null &&
        runLoop.status === 'completed' &&
        runLoop.finalSurface.pendingPacketStatus === 'completed' &&
        runLoop.taskReport?.evidence.some((item) => item.includes(`bmad-help-five-layer:${input.layer.id}:${input.stage}`)) === true;
    return {
...
function runBmadHelpFiveLayerMatrix(input) {
    const projectRoot = path.resolve(input.projectRoot);
    ...
    const steps = layers.flatMap((layer) => layer.stages.map((stage) => runLayerStage({ projectRoot, layer, stage })));
    const layerReports = layers.map((layer) => ({
        id: layer.id,
        name: layer.name,
        stages: layer.stages,
        passed: layer.stages.length > 0 &&
            layer.stages.every((stage) => steps.some((step) => step.layer === layer.id && step.stage === stage && step.passed)),
    }));
    return {
        ...
        layers: layerReports,
        steps,
        allPassed: bmadHelpEntry.catalogLoaded &&
...
function main(argv) {
    ...
    const reportPath = path.resolve(args.reportPath ??
        path.join(report.projectRoot, '_bmad-output', 'runtime', 'e2e', 'bmad-help-five-layer-main-agent-matrix.json'));
