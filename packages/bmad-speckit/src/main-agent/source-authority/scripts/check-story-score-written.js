...
const parse_epic_story_1 = require("../packages/scoring/query/parse-epic-story");
function parseArgs() {
    const args = {};
...
function main() {
    ...
    if (stageFilter === 'story') {
        matching = matching.filter((r) => r.stage === 'story' || r.trigger_stage === 'bmad_story_stage2');
    }
    else if (stageFilter === 'implement') {
        matching = matching.filter((r) => r.stage === 'implement' || r.trigger_stage === 'bmad_story_stage4');
    }
    if (matching.length > 0) {
        console.log('STORY_SCORE_WRITTEN:yes');
        console.log(`Found ${matching.length} record(s) for epic=${epicId} story=${storyId}`);
        const hasMissingDimensions = matching.some((r) => r.stage === 'implement' && (!r.dimension_scores || r.dimension_scores.length === 0));
