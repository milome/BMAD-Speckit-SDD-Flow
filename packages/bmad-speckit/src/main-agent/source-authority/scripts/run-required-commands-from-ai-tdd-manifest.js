...
exports.mainRunRequiredCommandsFromAiTddManifest = mainRunRequiredCommandsFromAiTddManifest;
...
const ingest_implementation_evidence_1 = require("./ingest-implementation-evidence");
const target_artifact_realization_gate_1 = require("./target-artifact-realization-gate");
function isDirectRunRequiredCommandsFromAiTddManifestCli(entry) {
    return /(^|[\\/])run-required-commands-from-ai-tdd-manifest(\.[cm]?js|\.ts)?$/iu.test(entry ?? '');
}
...
function hashExistingPath(absolutePath) {
...
function commandSpawnOptions(commandText) {
    if (!isWindows())
        return { command: commandText, args: [], shell: true };
    return {
        command: 'pwsh.exe',
        args: [
            '-NoLogo',
...
function commandExecutionMode(commandText) {
    const normalized = commandText.trim();
    if (/^rg\s+-n\s+-e\s+/iu.test(normalized) &&
        /delivery_closeout|architecture_confirmation_state_checked|implementation_readiness_check_recorded|readiness_baseline_activation_requested|closeout_recorded|gate_check_recorded\.\*sixModelResults|sixModelResults\.\*gate_check_recorded/u.test(normalized)) {
...
function checkPreRunSections(manifest) {
...
function checkRequiredCommandDefinitions(manifest) {
    ...
    const failures = [];
    if (ids.length === 0)
        failures.push('closeoutProof.requiredCommands_missing');
    for (const [id, rows] of byId) {
        if (id && rows.length > 1)
            failures.push(`manifest.requiredCommands_duplicate:${id}`);
    }
    for (const id of ids) {
        ...
        if (!text(command.command))
            failures.push(`manifest.requiredCommand.command_missing:${id}`);
...
function commandDefinitions(manifest) {
    const ids = strings(nested(manifest.closeoutProof).requiredCommands);
    const commands = objects(manifest.requiredCommands);
    return ids
...
function normalizeArtifactRole(value) {
    ...
    if (role)
        return role === 'post_closeout_review_projection' ? 'projection' : role;
    return 'evidence';
}
...
function latestAuditAttemptId(record) {
...
function replaceKnownTargetPlaceholders(value, record, closeoutAttemptId) {
...
function recordFieldValue(record, pathOrField) {
    ...
    return fieldPath
        .split('.')
        .filter(Boolean)
        .reduce((current, key) => current && typeof current === 'object' && !Array.isArray(current)
        ? current[key]
        : undefined, record);
}
function targetConcretePath(target, record, closeoutAttemptId) {
    const targetPath = filePathPrefix(text(target.pathOrField));
    if (!targetPath || /\s/u.test(targetPath))
        return '';
...
function writeSemanticCoverageClosureReport(input) {
    ...
    const closures = criticalMustRows.map((must) => {
        ...
        const commandRefs = unique(relatedTraceRows.flatMap((trace) => [
...
function writeAuditConvergenceReceipt(filePath, closeoutAttemptId) {
...
function materializeTargetArtifacts(input) {
    for (const target of objects(input.manifest.targetArtifacts)) {
        ...
        else if (pathOrField.endsWith('/audit-scoring-convergence-receipt.json') &&
            !fs.existsSync(concretePath)) {
            writeAuditConvergenceReceipt(concretePath, input.closeoutAttemptId);
        }
    }
}
function targetArtifactRefs(manifest, closeoutAttemptId, record) {
    return objects(manifest.targetArtifacts).flatMap((target) => {
        ...
        if (text(target.kind) !== 'file_artifact')
            return [];
        const targetPath = filePathPrefix(text(target.pathOrField));
        if (!targetPath || /\s/u.test(targetPath))
            return [];
...
function runCommand(input) {
    ...
    const result = (0, node_child_process_1.spawnSync)(spawn.command, spawn.args, {
        cwd: process.cwd(),
        shell: spawn.shell,
        encoding: 'utf8',
        windowsHide: true,
...
function commandCoversRefs(run, traceRefs, evidenceRefs) {
    ...
    return (traceRefs.some((ref) => runTraceRefs.has(ref)) ||
        evidenceRefs.some((ref) => runEvidenceRefs.has(ref)));
}
function failureCaseCoverageRefs(registry) {
...
function buildFailureCaseCoverage(input) {
    let confirmation;
    try {
        confirmation = (0, target_artifact_realization_gate_1.readImplementationConfirmation)(input.sourcePath).confirmation;
    }
    catch {
        return { artifactRefs: [] };
    }
    ...
    const failureCases = objects(registry.failureCases);
    const applies = registry.applies === true ||
    ...
    if (!applies || failureCases.length === 0)
        return { artifactRefs: [] };
    ...
    const coverageCommandRun = syntheticCommandRun({
        ...
        command: 'internal: generate failure-case coverage from implementationConfirmation.functionalResumeFailureCaseRegistry',
        runId: input.runId,
        closeoutAttemptId: input.closeoutAttemptId,
        evidenceDir: input.evidenceDir,
    ...
    writeJson(reportPath, {
        ...
        sourceDocumentHash: text(input.record.sourceDocumentHash),
        implementationConfirmationHash: text(input.record.implementationConfirmationHash),
        architectureConfirmationHash: text(input.record.architectureConfirmationHash) ||
...
function buildImplementationEvidencePacket(input) {
    ...
    const negativeAssertionArtifactRefs = artifactRefs.filter((artifact) => text(artifact.sourceOfTruthRole) === 'evidence');
    const manifestCommandIds = new Set(commandDefinitions(input.manifest).map((command) => commandId(command)));
    const deliveryCommandRuns = input.commandRuns.filter((run) => run.deliveryEvidenceRequired !== false && manifestCommandIds.has(run.commandId));
    return {
        ...
        gateChecks: [
            {
                ...
                decision: input.status === 'done' ? 'pass' : 'blocked',
            },
        ],
        contractChecks: [
            {
...
function writeFailureAndReturn(input) {
    ...
    writeJson(failedPath, failedEvidencePacket({
        record: input.record,
        runId: input.runId,
        closeoutAttemptId: input.closeoutAttemptId,
        status: 'failed',
...
function mainRunRequiredCommandsFromAiTddManifest(argv) {
    const args = parseArgs(argv);
    if (args.help) {
        console.log('Usage: run-required-commands-from-ai-tdd-manifest --source <source-document.md> --requirement-record <requirement-record.json> --mode closeout --attempt-id <closeoutAttemptId> --run-id <runId> --evidence-dir <dir> [--json]');
        return 0;
    }
    if (!args.source ||
    ...
    if (args.mode !== 'closeout')
        throw new Error('only --mode closeout is supported');
    const sourcePath = path.resolve(args.source);
    const recordPath = path.resolve(args.requirementRecord);
    const evidenceDir = path.resolve(args.evidenceDir);
    fs.mkdirSync(evidenceDir, { recursive: true });
    const record = readJson(recordPath);
    const preRunReport = (0, ai_tdd_contract_gate_1.evaluateAiTddContractGate)({
        sourcePath,
        record,
        recordPath,
        mode: 'pre-rerun',
    ...
    if (failureCaseCoverage.commandRun)
        commandRuns.push(failureCaseCoverage.commandRun);
    ...
    const ingestCode = (0, ingest_implementation_evidence_1.mainIngestImplementationEvidence)([
        '--evidence',
        packetPath,
        '--requirement-record',
        recordPath,
    ...
    const closeoutReport = (0, ai_tdd_contract_gate_1.evaluateAiTddContractGate)({
        sourcePath,
