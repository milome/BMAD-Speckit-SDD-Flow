...
exports.mainDashboardProjectionMapping = mainDashboardProjectionMapping;
...
function collectValidationIssues(projection) {
    const issues = [];
    if (projection.sourceOfTruthRole !== 'read_model')
        issues.push('projection_source_role_not_read_model');
    if (projection.canAffectControlFlow !== false)
        issues.push('projection_can_affect_control');
    ...
    for (const viewName of expectedViews) {
        if (!projection.businessObjectViews[viewName]) {
            issues.push(`view_missing:${viewName}`);
        }
    }
    for (const [viewName, view] of viewEntries) {
        if (view.canAffectControlFlow !== false)
            issues.push(`view_can_affect_control:${viewName}`);
        for (const item of view.items) {
            ...
            if (item.traceRows.length === 0)
                issues.push(`item_trace_rows_missing:${item.objectId}`);
    ...
    for (const id of [
        ...
        if (!forbiddenIds.includes(id))
            issues.push(`forbidden_display_missing:${id}`);
    }
...
function mainDashboardProjectionMapping(argv) {
    ...
    const projection = (0, dashboard_1.buildSixMentalModelProjection)(input);
    ...
    const report = {
        ...
        businessObjectViews: Object.fromEntries(Object.entries(projection.businessObjectViews).map(([viewName, view]) => [
            viewName,
            {
