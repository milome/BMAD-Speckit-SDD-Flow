...
exports.mainDatasetReleaseGate = mainDatasetReleaseGate;
...
function artifactPath(recordPath, artifactRef) {
...
function latestActiveSubsystemExtension(record, recordPath) {
...
function productionSubsystemCoverageRequired(record) {
...
function subsystemCoverageIssues(extension, required = true) {
    if (!required)
        return [];
    if (!extension)
        return ['subsystem_extension_missing'];
    ...
    for (const id of REQUIRED_SUBSYSTEM_IDS) {
        ...
        if (strings(item.inputRefs).length === 0)
            issues.push(`subsystem_input_refs_missing:${id}`);
        if (strings(item.outputRefs).length === 0)
            issues.push(`subsystem_output_refs_missing:${id}`);
        if (!text(item.status))
            issues.push(`subsystem_status_missing:${id}`);
        if (strings(item.evidenceRefs).length === 0)
            issues.push(`subsystem_evidence_refs_missing:${id}`);
        if (!isSha256(text(item.hash)))
            issues.push(`subsystem_hash_missing:${id}`);
...
function sourceSnapshotIssues(record, sourceManifest) {
...
function governanceIssues(governanceDir, governanceReport) {
    const issues = REQUIRED_GOVERNANCE_FILES.filter((file) => !fs.existsSync(path.join(governanceDir, file))).map((file) => `governance_file_missing:${file}`);
    if (!governanceReport)
        return [...issues, 'governance_report_missing'];
    if (text(governanceReport.decision) !== 'pass')
        issues.push('governance_decision_not_pass');
    const checks = nested(governanceReport.checks);
    for (const key of ['split', 'dedup', 'contamination']) {
        if (text(nested(checks[key]).decision) !== 'pass')
            issues.push(`${key}_decision_not_pass`);
    }
...
function validationSummaryIssues(sourceManifest) {
...
function trainingRunIssues(trainingRun, datasetId, datasetVersion) {
    if (!trainingRun)
        return ['training_run_missing'];
    const issues = [];
    if (!text(trainingRun.trainingRunId))
        issues.push('training_run_id_missing');
    if (text(trainingRun.status) !== 'completed')
        issues.push('training_run_not_completed');
    if (text(trainingRun.datasetId) !== datasetId)
        issues.push('training_run_dataset_id_mismatch');
    if (text(trainingRun.datasetVersion) !== datasetVersion)
        issues.push('training_run_dataset_version_mismatch');
    return issues;
}
function evalReportIssues(evalReport, trainingRun) {
    if (!evalReport)
        return ['eval_report_missing'];
    const issues = [];
    if (!text(evalReport.evalReportId))
        issues.push('eval_report_id_missing');
    if (text(evalReport.trainingRunId) !== text(trainingRun?.trainingRunId))
        issues.push('eval_report_training_run_mismatch');
    if (text(evalReport.decision) !== 'pass')
        issues.push('eval_report_decision_not_pass');
    if (evalReport.trainingLossOnly === true)
        issues.push('training_loss_only_eval_forbidden');
...
function qualityReport(sourceManifest, samples, routes, generatedAt) {
    ...
    return {
        ...
        generatedAt,
        decision: samples.length > 0 && Number(counts.accepted ?? samples.length) === samples.length
            ? 'pass'
            : 'blocked',
...
function redactionReport(sourceManifest, samples, generatedAt) {
...
function revokedSamples(routes, generatedAt) {
    const items = routes
...
function artifactSummary(file, artifactType) {
    return {
        artifactType,
...
function defaultJsonBinding(file) {
    return fs.existsSync(file) ? readJson(file) : null;
}
function defaultEvalReport(governanceDir) {
...
function buildRelease(input) {
    ...
    const releaseDecision = blockingIssues.length === 0 ? 'pass' : 'blocked';
    ...
    if (fs.existsSync(path.join(input.dataDir, 'test.jsonl')))
        ...
        writeText(testPath, '');
    writeText(openaiProjectionPath, samples
        .map((sample, index) => JSON.stringify({
        custom_id: text(sample.sample_id) || `sample-${index + 1}`,
        messages: objects(sample.messages),
        metadata: nested(sample.metadata),
    }))
        .join('\n') + (samples.length > 0 ? '\n' : ''));
    writeText(huggingfaceProjectionPath, samples
        .map((sample, index) => JSON.stringify({
    ...
    writeText(paths.datasetCard, releaseCard({
        record: input.record,
        datasetId: input.datasetId,
        datasetVersion: input.datasetVersion,
        sampleCount: samples.length,
    ...
    const report = {
        ...
        checks: [
            ...
            },
            {
                ...
                passed: trainingRunIssues(input.trainingRun, input.datasetId, input.datasetVersion).length === 0,
            },
            {
        ...
        artifactPaths: Object.fromEntries(Object.entries({
            ...paths,
            trainPath,
            validationPath,
            testPath,
...
function mainDatasetReleaseGate(argv) {
