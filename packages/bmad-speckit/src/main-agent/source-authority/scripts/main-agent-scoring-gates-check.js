...
exports.mainScoringGatesCheck = mainScoringGatesCheck;
...
function readRuntimePolicySnapshot(root, record) {
    ...
    if (expectedHash && sha256File(absolute) !== expectedHash)
        return null;
    return readJson(absolute);
}
function scoringRequiredFromSnapshot(snapshot) {
    if (!snapshot)
        return null;
    const policy = nested(snapshot, 'policy');
    const scoring = nested(policy, 'scoring');
    if (scoring?.required === true)
        return true;
    if (scoring?.required === false)
        return false;
    if (policy?.scoringRequired === true || snapshot.scoringRequired === true)
        return true;
    if (policy?.scoringRequired === false || snapshot.scoringRequired === false)
        return false;
    if (policy?.scoringEnabled === true || snapshot.scoringEnabled === true)
        return true;
    if (policy?.scoringEnabled === false || snapshot.scoringEnabled === false)
        return false;
    if (nested(policy, 'control')?.scoringEnabled === true)
        return true;
    if (nested(policy, 'control')?.scoringEnabled === false)
        return false;
    return null;
}
function resolvedScoringPolicyFromSnapshot(snapshot) {
    const resolved = nested(snapshot, 'resolvedScoringPolicy');
    if (!resolved)
        return null;
    if (text(resolved.schemaVersion) !== 'resolved-scoring-policy/v1')
        return null;
    if (!/^sha256:[a-f0-9]{64}$/u.test(text(resolved.scoringPolicyHash)))
        return null;
    return resolved;
}
function latestGate(record, gate) {
    return (objects(record.gateChecks)
...
function scoreArtifacts(record) {
    const isScoreArtifact = (artifact) => {
        const artifactType = text(artifact.artifactType);
        const artifactPath = normalizePathForRecord(text(artifact.path));
        return (['score', 'score_record', 'score_report', 'scoring_gate_report'].includes(artifactType) ||
...
function readScoreArtifact(root, artifact) {
...
function scoreArtifactIssues(root, record, resolvedPolicy) {
    const issues = [];
    ...
    if (controlArtifact)
        issues.push('score_artifact_must_not_be_control');
    ...
    if (!evidenceArtifact)
        issues.push('score_artifact_evidence_or_read_model_missing');
    ...
    if (text(scoreRecord.scoreWriteResult) !== 'ok')
        issues.push('score_write_result_not_ok');
    const scoringPolicyHash = text(scoreRecord.scoringPolicyHash ?? scoreRecord.policyHash);
    if (!/^sha256:[a-f0-9]{64}$/u.test(scoringPolicyHash))
        issues.push('score_policy_hash_missing');
    ...
    if (display && display.visible !== true)
        issues.push('score_display_not_visible');
    const evaluation = nested(scoreRecord, 'evaluation');
    if (evaluation) {
        if (text(evaluation.decision) !== 'pass')
            issues.push('score_record_evaluation_not_pass');
        if (evaluation.thresholdPassed === false)
            issues.push('score_threshold_failed');
...
function hasFailure(record, type, sourceId) {
    return objects(record.failureRecords).some((failure) => {
        if (text(failure.type) !== type)
            return false;
        if (!['open', 'in_progress', 'blocked'].includes(text(failure.status)))
            return false;
        return objects(failure.sourceRefs).some((ref) => text(ref.id) === sourceId || text(ref.sourceType) === 'gate_check');
    });
}
function hasRerunForGate(record, sourceId) {
...
function openScoreFailures(record) {
    return objects(record.failureRecords).filter((failure) => ['score_write_failed', 'score_threshold_or_dimension_failed'].includes(text(failure.type)) &&
        ['open', 'in_progress', 'blocked'].includes(text(failure.status)));
}
function evaluateScoringGates(root, record) {
    const checks = [];
    ...
    if (scoringRequired === null)
        blockingReasons.push('runtime_policy_scoring_required_unresolved');
    let resolvedPolicy = null;
    try {
        resolvedPolicy = (0, policy_1.resolveScoringPolicy)({ root });
    }
    catch (error) {
    ...
    if (scoringRequired === false) {
        ...
        if (!skipped)
            blockingReasons.push('score_materialization_policy_skip_gate_missing');
        return {
    ...
    if (!materialization)
        blockingReasons.push('score_materialization_gate_missing');
    ...
    if (!evaluation)
        blockingReasons.push('score_evaluation_gate_missing');
    ...
    if (openFailures.length > 0)
        blockingReasons.push('open_score_failure_record_exists');
    return {
        decision: blockingReasons.length === 0 ? 'pass' : 'blocked',
        blockingReasons: [...new Set(blockingReasons)],
        checks,
...
function mainScoringGatesCheck(argv) {
