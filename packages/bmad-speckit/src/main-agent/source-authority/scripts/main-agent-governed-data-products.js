...
exports.mainGovernedDataProducts = mainGovernedDataProducts;
...
function readJsonOrJsonl(file) {
    ...
    if (content.startsWith('[')) {
        ...
        return Array.isArray(parsed)
            ? parsed.filter((item) => Boolean(item) && typeof item === 'object' && !Array.isArray(item))
            : [];
    }
    return content
        .split(/\r?\n/u)
...
function mergeJsonlByStableKey(file, rows) {
    const existing = fs.existsSync(file) ? readJsonOrJsonl(file) : [];
    const merged = new Map();
    const keyOf = (row) => text(row.mentorEventId) ||
...
function recordDataDir(recordPath) {
    return path.join(path.dirname(recordPath), 'data');
}
function latestClosureStatusByRequirement(record) {
    const statuses = new Map();
    for (const closure of objects(record.requirementClosures)) {
        const requirementId = text(closure.requirementId);
        if (requirementId)
            statuses.set(requirementId, text(closure.status));
    }
    return statuses;
}
function artifactRefsForIteration(record, iteration) {
...
function mentorEventsFromRecord(record, generatedAt, generatedBy) {
...
function candidateEvents(record, candidateEventsPath, generatedAt, generatedBy) {
    if (!candidateEventsPath)
        return [];
    const recordId = text(record.recordId);
    const requirementSetId = text(record.requirementSetId);
...
function routeReasonsForEvent(event, closureStatuses) {
    const reasons = [];
    ...
    const quality = event.quality && typeof event.quality === 'object' && !Array.isArray(event.quality)
        ? event.quality
        : {};
    ...
    const redaction = event.redaction && typeof event.redaction === 'object' && !Array.isArray(event.redaction)
        ? event.redaction
        : {};
    if (text(redaction.status) === 'blocked')
        reasons.push('redaction_blocked');
    const contamination = event.contamination &&
    ...
    const withdrawal = event.withdrawal && typeof event.withdrawal === 'object' && !Array.isArray(event.withdrawal)
        ? event.withdrawal
        : {};
...
function routeDestination(reasons) {
    if (reasons.some((reason) => reason.startsWith('forbidden_direct_source')))
        return 'discard';
    if (reasons.includes('contamination_detected') ||
        reasons.includes('redaction_blocked') ||
        reasons.includes('withdrawal_requested')) {
        return 'quarantine';
    }
    if (reasons.some((reason) => reason.startsWith('non_positive_status:failed') ||
        reason.startsWith('non_positive_status:blocked'))) {
        return 'rca';
    }
    if (reasons.some((reason) => reason.startsWith('non_positive_status') || reason.startsWith('requirement_not_closed')))
        return 'eval';
    if (reasons.some((reason) => reason.startsWith('quality')))
        return 'preference';
    if (reasons.includes('training_not_ready'))
        return 'error_library';
    return 'sft_positive';
}
function sampleRoutes(events, closureStatuses, generatedAt) {
    return events
...
function canonicalSamples(record, events, routes, generatedAt) {
    ...
    return routes
        ...
        return {
            ...
            source: {
                ...
                artifact_refs: artifactRefs.length > 0
                    ? artifactRefs
                    : [
                        {
...
function datasetManifest(input) {
    ...
    return {
        ...
        source_snapshot: {
            ...
            architectureConfirmationHash: text(input.record.architectureConfirmationState
                ?.currentArchitectureConfirmationHash),
...
function governanceReport(input) {
    const destinationCounts = input.routes.reduce((counts, route) => {
...
function releaseValidationEvalReport(input) {
    const datasetId = `${text(input.record.recordId)}-governed-sft`.toLowerCase();
    const metric = (baseline, current, lowerIsBetter = false) => ({
        baseline,
        current,
...
function buildProducts(input) {
    ...
    writeJson(paths.manifest, datasetManifest({
        record: input.record,
        outDir: input.outDir,
        generatedAt: input.generatedAt,
        samples,
    ...
    writeJson(paths.report, governanceReport({
        record: input.record,
        events,
        routes,
        samples,
...
function mainGovernedDataProducts(argv) {
