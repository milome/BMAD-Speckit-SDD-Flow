...
const PACKAGE_SOURCE_AUTHORITY_ROOT = path.resolve(__dirname, '..');
const ROOT = path.resolve(process.env.BMAD_SOURCE_REPO_EQUIVALENT_ROOT || process.cwd());
...
function runReleaseGate(fixtures) {
  const releaseGate = path.join(PACKAGE_SOURCE_AUTHORITY_ROOT, 'scripts', 'main-agent-release-gate.js');
  runNodeScript(releaseGate, [
    '--hostMatrixPath',
...
runNodeScript(path.join(PACKAGE_SOURCE_AUTHORITY_ROOT, 'scripts', 'ensure-governance-user-story-mapping-fixture.js'));
