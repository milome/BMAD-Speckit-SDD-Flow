...
function modelResult(record, model, status, recordedAt) {
    return {
        ...
        sourceRefs: [
            {
                ...
                id: model === 'requirement_confirmation'
...
function main(argv) {
    ...
    if (!requirementRecordArg)
        throw new Error('missing --requirement-record');
    ...
    const commit = (0, requirement_record_control_store_1.appendControlEventAndReplay)({
        ...
        reduce: (record) => {
            ...
            for (const model of MODELS) {
                sixModelResults[model] = modelResult(record, model, model === 'requirement_confirmation' ? 'pass' : 'not_established', recordedAt);
            }
            return {
                ...record,
    ...
    const result = {
        ...
        requirementRecordHash: (0, requirement_record_control_store_1.sha256Text)(fs.readFileSync(recordPath, 'utf8')),
