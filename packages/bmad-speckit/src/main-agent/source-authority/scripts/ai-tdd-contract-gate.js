...
exports.evaluateAiTddContractGate = evaluateAiTddContractGate;
exports.mainAiTddContractGate = mainAiTddContractGate;
exports.aiTddContractGateRequired = aiTddContractGateRequired;
...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--help' || arg === '-h')
            ...
            out.json = true;
        else if (arg === '--execute-red-proof')
            out.executeRedProof = true;
...
function isFailurePathId(value) {
    return /^FAIL-\d+/u.test(value);
}
function isEdgeCaseId(value) {
    return /^EDGE-\d+/u.test(value);
}
function requirementCoverageRefs(values) {
...
function findProjectRoot(start) {
    ...
    while (current) {
        if (fs.existsSync(path.join(current, 'package.json')) ||
            ...
            fs.existsSync(path.join(current, '_bmad-output'))) {
            return current;
        }
...
function withActiveSourceRoot(sourcePath, fn) {
    const previousSourceRoot = activeSourceRoot;
    const previousProjectRoot = activeProjectRoot;
    activeSourceRoot = path.dirname(path.resolve(sourcePath));
    activeProjectRoot = findProjectRoot(activeSourceRoot);
    try {
        return fn();
    }
    finally {
        activeSourceRoot = previousSourceRoot;
        activeProjectRoot = previousProjectRoot;
    }
}
...
function resolvedFileExistence(ref) {
    const resolved = resolveLogicalSkillRef(resolveSkillPlaceholders(ref));
    const candidates = path.isAbsolute(resolved)
        ? [resolved]
        : unique([
            path.resolve(activeSourceRoot, resolved),
            path.resolve(activeProjectRoot, resolved),
    ...
    for (const candidate of candidates) {
        try {
            if (fs.statSync(candidate).isFile()) {
                return { absolutePath: candidate, exists: true, parentEntries: [] };
            }
        }
        catch {
    ...
    try {
        if (fs.statSync(absolute).isFile())
            return { absolutePath: absolute, exists: true, parentEntries: [] };
    }
    catch {
        // Fall through to directory-entry matching below.
    }
    ...
    if (!fs.existsSync(parent))
        return { absolutePath: absolute, exists: false, parentEntries: [] };
    const parentEntries = fs.readdirSync(parent);
    const normalizeName = (value) => value.normalize('NFC').trim();
...
function normalizePreState(value) {
    ...
    if (!raw)
        return null;
    if (['expected_red', 'red', 'valid_red', 'old_red'].includes(raw))
        return 'expected_red';
    if (['unexpected_green', 'green', 'old_green'].includes(raw))
...
function requirementRows(confirmation) {
    const traceRefsByCoveredRequirement = new Map();
    for (const trace of objects(confirmation.traceRows)) {
        const traceId = text(trace.id);
        if (!traceId)
            continue;
        for (const coveredId of strings(trace.covers)) {
            traceRefsByCoveredRequirement.set(coveredId, unique([...(traceRefsByCoveredRequirement.get(coveredId) ?? []), traceId]));
        }
    }
    const traceRefsFor = (row) => unique([
        ...idRefs(row, ['coveredByTraceRows', 'traceRows', 'traceRefs']),
        ...(traceRefsByCoveredRequirement.get(text(row.id)) ?? []),
    ]);
...
function traceCommandIndex(confirmation) {
    const index = new Map();
    for (const trace of objects(confirmation.traceRows)) {
...
function explicitAcceptanceRows(confirmation) {
    const sections = [
    ...
    return sections.flatMap(([section, kind]) => objects(confirmation[section]).map((row, index) => {
        const files = unique([
            text(row.file),
            text(row.path),
            text(row.testFile),
        ...
        return {
            id: text(row.id) ||
                text(row.testId) ||
                `${kind === 'e2e' ? 'E2E' : 'ACC'}-${section}-${index + 1}`,
            kind,
            sourceSection: section,
            files,
            commandRefs: commandRefs(row),
            covers: requirementCoverageRefs(idRefs(row, ['covers', 'requirementRefs', 'linkedRequirementIds', 'mustRefs', 'negRefs'])),
            ...
            evidenceRefs: idRefs(row, ['evidenceRefs', 'linkedEvidenceIds']),
            negativeControlRefs: idRefs(row, [
                'negativeControlRefs',
                'negativeControls',
                'linkedNegIds',
            ]),
            expectedPreImplementationState: normalizePreState(row.expectedPreImplementationState ??
...
function derivedAcceptanceRows(confirmation) {
    ...
    return objects(confirmation.requiredCommands).flatMap((command, index) => {
        ...
        const files = commandFileRefs(command).filter((file) => /\.(?:test|spec)\.(?:ts|tsx|js|jsx|mjs|cjs)$/iu.test(file));
        if (files.length === 0)
            return [];
        ...
        return [
            {
                ...
                expectedPreImplementationState: normalizePreState(command.expectedPreImplementationState ??
...
function acceptanceRows(confirmation) {
    ...
    const derived = derivedAcceptanceRows(confirmation).filter((row) => !row.commandRefs.some((ref) => explicitCommandRefs.has(ref)) &&
...
function targetModificationPaths(confirmation) {
    ...
    const taskRows = objects(confirmation.implementationTasks).flatMap((task, taskIndex) => strings(task.targetPaths).map((value, pathIndex) => ({
    ...
    return [...directRows, ...taskRows].map((row) => ({
        ...row,
...
function targetModificationPathCoverage(input) {
    ...
    const rows = input.targetModificationPaths.map((row, index) => {
        ...
        for (const ref of traceRefs) {
            if (!traceIds.has(ref))
                missing.push(issue('targetModificationPaths', 'trace_ref_missing', id, { ref }));
        }
...
function negativeControls(confirmation) {
    const controls = [
        ...objects(confirmation.notDone).map((row) => ({
            id: text(row.id),
            source: 'notDone',
            oracle: text(row.oracle) ||
                text(row.whyItBlocksCompletion) ||
        ...
        ...objects(confirmation.failurePaths).map((row) => ({
            id: text(row.id),
            source: 'failurePaths',
            oracle: text(row.expectedBehavior) ||
...
function viewCoverageIndex(confirmation) {
    const index = new Map();
    const views = [
...
function traceRefsFor(traceRows, requirementRefs, evidenceRefs, explicitRefs) {
    return unique([
        ...explicitRefs,
        ...traceRows
            .filter((trace) => strings(trace.covers).some((ref) => requirementRefs.includes(ref)) ||
            strings(trace.evidenceRefs).some((ref) => evidenceRefs.includes(ref)))
            .map((trace) => text(trace.id)),
    ]);
}
function acceptanceRefsFor(acceptance, key, id) {
...
function errorCaseCoverage(confirmation, acceptance) {
    ...
    const failureNegRefs = new Map(failureRows.map((row) => [text(row.id), idRefs(row, ['linkedNegIds', 'negRefs'])]));
    const failurePaths = failureRows
        ...
        const traceRefs = traceRefsFor(traceRows, linkedNegIds, linkedEvidenceIds, idRefs(row, ['traceRows', 'traceRefs']));
        const viewRefs = unique([
            ...idRefs(row, ['viewRefs', 'sequenceViewRefs', 'boundaryViewRefs']),
            ...(views.get(id) ?? []),
        ]);
        ...
        const missing = [
            ...
            ...(acceptanceRefs.length + e2eRefs.length > 0
                ? []
                : ['failure_path_acceptance_coverage_missing']),
    ...
    const edgeCases = edgeRows
        ...
        const missing = [
            ...(linkedFailurePathIds.length + linkedNegIds.length > 0
                ? []
                : ['edge_case_failure_or_neg_missing']),
            ...(linkedEvidenceIds.length > 0 ? [] : ['edge_case_evidence_refs_missing']),
            ...(traceRefs.length > 0 ? [] : ['edge_case_trace_coverage_missing']),
            ...(acceptanceRefs.length + e2eRefs.length > 0
                ? []
                : ['edge_case_acceptance_coverage_missing']),
        ...
        .filter((row) => text(row.id));
    const missing = [...failurePaths, ...edgeCases].flatMap((row) => strings(row.missing).map((code) => ({
        id: text(row.id),
        code,
...
function commandTargetCollection(confirmation) {
    ...
    const rows = objects(confirmation.requiredCommands)
        ...
        const collectedTraceRefs = unique([
            ...traceRefs,
            ...linkedTraceRows.map((trace) => text(trace.id)),
        ]);
        ...
        .filter((row) => row.id);
    const missing = rows.flatMap((row) => strings(row.missing).map((code) => ({ id: text(row.id), code, category: 'CMD_TARGET' })));
    return {
        ...
        missing: rows.length === 0
            ? [
                {
                    id: 'requiredCommands',
                    code: 'command_target_collection_missing',
                    category: 'CMD_TARGET',
                },
            ]
            : missing,
...
function traceClosureAssertions(confirmation) {
    const rows = objects(confirmation.traceRows)
        ...
        return {
            ...
            acceptanceRefs,
            artifactRefs,
            closureAssertion: text(row.closureAssertion) ||
                text(row.oracle) ||
                'target-state closure requires current command, evidence, acceptance, and artifact binding',
            missing,
            ready: missing.length === 0,
        };
    })
        .filter((row) => row.id);
    const missing = rows.flatMap((row) => strings(row.missing).map((code) => ({ id: text(row.id), code, category: 'TRACE_CLOSURE' })));
    return {
        decision: rows.length > 0 && missing.length === 0 ? 'pass' : 'blocked',
        ready: rows.length > 0 && missing.length === 0,
        rows,
        missing: rows.length === 0
            ? [{ id: 'traceRows', code: 'trace_closure_assertions_missing', category: 'TRACE_CLOSURE' }]
            : missing,
        summary: {
...
function canonicalSurfaceReconciliation(confirmation, targetArtifacts) {
    ...
    const missing = rows.flatMap((row) => strings(row.missing).map((code) => ({ id: text(row.id), code, category: 'CANONICAL_SURFACE' })));
    return {
        decision: rows.length > 0 && missing.length === 0 ? 'pass' : 'blocked',
        ready: rows.length > 0 && missing.length === 0,
        applies: nested(confirmation.applicability).currentTargetMap
            ? nested(nested(confirmation.applicability).currentTargetMap).applies === true
            : Object.keys(currentTargetMap).length > 0,
        rows,
        missing: rows.length === 0
            ? [
                {
                    id: 'currentTargetMap',
                    code: 'canonical_surface_reconciliation_missing',
                    category: 'CANONICAL_SURFACE',
                },
            ]
            : missing,
        summary: {
...
function currentTargetMapSection(confirmation) {
    ...
    for (const [groupName, rows] of requiredGroups) {
        if (rows.length === 0) {
            missing.push({
                id: groupName,
                code: 'current_target_map_group_missing',
    ...
    if (text(map.schemaVersion) !== 'current-target-map/v1') {
        missing.push({
            id: 'schemaVersion',
            code: 'current_target_map_schema_version_missing_or_invalid',
            category: 'CURRENT_TARGET_MAP',
        });
    }
    if (text(map.displayProfile) !== 'closed_loop_current_target_map') {
        missing.push({
            id: 'displayProfile',
            code: 'current_target_map_display_profile_missing_or_invalid',
            category: 'CURRENT_TARGET_MAP',
        });
    }
...
function legacyDenial(confirmation, targetArtifacts, controls) {
    const legacyRows = [
    ...
    const rows = legacyRows
        ...
        const missing = [
            ...
            ...(traceRefs.length + evidenceRefs.length + linkedIds.length > 0
                ? []
                : ['legacy_denial_refs_missing']),
        ];
        return {
            id,
            pathOrField: text(row.pathOrField),
    ...
    const missing = rows.flatMap((row) => strings(row.missing).map((code) => ({
        id: text(row.id) || text(row.pathOrField),
        code,
    ...
    return {
        ...
        missing: rows.length === 0
            ? [
                {
                    id: 'currentTargetMap.existingArtifacts',
                    code: 'legacy_denial_missing',
                    category: 'LEGACY_DENIAL',
                },
            ]
            : missing,
...
function closeoutProof(confirmation, targetArtifacts, requiredCommandRows, commandTargets) {
...
function evidenceTrustStates(confirmation) {
    ...
    const rows = evidenceRows
        ...
        .filter((row) => row.id);
    const missing = rows.flatMap((row) => strings(row.missing).map((code) => ({ id: text(row.id), code, category: 'EVIDENCE_TRUST' })));
    return {
        decision: rows.length > 0 && missing.length === 0 ? 'pass' : 'blocked',
        ready: rows.length > 0 && missing.length === 0,
        rows,
        missing: rows.length === 0
            ? [{ id: 'evidence', code: 'evidence_trust_states_missing', category: 'EVIDENCE_TRUST' }]
            : missing,
        summary: {
...
function mergeUniqueRefs(row, key, refs) {
    const merged = unique([...strings(row[key]), ...refs]);
    return merged.length > 0 ? { ...row, [key]: merged } : row;
}
function applyOverlayRows(rows, overlayRows, merge) {
    ...
    for (const row of overlayRows) {
        const id = text(row.id);
        if (id)
            overlaysById.set(id, row);
    }
    return rows.map((row) => {
        const overlay = overlaysById.get(text(row.id));
        return overlay ? merge(row, overlay) : row;
    });
}
function applyReadinessAutoRemediationOverlay(confirmation, record) {
    const overlay = nested(nested(record.aiTddContractGate).readinessAutoRemediationOverlay);
    if (text(overlay.schemaVersion) !== 'readiness-auto-remediation-overlay/v1')
        return confirmation;
    const sourceMutationPolicy = text(overlay.sourceMutationPolicy);
    if (sourceMutationPolicy && sourceMutationPolicy !== 'non_semantic_projection_only')
        return confirmation;
    return {
        ...confirmation,
        acceptanceTests: applyOverlayRows(objects(confirmation.acceptanceTests), objects(overlay.acceptanceBindings), (row, patch) => mergeUniqueRefs(row, 'covers', strings(patch.covers))),
        e2eSuites: applyOverlayRows(objects(confirmation.e2eSuites), objects(overlay.acceptanceBindings), (row, patch) => mergeUniqueRefs(row, 'covers', strings(patch.covers))),
        traceRows: applyOverlayRows(objects(confirmation.traceRows), objects(overlay.traceBindings), (row, patch) => {
            let out = mergeUniqueRefs(row, 'covers', strings(patch.covers));
            out = mergeUniqueRefs(out, 'evidenceRefs', strings(patch.evidenceRefs));
            out = mergeUniqueRefs(out, 'commandRefs', strings(patch.commandRefs));
            out = mergeUniqueRefs(out, 'acceptanceRefs', strings(patch.acceptanceRefs));
            return mergeUniqueRefs(out, 'artifactRefs', strings(patch.artifactRefs));
        }),
        evidence: applyOverlayRows(objects(confirmation.evidence), objects(overlay.evidenceBindings), (row, patch) => {
            const out = mergeUniqueRefs(row, 'requiredCommandRefs', strings(patch.requiredCommandRefs));
            return mergeUniqueRefs(out, 'artifactRefs', strings(patch.artifactRefs));
        }),
        requiredCommands: applyOverlayRows(objects(confirmation.requiredCommands), objects(overlay.commandBindings), (row, patch) => {
        ...
        artifactAutomationPlan: applyOverlayRows(objects(confirmation.artifactAutomationPlan), objects(overlay.artifactBindings), (row, patch) => {
...
function buildManifest(input) {
    ...
    const targetArtifacts = (0, target_artifact_realization_gate_1.deriveTargetArtifactChecklist)(confirmation);
    const controls = negativeControls(confirmation);
    ...
    const canonicalSurfaces = canonicalSurfaceReconciliation(confirmation, targetArtifacts);
    const legacyControls = legacyDenial(confirmation, targetArtifacts, controls);
    const requiredCommandRows = objects(confirmation.requiredCommands);
    const closeoutProofPlan = closeoutProof(confirmation, targetArtifacts, requiredCommandRows, commandTargets);
    ...
    const rawManifest = {
        ...
        implementationConfirmationHash: (0, target_artifact_realization_gate_1.implementationConfirmationHash)(input.confirmation),
...
function missingTestPlan(manifest) {
    const requirements = objects(manifest.requirements).filter((row) => ['MUST', 'NEG'].includes(row.kind));
    ...
    const missingFiles = acceptance.flatMap((row) => row.files
        .filter((file) => !fileExists(file))
        .map((file) => ({
        id: row.id,
        file: normalizePath(file),
        reason: 'acceptance_test_file_missing',
    })));
    return {
...
function negativeControlPlan(manifest) {
...
function targetArtifactPlan(manifest) {
    const targets = objects(manifest.targetArtifacts);
    return {
        decision: targets.length > 0 ? 'pass' : 'blocked',
        targets,
        missingPlan: targets.length === 0 ? [{ reason: 'target_artifact_plan_empty' }] : [],
    };
}
function issue(category, code, id, details = {}) {
    return { category, code, id, ...details };
}
function idSet(rows) {
    return new Set(rows.map((row) => text(row.id)).filter(Boolean));
}
function contractCompletenessReport(manifest) {
    ...
    const acc = explicitAcceptance.filter((row) => !(text(row.kind) === 'e2e' || text(row.id).startsWith('E2E-')));
    ...
    const closeoutCommandRefs = new Set(strings(nested(manifest.closeoutProof).requiredCommands));
    const artifactIds = new Set(proofArtifacts
        .flatMap((row) => [text(row.id), text(row.pathOrField), ...strings(row.aliases)])
        .filter(Boolean));
    const isContractBoundArtifact = (artifact) => {
        const traceRefs = strings(artifact.traceRefs);
        const evidenceRefs = strings(artifact.evidenceRefs);
        return (artifact.contractBound === true &&
            traceRefs.length > 0 &&
            evidenceRefs.length > 0 &&
            traceRefs.every((ref) => traceIds.has(ref)) &&
            evidenceRefs.every((ref) => evidenceIds.has(ref)));
    };
    const artifactReferenced = (artifact) => {
        const aliases = [
            text(artifact.id),
            text(artifact.pathOrField),
            ...strings(artifact.aliases),
        ].filter(Boolean);
        return (isContractBoundArtifact(artifact) ||
            aliases.some((id) => evidence.some((evd) => strings(evd.artifactRefs).includes(id)) ||
                traceRows.some((trace) => [
                    ...strings(trace.artifactRefs),
                    ...strings(trace.canonicalSurfaceRefs),
                    ...strings(trace.currentTargetMapRefs),
                ].includes(id)) ||
                targetModifications.some((target) => strings(target.artifactRefs).includes(id))));
    };
    const evidenceReferenced = (id) => requirements.some((req) => strings(req.evidenceRefs).includes(id)) ||
        traceRows.some((trace) => strings(trace.evidenceRefs).includes(id)) ||
        commands.some((command) => strings(command.evidenceRefs).includes(id)) ||
        proofArtifacts.some((artifact) => isContractBoundArtifact(artifact) && strings(artifact.evidenceRefs).includes(id));
    const commandReferenced = (id) => acceptance.some((accRow) => strings(accRow.commandRefs).includes(id)) ||
        traceRows.some((trace) => strings(trace.commandRefs).includes(id)) ||
        closeoutCommandRefs.has(id);
    const issues = [];
    if (must.length === 0)
        issues.push(issue('MUST', 'must_missing', 'MUST-*'));
    if (neg.length === 0)
        issues.push(issue('NEG', 'neg_missing', 'NEG-*'));
    if (evidence.length === 0)
        issues.push(issue('EVD', 'evidence_missing', 'EVD-*'));
    if (traceRows.length === 0)
        issues.push(issue('TRACE', 'trace_rows_missing', 'TRACE-*'));
    if (commands.length === 0)
        issues.push(issue('CMD', 'required_commands_missing', 'CMD-*'));
    if (acc.length === 0)
        issues.push(issue('ACC', 'acceptance_tests_missing', 'ACC-*'));
    if (e2e.length === 0)
        issues.push(issue('E2E', 'e2e_suites_missing', 'E2E-*'));
    if (proofArtifacts.length === 0)
        issues.push(issue('ART', 'target_artifacts_missing', 'ART-*'));
    ...
    for (const row of requirements) {
        ...
        if (!text(row.id))
            issues.push(issue(text(row.kind) || 'REQ', 'requirement_id_missing', id));
        if (!text(row.text))
            issues.push(issue(text(row.kind) || 'REQ', 'requirement_text_missing', id));
        ...
        for (const ref of strings(row.evidenceRefs)) {
            if (!evidenceIds.has(ref))
                issues.push(issue(text(row.kind) || 'REQ', 'evidence_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.traceRefs)) {
            if (!traceIds.has(ref))
                issues.push(issue(text(row.kind) || 'REQ', 'trace_ref_missing', id, { ref }));
        }
    }
    for (const row of evidence) {
        const id = text(row.id) || '<missing-evidence-id>';
        if (!text(row.id))
            issues.push(issue('EVD', 'evidence_id_missing', id));
        if (!text(row.oracle))
            issues.push(issue('EVD', 'evidence_oracle_missing', id));
        const refs = [...strings(row.requiredCommandRefs), ...strings(row.artifactRefs)];
        if (refs.length === 0)
            issues.push(issue('EVD', 'evidence_refs_missing', id));
        for (const ref of strings(row.requiredCommandRefs)) {
            if (!commandIds.has(ref))
                issues.push(issue('EVD', 'command_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.artifactRefs)) {
            if (!artifactIds.has(ref))
                issues.push(issue('EVD', 'artifact_ref_missing', id, { ref }));
        }
    }
    for (const row of traceRows) {
        const id = text(row.id) || '<missing-trace-id>';
        if (!text(row.id))
            issues.push(issue('TRACE', 'trace_id_missing', id));
        for (const ref of strings(row.covers)) {
            if (!requirementIds.has(ref))
                issues.push(issue('TRACE', 'requirement_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.evidenceRefs)) {
            if (!evidenceIds.has(ref))
                issues.push(issue('TRACE', 'evidence_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.commandRefs)) {
            if (!commandIds.has(ref))
                issues.push(issue('TRACE', 'command_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.acceptanceRefs)) {
            if (!acceptanceIds.has(ref))
                issues.push(issue('TRACE', 'acceptance_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.artifactRefs)) {
            if (!artifactIds.has(ref))
                issues.push(issue('TRACE', 'artifact_ref_missing', id, { ref }));
        }
        const linkedSlices = [
    ...
    for (const row of commands) {
        ...
        if (!text(row.id))
            issues.push(issue('CMD', 'command_id_missing', id));
        if (!text(row.command))
            issues.push(issue('CMD', 'command_text_missing', id));
        const refs = [...strings(row.traceRefs), ...strings(row.evidenceRefs)];
        if (refs.length === 0)
            issues.push(issue('CMD', 'command_trace_or_evidence_refs_missing', id));
        for (const ref of strings(row.traceRefs)) {
            if (!traceIds.has(ref))
                issues.push(issue('CMD', 'trace_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.evidenceRefs)) {
            if (!evidenceIds.has(ref))
                issues.push(issue('CMD', 'evidence_ref_missing', id, { ref }));
        }
    }
    for (const row of acceptance) {
        ...
        if (!text(row.id))
            issues.push(issue(category, 'acceptance_id_missing', id));
        if (strings(row.files).length === 0)
            issues.push(issue(category, 'acceptance_test_file_ref_missing', id));
        if (!text(row.oracle))
            issues.push(issue(category, 'acceptance_oracle_missing', id));
        for (const ref of strings(row.covers)) {
            if (!mustNegIds.has(ref))
                issues.push(issue(category, 'requirement_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.traceRefs)) {
            if (!traceIds.has(ref))
                issues.push(issue(category, 'trace_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.evidenceRefs)) {
            if (!evidenceIds.has(ref))
                issues.push(issue(category, 'evidence_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.commandRefs)) {
            if (!commandIds.has(ref))
                issues.push(issue(category, 'command_ref_missing', id, { ref }));
        }
    }
    ...
    for (const row of proofArtifacts) {
        ...
        if (!text(row.id))
            issues.push(issue('ART', 'artifact_id_missing', id));
        if (!text(row.pathOrField))
            issues.push(issue('ART', 'artifact_path_or_field_missing', id));
        const refs = [...strings(row.traceRefs), ...strings(row.evidenceRefs)];
        if (refs.length === 0)
            issues.push(issue('ART', 'artifact_trace_or_evidence_refs_missing', id));
        for (const ref of strings(row.traceRefs)) {
            if (!traceIds.has(ref))
                issues.push(issue('ART', 'trace_ref_missing', id, { ref }));
        }
        for (const ref of strings(row.evidenceRefs)) {
            if (!evidenceIds.has(ref))
                issues.push(issue('ART', 'evidence_ref_missing', id, { ref }));
        }
    }
    for (const row of evidence) {
        const id = text(row.id);
        if (id && !evidenceReferenced(id)) {
            issues.push(issue('EVD', 'orphan_evidence', id));
        }
    }
    for (const row of commands) {
        const id = text(row.id);
        if (id && !commandReferenced(id)) {
    ...
    return {
        ...
        categoryCounts: {
            ...
            targetModificationPathCoverage: objects(nested(manifest.targetModificationPathCoverage).rows)
                .length,
            canonicalSurfaceReconciliation: objects(nested(manifest.canonicalSurfaceReconciliation).rows)
                .length,
...
function commandEvidence(record, commandIdValue, attemptId) {
    const runs = commandRunsForAttempt(record, attemptId);
    const run = runs.find((item) => text(item.commandId) === commandIdValue);
    const deliveryCommand = objects(nested(record.deliveryEvidence).requiredCommands).find((item) => text(item.commandId) === commandIdValue || text(item.id) === commandIdValue);
    const artifactRefs = [
    ...
    if (!run) {
        ...
        return {
            state: staleRun ? 'stale' : 'blocked',
            reason: staleRun ? 'stale_attempt' : 'current_attempt_command_missing',
        };
    }
    if (run.exitCode !== 0)
        return { state: 'blocked', reason: 'current_attempt_command_failed' };
    if (artifactRefs.length === 0)
        return { state: 'blocked', reason: 'exitCode_only_proof' };
    return { state: 'green', reason: 'current_attempt_command_with_evidence' };
}
function acceptanceFromManifest(manifest) {
    const explicit = objects(manifest.acceptanceSuites);
    if (explicit.length > 0)
        return explicit;
    return [
        ...objects(manifest.acceptanceTests),
        ...objects(manifest.e2eSuites),
    ];
}
function commandById(manifest) {
    return new Map(objects(manifest.requiredCommands).map((row) => [text(row.id), row]));
}
function preProofFor(row, manifest) {
    return objects(manifest.preImplementationRedProofs).find((proof) => proof.acceptanceId === row.id || row.commandRefs.includes(proof.commandId));
}
function invalidRedOutput(output) {
    return /(?:ENOENT|no such file|cannot find module|syntaxerror|timed out|command not found|not recognized)/iu.test(output);
}
function executePreImplementationRedProof(row, manifest, timeoutMs) {
    ...
    if (!commandText)
        return undefined;
    const result = (0, node_child_process_1.spawnSync)(commandText, {
        cwd: process.cwd(),
        shell: true,
        encoding: 'utf8',
        timeout: timeoutMs,
...
function modeMatrix(input) {
    ...
    const acceptanceRows = input.mode === 'pre-implementation'
    ...
    const gateRows = input.subReports.map((report) => matrixRow({
...
function requirementMatrixRows(manifest, acceptanceRows, mode) {
    return objects(manifest.requirements).map((req) => {
        if (req.kind === 'OUT') {
            return matrixRow({
                id: req.id,
                category: req.kind,
        ...
        return matrixRow({
            ...
            currentState: mode === 'iteration'
                ...
                : covering.every((row) => row.currentState === 'green')
                    ? 'green'
                    : 'blocked',
            attemptBinding: covering.every((row) => row.attemptBinding === 'current')
                ? 'current'
                : covering.some((row) => row.attemptBinding === 'stale')
                    ? 'stale'
                    : 'missing',
            blockingReasons: mode === 'iteration'
                ? []
                : covering.every((row) => row.currentState === 'green')
                    ? []
...
function acceptanceRuntimeMatrix(row, record, attemptId, mode) {
    if (row.mockOnly) {
        return matrixRow({
            id: row.id,
            category: row.id.startsWith('E2E-') || row.kind === 'e2e' ? 'E2E' : 'ACC',
...
function requiredCommandMatrixRows(manifest, record, attemptId, mode) {
    return objects(manifest.requiredCommands).map((command) => {
        ...
        return matrixRow({
            ...
            commandRefs: [id],
            attemptBinding: state === 'green' ? 'current' : state === 'stale' ? 'stale' : 'missing',
            blockingReasons: state === 'green' ? [] : [text(proof.reason)],
        });
    });
}
function artifactIssueIndex(subReports) {
...
function artifactMatrixRows(manifest, subReports, mode) {
    ...
    return objects(manifest.targetArtifacts).map((artifact) => {
        ...
        return matrixRow({
            ...
            category: 'ART',
            refs: [...strings(artifact.traceRefs), ...strings(artifact.evidenceRefs)],
            currentState: issues.length === 0
                ? mode === 'iteration'
                    ? 'partial_green'
                    : 'green'
                : 'missing_artifact',
            artifactRefs: [text(artifact.pathOrField)],
            attemptBinding: issues.includes('target_artifact_attempt_binding_missing')
                ? 'missing'
                : issues.length
                    ? 'missing'
                    : 'current',
...
function traceMatrixRows(manifest, acceptanceRows, commandRows, artifactRows, mode) {
    return objects(manifest.traceRows).map((trace) => {
        ...
        return matrixRow({
            ...
            currentState: mode === 'pre-implementation'
                ? hasPartial
                    ? 'expected_red'
                    : 'missing_plan'
                : mode === 'iteration'
                    ...
                    : allGreen
                        ? 'green'
                        : 'blocked',
            attemptBinding: linked.length && linked.every((row) => row.attemptBinding === 'current')
                ? 'current'
                : 'missing',
            blockingReasons: mode === 'pre-implementation'
                ? hasPartial
                    ? []
...
function evidenceMatrixRows(manifest, acceptanceRows, commandRows, artifactRows, mode) {
...
function subReportsFor(input) {
    if (input.mode === 'pre-implementation')
        return [];
    const common = [
        (0, target_artifact_realization_gate_1.evaluateRequiredCommandFileExistence)(input),
        (0, target_artifact_realization_gate_1.evaluateTargetArtifactRealization)(input),
        (0, target_artifact_realization_gate_1.evaluateCanonicalSchemaReducerGate)(input),
        (0, target_artifact_realization_gate_1.evaluateCanonicalEventRegistryGate)(input),
        (0, target_artifact_realization_gate_1.evaluateReverseAuditReadinessGate)(input),
        (0, target_artifact_realization_gate_1.evaluateCloseoutTargetControlFlowGate)(input),
        (0, target_artifact_realization_gate_1.evaluateExternalBoundaryGate)(input),
    ];
    if (input.mode !== 'closeout')
        return common;
    return [
        ...common,
        (0, strict_closeout_proof_gate_1.evaluateStrictCloseoutProof)({
            record: input.record,
...
function preImplementationReadinessReport(input) {
    ...
    const validExpectedRed = acceptanceRows.length > 0 && acceptanceRows.every((row) => row.currentState === 'expected_red');
    ...
    return {
        ...
        mode: input.mode,
        applicable: true,
        contractComplete: input.sourceConfirmed &&
...
function evaluateAiTddContractGate(input) {
    return withActiveSourceRoot(input.sourcePath, () => {
        ...
        const source = (0, target_artifact_realization_gate_1.readImplementationConfirmation)(input.sourcePath);
        ...
        const subReportBlockers = subReports.flatMap((report) => text(report.decision) === 'pass'
            ? []
        ...
        const preImplementationBlockers = input.mode === 'pre-implementation'
            ? strings(preImplementationReadiness.blockingReasons)
            : [];
        const contractCompletenessBlockers = contractCompleteness.ready === true
            ? []
            : [
                'contract_completeness_report_blocked',
                ...strings(contractCompleteness.blockingReasons),
            ];
        const blockers = unique([
            ...baseBlockers,
...
function mainAiTddContractGate(argv) {
    const args = parseArgs(argv);
    if (args.help) {
        console.log('Usage: ai-tdd-contract-gate --source <requirement.md> --requirement-record <json> --mode <pre-implementation|pre-rerun|iteration|closeout> [--attempt-id <id>] [--report-path <json>] [--json]');
        return 0;
    }
...
function aiTddContractGateRequired(record, sourcePath) {
    if (text(nested(record.aiTddContractGate).enforcementMode) === 'skipped_by_policy')
        return false;
    if (nested(record.aiTddContractGate).required === true)
        return true;
    if (nested(record.closeout).aiTddContractGateRequired === true)
        return true;
    if (text(record.status) === 'user_confirmed' && (text(sourcePath) || text(record.sourcePath)))
        return true;
    return objects(nested(record.deliveryEvidence).requiredCommands).some((command) => AI_TDD_COMMAND_IDS.has(text(command.commandId) || text(command.id)));
}
