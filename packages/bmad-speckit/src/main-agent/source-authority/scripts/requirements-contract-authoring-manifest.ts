...
export function createRequirementsContractBuildManifestV2(input: {
  ...
  const artifactEntries = [...input.artifactEntries].sort((left, right) =>
  ...
  try {
    ...
    if (
      ...
      manifest.artifactEntries.some((entry) =>
        !entry || !nonEmptyString(entry.role) || !REQUIREMENTS_CONTRACT_AUTHORING_ARTIFACT_ROLES.includes(entry.role as RequirementsAuthoringArtifactRole) || !nonEmptyString(entry.schemaVersion) ||
