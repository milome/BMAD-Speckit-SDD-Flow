...
function Resolve-RepoRoot([string]$StartPath) {
  ...
  while ($true) {
    ...
    if (Test-Path $packageJson) {
      $packageText = Get-Content -Raw $packageJson
      if ($packageText -match '"build:scoring"') { return $current }
    }
...
'@
  ...
  if (Test-Path $ConfigBackupPath) {
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $CanonicalConfigPath) | Out-Null
    Copy-Item -Path $ConfigBackupPath -Destination $CanonicalConfigPath -Force
  }
}
