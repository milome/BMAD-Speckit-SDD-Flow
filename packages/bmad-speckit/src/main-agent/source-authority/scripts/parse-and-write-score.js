...
function parseEpicStoryFromPath(reportPath) {
    ...
    if (fileMatch)
        return { epic: fileMatch[1], story: fileMatch[2] };
    const dirMatch = reportPath.match(/story-(\d+)-(\d+)/);
    if (dirMatch)
        return { epic: dirMatch[1], story: dirMatch[2] };
    return {};
}
async function main() {
    const args = parseArgs();
    const reportPath = args.reportPath;
    const stage = (args.stage ?? 'prd');
    ...
    const scenario = (args.scenario ?? 'real_dev');
    const writeMode = (args.writeMode ?? 'single_file');
    ...
    const iterationCountParsed = iterationCountRaw != null ? parseInt(iterationCountRaw, 10) : undefined;
    const iterationCount = iterationCountParsed != null && !isNaN(iterationCountParsed) ? iterationCountParsed : undefined;
    const iterationReportPathsRaw = args.iterationReportPaths;
    const iterationReportPaths = iterationReportPathsRaw != null && iterationReportPathsRaw.trim().length > 0
    ...
    if (!skipTriggerCheck) {
        const decision = (0, trigger_loader_1.shouldWriteScore)(event, triggerStage, scenario);
        if (!decision.write) {
