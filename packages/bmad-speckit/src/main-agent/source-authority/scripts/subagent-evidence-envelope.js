...
exports.SUBAGENT_EVIDENCE_ENVELOPE_SCHEMA = void 0;
exports.sha256Object = sha256Object;
exports.sha256File = sha256File;
exports.validateSubagentEvidenceEnvelope = validateSubagentEvidenceEnvelope;
exports.buildSubagentEvidenceEnvelopeFromTaskReport = buildSubagentEvidenceEnvelopeFromTaskReport;
exports.validateTaskReportLegacyBoundary = validateTaskReportLegacyBoundary;
exports.artifactRefForPath = artifactRefForPath;
exports.writeSubagentEvidenceEnvelopeSchema = writeSubagentEvidenceEnvelopeSchema;
exports.runSubagentEvidenceEnvelopeAcceptance = runSubagentEvidenceEnvelopeAcceptance;
...
const FORBIDDEN_FIELDS = ['result', 'decision', 'closeoutDecision', 'requirementDecision'];
...
const WORKSPACE_REF_REQUIRED_FIELDS = ['kind', 'path', 'commitBefore', 'commitAfter'];
...
exports.SUBAGENT_EVIDENCE_ENVELOPE_SCHEMA = {
...
function addRecursiveFieldHits(value, fields, prefix, out) {
    if (!value || typeof value !== 'object')
        return;
    if (Array.isArray(value)) {
        for (const item of value)
            addRecursiveFieldHits(item, fields, prefix, out);
        return;
    }
    const objectValue = value;
...
function validateArtifactRef(artifact, index, projectRoot) {
    const mismatches = [];
    ...
    if (!artifactPath)
        mismatches.push(`${prefix}_path_missing`);
    if (!hash || !SHA256_PATTERN.test(hash))
        mismatches.push(`${prefix}_hash_missing_or_invalid`);
    if (!text(artifact.producer))
        mismatches.push(`${prefix}_producer_missing`);
    if (!text(artifact.purpose))
        mismatches.push(`${prefix}_purpose_missing`);
    ...
    if (!text(artifact.status))
        mismatches.push(`${prefix}_status_missing`);
    if (!text(artifact.inputVersion))
        mismatches.push(`${prefix}_input_version_missing`);
    if (!text(artifact.outputVersion))
        mismatches.push(`${prefix}_output_version_missing`);
...
function validateSubagentEvidenceEnvelope(rawEnvelope, options = {}) {
    const mismatches = [];
    ...
    if (options.expectedParentCloseoutAttemptId &&
    ...
    for (const [field, values] of [
        ...
        if (values.length === 0)
            mismatches.push(`subagent_envelope_${field}_empty`);
    }
    const workspaceRef = isObject(envelope.workspaceRef) ? envelope.workspaceRef : {};
    if (!isObject(envelope.workspaceRef))
        mismatches.push('subagent_envelope_workspace_ref_invalid');
...
function packetKind(packet) {
    if ('taskType' in packet)
        return 'execution';
    if ('resumeReason' in packet)
        return 'resume';
    return 'recommendation';
}
function buildSubagentEvidenceEnvelopeFromTaskReport(input) {
    const status = input.taskReport.status === 'done'
        ? 'accepted'
...
function validateTaskReportLegacyBoundary(taskReport, envelope) {
...
function artifactRefForPath(input) {
...
function writeSubagentEvidenceEnvelopeSchema(file) {
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, `${JSON.stringify(exports.SUBAGENT_EVIDENCE_ENVELOPE_SCHEMA, null, 2)}\n`, 'utf8');
}
function runSubagentEvidenceEnvelopeAcceptance(argv) {
    const args = parseArgs(argv);
    if (args.help) {
        console.log('Usage: subagent-evidence-envelope --envelope <json> --requirement-record <json> --schema-out <json> --report-out <json> [--project-root <dir>] [--json]');
        return 0;
    }
