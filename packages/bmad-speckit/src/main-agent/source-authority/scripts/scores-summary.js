...
const format_table_1 = require("../packages/scoring/scores/format-table");
...
function hasParsableRealDevRecords(dataPath) {
    ...
    return realDev.some((r) => (0, query_1.parseEpicStoryFromRecord)(r) != null);
}
function main() {
    ...
    if (epicArg == null && storyArg == null) {
        const records = (0, query_1.queryLatest)(DEFAULT_LIMIT, dataPath);
        if (records.length === 0) {
            console.log(EMPTY_DATA_MESSAGE);
            return;
        }
        console.log((0, format_table_1.formatScoresToTable)(records, 'all'));
        return;
    }
    if (epicArg != null) {
        const epicId = parseInt(String(epicArg), 10);
        const records = (0, query_1.queryByEpic)(epicId, dataPath);
        if (records.length === 0) {
        ...
        console.log((0, format_table_1.formatScoresToTable)(records, 'epic'));
        return;
    }
    if (storyArg != null) {
        ...
        const records = (0, query_1.queryByStory)(epicId, storyId, dataPath);
        if (records.length === 0) {
            const latest = (0, query_1.queryLatest)(1, dataPath);
            if (latest.length === 0) {
                console.log(EMPTY_DATA_MESSAGE);
        ...
        console.log((0, format_table_1.formatScoresToTable)(records, 'story'));
    }
}
main();
