...
exports.DEFAULT_PROTECTED_WRITE_PATHS = void 0;
exports.evaluateParallelMissionEvidenceIntegration = evaluateParallelMissionEvidenceIntegration;
exports.buildParallelMissionPlan = buildParallelMissionPlan;
exports.buildPrTopology = buildPrTopology;
exports.validatePrTopologyForReleaseGate = validatePrTopologyForReleaseGate;
const subagent_evidence_envelope_1 = require("./subagent-evidence-envelope");
exports.DEFAULT_PROTECTED_WRITE_PATHS = [
    ...
    for (let leftIndex = 0; leftIndex < plan.nodes.length; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < plan.nodes.length; rightIndex += 1) {
            ...
            for (const scope of shared) {
                const conflict = plan.conflicts.find((candidate) => candidate.scope === scope &&
                    left.node_id &&
                    right.node_id &&
...
function topologyIssues(plan, topology) {
    const issues = [];
    if (topology.batch_id !== plan.batch_id)
        issues.push('pr_topology_batch_id_mismatch');
    ...
    for (const id of planIds) {
        if (!topologyIds.has(id))
            issues.push(`pr_topology_required_node_missing:${id}`);
    }
    for (const id of topologyIds) {
        if (!planIds.has(id))
            issues.push(`pr_topology_unknown_node:${id}`);
    }
...
function integratedVerificationIssues(input) {
    ...
    const issues = [];
    if (!verification)
        return ['integrated_verification_missing'];
    ...
    if (commandRuns.length === 0)
        issues.push('integrated_verification_command_runs_missing');
    if (artifactRefs.length === 0)
        issues.push('integrated_verification_artifact_refs_missing');
...
function conflictProof(plan) {
    ...
    return plan.conflicts.map((conflict) => {
        const issues = [];
        if (conflict.resolution === 'block')
            issues.push(`conflict_resolution_block:${conflict.scope}`);
        if (conflict.resolution === 'serialize') {
            const positions = conflict.contenders.map((nodeId) => orderIndex.get(nodeId));
            if (positions.some((item) => item === undefined) ||
...
function evaluateParallelMissionEvidenceIntegration(input) {
    ...
    const nodeResults = input.plan.nodes.map((node) => {
        ...
        if (!envelope) {
            ...
            envelopeHash = validation.envelopeHash ?? (0, subagent_evidence_envelope_1.sha256Object)(envelope);
            issues.push(...validation.mismatches.map((issue) => `node_envelope_invalid:${node.node_id}:${issue}`));
            if (!validation.ok)
                issues.push(`node_envelope_not_accepted:${node.node_id}`);
    ...
    for (const evidence of input.nodeEvidence) {
        if (!input.plan.nodes.some((node) => node.node_id === evidence.node_id)) {
            nodeResults.push({
                ...
                packet_id: '<unknown>',
                prState: '<unknown>',
                envelopeHash: evidence.envelope ? (0, subagent_evidence_envelope_1.sha256Object)(evidence.envelope) : null,
                passed: false,
...
function buildParallelMissionPlan(input) {
    const nodes = input.nodes.map((node) => ({
        ...node,
        protected_write_paths: exports.DEFAULT_PROTECTED_WRITE_PATHS,
    }));
    const conflicts = [];
    for (let leftIndex = 0; leftIndex < nodes.length; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < nodes.length; rightIndex += 1) {
            ...
            const shared = intersects(left.write_scope, right.write_scope);
            const protectedShared = intersects([...left.write_scope, ...right.write_scope], exports.DEFAULT_PROTECTED_WRITE_PATHS);
...
function buildPrTopology(input) {
    ...
    const all_affected_stories_passed = required_nodes.every((node) => ['merged', 'closed_not_needed'].includes(node.state));
    return {
        version: 1,
