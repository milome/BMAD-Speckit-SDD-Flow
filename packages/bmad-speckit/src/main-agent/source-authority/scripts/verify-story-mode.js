...
function runVerification() {
    ...
    const currentMode = (0, bmad_config_1.getCurrentMode)(config);
    ...
    for (const stageDef of STAGE_DEFINITIONS) {
        const needsAudit = (0, bmad_config_1.shouldAudit)(stageDef.name, config);
        const validation = (0, bmad_config_1.shouldValidate)(stageDef.name, config);
        const result = {
            stage: stageDef.name,
            passed: true,
        ...
        console.log(`  审计: ${needsAudit ? '是' : '否'} (期望: ${stageDef.expectAudit ? '是' : '否'})`);
