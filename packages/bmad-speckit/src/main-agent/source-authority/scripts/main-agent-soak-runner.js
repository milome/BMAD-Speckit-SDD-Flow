...
exports.buildDeterministicSoakReport = buildDeterministicSoakReport;
exports.evaluateSoakReport = evaluateSoakReport;
exports.runWallClockSoak = runWallClockSoak;
exports.main = main;
...
function buildDeterministicSoakReport(input) {
...
function evaluateSoakReport(report) {
    const reasons = [];
    if (report.observed_duration_ms < report.target_duration_ms) {
        reasons.push('observed duration is shorter than target duration');
    }
    if (report.manual_restarts > 0)
        reasons.push('manual restarts detected');
    if (report.silent_hangs > 0)
        reasons.push('silent hangs detected');
    if (report.false_completions > 0)
        reasons.push('false completions detected');
    if (report.tick_count <= 0)
        reasons.push('no ticks recorded');
    ...
    if (report.run_kind === 'development_run_loop') {
        ...
        else if (!report.developmentRun.runLoopInvocations.some((item) => item.tickCommand && item.tickCommand.exitCode === 0)) {
    ...
    for (const event of report.recoveries) {
        ...
        if (event.duplicate_side_effect)
            reasons.push(`duplicate side-effect at tick ${event.tick}`);
        if (event.owner_count !== 1)
            reasons.push(`invalid owner count at tick ${event.tick}`);
...
function parseArgs(argv) {
    ...
    if (!out.durationMs && positional[0])
        out.durationMs = positional[0];
    if (!out.tickIntervalMs && positional[1])
        out.tickIntervalMs = positional[1];
    return out;
}
...
function startBackgroundSoak(input) {
    ...
    if (input.injectRecoverableFault)
        args.push('--injectRecoverableFault');
    if (input.developmentRunLoop)
        args.push('--developmentRunLoop');
    if (input.flow)
        args.push('--flow', input.flow);
    if (input.stage)
        args.push('--stage', input.stage);
    if (input.hostKind)
        args.push('--hostKind', input.hostKind);
    if (input.tickCommand)
        args.push('--tickCommand', input.tickCommand);
    if (input.tickCommandTimeoutMs) {
        args.push('--tickCommandTimeoutMs', String(input.tickCommandTimeoutMs));
    }
    if (input.evidenceSessionDir)
        args.push('--evidenceSessionDir', input.evidenceSessionDir);
    if (input.initialTickOffset)
        args.push('--initialTickOffset', String(input.initialTickOffset));
    if (input.runId)
        args.push('--runId', input.runId);
    if (input.storyKey)
        args.push('--storyKey', input.storyKey);
    if (input.evidenceBundleId)
        args.push('--evidenceBundleId', input.evidenceBundleId);
    ...
    const metadata = {
        ...
        completionCheck: 'Run npm run main-agent:delivery-truth-gate after the background process exits; do not claim 8h completion before the report observed_duration_ms reaches target_duration_ms.',
    };
...
function invocationFromTickRecord(input) {
    const record = readJsonFile(path.join(input.tickDir, 'tick-record.json'));
    ...
    return {
        ...
        status: input.commandExitCode === 0 &&
...
function invocationFromExistingTickRecord(tickDir) {
    const record = readJsonFile(path.join(tickDir, 'tick-record.json'));
...
function hydrateRunLoopInvocationsFromSessionDir(evidenceSessionDir, existing) {
...
async function runWallClockSoak(input) {
    const start = Date.now();
    const heartbeats = [];
    const recoveries = [];
    const runLoopInvocations = [];
    ...
    while (Date.now() - start < input.durationMs) {
        ...
        if (input.developmentRunLoop) {
            ...
            if (input.tickCommand) {
                ...
                const timeoutMs = input.tickCommandTimeoutMs && input.tickCommandTimeoutMs > 0
                    ? input.tickCommandTimeoutMs
                    : Math.max(1_000, Math.min(input.tickIntervalMs - 1_000, 120_000));
                const commandResult = (0, node_child_process_1.spawnSync)(input.tickCommand, {
                    cwd: projectRoot,
                    shell: true,
                    encoding: 'utf8',
                    timeout: timeoutMs,
            ...
            if (input.tickCommand && tickCommand) {
                ...
                const result = (0, main_agent_orchestration_1.runMainAgentAutomaticLoop)({
                    projectRoot,
                    flow,
                    stage,
                    args: {
    ...
    if (input.developmentRunLoop) {
        const hydratedInvocations = hydrateRunLoopInvocationsFromSessionDir(input.evidenceSessionDir, runLoopInvocations);
...
function gitDiffHash(projectRoot) {
    const result = (0, node_child_process_1.spawnSync)('git', ['diff', '--binary'], {
        cwd: projectRoot,
        encoding: 'utf8',
...
function main(argv) {
    ...
    if (args.startBackground === 'true') {
        const result = startBackgroundSoak({
            ...
            flow: args.flow,
            stage: args.stage,
            hostKind: args.hostKind === 'codex' || args.hostKind === 'claude' || args.hostKind === 'cursor'
                ? args.hostKind
                : undefined,
    ...
    return emit(buildDeterministicSoakReport({
