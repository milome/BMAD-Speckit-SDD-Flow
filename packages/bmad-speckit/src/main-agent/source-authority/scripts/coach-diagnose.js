...
async function main() {
    ...
    const scenarioFilter = scenarioArg === 'all'
        ? undefined
    ...
    if (epicArg != null || storyArg != null) {
        ...
        const records = storyArg != null
            ? (0, query_1.queryByStory)(epicForStory, storyId, dataPath)
            : (0, query_1.queryByEpic)(epicForStory, dataPath);
        if (records.length === 0) {
            const latest = (0, query_1.queryLatest)(1, dataPath);
            if (latest.length === 0) {
                console.log(EMPTY_DATA_MESSAGE);
            }
            else {
                const all = (0, loader_1.loadAndDedupeRecords)(dataPath);
                const realDev = all.filter((r) => r.scenario !== 'eval_question');
                const hasParsable = realDev.some((r) => (0, query_1.parseEpicStoryFromRecord)(r) != null);
                console.log(hasParsable
        ...
        const allRecords = (0, loader_1.loadAndDedupeRecords)(dataPath).filter((r) => r.run_id === latestRunId);
        const result = await (0, coach_1.coachDiagnose)(latestRunId, {
            dataPath,
            records: allRecords,
        });
        if ('error' in result) {
        ...
        if (format === 'markdown') {
            console.log((0, coach_1.formatToMarkdown)(result));
        }
        else {
    ...
    if (effectiveRunId == null) {
        const discovered = (0, coach_1.discoverLatestRunId)(dataPath, limit, scenarioFilter);
        if (discovered == null) {
            const emptyMsg = scenarioFilter === 'real_dev'
    ...
    const result = await (0, coach_1.coachDiagnose)(effectiveRunId, { dataPath });
    if ('error' in result) {
        console.error(JSON.stringify(result, null, 2));
        process.exit(1);
    }
    if (format === 'markdown') {
        const markdown = (0, coach_1.formatToMarkdown)(result);
        if (truncated) {
