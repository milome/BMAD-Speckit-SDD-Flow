...
exports.runCodexWorkerAdapter = runCodexWorkerAdapter;
exports.main = main;
...
function readPacket(packetPath) {
    return JSON.parse(readStrictUtf8JsonText(packetPath, { allowUtf8Bom: true }));
}
function sha256File(filePath) {
    return `sha256:${(0, node_crypto_1.createHash)('sha256').update(fs.readFileSync(filePath)).digest('hex')}`;
}
function packetExpectedDelta(packet) {
    return packet.expectedDelta;
}
function packetAllowedWriteScope(packet) {
...
function defaultSmokeTargetPath(input) {
    const requirementId = safePathSegment(input.recordId ?? input.requirementSetId ?? input.packet.parentSessionId);
...
function safeAgentName(role) {
    return role
        .replace(/\\/g, '/')
        .replace(/\.toml$/u, '')
        .replace(/^\/+/u, '');
}
function resolveCodexAgentSpec(projectRoot, role) {
...
function resolveRuntimeGovernanceBlock(projectRoot, options) {
    try {
        const loaded = (0, emit_runtime_policy_1.loadPolicyContextFromRegistry)(projectRoot, options);
        const currentSurface = (0, main_agent_orchestration_1.resolveMainAgentOrchestrationSurface)({
            projectRoot,
        ...
        const governance = alignPolicyWithCurrentDispatchSurface(policy, currentSurface, options?.packet);
        return {
            status: 'resolved',
            content: (0, stable_runtime_policy_json_1.stableStringifyPolicy)({
                ...governance,
...
function passImplementationEntryGateFromSurface(surface) {
    ...
    const flow = surface.orchestrationState?.flow === 'bugfix' ||
...
function alignPolicyWithCurrentDispatchSurface(policy, surface, packet) {
    ...
    const activeDispatchPacketMatches = Boolean(packet) &&
    ...
    const activeImplementDispatchPacket = activeDispatchPacketMatches && packetTaskType === 'implement';
    const packetImpliesImplement = (surface.mainAgentNextAction === 'dispatch_implement' && surface.mainAgentReady === true) ||
    ...
    const alignedSurface = activeImplementDispatchPacket
        ...
        : surface;
    const gate = passImplementationEntryGateFromSurface(alignedSurface) ??
    ...
    const helpRouting = policy.helpRouting &&
...
function ensureWithinProject(projectRoot, targetPath) {
...
function writeSmokeProof(projectRoot, relativePath, packet) {
...
function readTaskReport(taskReportPath) {
    const parsed = JSON.parse(readStrictUtf8JsonText(taskReportPath, { allowUtf8Bom: false }));
    return normalizeTaskReport(parsed);
}
function readStrictUtf8JsonText(filePath, options) {
...
function readCompiledPromptProjection(input) {
    if (!('authorityMode' in input.packet) ||
    ...
    const packetProfile = nested(input.packet.executionDisciplineProfile);
    ...
    return {
        status: 'pass',
        content,
        goalExplanation: goalMode === 'native_goal_document_ref'
...
function isNativeGoalDocumentRefPacket(packet) {
    if (!('authorityMode' in packet) ||
...
function validateTaskReport(report, packet, scopes) {
    const errors = [];
...
function taskReportStatusToEnvelopeStatus(status) {
    if (status === 'done')
        return 'done';
    if (status === 'partial')
        return 'partial';
...
function listGitVisibleFiles(projectRoot) {
    const result = (0, node_child_process_1.spawnSync)('git', ['ls-files', '--cached', '--others', '--exclude-standard', '-z'], {
        cwd: projectRoot,
        encoding: 'buffer',
...
function listFilesystemVisibleFiles(projectRoot) {
    const out = [];
...
function snapshotGitVisibleFiles(projectRoot) {
    ...
    for (const file of files.length > 0 ? files : listFilesystemVisibleFiles(projectRoot)) {
        ...
        if (!fs.existsSync(absolute) || !fs.statSync(absolute).isFile())
            continue;
        const digest = (0, node_crypto_1.createHash)('sha256').update(fs.readFileSync(absolute)).digest('hex');
...
function isAdapterOwnedPath(projectRoot, candidate, ownedPaths) {
...
function writeTaskReport(taskReportPath, report) {
    fs.mkdirSync(path.dirname(taskReportPath), { recursive: true });
    fs.writeFileSync(taskReportPath, JSON.stringify(report, null, 2) + '\n', 'utf8');
}
function readRequirementRecord(projectRoot, ...recordIds) {
    const candidates = [...new Set(recordIds.map(text).filter(Boolean))];
    for (const recordId of candidates) {
        const recordPath = path.join(projectRoot, '_bmad-output', 'runtime', 'requirement-records', recordId, 'requirement-record.json');
...
function implementationConfirmationFromSource(projectRoot, record) {
...
function confirmedExecutionRefs(input) {
    ...
    const taskRefs = uniqueStrings([
        ...
        ...objects(confirmation.mustExecutionDecompositionMatrix).flatMap((row) => strings(row.atomicTaskRefs)),
        ...objects(confirmation.traceRows).flatMap((row) => strings(row.taskRefs)),
    ]);
    return { traceRows, coveredRequirementIds, taskRefs };
}
function recordArchitectureHash(record) {
...
function recordCloseoutAttemptId(record) {
...
function evidencePathFromTaskReportItem(item) {
    const codexSmokePrefix = 'codex-smoke:';
    if (item.startsWith(codexSmokePrefix))
        return item.slice(codexSmokePrefix.length);
...
function taskReportArtifactRefs(input) {
    return input.taskReport.evidence
        ...
        .map((artifactPath) => (0, subagent_evidence_envelope_1.artifactRefForPath)({
...
function resolveCodexWritableTaskReportPath(projectRoot, taskReportPath) {
    const root = path.resolve(projectRoot);
    const requested = path.resolve(taskReportPath);
    if (requested === root || requested.startsWith(`${root}${path.sep}`)) {
        return requested;
    }
    return path.join(projectRoot, '_bmad-output', 'runtime', 'codex', 'task-reports', path.basename(path.dirname(requested)), path.basename(requested));
}
function copyTaskReportIfNeeded(sourcePath, targetPath) {
...
function runCodexWorkerAdapter(input) {
    const projectRoot = path.resolve(input.projectRoot);
    const packetPath = path.resolve(input.packetPath);
    const packet = readPacket(packetPath);
    const requirementRecord = readRequirementRecord(projectRoot, input.requirementSetId, input.recordId, packet.parentSessionId);
    const sourceDocumentHash = input.sourceDocumentHash ?? text(requirementRecord?.sourceDocumentHash);
    const implementationConfirmationHash = input.implementationConfirmationHash ?? text(requirementRecord?.implementationConfirmationHash);
    const architectureConfirmationHash = input.architectureConfirmationHash ?? recordArchitectureHash(requirementRecord);
    const parentCloseoutAttemptId = input.parentCloseoutAttemptId ||
    ...
    const taskReportPath = path.resolve(input.taskReportPath ??
        path.join(projectRoot, '_bmad-output', 'runtime', 'codex', 'task-reports', `${packet.packetId}.json`));
    const codexWritableTaskReportPath = resolveCodexWritableTaskReportPath(projectRoot, taskReportPath);
    ...
    if (runtimeGovernance.status === 'blocked' &&
    ...
    if (input.smoke) {
        ...
        const result = (0, node_child_process_1.spawnSync)(codexCommand[0], codexCommand.slice(1), {
            cwd: projectRoot,
            input: prompt,
            encoding: 'utf8',
    ...
    let taskReportStrictJsonError = null;
    ...
    const actualFilesChanged = diffFileSnapshots(beforeFileSnapshot, snapshotGitVisibleFiles(projectRoot)).filter((file) => !isAdapterOwnedPath(projectRoot, file, [
        taskReportPath,
        codexWritableTaskReportPath,
        stdinPath,
        stdoutPath,
    ...
    const transportEnvelopeValidation = (0, governance_transport_envelope_1.validateGovernanceTransportEnvelope)(transportEnvelope, {
        governanceEventTypeRegistryPolicy: input.governanceEventTypeRegistryPolicy,
        registryPolicyHash: input.governanceEventTypeRegistryPolicyHash ??
        ...
        registryHash: input.governanceEventTypeRegistryHash ??
    ...
    const coveredRequirementIds = input.coveredRequirementIds ?? confirmedRefs.coveredRequirementIds;
    ...
    const commandRun = {
        ...
        outputSummary: taskReport.evidence.join('; ').slice(0, 240) || `TaskReport status ${taskReport.status}`,
        artifactRefs: subagentArtifactRefs,
        closeoutAttemptId: parentCloseoutAttemptId,
    };
    const subagentEvidenceEnvelope = sourceDocumentHash && implementationConfirmationHash && architectureConfirmationHash
        ? (0, subagent_evidence_envelope_1.buildSubagentEvidenceEnvelopeFromTaskReport)({
            packet,
            taskReport,
            ...
            taskRefs,
            actualFilesChanged,
            diffHash: (0, subagent_evidence_envelope_1.sha256Object)(actualFilesChanged),
            workspaceRef: {
                kind: 'main_workspace',
                path: projectRoot,
                commitBefore,
    ...
    const subagentEvidenceEnvelopeValidation = subagentEvidenceEnvelope
        ? (0, subagent_evidence_envelope_1.validateSubagentEvidenceEnvelope)(subagentEvidenceEnvelope, {
...
function isDirectMainAgentCodexWorkerAdapterCli(entry) {
