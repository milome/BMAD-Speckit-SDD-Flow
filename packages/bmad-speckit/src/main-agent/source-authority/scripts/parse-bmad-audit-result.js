...
exports.parseBmadAuditResult = parseBmadAuditResult;
...
function parseBmadAuditResult(input) {
    const status = matchCanonicalThenCompatibility(input, /status:\s*(PASS|FAIL)/, /状态:\s*(PASS|FAIL)/);
    const stage = matchCanonicalThenCompatibility(input, /stage:\s*([A-Za-z_]+)/, /阶段:\s*([A-Za-z_]+)/);
    const reportPath = matchCanonicalThenCompatibility(input, /reportPath:\s*(.+)/, /报告路径:\s*(.+)/);
    const storyPath = matchCanonicalThenCompatibility(input, /storyPath:\s*(.+)/, /Story\s*文档路径:\s*(.+)/);
    const iterationCountRaw = matchCanonicalThenCompatibility(input, /iteration_count:\s*(\d+)/, /迭代次数:\s*(\d+)/);
    const requiredFixesCountRaw = matchCanonicalThenCompatibility(input, /required_fixes_count:\s*(\d+)/, /待修复项数:\s*(\d+)/);
    const scoreTriggerPresentRaw = matchCanonicalThenCompatibility(input, /score_trigger_present:\s*(true|false)/, /已检测到评分触发器:\s*(true|false)/);
    const artifactDocPath = matchCanonicalThenCompatibility(input, /artifactDocPath:\s*(.+)/, /产物文档路径:\s*(.+)/);
    const convergedRaw = matchCanonicalThenCompatibility(input, /converged:\s*(true|false)/, /是否已收敛:\s*(true|false)/);
