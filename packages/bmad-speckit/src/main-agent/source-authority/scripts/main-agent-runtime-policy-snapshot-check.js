...
exports.mainRuntimePolicySnapshotCheck = mainRuntimePolicySnapshotCheck;
...
function resolveArtifactPath(artifactPath, recordPath) {
...
function artifactMatches(ref, artifactPath, hash) {
    return (normalizePathForRecord(text(ref.path)) === normalizePathForRecord(artifactPath) &&
        firstHash(ref) === hash);
}
function findIndexedArtifact(record, artifactPath, hash) {
    return objects(record.artifactIndex).find((ref) => artifactMatches(ref, artifactPath, hash));
}
function runtimePolicySnapshotRef(record) {
...
function validateArtifactRef(input) {
    ...
    if (!ref && !input.snapshotPath)
        issues.push('runtimePolicySnapshotRef_missing');
    if (!artifactPath)
        issues.push('runtimePolicySnapshotRef_path_missing');
    if (ref) {
        ...
        if (!artifactHash)
            issues.push('runtimePolicySnapshotRef_hash_missing');
        if (!text(ref.producer))
            issues.push('runtimePolicySnapshotRef_producer_missing');
        if (!text(ref.purpose))
            issues.push('runtimePolicySnapshotRef_purpose_missing');
        if (strings(ref.relatedRequirementIds).length === 0)
            issues.push('runtimePolicySnapshotRef_relatedRequirementIds_missing');
        if (!text(ref.status))
            issues.push('runtimePolicySnapshotRef_status_missing');
        if (!text(ref.inputVersion))
            issues.push('runtimePolicySnapshotRef_inputVersion_missing');
        if (!text(ref.outputVersion))
            issues.push('runtimePolicySnapshotRef_outputVersion_missing');
    }
...
function validateSnapshot(input) {
    ...
    const expectedHashes = {
        ...
        implementationConfirmationHash: input.expectedImplementationConfirmationHash ?? text(record.implementationConfirmationHash),
        architectureConfirmationHash: input.expectedArchitectureConfirmationHash ?? architectureHash(record),
    };
    for (const [field, expected] of Object.entries(expectedHashes)) {
        const actual = text(snapshot[field]);
        if (!expected)
            issues.push(`runtimePolicySnapshot_${field}_expected_missing`);
        if (!actual)
            issues.push(`runtimePolicySnapshot_${field}_missing`);
    ...
    if (!text(snapshot.host))
        issues.push('runtimePolicySnapshot_host_missing');
    if (!text(snapshot.stage))
        issues.push('runtimePolicySnapshot_stage_missing');
...
function buildReport(args) {
    ...
    const snapshot = artifact.absolutePath && fs.existsSync(artifact.absolutePath)
...
function mainRuntimePolicySnapshotCheck(argv) {
    ...
    const reportPath = path.resolve(args.reportPath ??
        path.join(path.dirname(path.resolve(args.requirementRecord)), 'runtime-policy-snapshot-check.json'));
