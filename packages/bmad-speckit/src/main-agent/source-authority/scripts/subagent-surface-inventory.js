...
exports.subagentSurfaceInventoryInternals = void 0;
exports.runSubagentSurfaceInventory = runSubagentSurfaceInventory;
...
function defaultInventoryOutputPath(recordPath) {
...
function extractImplementationConfirmation(sourceText) {
    ...
    const parsed = nested(js_yaml_1.default.load(lines.slice(start, end).join('\n')));
...
function registryFromSource(sourcePath) {
...
function splitRegistryPaths(registryPath) {
    return registryPath
        .split(',')
        .map((item) => item.trim())
        .filter(Boolean);
}
function fileExistsForRegistryPath(root, registryPath) {
    const parts = splitRegistryPaths(registryPath);
    if (parts.length === 0)
        return false;
    return parts.some((part) => {
        if (part.includes('*'))
            return discoverFiles(root, [part]).length > 0;
        return fs.existsSync(path.resolve(root, part));
    });
}
function registryIssues(root, registry) {
    const issues = [];
    if (registry.length === 0)
        issues.push('subagentExecutionSurfaceRegistry_missing');
    const ids = new Set();
    for (const surface of registry) {
        if (!surface.surfaceId)
            issues.push('registry_surface_id_missing');
        ...
        if (!surface.surfaceType)
            issues.push(`registry_surface_type_missing:${surface.surfaceId}`);
...
function walkFiles(root, relativePath) {
    ...
    if (stat.isFile())
        return [normalizePathForRecord(path.relative(root, absolute))];
    if (!stat.isDirectory())
        return [];
    const out = [];
...
function discoverFiles(root, includePaths = TARGET_PATHS) {
...
function findMatches(filePath, content) {
    ...
    for (const pattern of MATCH_PATTERNS) {
        ...
        if (line)
            rows.push({ pattern, line: line.trim().slice(0, 500) || pattern });
    }
    return rows.map((row) => ({ ...row, pattern: canonicalPatternForPath(filePath, row.pattern) }));
}
function canonicalPatternForPath(filePath, pattern) {
    if (filePath.includes('main-agent-codex-worker-adapter.ts'))
        return 'Codex worker adapter';
    if (filePath.includes('run-auditor-host.ts'))
        return 'runAuditorHost';
    if (filePath.includes('parallel'))
        return 'parallel processing';
    if (filePath.includes('governance-host-dispatch-adapter.ts'))
        return 'external host dispatch';
    return pattern;
}
function registryForPath(filePath, registry) {
    ...
    if (normalized.startsWith('_bmad/codex/skills/bmad-story-assistant/')) {
        ...
        if (storyAndBugSurface)
            return storyAndBugSurface;
    }
    for (const surface of registry) {
        ...
        if (parts.some((part) => {
            ...
            if (normalizedPart.includes('*'))
                return globToRegex(normalizedPart).test(normalized);
...
function exclusionForPath(filePath) {
    const normalized = normalizePathForRecord(filePath);
    if (/(^|\/)(README|messages\.(en|zh))\.(md|json)$/iu.test(normalized) ||
    ...
    if (normalized.startsWith('docs/fixes/') ||
...
function surfaceKindForPath(filePath, matchPattern) {
    const normalized = normalizePathForRecord(filePath);
    if (normalized.includes('/hooks/'))
        return 'hooks_and_hook_summaries';
    if (normalized.includes('/agents/'))
        return 'agent_prompt_publication_surfaces';
    if (normalized.includes('/skills/'))
        return 'skill_and_workflow_markdown_subagent_mentions';
    ...
    if (normalized.startsWith('tests/'))
        return 'tests_and_fixtures_that_model_subagent_behavior';
...
function linkedEvidenceIds(surface, excluded) {
    if (surface || excluded)
        return ['EVD-044'];
    return [];
}
...
function validateRow(row, registryIds, scannerConfigHash) {
    const issues = [];
    for (const field of [
        'surfacePath',
        'matchPattern',
        ...
        if (!text(row[field]))
            issues.push(`row_field_missing:${row.surfacePath}:${field}`);
    }
    ...
    if (row.coverageStatus === 'excluded') {
        if (!row.explicitExclusionReason)
            issues.push(`excluded_reason_missing:${row.surfacePath}`);
    ...
    if (row.coverageStatus === 'blocked')
        issues.push(`inventory_row_blocked:${row.surfacePath}`);
    ...
    if (!isSha256(row.matchedTextHash))
        issues.push(`matched_text_hash_invalid:${row.surfacePath}`);
    return issues;
}
function validateInventory(input) {
    const issues = [];
    const registryIds = new Set(input.registry.map((surface) => surface.surfaceId));
    if (!isSha256(input.scannerConfigHash))
        issues.push('scannerConfigHash_missing_or_invalid');
    if (!isSha256(input.registryHash))
        issues.push('registryHash_missing_or_invalid');
    if (input.rows.length === 0)
        issues.push('inventory_rows_missing');
    const counts = input.rows.reduce((acc, row) => {
        acc[row.coverageStatus] += 1;
        return acc;
    }, { covered: 0, excluded: 0, blocked: 0 });
...
function runSubagentSurfaceInventory(argv) {
...
exports.subagentSurfaceInventoryInternals = {
    buildRows,
