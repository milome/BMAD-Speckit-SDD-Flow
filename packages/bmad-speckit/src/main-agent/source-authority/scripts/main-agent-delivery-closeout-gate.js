...
exports.mainDeliveryCloseoutGate = mainDeliveryCloseoutGate;
...
const per_must_closure_evidence_index_1 = require("./per-must-closure-evidence-index");
...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--help' || arg === '-h')
            ...
            out.json = true;
        else if (arg === '--allow-existing-attempt')
            out.allowExistingAttempt = true;
...
function writeSyntheticCloseoutSource(input) {
    ...
    const implementationConfirmationHash = text(input.record.implementationConfirmationHash) ||
...
function renderCloseoutConfirmation(input) {
    ...
    const rendererPath = path.resolve(process.cwd(), '_bmad', 'skills', 'requirements-contract-authoring', 'scripts', 'render-requirements-confirmation-html.ts');
    ...
    const step = (0, node_child_process_1.spawnSync)(process.execPath, renderArgs, { cwd: process.cwd(), encoding: 'utf8' });
    if (step.status !== 0) {
        throw new Error(`closeout confirmation HTML render failed (${step.status ?? 'unknown'}): ${step.stderr.trim() || step.stdout.trim()}`);
    }
...
function resolveArtifactPath(recordPath, artifactPath) {
    if (path.isAbsolute(artifactPath))
        return artifactPath;
    return path.resolve(path.dirname(recordPath), '..', '..', '..', '..', artifactPath);
}
function deliveryEvidence(record) {
...
function candidateModelPacketPaths(record, recordPath) {
    const baseDir = path.dirname(recordPath);
    const rootDir = path.resolve(baseDir, '..', '..', '..');
    const candidates = [
        ...objects(record.executionStrategySelections).flatMap((selection) => objects(selection.compiledPromptRefs)
            .map((ref) => text(ref.modelPacketPath))
            .filter(Boolean)),
        ...objects(record.executionStrategySelections).map((selection) => text(selection.modelPacketPath)),
        ...objects(record.executionIterations).flatMap((iteration) => objects(iteration.sourceRefs)
...
function selectCurrentModelPacketPath(record, recordPath) {
    ...
    const matching = candidates
        ...
        try {
            ...
            const hashMatch = (!sourceHash || packetSourceHash === sourceHash) &&
    ...
    return (matching.filter((item) => item.hashMatch).sort((left, right) => right.mtimeMs - left.mtimeMs)[0]
        ?.candidate ?? '');
}
function perMustIndexOutPath(reportPath) {
    return path.join(path.dirname(reportPath), 'per-must-closure-evidence-index.json');
}
function compiledModelPacketRequired(record) {
    return objects(record.executionStrategySelections).some((selection) => text(selection.strategyId) === 'compiled_trace_direct' ||
        text(selection.eventType) === 'execution_strategy_selected' ||
        text(selection.modelPacketHash));
}
function strictCloseoutProofContractApplies(record) {
    const referencedIds = [
...
function commandAttemptId(command) {
    const direct = text(command.closeoutAttemptId);
    if (direct)
        return direct;
    const lastRunRef = command.lastRunRef;
    return lastRunRef && typeof lastRunRef === 'object' && !Array.isArray(lastRunRef)
        ? text(lastRunRef.closeoutAttemptId)
        : '';
}
function attemptedCloseoutIds(record) {
    return new Set(objects(nested(record.closeout).attempts)
        .map((attempt) => text(attempt.closeoutAttemptId))
        .filter(Boolean));
}
function commandAttemptCompletedAt(command) {
    const lastRunRef = command.lastRunRef;
    if (lastRunRef && typeof lastRunRef === 'object' && !Array.isArray(lastRunRef)) {
        const completedAt = text(lastRunRef.completedAt);
        if (completedAt)
            return completedAt;
    }
    return text(command.completedAt);
}
function selectDefaultCloseoutAttemptId(record, fallbackAttemptId) {
    ...
    for (const candidate of orderedCandidates) {
        if (alreadyAttempted.has(candidate))
            continue;
        if (requiredCommandsForAttempt(record, candidate).length === 0)
            continue;
        if (commandRunsForAttempt(record, candidate).length === 0)
            continue;
        return candidate;
    }
    for (const attempt of objects(nested(record.closeout).attempts).reverse()) {
        const candidate = text(attempt.closeoutAttemptId);
        if (!candidate || text(attempt.decision) !== 'pass')
            continue;
        if (requiredCommandsForAttempt(record, candidate).length === 0)
            continue;
        if (commandRunsForAttempt(record, candidate).length === 0)
...
function strictCloseoutProofRequired(record, attemptId) {
    return (strictCloseoutProofContractApplies(record) ||
        requiredCommandsForAttempt(record, attemptId).some((command) => text(command.commandId) === STRICT_CLOSEOUT_PROOF_COMMAND_ID));
}
function latestRequirementClosures(record) {
    const latestByRequirementId = new Map();
    for (const closure of objects(record.requirementClosures)) {
        const requirementId = text(closure.requirementId);
        if (requirementId)
            latestByRequirementId.set(requirementId, closure);
    }
    return [...latestByRequirementId.values()];
}
function latestFailureRecords(record) {
    const latestByFailureId = new Map();
    for (const failure of objects(record.failureRecords)) {
        const failureId = text(failure.failureId);
        if (failureId)
            latestByFailureId.set(failureId, failure);
    }
    return [...latestByFailureId.values()];
}
function isOpenLifecycleStatus(value) {
    return ['open', 'in_progress', 'blocked'].includes(text(value));
}
function isSupersededCloseoutFailure(failure, currentAttemptId) {
    return (text(failure.type) === 'delivery_closeout_blocked' &&
        text(failure.closeoutAttemptId) !== '' &&
        text(failure.closeoutAttemptId) !== currentAttemptId);
}
function isCurrentAttemptCloseoutFailure(failure, currentAttemptId) {
    return (text(failure.type) === 'delivery_closeout_blocked' &&
        text(failure.closeoutAttemptId) === currentAttemptId &&
        text(failure.failureId) === `failure:${currentAttemptId}`);
}
function closeoutAttemptRefId(record) {
    return (text(record.closeoutAttemptId) ||
...
function isSupersededCloseoutRca(record, currentAttemptId) {
    if (text(record.type) !== 'closeout_blocker')
        return false;
    const attemptRef = closeoutAttemptRefId(record);
    return Boolean(attemptRef && attemptRef !== currentAttemptId);
}
function isCurrentAttemptCloseoutRca(record, currentAttemptId) {
    if (text(record.type) !== 'closeout_blocker')
        return false;
    const attemptRef = closeoutAttemptRefId(record);
    return Boolean(attemptRef && attemptRef === currentAttemptId);
}
function latestRcaRecords(record) {
    const latestByRcaId = new Map();
    for (const rca of objects(record.rcaRecords)) {
        const rcaId = text(rca.rcaId);
        if (rcaId)
            latestByRcaId.set(rcaId, rca);
    }
    return [...latestByRcaId.values()];
}
function latestRerunLoops(record) {
    const latestByRerunLoopId = new Map();
    for (const loop of objects(record.rerunLoops)) {
        const rerunLoopId = text(loop.rerunLoopId);
        if (rerunLoopId)
            latestByRerunLoopId.set(rerunLoopId, loop);
    }
    return [...latestByRerunLoopId.values()];
}
function nativeGoalAttemptFromObject(item, recordId, orderIndex) {
    if (text(item.executionRuntimeMode) !== 'native_goal')
        return null;
    ...
    if (!packetId || !attemptId || !goalExecutionHash)
        return null;
    return {
        recordId,
        attemptId,
        ...
        orderingKey: text(item.recordedAt) ||
            text(item.selectedAt) ||
            text(item.completedAt) ||
            String(orderIndex).padStart(8, '0'),
    };
}
function nativeGoalAttemptsFromRuntimeModeArtifacts(projectRoot, recordId) {
    const runtimeRoot = (0, host_runtime_mode_1.runtimeModeDir)(projectRoot, recordId, '.');
    ...
    return fs
        ...
        .map((entry, index) => {
        const selectionPath = path.join(parentDir, entry.name, 'execution-runtime-mode-selection.json');
...
function latestNativeGoalCloseoutAttempt(record, recordPath) {
    ...
    for (const attempt of [...runtimeAttempts, ...explicitAttempts]) {
        ...
        byAttempt.set(attempt.attemptId, {
            ...
            orderingKey: attempt.orderingKey > (existing?.orderingKey ?? '') ? attempt.orderingKey : existing?.orderingKey ?? attempt.orderingKey,
        });
    }
    return [...byAttempt.values()].sort((left, right) => left.orderingKey.localeCompare(right.orderingKey)).at(-1) ?? null;
}
function nativeGoalReceiptPath(input) {
    return path.join((0, host_runtime_mode_1.runtimeModeDir)(input.projectRoot, input.recordId, input.attemptId), 'native-goal-invocation-receipt.json');
}
function readNativeGoalReceiptIfExists(filePath) {
    if (!fs.existsSync(filePath))
        return null;
    return readJson(filePath);
}
function strictNativeTaskReportIssues(filePath, packetId) {
    if (!filePath || filePath === 'not_available')
    ...
    try {
        ...
        const issues = [];
        if (text(report.packetId) !== packetId)
            issues.push('packetId');
        if (text(report.status) !== 'done')
            issues.push('status');
        if (!Array.isArray(report.filesChanged))
            issues.push('filesChanged');
        if (!Array.isArray(report.validationsRun))
            issues.push('validationsRun');
        if (!Array.isArray(report.evidence))
            issues.push('evidence');
        if (!Array.isArray(report.downstreamContext))
            issues.push('downstreamContext');
        return issues;
    }
    catch {
        return ['taskReportPath'];
...
function nativeGoalCloseoutEvidenceIssues(record, recordPath) {
    ...
    const projectRoot = projectRootForRecordPath(recordPath);
    const receiptBlocker = (0, host_runtime_mode_1.validateNativeGoalInvocationReceipt)({
        projectRoot,
...
function artifactCompletenessIssues(artifactRef) {
    ...
    if (!text(ref.hash ?? ref.contentHash))
        issues.push('hash_missing');
    if (!text(ref.producer))
...
function artifactIndexed(record, artifactRef) {
    ...
    return objects(record.artifactIndex).some((item) => {
        if (artifactCompletenessIssues(item).length > 0)
            return false;
        return (normalizePathForRecord(text(item.path)) === refPath &&
            text(item.contentHash ?? item.hash) === refHash &&
            text(item.sourceOfTruthRole) === 'evidence');
    });
}
function concreteEvidenceIssues(item, prefix, itemId) {
    const issues = [];
...
function latestActiveArtifact(record, predicate) {
    const artifacts = objects(record.artifactIndex).filter((artifact) => text(artifact.sourceOfTruthRole) === 'evidence' &&
        text(artifact.status) === 'active' &&
        predicate(artifact));
    return artifacts.at(-1) ?? null;
}
function latestFailureCaseCoverageArtifact(record) {
    return latestActiveArtifact(record, (artifact) => text(artifact.artifactType) === 'failure_case_coverage');
}
function failureCaseCoverageIssues(record) {
    const artifact = latestFailureCaseCoverageArtifact(record);
    if (!artifact)
        return ['failure_case_coverage_artifact_missing'];
    ...
    const coverage = report.resumeFailureCaseRegistryCoverage;
    ...
    if (text(report.sourceDocumentHash) &&
    ...
    if (text(report.implementationConfirmationHash) &&
    ...
    if (text(report.architectureConfirmationHash) &&
    ...
    if (!total)
        out.push('failure_case_coverage_total_missing');
    if (total !== exercised)
        out.push(`failure_case_coverage_incomplete:${exercised}/${total}`);
    for (const caseId of unexercised)
        out.push(`failure_case_unexercised:${caseId}`);
    if (issues.length > 0)
        out.push('failure_case_coverage_registry_issues');
    if (blockingIssues.length > 0)
        out.push('failure_case_coverage_blocking_issues');
    const caseEvidence = objects(coverage?.caseEvidence).length > 0
...
function runtimeRootFromRecordPath(recordPath) {
    return path.dirname(path.dirname(path.dirname(recordPath)));
}
function deliveryTruthGateReportCandidates(recordPath) {
...
function deliveryTruthGateIssues(recordPath) {
...
function defaultDatasetId(record) {
    return `${text(record.recordId)}-governed-sft`.toLowerCase();
}
function resolveDefaultDatasetReleaseReport(record, recordPath) {
    return path.join(runtimeRootFromRecordPath(recordPath), 'datasets', defaultDatasetId(record), 'v1', 'dataset-release-gate-report.json');
}
function resolveDefaultDatasetManifest(record, recordPath) {
...
function boolAt(root, pathSegments) {
    let current = root;
    for (const segment of pathSegments)
        current = nested(current)[segment];
    return typeof current === 'boolean' ? current : null;
}
function contractApplicability(sourcePath) {
    if (!sourcePath)
        return {};
    try {
        return nested((0, target_artifact_realization_gate_1.readImplementationConfirmation)(sourcePath).confirmation.applicability);
    }
    catch {
        return {};
    }
}
function failureCaseCoverageRequired(record, applicability) {
...
function productionSubsystemEvidenceRequired(record, applicability, attemptId) {
    const explicit = boolAt(applicability, ['productionSubsystems', 'applies']);
    if (explicit !== null)
        return explicit;
    return Boolean(latestActiveExtensionRef(record) ||
        latestActiveArtifact(record, (artifact) => ['production_subsystem_acceptance_report', 'production_loop_ready_report'].includes(text(artifact.artifactType))) ||
        commandRunsForAttempt(record, attemptId).some((run) => text(run.commandId) === 'CMD-PRODUCTION-SUBSYSTEM-ACCEPTANCE'));
}
function datasetReleaseEvidenceRequired(record, applicability) {
    const explicit = boolAt(applicability, ['scoringDashboardSft', 'applies']);
    if (explicit !== null)
        return explicit;
    return Boolean(latestActiveArtifact(record, (artifact) => ['dataset_release_manifest', 'dataset_manifest', 'dataset_release_gate_report'].includes(text(artifact.artifactType))));
}
function latestActiveExtensionRef(record) {
    const refs = objects(record.extensionRefs).filter((ref) => {
        const relatedIds = strings(ref.relatedRequirementIds);
        return (text(ref.status) === 'active' &&
...
function artifactRefIssues(ref, prefix) {
    if (!ref)
        return [`${prefix}_missing`];
    const issues = [];
    if (!text(ref.path))
        issues.push(`${prefix}_path_missing`);
    if (!isSha256(artifactHash(ref)))
        issues.push(`${prefix}_hash_missing`);
    if (!text(ref.producer))
        issues.push(`${prefix}_producer_missing`);
    if (!text(ref.purpose))
        issues.push(`${prefix}_purpose_missing`);
    if (strings(ref.relatedRequirementIds).length === 0)
        issues.push(`${prefix}_related_requirement_ids_missing`);
    if (!text(ref.inputVersion))
        issues.push(`${prefix}_input_version_missing`);
    if (!text(ref.outputVersion))
        issues.push(`${prefix}_output_version_missing`);
    if (text(ref.sourceOfTruthRole) !== 'evidence')
        issues.push(`${prefix}_source_of_truth_role_not_evidence`);
    if (text(ref.status) !== 'active')
        issues.push(`${prefix}_not_active`);
    return issues;
}
function hashBindingIssues(record, binding, prefix) {
    const issues = [];
...
function subsystemAcceptanceIssues(record, extension) {
    const issues = [];
    ...
    if (!registryItems.length)
        issues.push('production_subsystem_acceptance_registry_missing');
    ...
    for (const subsystemId of REQUIRED_SUBSYSTEM_IDS) {
        ...
        if (!isSha256(text(subsystem.hash)))
            issues.push(`subsystem_hash_missing:${subsystemId}`);
        ...
        for (const failureCase of strings(acceptance.requiredFailureCases)) {
            if (!failureModes.includes(failureCase))
                issues.push(`subsystem_acceptance_failure_case_not_satisfied:${subsystemId}:${failureCase}`);
        }
...
function extensionProductionIssues(record, recordPath) {
    const issues = [];
    ...
    if (!extensionRef || refIssues.length > 0)
...
function productionLoopReadyReportIssues(record, recordPath) {
    const artifact = latestActiveArtifact(record, (item) => ['production_subsystem_acceptance_report', 'production_loop_ready_report'].includes(text(item.artifactType)));
    if (!artifact)
        return ['production_loop_ready_report_artifact_missing'];
    ...
    if (!fs.existsSync(reportPath))
        return ['production_loop_ready_report_missing'];
    ...
    if (text(report.decision) !== 'pass')
        issues.push('production_loop_ready_report_not_pass');
...
function datasetReleaseIssues(record, recordPath) {
    const issues = [];
    ...
    const manifestArtifact = latestActiveArtifact(record, (item) => ['dataset_release_manifest', 'dataset_manifest'].includes(text(item.artifactType)) &&
        normalizePathForRecord(text(item.path)) ===
            normalizePathForRecord(path.relative(process.cwd(), manifestPath)));
    const reportArtifact = latestActiveArtifact(record, (item) => text(item.artifactType) === 'dataset_release_gate_report' &&
        normalizePathForRecord(text(item.path)) ===
    ...
    if (text(report.decision) !== 'pass')
        issues.push('dataset_release_gate_not_pass');
    if (text(report.recordId) !== text(record.recordId))
...
function productionBlockerIssues(record, recordPath, options = {
...
function subagentCurrentAttemptRevalidationIssues(record, attemptId) {
    const subagentEnvelopeEvents = objects(record.executionIterations).filter((iteration) => text(iteration.eventType) === 'subagent_evidence_envelope_recorded' &&
    ...
    const reports = objects(record.artifactIndex).filter((artifact) => text(artifact.sourceOfTruthRole) === 'evidence' &&
        text(artifact.status) === 'active' &&
    ...
    for (const event of subagentEnvelopeEvents) {
        ...
        const matching = reports.find((artifact) => {
            ...
            const absolute = artifactPath
                ? path.isAbsolute(artifactPath)
                    ...
                    : path.resolve(process.cwd(), artifactPath)
                : '';
            if (!artifactPath || !fs.existsSync(absolute))
                return false;
            if (sha256File(absolute) !== text(artifact.hash ?? artifact.contentHash))
                return false;
            const report = readJson(absolute);
            return (text(report.schemaVersion) === 'subagent-current-attempt-revalidation/v1' &&
        ...
        if (!passed)
            issues.push(`subagent_current_attempt_revalidation_missing:${envelopeHash || text(event.executionIterationId) || '<missing>'}`);
    }
    return { issues: [...new Set(issues)], checks };
}
function parallelMissionEvidenceRequired(record) {
    const hasParallelCommand = objects(deliveryEvidence(record).requiredCommands).some((command) => text(command.commandId) === 'CMD-PARALLEL-MISSION-EVIDENCE-INTEGRATION');
    const hasTrace037Execution = objects(record.executionIterations).some((iteration) => strings(iteration.traceRows).includes('TRACE-037'));
    const hasTrace037Closure = objects(record.requirementClosures).some((closure) => text(closure.requirementId) === 'TRACE-037' ||
        strings(closure.traceRows).includes('TRACE-037'));
    const hasParallelArtifact = objects(record.artifactIndex).some((artifact) => text(artifact.artifactType) === 'parallel_mission_evidence_integration_report');
    return hasParallelCommand || hasTrace037Execution || hasTrace037Closure || hasParallelArtifact;
}
function parallelMissionEvidenceIntegrationIssues(record, attemptId) {
    ...
    for (const artifact of artifacts) {
        ...
        if (!artifactPath)
            artifactIssues.push('parallel_mission_report_path_missing');
        if (absolute && !fs.existsSync(absolute))
            artifactIssues.push(`parallel_mission_report_file_missing:${normalizePathForRecord(artifactPath)}`);
        if (absolute &&
        ...
        if (text(report.decision) !== 'pass')
            artifactIssues.push('parallel_mission_report_not_pass');
        if (!isCurrentAttemptReport)
        ...
        for (const check of objects(report.checks)) {
            if (check.passed !== true)
                artifactIssues.push(`parallel_mission_report_check_not_passed:${text(check.id) || '<missing>'}`);
        }
        ...
        for (const node of nodeResults) {
            if (node.passed !== true)
                artifactIssues.push(`parallel_mission_node_not_passed:${text(node.node_id) || '<missing>'}`);
            if (!text(node.envelopeHash))
                artifactIssues.push(`parallel_mission_node_envelope_hash_missing:${text(node.node_id) || '<missing>'}`);
        }
        const integratedVerification = report.integratedVerification &&
...
function hasImplementationReadinessPass(record) {
    return objects(record.gateChecks).some((check) => text(check.gate) === 'Implementation Readiness Gate' && text(check.decision) === 'pass');
}
function sixModelResult(record, model) {
    const results = nested(record.sixModelResults);
    return nested(results[model]);
}
function currentSixModelPassIssues(record, model) {
...
function architectureConfirmationIssues(record) {
    ...
    try {
        resolvedRecipeHash = (0, architecture_confirmation_hash_recipe_1.resolveArchitectureConfirmationHashRecipe)().resolvedRecipeHash;
    }
    catch {
    ...
    const transition = stateCheck?.stateTransition &&
...
function hasBlockingScoringState(record) {
    ...
    const openScoreFailures = objects(record.failureRecords).filter((failure) => ['score_write_failed', 'score_threshold_or_dimension_failed'].includes(text(failure.type)) &&
        ['open', 'in_progress', 'blocked'].includes(text(failure.status)));
    return (['fail', 'blocked'].includes(materializationDecision) ||
        ['fail', 'blocked'].includes(evaluationDecision) ||
        openScoreFailures.length > 0);
}
function legacyClosedLoopEvidenceRequired(record) {
    if (objects(record.extensionRefs).length > 0)
        return true;
    if (objects(record.artifactIndex).some((artifact) => [
    ...
    return ids.some((id) => /^TRACE-0(?:0[6-9]|[1-3][0-9]|40)$/u.test(id) ||
        [
            'EVD-039',
            'EVD-040',
            'EVD-041',
...
function rerunLoopSourceIssues(loop) {
    ...
    for (const sourceRef of sourceRefs) {
        ...
        if (!text(sourceRef.id))
            issues.push(`rerun_loop_source_ref_id_missing:${loopId}`);
    }
...
function hookReconciliationIssues(record) {
    const reconciliation = record.hookReconciliation &&
    ...
    const sequenceLedger = reconciliation.sequenceLedger &&
    ...
    const hookTrustReconciled = fallbackModeAllowed &&
    ...
    for (const receipt of missingReceipts) {
        issues.push(`hook_missing_receipt:${text(receipt.receiptType) || '<missing>'}:${text(receipt.expectedEventId) || '<missing>'}`);
    }
...
function evaluate(record, recordPath, attemptId, sourcePath, modelPacketPath, reportPath) {
    const checks = [];
    const blockingReasons = [];
    const openReconfirmations = (0, reconfirmation_runtime_1.openReconfirmationRequests)(record);
    checks.push({
    ...
    if (!readinessPassed)
        blockingReasons.push('implementation_readiness_gate_not_passed');
    ...
    if (scoringBlocked)
        blockingReasons.push('score_gate_failure_unresolved');
    ...
    if (strictCloseoutProofRequired(record, attemptId)) {
        ...
        const strictProof = (0, strict_closeout_proof_gate_1.evaluateStrictCloseoutProof)({
            record,
            recordPath,
            attemptId,
        ...
        checks.push({
            ...
            passed: Boolean(strictProofCommand) &&
        ...
        if (!strictProofCommand)
            blockingReasons.push('strict_closeout_proof_required_command_missing');
        if (!strictProofRun)
    ...
    const resolvedAiTddSourcePath = sourcePath
        ...
        : text(record.sourcePath) && fs.existsSync(text(record.sourcePath))
            ? path.resolve(text(record.sourcePath))
            : '';
    if ((0, ai_tdd_contract_gate_1.aiTddContractGateRequired)(record, resolvedAiTddSourcePath)) {
        const resolvedSourcePath = sourcePath
            ? path.resolve(sourcePath)
            : text(record.sourcePath) && fs.existsSync(text(record.sourcePath))
                ? path.resolve(text(record.sourcePath))
        ...
        if (!resolvedSourcePath) {
            ...
            const aiTddGate = (0, ai_tdd_contract_gate_1.evaluateAiTddContractGate)({
            ...
            if (text(aiTddGate.decision) !== 'pass') {
                blockingReasons.push('ai_tdd_contract_gate_not_passed', ...strings(aiTddGate.blockingReasons));
            }
        }
    }
    for (const command of requiredCommands) {
        ...
        const artifactIssues = artifactRefs.flatMap((ref) => artifactCompletenessIssues(ref).map((issue) => `required_command_artifact_incomplete:${commandId}:${issue}`));
        const artifactsPresent = artifactRefs.length > 0 &&
        ...
        if (!passed)
            blockingReasons.push(`required_command_not_satisfied:${commandId || '<missing>'}`);
    }
    ...
    if (!hasNegative)
        blockingReasons.push('negative_or_regression_command_missing');
    const openClosures = latestRequirementClosures(record).filter((closure) => ['open', 'fail', 'blocked'].includes(text(closure.status)));
    checks.push({
    ...
    if (openClosures.length > 0)
        blockingReasons.push('requirement_closures_not_terminal');
    ...
    if (resolvedModelPacketPath) {
        const perMustIndex = (0, per_must_closure_evidence_index_1.buildPerMustClosureEvidenceIndex)({
        ...
        if (perMustOutPath)
            writeJsonAtomic(perMustOutPath, perMustIndex);
        checks.push({
        ...
        if (text(perMustIndex.decision) !== 'pass') {
            blockingReasons.push('per_must_closure_evidence_index_not_passed', ...strings(perMustIndex.blockingReasons));
        }
    }
    else {
        ...
        if (modelPacketRequired) {
            blockingReasons.push('per_must_closure_evidence_index_not_passed', 'model_packet_not_available');
        }
    }
    const openReruns = latestRerunLoops(record).filter((loop) => ['open', 'in_progress', 'no_progress', 'blocked'].includes(text(loop.status)));
    checks.push({
        id: 'rerun-loops-closed',
        passed: openReruns.length === 0,
        openCount: openReruns.length,
    });
    if (openReruns.length > 0)
        blockingReasons.push('pending_rerun_exists');
    ...
    const openFailureRecords = latestFailureRecords(record).filter((record) => isOpenLifecycleStatus(record.status) &&
    ...
    if (openFailureRecords.length > 0)
        blockingReasons.push('open_failure_record_exists');
    ...
    if (openRcaRecords.length > 0)
        blockingReasons.push('open_rca_action_exists');
    const decision = blockingReasons.length === 0 ? 'pass' : 'blocked';
    return { decision, blockingReasons, checks };
}
function closeoutFailureSourceRefs(record, attemptId, gateCheckId) {
    const refs = [
...
function failureRecordsForCloseout(record, input) {
    if (input.decision === 'pass')
        return objects(record.failureRecords);
    const existing = objects(record.failureRecords);
    const failureId = `failure:${input.attemptId}`;
    if (existing.some((failure) => text(failure.failureId) === failureId))
        return existing;
...
function rcaRecordsForCloseout(record, input) {
    if (input.decision === 'pass')
        return objects(record.rcaRecords);
    ...
    if (existing.some((rca) => text(rca.rcaId) === rcaId))
        return existing;
    return [
        ...existing,
        {
...
function updateRecord(record, input) {
    ...
    const closeoutEventType = input.decision === 'pass'
        ? 'delivery_confirmation_user_acceptance_requested'
        : 'delivery_confirmation_result_recorded';
    const deliveryConfirmationStatus = input.decision === 'pass' ? 'awaiting_user_acceptance' : input.decision;
    ...
    const nextAttempts = attemptExists && input.allowExistingAttempt === true
        ? attempts.map((existingAttempt) => text(existingAttempt.closeoutAttemptId) === input.attemptId ? attempt : existingAttempt)
        : [...attempts, attempt];
    return {
        ...record,
        closeout: {
        ...
        currentMentalModel: input.decision === 'pass'
            ? 'delivery_confirmation'
            : text(record.currentMentalModel) || 'delivery_confirmation',
        currentStage: input.decision === 'pass'
            ? 'delivery_confirmation'
...
function mainDeliveryCloseoutGate(argv) {
    ...
    const existingCloseout = record.closeout && typeof record.closeout === 'object' && !Array.isArray(record.closeout)
        ? record.closeout
        : {};
    ...
    const evaluation = evaluate(record, recordPath, attemptId, sourcePathForCloseout, args.modelPacket, reportPath);
    const report = {
    ...
    const commit = (0, requirement_record_control_store_1.appendControlEventAndReplay)({
        ...
        eventType: evaluation.decision === 'pass'
            ? 'delivery_confirmation_user_acceptance_requested'
            : 'delivery_confirmation_result_recorded',
