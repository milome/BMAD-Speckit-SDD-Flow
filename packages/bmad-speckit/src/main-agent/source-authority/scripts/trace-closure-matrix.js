...
exports.buildTraceClosureMatrix = buildTraceClosureMatrix;
exports.mainTraceClosureMatrix = mainTraceClosureMatrix;
...
function latestClosure(record, id) {
    const matches = objects(record.requirementClosures).filter((closure) => text(closure.requirementId) === id);
    return matches.length > 0 ? matches[matches.length - 1] : null;
}
function sourceRefsContain(sourceRefs, sourceType, id) {
    return objects(sourceRefs).some((ref) => text(ref.sourceType) === sourceType && text(ref.id) === id);
}
function executionsFor(record, id) {
    return objects(record.executionIterations).filter((iteration) => strings(iteration.traceRows).includes(id) ||
...
function requiredCommandsFor(record, id) {
    const deliveryEvidence = record.deliveryEvidence;
    return objects(deliveryEvidence?.requiredCommands).filter((command) => strings(command.traceRows).includes(id) || strings(command.evidenceRefs).includes(id));
}
function matrixRow(record, id) {
...
function referencedIds(record) {
    ...
    for (const iteration of objects(record.executionIterations)) {
        for (const id of strings(iteration.traceRows))
            ids.add(id);
        for (const id of strings(iteration.evidenceRefs))
            ids.add(id);
    }
    return [...ids].sort();
}
function buildTraceClosureMatrix(record) {
    ...
    return {
        ...
        projectionClosurePolicy: {
            ...
            pendingSourceStatusMeaning: 'PENDING in the confirmed source document preserves the user-confirmed implementationConfirmation projection and does not represent runtime delivery closure.',
            passCriteria: 'Runtime PASS requires latest requirementClosures status=pass plus current deliveryEvidence.requiredCommands and supporting evidence artifacts.',
            semanticMutationPolicy: 'Changing source traceRows status from PENDING to PASS would mutate the confirmed contract and requires reconfirmation; closeout records projection evidence instead.',
        },
        readModelBoundary: 'TaskReport done, agent said done, smoke passed, quality gate passed, release gate passed, delivery truth passed, and tests passed are evidence inputs only; they cannot close a requirement without requirementClosures plus deliveryEvidence.requiredCommands.',
        counts: {
            rows: rows.length,
...
function mainTraceClosureMatrix(argv) {
