...
exports.syncAuditIndexFromReport = syncAuditIndexFromReport;
exports.recordAuthoritativeAuditCloseout = recordAuthoritativeAuditCloseout;
exports.syncAuditIndexFromAllReports = syncAuditIndexFromAllReports;
exports.recordLatestReviewerCloseout = recordLatestReviewerCloseout;
exports.buildImplementationEntryIndexKey = buildImplementationEntryIndexKey;
exports.recordImplementationEntryGate = recordImplementationEntryGate;
exports.invalidateImplementationEntryGates = invalidateImplementationEntryGates;
...
function syncAuditIndexFromReport(root, reportPath) {
    ...
    const parsed = (0, parse_bmad_audit_result_1.parseBmadAuditResult)(fs.readFileSync(reportPath, 'utf8'));
...
function recordAuthoritativeAuditCloseout(root, input) {
    const registry = readRegistryOrDefault(root);
...
function writeJsonWithFsync(file, payload) {
    fs.mkdirSync(path.dirname(file), { recursive: true });
...
function mergeLatestReviewerCloseoutIntoContextFile(contextPath, closeout) {
...
function recordLatestReviewerCloseout(root, closeout) {
    const registry = readRegistryOrDefault(root);
...
function buildImplementationEntryIndexKey(input) {
    if (input.flow === 'story') {
        const key = normalizeText(input.runId) ||
...
function invalidateImplementationEntryGates(root, input) {
    const registry = readRegistryOrDefault(root);
