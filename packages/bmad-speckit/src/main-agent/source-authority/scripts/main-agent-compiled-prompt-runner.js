...
exports.resolveConfirmedSource = resolveConfirmedSource;
exports.runMainAgentCompiledPrompt = runMainAgentCompiledPrompt;
...
function hasForbiddenProfileField(value, pathPrefix = '') {
...
function writeProfileSnapshot(filePath, profile) {
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
    fs.writeFileSync(filePath, `${JSON.stringify(profile, null, 2)}\n`, 'utf8');
}
function defaultTaskReportPath(projectRoot, sessionId, packetId) {
    return path.join(projectRoot, '_bmad-output', 'runtime', 'governance', 'task-reports', sessionId, `${packetId}.json`);
}
function hasControlledConfirmation(record) {
...
function resolveConfirmedSource(input) {
    ...
    const hasInlineConfirmedImplementation = text(record.implementationConfirmation?.status) ===
        'user_confirmed';
    ...
    const sourcePath = path.resolve(input.projectRoot, input.sourcePath || text(record.sourcePath) || text(record.artifactPath));
    const blockingReasons = [];
    if (!fs.existsSync(sourcePath))
        blockingReasons.push('source_path_missing');
    ...
    if (!sourceDocumentHash)
        blockingReasons.push('source_document_hash_missing');
...
function resolveReqTraceSkillDir(projectRoot) {
    ...
    if (!found)
        throw new Error('req-trace skill generate_prompt.js not found');
    return found;
}
function runMainAgentCompiledPrompt(input) {
    ...
    if (confirmedSource.status !== 'confirmed') {
        return {
            ...
            blockingReasons: confirmedSource.status === 'confirmed_source_unresolvable'
    ...
    const sessionId = text(confirmedRecord.requirementSetId) ||
    ...
    const profileRefPath = input.profileRefPath ?? path.join(outDir, 'execution-discipline-profile.json');
    let expectedProfileHash = input.executionDisciplineProfile?.profileHash ?? null;
    ...
    if ((result.status ?? 1) !== 0)
        blockingReasons.push(`compiler_exit_${result.status ?? 1}`);
    if ((result.stdout ?? '').includes('BLOCK:'))
        blockingReasons.push('compiler_block');
    ...
    const goalMode = text(receipt.goalCommand?.mode);
    ...
    if (expectedProfileHash) {
        ...
        const receiptProfile = receipt.executionDisciplineProfile;
    ...
    if (input.goalCommandAvailable === 'true' &&
    ...
    const receiptGoalHash = text(receipt.goalCommand?.documentHash);
