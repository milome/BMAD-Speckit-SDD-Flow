...
exports.mainControlPlaneIsolationCheck = mainControlPlaneIsolationCheck;
...
function containsForbiddenControlField(value) {
    if (!value || typeof value !== 'object')
        return false;
    if (Array.isArray(value))
        return value.some(containsForbiddenControlField);
    const obj = value;
    if (Object.prototype.hasOwnProperty.call(obj, 'result'))
        return true;
    if (text(obj.sourceOfTruthRole) !== 'control')
        return false;
...
function sourceRefIssues(refs, owner) {
    return refs.flatMap((ref) => {
        const sourceType = text(ref.sourceType);
        if (!sourceType)
            return [`${owner}:source_type_missing`];
        ...
        if (!text(ref.id))
            return [`${owner}:source_ref_id_missing:${sourceType}`];
        return [];
    });
}
function artifactIssues(record) {
    const issues = [];
    for (const artifact of [...objects(record.artifactIndex), ...objects(record.extensionRefs)]) {
        ...
        if (READ_MODEL_ARTIFACT_TYPES.has(artifactType) &&
            role !== 'read_model' &&
            role !== 'evidence') {
            issues.push(`read_model_artifact_bad_role:${artifactType}`);
        }
        if (PROJECTION_ARTIFACT_TYPES.has(artifactType) &&
            role !== 'projection' &&
            role !== 'evidence') {
            issues.push(`projection_artifact_bad_role:${artifactType}`);
        }
        if ((artifactPath.includes('/dashboard/') ||
...
function gateAndContractIssues(record) {
    const issues = [];
    for (const [groupName, rows] of [
        ...
        for (const row of rows) {
            if (!text(row.decision))
                issues.push(`${groupName}_decision_missing:${text(row.checkId) || text(row.gate) || text(row.contract)}`);
...
function failureAndRerunIssues(record) {
    const issues = [];
    ...
    for (const rca of objects(record.rcaRecords)) {
        if (Object.prototype.hasOwnProperty.call(rca, 'result'))
            issues.push('rca_result_forbidden');
    ...
    for (const loop of objects(record.rerunLoops)) {
        if (Object.prototype.hasOwnProperty.call(loop, 'result'))
            issues.push('rerun_result_forbidden');
...
function closeoutBoundaryIssues(record) {
    const issues = [];
...
function mainControlPlaneIsolationCheck(argv) {
