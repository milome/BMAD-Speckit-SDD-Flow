...
exports.SHORT_FEEDBACK_WINDOW_MS = exports.SOURCE_MATERIALIZATION_RECEIPT_SCHEMA_VERSION = void 0;
exports.runMainAgentAuthoringRepair = runMainAgentAuthoringRepair;
exports.validateWrittenDeepReviewInput = validateWrittenDeepReviewInput;
exports.runMainAgentPreConfirmationDrilldown = runMainAgentPreConfirmationDrilldown;
exports.resolveMainAgentOrchestrationSurface = resolveMainAgentOrchestrationSurface;
exports.claimMainAgentPendingPacket = claimMainAgentPendingPacket;
exports.markMainAgentPacketDispatched = markMainAgentPacketDispatched;
exports.completeMainAgentPendingPacket = completeMainAgentPendingPacket;
exports.invalidateMainAgentPendingPacket = invalidateMainAgentPendingPacket;
exports.ingestMainAgentTaskReport = ingestMainAgentTaskReport;
exports.ensureMainAgentDispatchPacket = ensureMainAgentDispatchPacket;
exports.buildMainAgentDispatchInstruction = buildMainAgentDispatchInstruction;
exports.runMainAgentConfirmCloseoutAcceptance = runMainAgentConfirmCloseoutAcceptance;
exports.runMainAgentConfirmScope = runMainAgentConfirmScope;
exports.runMainAgentConfirmationBookkeepingRepair = runMainAgentConfirmationBookkeepingRepair;
exports.runMainAgentConfirmationDriftRoute = runMainAgentConfirmationDriftRoute;
exports.runMainAgentControlledReadinessAudit = runMainAgentControlledReadinessAudit;
exports.writeMainAgentRunLoopTaskReport = writeMainAgentRunLoopTaskReport;
exports.runMainAgentPostCloseDefectIntake = runMainAgentPostCloseDefectIntake;
exports.runMainAgentReadinessAutoRemediation = runMainAgentReadinessAutoRemediation;
exports.runMainAgentAutomaticLoop = runMainAgentAutomaticLoop;
exports.runMainAgentAutomaticLoopAsync = runMainAgentAutomaticLoopAsync;
exports.mainMainAgentOrchestration = mainMainAgentOrchestration;
exports.mainMainAgentOrchestrationAsync = mainMainAgentOrchestrationAsync;
...
const orchestration_dispatch_contract_1 = require("./orchestration-dispatch-contract");
const execution_discipline_profiles_1 = require("./execution-discipline-profiles");
const main_agent_compiled_prompt_runner_1 = require("./main-agent-compiled-prompt-runner");
const execution_strategy_selection_1 = require("./execution-strategy-selection");
const sdd_artifact_manifest_1 = require("./sdd-artifact-manifest");
...
const adaptive_intake_governance_gate_1 = require("./adaptive-intake-governance-gate");
...
const controlled_readiness_audit_bridge_1 = require("./controlled-readiness-audit-bridge");
const requirement_record_control_store_1 = require("./requirement-record-control-store");
const main_agent_implementation_readiness_gate_1 = require("./main-agent-implementation-readiness-gate");
const ai_tdd_contract_gate_1 = require("./ai-tdd-contract-gate");
const run_required_commands_from_ai_tdd_manifest_1 = require("./run-required-commands-from-ai-tdd-manifest");
...
const six_model_runtime_decision_1 = require("./six-model-runtime-decision");
const reconfirmation_runtime_1 = require("./reconfirmation-runtime");
const requireCommonJs = (0, node_module_1.createRequire)(__filename);
const { runNativeGoalInvocation } = requireCommonJs('../packages/bmad-speckit/src/main-agent/actions/native-goal-invoker.js');
...
exports.SOURCE_MATERIALIZATION_RECEIPT_SCHEMA_VERSION = 'source-materialization-receipt/v1';
exports.SHORT_FEEDBACK_WINDOW_MS = 300000;
...
function deriveNextActionFromTaskType(taskType, stage) {
    switch (taskType) {
        case 'implement':
...
function deriveNextActionFromFailedTaskType(taskType, status) {
...
function isTerminalPendingPacketStatus(status) {
    return status === 'completed' || status === 'invalidated';
}
function isRecordScopedOrchestrationStatePath(candidate, runtimeContext) {
    const recordPath = resolvedContext(runtimeContext)?.recordPath;
    if (!recordPath) {
        return false;
    }
    return candidate.startsWith(path.join(path.dirname(recordPath), 'orchestration', 'orchestration-state'));
}
function loadRuntimeContextForMainAgent(input) {
    ...
    try {
        return (0, emit_runtime_policy_1.loadPolicyContextFromRegistry)(input.projectRoot, {
        ...
        if ((0, resolve_active_requirement_1.isNoActiveRequirementError)(error)) {
            return null;
        }
        return null;
    }
}
function resolvedContext(runtimeContext) {
    const candidate = runtimeContext
        ?.resolvedRuntimeContext;
    return candidate?.kind === 'ResolvedRuntimeContext' ? candidate : null;
}
function listScopedOrchestrationStatePaths(projectRoot, runtimeContext = null) {
    if (!projectRoot) {
        return [];
    }
    const dirs = [
        (0, orchestration_state_1.orchestrationStateDirForRecordPath)(projectRoot, resolvedContext(runtimeContext)?.recordPath),
...
function readRequirementRecordFromRuntimeContext(runtimeContext) {
    const recordPath = resolvedContext(runtimeContext)?.recordPath;
...
function runtimeRecordPath(runtimeContext) {
    return resolvedContext(runtimeContext)?.recordPath ?? null;
}
function isSixMentalModel(value) {
    return SIX_MENTAL_MODEL_SEQUENCE.includes(value);
}
function modelResultFor(record, model) {
    const sixModelResults = record?.sixModelResults &&
...
function modelStatusFor(record, model) {
    return normalizeText(modelResultFor(record, model)?.status);
}
function packetHasCurrentHashCompiledPromptRef(packet, record) {
    if (!packet || !record) {
        return false;
    }
    ...
    const ref = candidate.compiledPromptRef &&
    ...
    return (normalizeText(ref.sourceDocumentHash) === normalizeText(record.sourceDocumentHash) &&
...
function nextMentalModelFor(currentMentalModel, nextAction) {
...
function buildMainAgentStageSummary(input) {
    ...
    const currentStageOrdinal = currentMentalModel && isSixMentalModel(currentMentalModel)
    ...
    const blocked = input.ready === false ||
...
function buildNoActiveRequirementStageSummary() {
    return {
        schemaVersion: 'main-agent-stage-summary/v1',
        recordId: null,
        ...
        lastEventType: null,
        userFacingMessage: '当前项目尚未创建需求契约。BMAD 不会把初始化占位状态当作真实需求。请先创建或导入一个可确认的需求源文档。',
    };
}
        ...
        diagnostics: [
            {
                ...
                nextCommand: 'bmads',
                message: 'NO_ACTIVE_REQUIREMENT: create or import a requirement contract and complete requirement confirmation before orchestration can continue.',
            },
        ],
...
function normalizeAllowedWriteScope(value) {
    return Array.isArray(value)
        ? value.filter((item) => typeof item === 'string' && item.trim().length > 0)
        : [];
}
function mergeAllowedWriteScope(base, extras) {
    return Array.from(new Set([...normalizeAllowedWriteScope(base), ...normalizeAllowedWriteScope(extras)]));
}
function resolveMappedAllowedWriteScope(projectRoot, runtimeContext, flow, taskType, requirementRecord = null) {
    ...
    const mapping = (0, user_story_mapping_1.selectBestMappingForRuntimeContext)((0, user_story_mapping_1.readUserStoryMappingIndexOrDefault)(projectRoot), runtimeContext, flow);
    if (!mapping) {
        return null;
    }
    if (taskType === 'audit') {
...
function resolveTaskBindingAllowedWriteScope(runtimeContext, requirementRecord) {
    const bindings = Array.isArray(requirementRecord?.taskBindings)
        ? requirementRecord.taskBindings
        : [];
    ...
    const candidates = bindings
        ...
        const scope = Array.isArray(binding.allowedWriteScope)
            ? binding.allowedWriteScope.filter((value) => typeof value === 'string' && value.trim().length > 0)
            : [];
        if (scope.length === 0)
            return null;
        ...
        let score = 0;
        if (bindingFlow && flow && bindingFlow === flow)
            score += 1;
        if (bindingEpic && epicId && bindingEpic === epicId)
            score += 2;
        if (bindingStory && storyId && bindingStory === storyId)
            score += 4;
        if (bindingRun && runId && bindingRun === runId)
            score += 8;
        return { scope, score };
    })
...
function resolveScopedOrchestrationState(projectRoot, runtimeContext) {
    ...
    const scored = candidates
        ...
        const state = (0, orchestration_state_1.readOrchestrationStateAtPath)(candidate);
        if (!state) {
            return null;
        }
        let score = 0;
        let requirementMatchScore = 0;
        const addRequirementMatchScore = (value) => {
            score += value;
        ...
        if (resolvedContext(runtimeContext)?.recordPath &&
...
function resolveMainAgentHost(projectRoot, explicitHost, surface) {
    if (explicitHost) {
        return explicitHost;
    }
    ...
    return (0, governance_remediation_config_1.readGovernanceRemediationConfig)(root).primaryHost === 'claude' ? 'claude' : 'cursor';
}
...
function packetTaskType(packet) {
    if (!packet) {
        return null;
    }
    const taskType = normalizeText(packet.taskType);
    if (taskType === 'implement' ||
        taskType === 'audit' ||
        taskType === 'remediate' ||
        taskType === 'document') {
...
function pendingPacketDispatchable(packet) {
...
function pendingPacketMatchesNextAction(surface) {
...
function isCodexSmokeOnlyTaskReport(report) {
    return (report.validationsRun.some((validation) => normalizeText(validation) === 'codex-worker-adapter-smoke') || report.evidence.some((evidence) => normalizeText(evidence).startsWith('codex-smoke:')));
}
function blockSmokeOnlyImplementationReport(report, instruction) {
...
function writePacketFile(projectRoot, sessionId, packetId, packet) {
    const file = (0, orchestration_dispatch_contract_1.packetArtifactPath)(projectRoot, sessionId, packetId);
...
function relativePathFromRoot(projectRoot, filePath) {
    return path.relative(projectRoot, filePath).replace(/\\/gu, '/');
}
function recordIdentityFromPath(recordPath, fallbackSessionId) {
    if (recordPath && fs.existsSync(recordPath)) {
        try {
...
function ensurePolicyDefaultExecutionStrategy(input) {
    const optionsResult = (0, execution_strategy_selection_1.buildExecutionStrategyOptions)({
        compiledPromptRef: input.compiledRun.compiledPromptRef,
        modelPacketGateDecision: input.compiledRun.status === 'pass' ? 'pass' : 'blocked',
    });
    if (optionsResult.status !== 'pass')
        return null;
    const selection = (0, execution_strategy_selection_1.selectExecutionStrategy)({
        optionsResult,
    ...
    (0, execution_strategy_selection_1.appendExecutionStrategySelection)({
        recordPath: input.recordPath,
        event: (0, execution_strategy_selection_1.toExecutionStrategySelectionEvent)({
            ...identity,
            selection,
            sourceRefs: [
...
function writeEmptySddArtifactManifestRef(input) {
    if (!input.compiledOutDir)
        return null;
    const identity = recordIdentityFromPath(input.recordPath, input.sessionId);
    const manifest = (0, sdd_artifact_manifest_1.createSddArtifactManifest)({
        recordId: identity.recordId,
        flow: input.flow,
        packetId: input.packetId,
        runtimeTraceExecutionDir: relativePathFromRoot(input.projectRoot, input.compiledOutDir),
    });
    const manifestPath = (0, sdd_artifact_manifest_1.defaultSddArtifactManifestPath)({
        runtimeTraceExecutionDir: input.compiledOutDir,
    });
    (0, sdd_artifact_manifest_1.writeSddArtifactManifest)(manifestPath, manifest);
    const validation = (0, sdd_artifact_manifest_1.validateSddArtifactManifest)({
        manifest,
...
function latestCurrentCompiledPromptRef(input) {
    if (!input.record)
        return null;
    const packetDir = path.join(input.projectRoot, '_bmad-output', 'runtime', 'requirement-records', safeSegment(input.sessionId), 'prompts', 'prompt-packets');
    ...
    const candidates = fs
        ...
        return (normalizeText(packet?.taskType) === 'implement' &&
    ...
    return candidates[0]?.packet?.compiledPromptRef ?? null;
}
function auditArtifactsDir(projectRoot, recordId, attemptId) {
    return path.join(projectRoot, '_bmad-output', 'runtime', 'requirement-records', safeSegment(recordId), 'audit-review', safeSegment(attemptId));
}
function writeAuditExecutionProfile(input) {
    const criticalProfile = (0, critical_auditor_profile_1.resolveCriticalAuditorProfile)(input.projectRoot);
    const stageProfileId = (0, critical_auditor_profile_1.stageProfileForCallPoint)('audit_review');
    const validation = (0, critical_auditor_profile_1.validateCriticalAuditorProfileForStage)({
    ...
    const requiredCheckItemSetHash = (0, audit_triad_orchestrator_1.sha256Json)(validation.stageProfile.requiredCheckItemIds);
    ...
    const triadPlan = (0, audit_triad_orchestrator_1.createAuditTriadExecutionPlan)({
        projectRoot: input.projectRoot,
        recordId: input.recordId,
        stage: input.stage,
    ...
    const triadPath = (0, audit_triad_orchestrator_1.writeAuditTriadExecutionPlan)(input.projectRoot, triadPlan);
    const triadRef = {
        path: triadPath,
...
function preConfirmationIssue(code, message, refs = [], source = 'main_agent_orchestration') {
    return { code, message, refs, severity: 'blocker', source };
}
function sanitizeRequirementIdSegment(value) {
    return value.replace(/[^A-Za-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '') || 'pre-confirmation';
}
function assertSafeRequirementIdPathSegment(label, value) {
    ...
    if (!/^[A-Za-z0-9_-]+$/u.test(value)) {
        throw new Error(`${label} must contain only alphanumeric characters, hyphens, and underscores: ${value}`);
    }
}
function toRootRelativePath(root, filePath) {
...
function resolveRootRelativePath(root, filePath) {
    return path.isAbsolute(filePath) ? filePath : path.resolve(root, filePath);
}
function derivePreConfirmationIdentity(root, sourcePath, options) {
...
function preConfirmationPaths(root, recordId, requirementSetId = recordId) {
    ...
    return {
        ...
        scaleAssessmentPostMaterialization: path.join(authoringDir, 'scale-assessment-post-materialization.json'),
        ...
        receiptPaths: [1, 2, 3].map((round) => path.join(authoringDir, `critical-auditor-receipt-round-${round}.json`)),
...
function sourceMaterializationReceiptPath(root, requirementSetId) {
    assertSafeRequirementIdPathSegment('requirementSetId', requirementSetId);
    return path.join(root, '_bmad-output', 'runtime', 'requirement-records', requirementSetId, 'authoring', 'source-materialization-receipt.json');
}
function extractImplementationConfirmationBlock(sourceText) {
...
function dumpImplementationConfirmation(confirmation) {
    return yaml
...
function sourceDocumentHashForPreConfirmation(sourceText, blockText, confirmation) {
...
function implementationConfirmationHashForPreConfirmation(confirmation) {
    return sha256Json(semanticConfirmationForHash(confirmation));
}
function projectionBackRef(packetHash, mustRef = 'MUST-001') {
    return {
...
function mdmIdForMust(index, total) {
    return total === 1 ? 'MDM-001' : `MDM-${requirementOrdinal(index)}`;
}
function extractExplicitSourceMustRequirements(sourceText) {
    ...
    semanticText
        ...
        rows.push({
            ...
            text,
            source: 'explicit_source_must',
            sourceLine: index + 1,
        });
    });
    return rows.map((item, index) => ({ ...item, id: normalizeMustRef(item.id, index) }));
}
function extractInlineMustRequirements(confirmation) {
...
function resolveSourceMustRequirements(sourcePath) {
...
function buildPreConfirmationImplementationConfirmation(input) {
    const rel = (filePath) => toRootRelativePath(input.root, filePath);
    ...
    const acceptanceTestFile = path.resolve(__dirname, '..', 'tests', 'acceptance', 'main-agent-pre-confirmation-drilldown-lane.test.ts');
    return {
        ...
        evidence: [
            {
                ...
                oracleZh: '所有 drilldown 产物、双向 reconciliation、预渲染门禁和 renderer 可确认性通过，同时 controlled ingest 前 deliveryReadiness.ready 保持 false。',
        ...
        targetModificationPaths: [
            {
                id: 'TARGET-MOD-001',
                path: 'packages/bmad-speckit/src/main-agent/source-authority/scripts/main-agent-orchestration.js',
                changeType: 'modify',
                coverageRole: 'modify',
...
function materializeImplementationConfirmationSource(sourcePath, confirmation) {
    const sourceText = fs.readFileSync(sourcePath, 'utf8');
    const extraction = extractImplementationConfirmationBlock(sourceText);
...
function safeCurrentSourceDocumentHash(sourcePath) {
    const sourceText = fs.existsSync(sourcePath) ? fs.readFileSync(sourcePath, 'utf8') : '';
    const extraction = extractImplementationConfirmationBlock(sourceText);
    if (!extraction) {
        return sha256Text(sourceText);
    }
    return sourceDocumentHashForPreConfirmation(sourceText, extraction.blockText, extraction.confirmation);
}
function writtenIdRangesFromConfirmation(confirmation) {
...
function nextSourceAuditCommand(root, sourcePath) {
...
function writeSourceMaterializationReceipt(input) {
    const receipt = {
        schemaVersion: exports.SOURCE_MATERIALIZATION_RECEIPT_SCHEMA_VERSION,
...
function sourceMaterializationGateIssue(code, message, refs) {
...
function verifySourceMaterializationBeforeAudit(input) {
    ...
    if (!fs.existsSync(receiptPath)) {
        return {
            ok: false,
            issues: [
                sourceMaterializationGateIssue('source_materialization_receipt_missing', 'Deep audit requires a source materialization receipt before request generation.', [toRootRelativePath(input.root, receiptPath)]),
            ],
        };
    }
    ...
    for (const field of requiredFields) {
        if (!(field in receipt) || receipt[field] == null) {
            issues.push(sourceMaterializationGateIssue('source_materialization_receipt_field_missing', `Source materialization receipt is missing required field ${field}.`, [toRootRelativePath(input.root, receiptPath), field]));
        }
    }
    ...
    if (normalizeText(receipt.schemaVersion) !== exports.SOURCE_MATERIALIZATION_RECEIPT_SCHEMA_VERSION) {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_schema_mismatch', 'Source materialization receipt schema version is not supported.', [toRootRelativePath(input.root, receiptPath)]));
    }
    if (normalizeText(receipt.sourcePath) !== sourceRelative) {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_source_path_mismatch', 'Source materialization receipt sourcePath does not match the current source document.', [sourceRelative, normalizeText(receipt.sourcePath)]));
    }
    if (normalizeText(receipt.requirementSetId) !== input.requirementSetId) {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_requirement_set_mismatch', 'Source materialization receipt requirementSetId does not match the current authoring lane.', [input.requirementSetId, normalizeText(receipt.requirementSetId)]));
    }
    if (normalizeText(receipt.sourceDocumentHashAfter) !== current.sourceDocumentHash) {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_source_hash_stale', 'Current source document hash differs from receipt sourceDocumentHashAfter.', [current.sourceDocumentHash, normalizeText(receipt.sourceDocumentHashAfter)]));
    }
    if (normalizeText(receipt.sourceDocumentHashAfter) !== input.expectedSourceDocumentHash) {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_expected_source_hash_mismatch', 'Receipt sourceDocumentHashAfter does not match the current audit input hash.', [input.expectedSourceDocumentHash, normalizeText(receipt.sourceDocumentHashAfter)]));
    }
    if (normalizeText(receipt.implementationConfirmationHash) !==
        ...
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_confirmation_hash_stale', 'Inline implementationConfirmation hash differs from the materialization receipt.', [
            current.implementationConfirmationHash,
            normalizeText(receipt.implementationConfirmationHash),
        ]));
    }
    if (normalizeText(receipt.draftStatus) !== 'confirmation_ready') {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_not_confirmation_ready', 'Deep audit requires draftStatus=confirmation_ready.', [normalizeText(receipt.draftStatus)]));
    }
    if (!Array.isArray(receipt.writtenIdRanges)) {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_written_id_ranges_invalid', 'Source materialization receipt writtenIdRanges must be an array.', [toRootRelativePath(input.root, receiptPath)]));
    }
    if (normalizeText(receipt.createdBy) !== 'main-agent-source-materialization') {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_created_by_invalid', 'Source materialization receipt createdBy must be main-agent-source-materialization.', [normalizeText(receipt.createdBy)]));
    }
    if (!Number.isFinite(Date.parse(normalizeText(receipt.createdAt)))) {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_created_at_invalid', 'Source materialization receipt createdAt must be an ISO-8601 timestamp.', [normalizeText(receipt.createdAt)]));
    }
    if (normalizeText(receipt.receiptHash) !== expectedReceiptHash) {
        issues.push(sourceMaterializationGateIssue('source_materialization_receipt_hash_stale', 'Source materialization receiptHash does not match receipt content.', [expectedReceiptHash, normalizeText(receipt.receiptHash)]));
    }
...
function materializeCriticalAuditorGapFix(input) {
    ...
    const nextConfirmation = {
        ...
        sourceGapFixes: [
            ...existingRows,
            {
                ...
                status: 'materialized',
                validatedGapRefs: asRecordArray(input.response.validatedGaps).map((gap, index) => normalizeText(gap.id ?? gap.code ?? `validated-gap-${index + 1}`)),
    ...
    if (materialization.materialized.sourceDocumentHash === input.previousSourceDocumentHash) {
        return {
            ok: false,
            issue: sourceMaterializationGateIssue('source_gap_fix_hash_unchanged', 'Validated Critical Auditor gaps require a source document materialization that changes sourceDocumentHashAfter before the next audit round.', [input.previousSourceDocumentHash]),
        };
    }
    return { ok: true, ...materialization };
}
function readMaterializedConfirmation(sourcePath) {
    const sourceText = fs.readFileSync(sourcePath, 'utf8');
    const extraction = extractImplementationConfirmationBlock(sourceText);
    if (!extraction) {
    ...
    return {
        sourceText,
        extraction,
        sourceDocumentHash: sourceDocumentHashForPreConfirmation(sourceText, extraction.blockText, extraction.confirmation),
...
function buildSemanticKernel(input) {
    ...
    const payload = {
        ...
        currentState: input.mustRequirements.map((requirement) => `${requirement.id} is an authored source requirement awaiting packet-backed drilldown.`),
        targetState: [
...
function buildMustDecompositionPacket(input) {
    const rel = (filePath) => toRootRelativePath(input.root, filePath);
    const materialized = (target) => [target];
    const mustRefs = input.mustRequirements.map((requirement) => requirement.id);
    const mustTexts = input.mustRequirements.map((requirement) => requirement.text);
        ...
        mustPackets: input.mustRequirements.map((requirement, index) => {
            ...
            return {
                ...
                decompositionBasis: {
                    ...
                    targetSurfaces: [
                        'packages/bmad-speckit/src/main-agent/source-authority/scripts/main-agent-orchestration.js',
                ...
                mustArtifactProjection: [
                    {
                        id: 'ART-001',
                        materializedTo: materialized('implementationConfirmation.artifactAutomationPlan[ART-001]'),
                    },
                    {
                        id: 'ART-002',
                        materializedTo: materialized('implementationConfirmation.artifactAutomationPlan[ART-002]'),
                    },
                ],
...
function projectionBackRefForRepair(packetHash, mustRef) {
    return {
        derivedFromMustRef: mustRef,
        derivedFromPacketHash: packetHash,
        projectionStatus: 'synchronized',
...
function sourceRowsForRepairProjection(confirmation, key) {
    ...
    if (key === 'aiTddContractExecutionManifestProjection') {
        return Object.entries(recordObject(confirmation.aiTddContractExecutionManifestProjection)).map(([id, value]) => ({
            id,
...
function rowIdForRepair(row, fallback) {
    return normalizeText(row.id ?? row.taskId ?? row.commandId ?? row.artifactId ?? row.path ?? fallback);
}
function selectExistingRowIds(confirmation, keys, fallbackIds) {
    const ids = keys
...
function buildPreserveExistingMustDecompositionPacket(input) {
    ...
    const existingTaskRows = sourceRowsForRepairProjection(input.confirmation, 'atomicImplementationTaskList');
    const existingMatrixRows = sourceRowsForRepairProjection(input.confirmation, 'mustExecutionDecompositionMatrix');
    ...
    const targetIds = selectExistingRowIds(input.confirmation, ['targetModificationPaths'], ['TARGET-MOD-001']);
    const artifactIds = selectExistingRowIds(input.confirmation, ['artifactAutomationPlan'], ['ART-001']);
    const commandIds = selectExistingRowIds(input.confirmation, ['requiredCommands'], ['CMD-001']);
    const currentTargetIds = selectExistingRowIds(input.confirmation, ['currentTargetMap'], ['CT-CANON-001']);
    const aiTddIds = selectExistingRowIds(input.confirmation, ['aiTddContractExecutionManifestProjection'], ['AI-TDD-001']);
    const projection = (id, target, mustRef = firstMustRef) => ({
        id,
        mustRef,
    ...
    const payload = {
        ...
        packetHash: input.packetHash,
        status: 'synchronized',
        generatedBy: 'main_agent_orchestration.requirement_confirmation.authoring_repair.preserve_existing',
        createdBy: 'main_agent_orchestration.requirement_confirmation.authoring_repair.preserve_existing',
        createdAt: input.createdAt,
        inputRefs: [rel(input.sourcePath), rel(input.paths.semanticKernel)],
        materializationTarget: 'implementationConfirmation',
        lifecycle: {
        ...
        authorClaims: [
            {
                ...
                claim: 'Preserve-existing authoring repair synchronizes artifacts without replacing the rich source contract.',
                criticDisposition: input.consecutiveNoNewGapRounds >= 3
        ...
        mustPackets: input.mustRequirements.map((requirement, index) => {
            ...
            const existingMatrixId = existingMatrixIds[index] ||
                existingMatrixIds[0] ||
            ...
            return {
                ...
                mustExecutionDecompositionMatrix: [
                    projection(existingMatrixId, existingMatrixRows.length > 0
                        ? `implementationConfirmation.mustExecutionDecompositionMatrix[${existingMatrixId}]`
                        : `implementationConfirmation.traceRows[${traceIds[0]}]`, requirement.id),
                ],
                mustAtomicTasks: [
                    {
                        id: existingTaskId,
                ...
                mustEvidenceProjection: evidenceIds.map((id) => projection(id, `implementationConfirmation.evidence[${id}]`, requirement.id)),
                mustTraceProjection: traceIds.map((id) => projection(id, `implementationConfirmation.traceRows[${id}]`, requirement.id)),
                mustAcceptanceProjection: [
                    ...acceptanceIds.map((id) => projection(id, `implementationConfirmation.acceptanceTests[${id}]`, requirement.id)),
                    ...e2eIds.map((id) => projection(id, `implementationConfirmation.e2eSuites[${id}]`, requirement.id)),
                ],
                mustFailureEdgeProjection: [
                    ...failureIds.map((id) => projection(id, `implementationConfirmation.failurePaths[${id}]`, requirement.id)),
                    ...edgeIds.map((id) => projection(id, `implementationConfirmation.edgeCases[${id}]`, requirement.id)),
                ],
                mustTargetPathProjection: targetIds.map((id) => projection(id, `implementationConfirmation.targetModificationPaths[${id}]`, requirement.id)),
                mustCurrentTargetProjection: currentTargetIds.map((id) => projection(id, 'implementationConfirmation.currentTargetMap', requirement.id)),
                mustAiTddManifestProjection: aiTddIds.map((id) => projection(id, 'implementationConfirmation.aiTddContractExecutionManifestProjection', requirement.id)),
                mustArtifactProjection: artifactIds.map((id) => projection(id, `implementationConfirmation.artifactAutomationPlan[${id}]`, requirement.id)),
                mustCommandProjection: commandIds.map((id) => projection(id, `implementationConfirmation.requiredCommands[${id}]`, requirement.id)),
                mustCloseoutBoundaryProjection: [
                    projection('CLOSEOUT-BOUNDARY-001', 'implementationConfirmation.closeoutReadinessPreview', requirement.id),
                ],
            };
        }),
...
function buildCriticalAuditorReceipt(input) {
    const auditResult = input.auditResult ?? { verdict: 'no_new_valid_gap' };
    const isClean = auditResult.verdict === 'no_new_valid_gap' ||
    ...
    const receipt = {
        ...
        rejectedGapCandidates: auditResult.rejectedGapCandidates ??
        ...
        noNewGapRationale: auditResult.rationale ??
            (isClean
...
function isNoNewGapVerdict(verdict) {
...
function criticalAuditorRoundPerspective(roundIndex) {
    ...
    if (normalizedRound === 2) {
        return {
            ...
            focus: 'EVD / TRACE / ACC / E2E / FAIL / EDGE / artifact / command / AI-TDD projection materialization',
            requiredAttackSurface: [
    ...
    return {
        ...
        focus: 'stale hash, authority bypass, negative boundary, reconfirmation, and delivery-vs-confirmation confusion',
        requiredAttackSurface: [
...
function projectionRefsFromPacket(packet) {
    const refs = new Set();
...
function actionableDryRunIssues(report) {
    return collectBlockingIssuesFromReports(report).filter((issue) => !CRITICAL_AUDITOR_ACTIONABLE_DRY_RUN_EXCLUSIONS.has(issue.code));
}
function buildCriticalAuditorGateDryRunSummary(input) {
    const dryRunBase = path.join(input.paths.authoringDir, `pre-render-must-decomposition-gate-dry-run-round-${input.roundIndex}`);
    ...
    const dryRun = runNodeJson(mustGateScript, [
        '--source',
        input.sourcePath,
        '--authoring-dir',
    ...
    const actionableBlockingIssues = actionableDryRunIssues(report).concat(report
        ? []
        : [
            commandFailureIssue('critical_auditor_gate_dry_run_report_missing', mustGateScript, dryRun),
        ]);
    const summary = {
        ...
        reconciliation: {
            verdict: normalizeText(reconciliation?.verdict ?? recordObject(report?.packetSourceReconciliation).verdict),
...
function runCriticalAuditorReceiptLoop(input) {
    if (!input.roundProvider) {
        return {
            ...
            receipts: [],
            issues: [
                preConfirmationIssue('critical_auditor_round_provider_missing', 'A real Critical Auditor round provider is required; synthetic clean receipts are not allowed', ['critical_auditor_receipt_loop'], 'critical_auditor'),
            ],
        };
    }
    ...
    for (let roundIndex = 1; roundIndex <= maxRounds && consecutiveNoNewGapRounds < 3; roundIndex += 1) {
        ...
        writeJsonUtf8(path.join(input.paths.authoringDir, `critical-auditor-receipt-round-${roundIndex}.json`), { criticalAuditorReceipt: receipt });
        const verdict = auditResult.verdict;
        if (verdict === 'blocked' || verdict === 'insufficient_audit') {
            issues.push(preConfirmationIssue(`critical_auditor_${verdict}`, `Critical Auditor round ${roundIndex} returned ${verdict}`, [`critical-auditor-receipt-round-${roundIndex}.json`], 'critical_auditor'));
            break;
        }
        ...
        if (unresolvedGaps.length > 0) {
            issues.push(preConfirmationIssue('critical_auditor_unresolved_validated_gap', `Critical Auditor round ${roundIndex} returned unresolved validated gaps`, unresolvedGaps.map((gap) => normalizeText(gap.id ?? gap.code ?? 'gap')), 'critical_auditor'));
            consecutiveNoNewGapRounds = 0;
            break;
        }
    ...
    if (consecutiveNoNewGapRounds < 3 && issues.length === 0) {
        issues.push(preConfirmationIssue('critical_auditor_no_new_gap_convergence_not_reached', `Critical Auditor did not reach three consecutive no-new-gap rounds within ${maxRounds} rounds`, ['critical_auditor_receipt_loop'], 'critical_auditor'));
    }
    return { latestReceiptHash, consecutiveNoNewGapRounds, receipts, issues };
}
function readCriticalAuditorReceipts(authoringDir) {
...
function authoringRepairCommand(sourcePath, responsePath) {
...
function buildAuthoringRepairResult(input) {
    return {
        ...
        purposeGuard: {
            ...
            blockingStage: input.blockingStage === 'source_materialization_required_before_audit'
...
function buildCriticalAuditorRoundRequest(input) {
    ...
    const request = {
        ...
        gateDryRun: {
            ...
            reconciliationReportPath: toRootRelativePath(input.root, input.gateDryRun.reconciliationReportPath),
        ...
        requiredResponseSchema: {
            ...
            verdict: 'no_new_valid_gap | no_new_confirmation_blocking_gap | new_valid_gap | insufficient_audit | blocked',
            ...
            falsePositiveProofs: [
                {
                    blockerCode: '<required only when gateDryRun.actionableBlockingIssueCount > 0 and verdict is no_new_*>',
                    proofType: 'current_source_packet_hash_match | source_row_backref_resolves | projection_materialized | reconciliation_count_match',
                    evidenceRefs: [],
                },
            ],
            gapCandidates: [],
...
function findCriticalAuditorRequestPath(authoringDir, roundIndex) {
    return path.join(authoringDir, `critical-auditor-round-request-${roundIndex}.json`);
}
function findNextCriticalAuditorRound(authoringDir) {
...
function archiveCriticalAuditorArtifacts(input) {
    ...
    const artifactNames = fs
        ...
        .filter((fileName) => /^critical-auditor-round-request-\d+\.json$/u.test(fileName) ||
        /^critical-auditor-round-response-\d+\.json$/u.test(fileName) ||
        /^critical-auditor-receipt-round-\d+\.json$/u.test(fileName) ||
        /^pre-render-must-decomposition-gate-dry-run-round-\d+(?:-receipt|-reconciliation)?\.json$/u.test(fileName));
    if (artifactNames.length === 0) {
        return null;
    }
    const stamp = input.createdAt.replace(/[^0-9A-Za-z]+/gu, '').slice(0, 15) || 'archive';
    const archiveDir = path.join(input.authoringDir, `stale-critical-auditor-${stamp}-${sha256Text(input.auditInputHash).slice('sha256:'.length, 'sha256:'.length + 12)}`);
...
function collectStaleCriticalAuditorArtifactIssues(input) {
    ...
    const issues = [];
    const staleIfMismatch = (artifactName, artifact, hashFields) => {
        if (!artifact) {
            return;
        }
        for (const field of hashFields) {
            const expected = normalizeText(input[field]);
            if (expected &&
                normalizeText(artifact[field]) &&
                normalizeText(artifact[field]) !== expected) {
                issues.push(preConfirmationIssue(`critical_auditor_${artifactName.replace(/\W+/gu, '_')}_${field}_stale`, `Critical Auditor artifact ${artifactName} has stale ${field}`, [artifactName, field], 'critical_auditor'));
            }
        }
    };
    for (const fileName of fs.readdirSync(input.authoringDir)) {
        ...
        if (/^critical-auditor-receipt-round-\d+\.json$/u.test(fileName)) {
            ...
            if (receipt && normalizeText(receipt.inputHash) !== input.auditInputHash) {
                issues.push(preConfirmationIssue('critical_auditor_receipt_input_hash_stale', `Critical Auditor artifact ${fileName} has stale inputHash`, [fileName, 'inputHash'], 'critical_auditor'));
            }
            if (receipt && !normalizeText(receipt.gateDryRunHash)) {
                issues.push(preConfirmationIssue('critical_auditor_receipt_gate_dry_run_hash_missing', `Critical Auditor artifact ${fileName} lacks gateDryRunHash`, [fileName, 'gateDryRunHash'], 'critical_auditor'));
            }
        }
    }
    return issues;
}
function collectConsecutiveNoNewGapFromReceipts(receipts, auditInputHash) {
    const issues = [];
    let consecutive = 0;
    let latestReceiptHash = null;
    for (const receipt of receipts) {
        ...
        if (receipt.inputHash !== auditInputHash) {
            issues.push(preConfirmationIssue('critical_auditor_receipt_input_hash_stale', `Critical Auditor receipt round ${receipt.roundIndex ?? '?'} is stale for current source/packet hashes`, ['inputHash'], 'critical_auditor'));
            consecutive = 0;
            continue;
        }
        if (!normalizeText(receipt.gateDryRunHash)) {
            issues.push(preConfirmationIssue('critical_auditor_receipt_gate_dry_run_hash_missing', `Critical Auditor receipt round ${receipt.roundIndex ?? '?'} lacks gateDryRunHash and must be regenerated`, ['gateDryRunHash'], 'critical_auditor'));
            consecutive = 0;
            continue;
        }
...
function validateCriticalAuditorResponse(input) {
    ...
    if (!parsed) {
        return {
            response: null,
            issues: [
                preConfirmationIssue('critical_auditor_response_unreadable', 'Critical Auditor response is missing or unreadable', [input.responsePath], 'critical_auditor'),
            ],
        };
    }
    const requestHash = normalizeText(input.request.requestHash);
    if (normalizeText(parsed.requestHash) !== requestHash) {
        issues.push(preConfirmationIssue('critical_auditor_response_request_hash_mismatch', 'Critical Auditor response requestHash does not match the current request', ['requestHash'], 'critical_auditor'));
    }
    if (normalizeText(parsed.sourceDocumentHash) !== input.sourceDocumentHash) {
        issues.push(preConfirmationIssue('critical_auditor_response_source_hash_stale', 'Critical Auditor response sourceDocumentHash is stale', ['sourceDocumentHash'], 'critical_auditor'));
    }
    if (normalizeText(parsed.implementationConfirmationHash) !== input.implementationConfirmationHash) {
        issues.push(preConfirmationIssue('critical_auditor_response_implementation_hash_stale', 'Critical Auditor response implementationConfirmationHash is stale', ['implementationConfirmationHash'], 'critical_auditor'));
    }
    if (normalizeText(parsed.packetHash) !== input.packetHash) {
        issues.push(preConfirmationIssue('critical_auditor_response_packet_hash_stale', 'Critical Auditor response packetHash is stale', ['packetHash'], 'critical_auditor'));
    }
    const verdict = normalizeText(parsed.verdict);
    if (!verdict) {
        issues.push(preConfirmationIssue('critical_auditor_response_verdict_missing', 'Critical Auditor response must include verdict', ['verdict'], 'critical_auditor'));
    }
    if (!isNoNewGapVerdict(verdict) &&
        !['new_valid_gap', 'insufficient_audit', 'blocked'].includes(verdict)) {
        issues.push(preConfirmationIssue('critical_auditor_response_verdict_invalid', 'Critical Auditor response verdict is not an allowed value', ['verdict'], 'critical_auditor'));
    }
    if (normalizeText(parsed.gateDryRunHash) !== input.gateDryRun.hash) {
        issues.push(preConfirmationIssue('critical_auditor_response_gate_dry_run_hash_mismatch', 'Critical Auditor response gateDryRunHash does not match the current gate dry-run', ['gateDryRunHash'], 'critical_auditor'));
    }
    if (Number(parsed.reconciliationIssueCount) !== input.gateDryRun.reconciliation.issueCount) {
        issues.push(preConfirmationIssue('critical_auditor_response_reconciliation_issue_count_mismatch', 'Critical Auditor response reconciliationIssueCount does not match the current gate dry-run', ['reconciliationIssueCount'], 'critical_auditor'));
    }
    ...
    for (const group of CRITICAL_AUDITOR_PROJECTION_GROUPS) {
        if (!checkedProjectionGroups.has(group)) {
            issues.push(preConfirmationIssue('critical_auditor_response_checked_projection_group_missing', `Critical Auditor response did not check projection group ${group}`, [group], 'critical_auditor'));
        }
    }
    const priorFindingsDisposition = asRecordArray(parsed.priorFindingsDisposition);
    if (priorFindingsDisposition.length === 0) {
        issues.push(preConfirmationIssue('critical_auditor_response_prior_findings_disposition_missing', 'Critical Auditor response must classify prior findings as new/resolved/unchanged/rejected', ['priorFindingsDisposition'], 'critical_auditor'));
    }
    for (const disposition of priorFindingsDisposition) {
        ...
        if (!['new', 'resolved', 'unchanged', 'rejected'].includes(value)) {
            issues.push(preConfirmationIssue('critical_auditor_response_prior_findings_disposition_invalid', `Critical Auditor response has invalid prior finding disposition ${value || '<missing>'}`, ['priorFindingsDisposition.disposition'], 'critical_auditor'));
        }
    }
    ...
    for (const ref of reviewedRefs) {
        if (!allowedRefs.has(ref)) {
            issues.push(preConfirmationIssue('critical_auditor_response_unknown_must_ref', `Critical Auditor response references unknown MUST ${ref}`, [ref], 'critical_auditor'));
        }
    }
    ...
    if (reviewedProjectionRefs.length === 0) {
        issues.push(preConfirmationIssue('critical_auditor_response_reviewed_projection_refs_missing', 'Critical Auditor response must include non-empty reviewedProjectionRefs', ['reviewedProjectionRefs'], 'critical_auditor'));
    }
    for (const ref of [
        ...
        if (!allowedProjectionRefs.has(ref)) {
            issues.push(preConfirmationIssue('critical_auditor_response_unknown_projection_ref', `Critical Auditor response references unknown projection ${ref}`, [ref], 'critical_auditor'));
        }
    }
    if (isNoNewGapVerdict(verdict) && input.gateDryRun.actionableBlockingIssueCount > 0) {
        ...
        const provenCodes = new Set(proofs.map((proof) => normalizeText(proof.blockerCode)).filter(Boolean));
        const missingProofs = input.gateDryRun.actionableBlockingIssues
            .map((issue) => issue.code)
            .filter((code) => !provenCodes.has(code));
        if (missingProofs.length > 0) {
            issues.push(preConfirmationIssue('critical_auditor_no_new_gap_forbidden_by_gate_dry_run_blockers', 'Critical Auditor response cannot use no_new_* while gate dry-run has actionable blockers without machine-verifiable falsePositiveProofs', missingProofs, 'critical_auditor'));
        }
    }
    ...
    if (attemptedWrites.length > 0 ||
        parsed.mutatesSource === true ||
        parsed.writesControlState === true) {
        issues.push(preConfirmationIssue('critical_auditor_response_write_attempt_forbidden', 'Critical Auditor response must not write source, packet, receipt, or control state', attemptedWrites, 'critical_auditor'));
    }
    ...
    if (unresolvedGaps.length > 0 && verdict !== 'new_valid_gap') {
        issues.push(preConfirmationIssue('critical_auditor_unresolved_validated_gap', 'Critical Auditor response contains unresolved validated gaps', unresolvedGaps.map((gap) => normalizeText(gap.id ?? gap.code ?? 'gap')), 'critical_auditor'));
    }
    if (issues.length > 0) {
...
function validateCriticalAuditorRoundResultBinding(input) {
    ...
    if (normalizeText(result.gateDryRunHash) !== input.gateDryRun.hash) {
        issues.push(preConfirmationIssue('critical_auditor_response_gate_dry_run_hash_mismatch', `Critical Auditor provider round ${input.roundIndex} did not bind the current gate dry-run hash`, ['gateDryRunHash'], 'critical_auditor'));
    }
    if (Number(result.reconciliationIssueCount) !== input.gateDryRun.reconciliation.issueCount) {
        issues.push(preConfirmationIssue('critical_auditor_response_reconciliation_issue_count_mismatch', `Critical Auditor provider round ${input.roundIndex} did not bind reconciliationIssueCount`, ['reconciliationIssueCount'], 'critical_auditor'));
    }
    const reviewedProjectionRefs = new Set(result.reviewedProjectionRefs ?? []);
    if (reviewedProjectionRefs.size === 0) {
        issues.push(preConfirmationIssue('critical_auditor_response_reviewed_projection_refs_missing', `Critical Auditor provider round ${input.roundIndex} must include reviewedProjectionRefs`, ['reviewedProjectionRefs'], 'critical_auditor'));
    }
    const allowedProjectionRefs = new Set(projectionRefsFromPacket(input.packet));
    for (const ref of reviewedProjectionRefs) {
        if (!allowedProjectionRefs.has(ref)) {
            issues.push(preConfirmationIssue('critical_auditor_response_unknown_projection_ref', `Critical Auditor provider round ${input.roundIndex} references unknown projection ${ref}`, [ref], 'critical_auditor'));
        }
    }
    const checkedGroups = new Set(result.checkedProjectionGroups ?? []);
    for (const group of CRITICAL_AUDITOR_PROJECTION_GROUPS) {
        if (!checkedGroups.has(group)) {
            issues.push(preConfirmationIssue('critical_auditor_response_checked_projection_group_missing', `Critical Auditor provider round ${input.roundIndex} did not check projection group ${group}`, [group], 'critical_auditor'));
        }
    }
    const dispositions = result.priorFindingsDisposition ?? [];
    if (dispositions.length === 0) {
        issues.push(preConfirmationIssue('critical_auditor_response_prior_findings_disposition_missing', `Critical Auditor provider round ${input.roundIndex} must classify prior findings`, ['priorFindingsDisposition'], 'critical_auditor'));
    }
    for (const disposition of dispositions) {
        const value = normalizeText(disposition.disposition);
        if (!['new', 'resolved', 'unchanged', 'rejected'].includes(value)) {
            issues.push(preConfirmationIssue('critical_auditor_response_prior_findings_disposition_invalid', `Critical Auditor provider round ${input.roundIndex} has invalid disposition ${value || '<missing>'}`, ['priorFindingsDisposition.disposition'], 'critical_auditor'));
        }
    }
    if (input.gateDryRun.actionableBlockingIssueCount > 0) {
        ...
        const missingProofs = input.gateDryRun.actionableBlockingIssues
            ...
            .filter((code) => !provenCodes.has(code));
        if (missingProofs.length > 0) {
            issues.push(preConfirmationIssue('critical_auditor_no_new_gap_forbidden_by_gate_dry_run_blockers', `Critical Auditor provider round ${input.roundIndex} cannot return no_new_* while gate dry-run has actionable blockers`, missingProofs, 'critical_auditor'));
        }
    }
    return issues;
}
function detectIgnoredRequirementSource(root, sourcePath) {
...
function runMainAgentAuthoringRepair(root, options) {
    ...
    if (!extraction) {
        return buildAuthoringRepairResult({
            ...
            nextRequiredAction: 'materialize_source_document_before_deep_audit',
            issues: [
                preConfirmationIssue('implementation_confirmation_missing', 'preserve-existing authoring repair requires an existing inline implementationConfirmation block', [toRootRelativePath(root, sourcePath)], 'authoring_repair'),
            ],
        });
    }
    ...
    if (mustRequirements.length === 0) {
        return buildAuthoringRepairResult({
            ...
            issues: [
                preConfirmationIssue('controlled_must_candidates_missing', 'No inline implementationConfirmation.must[] entries were found; preserve-existing repair does not fall back to default MUST candidates.', ['implementationConfirmation.must'], 'semantic_kernel_authoring'),
            ],
        });
    }
    const sourceDocumentHash = sourceDocumentHashForPreConfirmation(sourceText, extraction.blockText, extraction.confirmation);
    const implementationConfirmationHash = implementationConfirmationHashForPreConfirmation(extraction.confirmation);
    ...
    const packet = buildPreserveExistingMustDecompositionPacket({
        ...
        consecutiveNoNewGapRounds: collectConsecutiveNoNewGapFromReceipts(previousReceiptsBeforePacket, sha256Json({
            sourceDocumentHash,
            implementationConfirmationHash,
            semanticKernelHash,
            packetHash,
    ...
    if (responsePath && !staleArtifactIssues.length) {
        ...
        if (!roundIndex || !request) {
            return buildAuthoringRepairResult({
                ...
                warnings,
                issues: [
                    preConfirmationIssue('critical_auditor_request_missing_for_response', 'Critical Auditor response does not match an existing round request', [responsePath], 'critical_auditor'),
                ],
            });
        }
        ...
        writeJsonUtf8(path.join(paths.authoringDir, `critical-auditor-receipt-round-${roundIndex}.json`), {
        ...
        try {
            if (fs.existsSync(responsePath) && path.dirname(responsePath) !== path.dirname(sourcePath)) {
                const archivedResponsePath = path.join(staleArchiveDir ?? paths.authoringDir, path.basename(responsePath));
    ...
    if (restartIssues.length > 0) {
        const archiveDir = staleArchiveDir ??
            archiveCriticalAuditorArtifacts({
                authoringDir: paths.authoringDir,
                reason: 'stale_or_unbound_critical_auditor_artifacts',
                auditInputHash,
        ...
        return buildAuthoringRepairResult({
            ...
            repairCommand: authoringRepairCommand(toRootRelativePath(root, sourcePath), toRootRelativePath(root, path.join(paths.authoringDir, 'critical-auditor-round-response-1.json'))),
            sourceDocumentHash,
            implementationConfirmationHash,
            packetHash,
            warnings,
    ...
    if (convergence.consecutive < 3) {
        ...
        return buildAuthoringRepairResult({
            ...
            blockingStage: 'critical_auditor_round_required',
            nextRequiredAction: 'write_critical_auditor_round_response',
            repairCommand: authoringRepairCommand(toRootRelativePath(root, sourcePath), toRootRelativePath(root, path.join(paths.authoringDir, `critical-auditor-round-response-${roundIndex}.json`))),
    ...
    const mustGate = runNodeJson(mustGateScript, [
        '--source',
        sourcePath,
        '--authoring-dir',
        paths.authoringDir,
    ...
    if (mustGateReport?.verdict !== 'PASS' || reconciliationReport?.verdict !== 'pass') {
        return buildAuthoringRepairResult({
            ...
            issues: mustGateReport
                ...
                : [
                    commandFailureIssue('pre_render_must_decomposition_gate_report_missing', mustGateScript, mustGate),
                ],
            consecutiveNoNewGapRounds: convergence.consecutive,
        });
    }
...
function enhanceArtifactMetadata(filePath, input) {
    ...
    const enhanced = {
        ...
        implementationConfirmationHash: normalizeText(existing.implementationConfirmationHash) ||
    ...
    const hashKey = normalizeText(enhanced.receiptHash)
        ? 'receiptHash'
        : 'contentHash';
    enhanced[hashKey] = sha256Json({
        ...enhanced,
        contentHash: null,
...
function writePreConfirmationRequirementRecord(input) {
    const record = {
        ...
        runtimePolicySnapshotRef: {
            path: toRootRelativePath(input.root, path.join(input.paths.recordRoot, 'recovery', 'runtime-policy-snapshot.json')),
        },
        recoveryContextRef: {
...
function resolveSkillScript(root, relativeScript) {
...
function runNodeJson(scriptPath, args, cwd) {
    ...
    const result = (0, node_child_process_1.spawnSync)(process.execPath, [scriptPath, ...args], {
        cwd,
        encoding: 'utf8',
        env: {
            ...process.env,
...
function validateWrittenDeepReviewInput(input) {
...
function commandFailureIssue(code, scriptPath, command) {
    const output = (command.stderr.trim() || command.stdout.trim()).slice(0, 600);
    return preConfirmationIssue(code, `Skill-local script failed or did not emit its required report: ${scriptPath}${output ? `; output: ${output}` : ''}`, [scriptPath], 'skill_script_resolution');
}
...
function collectBlockingIssuesFromReports(...reports) {
    return reports.flatMap((report) => {
        ...
        const normalized = issueRows
            .filter((item) => Boolean(item) && typeof item === 'object' && !Array.isArray(item))
            .map((item) => preConfirmationIssue(normalizeText(item.code) || 'unknown_pre_confirmation_issue', normalizeText(item.message) || normalizeText(item.code) || 'pre-confirmation issue', Array.isArray(item.refs) ? item.refs.map(String) : [], normalizeText(item.source) || 'pre_confirmation_gate'));
        if (normalized.length > 0) {
            return normalized;
        }
        return (Array.isArray(report.failedChecks) ? report.failedChecks : []).map((code) => preConfirmationIssue(String(code), `pre-confirmation gate failed: ${String(code)}`, [String(code)], 'pre_confirmation_gate'));
    });
}
function artifactMap(root, paths) {
    return {
        semanticKernel: toRootRelativePath(root, paths.semanticKernel),
        mustDecompositionPacket: toRootRelativePath(root, paths.mustDecompositionPacket),
...
function buildPreConfirmationResult(input) {
    ...
    const deliveryReadiness = input.renderReport?.deliveryReadiness &&
    ...
    return {
        ...
        visibleAuthoringLaneMessage: input.visibleAuthoringLaneMessage ??
            `author-confirmation-ready-source lane selected for ${toRootRelativePath(input.root, input.sourcePath)}`,
        advisoryScan: {
...
function updatePreConfirmationProgress(input) {
    const criticalAuditor = input.mustGateReport?.criticalAuditor;
    ...
    const progressCheckpoints = [
        ...
        { id: 'cp-07-human-readable-views', name: 'human-readable views', status: 'passed' },
        {
            ...
            status: input.mustGateReport?.verdict === 'PASS' && input.globalGateReport?.verdict === 'PASS'
                ? 'passed'
                : 'blocked',
        },
    ];
    writeJsonUtf8(input.paths.progress, {
        ...
        validation: {
            ...
            packetSourceReconciliation: input.mustGateReport?.packetSourceReconciliation
                ?.verdict === 'pass'
                ? 'pass'
                : 'fail',
            globalConsistency: input.globalGateReport?.verdict === 'PASS' ? 'pass' : 'fail',
            preRenderGate: input.mustGateReport?.verdict === 'PASS' && input.globalGateReport?.verdict === 'PASS'
                ? 'pass'
                : 'fail',
        },
...
function finalStandardsFromReports(input) {
    ...
    const lifecycle = packet?.lifecycle && typeof packet.lifecycle === 'object' && !Array.isArray(packet.lifecycle)
        ? packet.lifecycle
        : {};
    const renderSections = Array.isArray(input.renderReport?.renderedSections)
        ? input.renderReport.renderedSections.map(String)
        : [];
    const delivery = input.renderReport?.deliveryReadiness &&
        typeof input.renderReport.deliveryReadiness === 'object' &&
        !Array.isArray(input.renderReport.deliveryReadiness)
        ? input.renderReport.deliveryReadiness
        : {};
    return {
        newSkillFlowEntersAtomicDecompositionLoopBeforeMaterialization: lifecycle.atomicDecompositionLoopEnteredBeforeMaterialization === true,
        singlePassCannotSkipAtomicDecompositionLoop: true,
        threeConsecutiveNoNewValidGapRoundsRequired: Number(input.mustGateReport?.criticalAuditor &&
            input.mustGateReport.criticalAuditor
                .consecutiveNoNewGapRounds) >= 3,
        mustDecompositionPacketSynchronizedBeforeMaterialization: packet?.status === 'synchronized' && lifecycle.materializedAfterStatus === 'synchronized',
        sourceRowsMaterializedOnlyFromPacketProjections: input.mustGateReport?.packetSourceReconciliation
            ?.verdict === 'pass',
        packetSourceReconciliationPassesBidirectionally: input.mustGateReport?.packetSourceReconciliation
            ?.verdict === 'pass',
        preRenderGateBlocksMissingCoreSurfaces: input.missingSurfaceProbe?.substate === 'blocked_by_render_gate' &&
            input.missingSurfaceProbe.blockingIssues.some((issue) => issue.code === 'missing_semantic_kernel'),
        rendererShowsFullDrilldownInteraction: renderSections.includes('pre-confirmation-semantic-drilldown') &&
            Boolean(input.renderReport?.preConfirmationSemanticDrilldown) &&
            input.renderReport?.preConfirmationSemanticDrilldown
                ?.status === 'pass',
        confirmabilityBlocksOnMissingDrilldownSurfaces: input.missingSurfaceProbe?.confirmability === 'blocked',
        controlledIngestRequiredBeforeMentalModelProgression: normalizeText(input.requirementRecord?.status) !== 'user_confirmed' &&
            delivery.ready !== true,
    };
}
function runMainAgentPreConfirmationDrilldown(root, options) {
    const sourceArg = normalizeText(options.source);
    if (!sourceArg) {
    ...
    if (normalizeText(options.mode) === 'single_pass') {
        const issue = preConfirmationIssue('single_pass_cannot_skip_atomic_decomposition_loop', 'single_pass cannot skip the mandatory pre-confirmation atomic decomposition loop', ['atomic_decomposition_loop']);
    ...
    if (mustRequirements.length === 0) {
        const issue = preConfirmationIssue('controlled_must_candidates_missing', 'No explicit MUST: rows or inline implementationConfirmation.must[] entries were found; authoring must generate controlled MUST candidates before packet materialization.', [toRootRelativePath(root, sourcePath), 'implementationConfirmation.must'], 'semantic_kernel_authoring');
        return buildPreConfirmationResult({
            root,
            sourcePath,
            recordId: identity.recordId,
    ...
    if (Date.now() - authoringStartedAt > exports.SHORT_FEEDBACK_WINDOW_MS) {
        return buildPreConfirmationResult({
            root,
            sourcePath,
            recordId: identity.recordId,
            ...
            issues: [
                preConfirmationIssue('draft_updated_not_confirmation_ready', 'Source draft was materialized before the short feedback window expired; deep audit waits for confirmation_ready receipt.', [toRootRelativePath(root, paths.sourceMaterializationReceipt)], 'source_materialization'),
            ],
            sourceDocumentHash: materialized.sourceDocumentHash,
            implementationConfirmationHash: materialized.implementationConfirmationHash,
    ...
    if (options.skipDrilldownArtifacts) {
        const mustGateScript = resolveSkillScript(root, 'pre_render_must_decomposition_gate.js');
        const gate = runNodeJson(mustGateScript, [
            '--source',
            sourcePath,
            '--authoring-dir',
            paths.authoringDir,
    ...
    const combinedGate = runNodeJson(checkpointScript, ['--source', sourcePath, '--progress', paths.progress, '--mode', 'pre-render-gate', '--json'], root);
    ...
    if (!combinedReport) {
        return buildPreConfirmationResult({
            ...
            substate: 'blocked_by_render_gate',
            issues: [
                commandFailureIssue('pre_render_global_consistency_gate_report_missing', checkpointScript, combinedGate),
            ],
            mustGateReport,
            sourceDocumentHash: finalMaterialized.sourceDocumentHash,
            implementationConfirmationHash: finalMaterialized.implementationConfirmationHash,
    ...
    const globalGateReport = (combinedReport?.globalConsistencyGate &&
    ...
    if (routingDecision &&
        ['checkpoint_required', 'checkpoint_required_with_amendment'].includes(normalizeText(routingDecision.decision))) {
        const checkpointPersistence = runNodeJson(checkpointScript, [
            '--source',
            sourcePath,
            '--progress',
            paths.progress,
        ...
        if (checkpointPersistence.status !== 0 ||
            ...
            normalizeText(checkpointPersistenceEvidence?.checkpointPersistenceRef?.routeDecisionHash) !== normalizeText(routingDecision.routeDecisionHash)) {
            const issue = preConfirmationIssue('checkpoint_persistence_evidence_missing', 'Checkpoint persistence evidence is required before scale routing can continue after checkpoint_required.', [
    ...
    if (normalizeText(routingDecision?.decision) !== 'single_pass_final_allowed') {
        const issue = preConfirmationIssue(normalizeText(routingDecision?.blockingState) || 'scale_routing_decision_blocks_render', `Scale routing decision blocks confirmation render: ${normalizeText(routingDecision?.decision)}`, [toRootRelativePath(root, paths.scaleRoutingDecision)], 'requirements_contract_authoring_scale_routing');
        return buildPreConfirmationResult({
            root,
            sourcePath,
            recordId: identity.recordId,
    ...
    if (!language) {
        return buildPreConfirmationResult({
            ...
            issues: [
                preConfirmationIssue('language_required_before_render', 'Confirmation language is required before rendering the confirmation HTML.', ['confirmationLanguage'], 'render_confirmation'),
            ],
            mustGateReport,
            globalGateReport,
            sourceDocumentHash: finalMaterialized.sourceDocumentHash,
    ...
    const render = runNodeJson(rendererScript, [
        '--source',
        sourcePath,
        '--out',
...
function readinessBaselineActivation(record) {
    if (!record)
        return null;
...
function readinessDiagnostics(input) {
    ...
    if (input.drift?.effectiveVerdict === 'blocked_pending_rereadiness' &&
        ...
        diagnostics.push(buildDiagnostic({
            ...
            repairAction: activationStatus === 'audit_required'
                ? 'trigger_controlled_readiness_audit'
            ...
            message: input.drift.blockingReason ??
...
function deriveDriftSurface(projectRoot, closeout, requirementRecord = null) {
    if (closeout &&
    ...
    const scopedRecords = Array.isArray(requirementRecord?.readinessScoringRecords)
        ? requirementRecord.readinessScoringRecords.filter((item) => Boolean(item) && typeof item === 'object' && !Array.isArray(item))
        : [];
    ...
    try {
        ...
        if (projection.drift_signals.length === 0 &&
...
function resolveImplementationEntryGateFromRegistry(projectRoot, runtimeContext, flow) {
    ...
    const resolvedGate = runtimeContext?.implementationEntryGate;
    if (resolvedGate) {
        return resolvedGate;
    }
    let registry;
    try {
        registry = (0, runtime_context_registry_1.readRuntimeContextRegistry)(projectRoot);
    }
    catch {
        return null;
    }
    ...
    if (flow === 'story') {
        try {
            candidates.add((0, runtime_context_registry_1.buildImplementationEntryIndexKey)({
                flow,
                runId,
                artifactRoot,
                storyId,
...
function deriveContinueDecisionFromSurface(input) {
    ...
    if (closeout) {
        ...
        return {
            canContinue: (0, continue_state_contract_1.canMainAgentContinue)({
            ...
            continueDecision: closeout.canMainAgentContinue === true
                ? 'continue'
...
function latestRecordsById(records, idField) {
...
function hasOpenRerunLoop(record) {
    return latestRecordsById(record?.rerunLoops, 'rerunLoopId').some((loop) => ['open', 'in_progress', 'no_progress', 'blocked'].includes(normalizeText(loop.status)));
}
function gateIdentity(gate, index) {
    return ([normalizeText(gate.gate), normalizeText(gate.traceId), normalizeText(gate.requirementId)]
        .filter(Boolean)
        .join(':') ||
...
function latestGateChecks(records) {
    const latest = new Map();
    if (!Array.isArray(records)) {
        return [];
    }
...
function isRequirementRecordClosed(record) {
    if (!record) {
        return false;
    }
...
function architectureConfirmationRequiredForRecord(record) {
    if (!record)
        return false;
    if (record.architectureConfirmationRequired === true)
        return true;
    if (Array.isArray(record.architectureConfirmations) &&
        record.architectureConfirmations.length > 0)
        return true;
    if (Array.isArray(record.architectureConfirmationStateChecks) &&
...
function isRuntimeRegistryBridgeRecord(record) {
    return Boolean(record?.runtimeRegistryBridge);
}
function recordHasImplementationEntryGate(record) {
    return Boolean(record?.implementationEntryGate &&
        typeof record.implementationEntryGate === 'object' &&
        !Array.isArray(record.implementationEntryGate));
}
function readPreConfirmationLaneStateFromRecord(record) {
    if (!record) {
        return null;
    }
    const lane = record.preConfirmationDrilldownLane &&
    ...
    const substate = normalizeText(lane.substate);
    const confirmationHistory = Array.isArray(record.confirmationHistory)
        ? record.confirmationHistory.filter((item) => Boolean(item) && typeof item === 'object' && !Array.isArray(item))
        : [];
    const hasControlledConfirmationEvent = confirmationHistory.some((event) => normalizeText(event.eventType) === 'confirmation_recorded' &&
        (!normalizeText(record.sourceDocumentHash) ||
    ...
    const effectiveSubstate = userConfirmed
        ? 'user_confirmed'
        : substate || 'idle';
    return {
        currentMentalModel: 'requirement_confirmation',
...
function deriveNextActionFromRequirementRecord(input) {
    ...
    const currentModelResult = currentMentalModel &&
        ...
        ? sixModelResults[currentMentalModel]
        : null;
    const architectureState = input.record?.architectureConfirmationState &&
    ...
    const hasBlockingGate = latestGateChecks(input.record?.gateChecks).some((gate) => ['fail', 'blocked'].includes(normalizeText(gate.decision)));
    const blockingReasonRefs = [];
    if ((0, reconfirmation_runtime_1.hasOpenReconfirmationRequest)(input.record)) {
        blockingReasonRefs.push(...(0, reconfirmation_runtime_1.buildOpenReconfirmationBlockingReasonRefs)(input.record));
        return {
    ...
    if (architectureConfirmationRequiredForRecord(input.record) &&
    ...
    const hasCurrentCompiledPromptRef = packetHasCurrentHashCompiledPromptRef(input.pendingPacket, input.record);
    ...
    const canReuseStateTransition = stateNextAction === requirementNextAction ||
    ...
    if (input.state &&
        ...
        return {
            nextAction: input.state.nextAction === requirementNextAction
...
function normalizeBlockingReasonRefs(value) {
    if (!Array.isArray(value))
        return [];
    return value
        .filter((item) => {
...
function inferLatestGateFromState(state) {
    ...
    if (state.nextAction === 'dispatch_implement' ||
...
function resolveMainAgentOrchestrationSurface(input) {
    ...
    const runtimeContextImplementationEntryGate = resolveImplementationEntryGateFromRegistry(input.projectRoot, runtimeContext, input.flow);
    ...
    const displayLatestGateDecision = scopedState.state?.latestGate?.decision ??
    ...
    const controlLatestGateDecision = requirementRecord && !bridgeRecordWithoutControlledGate
        ? mapImplementationEntryDecision(implementationEntryGate)
        : displayLatestGateDecision;
    const latestGate = scopedState.state?.latestGate ??
        inferredLatestGate ??
    ...
    const useRequirementRecordProjection = Boolean(requirementRecord) &&
    ...
    const matrixAttemptId = scopedState.state?.pendingPacket?.packetId ??
    ...
    const sixModelRuntimeDecision = useRequirementRecordProjection
        ? (0, six_model_runtime_decision_1.resolveSixModelRuntimeDecision)({
            record: requirementRecord,
        ...
        : null;
    const sixModelRuntimeDecisionPath = input.projectRoot && sixModelRuntimeDecision
        ? (0, six_model_runtime_decision_1.writeSixModelRuntimeDecision)({
            projectRoot: input.projectRoot,
            decision: sixModelRuntimeDecision,
        })
        : null;
    const implementationEntryDecision = implementationEntryGate?.decision ?? null;
    const controlPlaneBlocksSixModelOverride = continueState.continueDecision === 'blocked' ||
    ...
    const bridgePendingPacketRemainsAuthoritative = runtimeRegistryBridgeRecord &&
        pendingPacketStatus !== 'none' &&
    ...
    const bridgeImplementDispatchFallback = bridgeRecordWithoutControlledGate &&
        input.stage === 'implement' &&
        action.nextAction === 'run_implementation_readiness_gate';
    const stateOnlyControlledRemediation = !requirementRecord && Boolean(scopedState.state) && action.source === 'orchestration_state';
    const bridgeCompletedRemediationReturnsToImplement = (runtimeRegistryBridgeRecord || stateOnlyControlledRemediation) &&
        bridgeCompletedRemediationState({
            pendingPacketStatus,
            pendingPacket,
            state: scopedState.state,
        });
    const sixModelRuntimeDecisionAuthoritative = Boolean(sixModelRuntimeDecision?.currentMentalModel) &&
    ...
    const fallbackActionNextAction = bridgeImplementDispatchFallback
        ...
        : bridgeCompletedRemediationReturnsToImplement
            ...
            : bridgePendingPacketRemainsAuthoritative
                ...
                : action.nextAction;
    const fallbackActionReady = bridgeImplementDispatchFallback || bridgeCompletedRemediationReturnsToImplement
        ? true
        : bridgePendingPacketRemainsAuthoritative
    ...
    const actionReady = sixModelRuntimeDecisionAuthoritative
        ...
        : fallbackActionReady;
    const splitBrainBlockerPath = input.projectRoot &&
        ...
        ? (0, six_model_runtime_decision_1.writeSplitBrainBlocker)({
            projectRoot: input.projectRoot,
            decision: sixModelRuntimeDecision,
        ...
        : null;
    const implementationEntryGateComesFromRecord = !explicitImplementationEntryGateProvided && recordHasImplementationEntryGate(requirementRecord);
    const implementationEntryGateIsBridgeOnly = runtimeRegistryBridgeRecord && implementationEntryGateComesFromRecord;
    const explicitImplementationEntryGateIsOnlySource = explicitImplementationEntryGateProvided && !recordHasImplementationEntryGate(requirementRecord);
    const surfaceSource = action.source === 'implementation_entry_gate' ||
    ...
    const terminalState = 'terminalState' in action && action.terminalState === 'completed_no_dispatch'
        ...
        ...(useRequirementRecordProjection
            ? {
                runtimeResumeProjection: {
                    ...
                    blockingReasonRefs: normalizeBlockingReasonRefs('blockingReasonRefs' in action ? action.blockingReasonRefs : []).concat(sixModelRuntimeDecision?.blockingReasonRefs ?? []),
...
function claimMainAgentPendingPacket(projectRoot, sessionId, owner = 'main-agent') {
    return (0, orchestration_state_1.claimPendingPacket)(projectRoot, sessionId, owner);
}
function markMainAgentPacketDispatched(projectRoot, sessionId, packetId) {
    return (0, orchestration_state_1.markPendingPacketDispatched)(projectRoot, sessionId, packetId);
}
function completeMainAgentPendingPacket(projectRoot, sessionId, packetId) {
    return (0, orchestration_state_1.completePendingPacket)(projectRoot, sessionId, packetId);
}
function invalidateMainAgentPendingPacket(projectRoot, sessionId, packetId) {
    return (0, orchestration_state_1.invalidatePendingPacket)(projectRoot, sessionId, packetId);
}
function ingestMainAgentTaskReport(projectRoot, sessionId, report, options = {}) {
    const state = (0, orchestration_state_1.readOrchestrationState)(projectRoot, sessionId);
    if (!state) {
    ...
    const taskType = packet && 'taskType' in packet
        ? packet.taskType
        : state.nextAction === 'dispatch_remediation'
            ...
            : state.nextAction === 'dispatch_review'
                ? 'audit'
                : 'implement';
    const legacyNextAction = options.nextActionHint ??
        (report.status === 'done'
    ...
    if (report.status === 'done') {
        (0, orchestration_state_1.completePendingPacket)(projectRoot, sessionId, report.packetId);
        (0, orchestration_state_1.resetGatesLoopProgress)(projectRoot, sessionId, {
            lastResult: `task-report:${report.status}`,
        });
    }
    else if (report.status === 'partial') {
        (0, orchestration_state_1.recordGatesLoopNoProgress)(projectRoot, sessionId, {
            lastResult: `task-report:${report.status}`,
        });
    }
    else {
        (0, orchestration_state_1.invalidatePendingPacket)(projectRoot, sessionId, report.packetId);
        (0, orchestration_state_1.recordGatesLoopNoProgress)(projectRoot, sessionId, {
            lastResult: `task-report:${report.status}`,
        });
    }
    const evidenceState = (0, orchestration_state_1.updateOrchestrationState)(projectRoot, sessionId, (current) => ({
        ...current,
    ...
    const runtimeContext = loadRuntimeContextForMainAgent({
        projectRoot,
        flow: evidenceState.flow,
        stage: options.currentStage ?? evidenceState.currentPhase,
    });
    const requirementRecord = readRequirementRecordFromRuntimeContext(runtimeContext);
    const matrix = (0, six_model_runtime_decision_1.resolveSixModelRuntimeDecision)({
        record: requirementRecord,
    ...
    const matrixPath = (0, six_model_runtime_decision_1.writeSixModelRuntimeDecision)({ projectRoot, decision: matrix });
    const keepLegacyTransition = isRuntimeRegistryBridgeRecord(requirementRecord) && taskType === 'remediate';
    ...
    return (0, orchestration_state_1.updateOrchestrationState)(projectRoot, sessionId, (current) => ({
        ...current,
        nextAction: (keepLegacyTransition
            ? legacyNextAction
            : (matrix.nextAction ?? 'await_user')),
    }));
}
function ensureMainAgentDispatchPacket(input) {
    ...
    if (currentSurface.pendingPacketStatus !== 'none' &&
    ...
    const sessionId = resolvedContext(runtimeContext)?.requirementSetId ??
    ...
    const executionDisciplineProfile = taskType === 'implement' || taskType === 'remediate'
        ? (0, execution_discipline_profiles_1.resolveExecutionDisciplineProfile)(flow)
        : null;
    ...
    const resolvedScope = resolveMappedAllowedWriteScope(input.projectRoot, runtimeContext, input.flow, taskType, currentSurface.runtimeResumeProjection
        ? readRequirementRecordFromRuntimeContext(runtimeContext)
        : null);
    const allowedWriteScope = resolvedScope && resolvedScope.length > 0
        ? resolvedScope
        : taskType === 'audit'
            ? ['docs/**', '_bmad-output/**', 'specs/**']
            : ['src/**', 'tests/**', 'docs/**', '_bmad-output/**'];
    const recordPath = resolvedContext(runtimeContext)?.recordPath;
    const compiledRun = taskType === 'implement' && !currentSurface.orchestrationState?.originalExecutionPacketId
        ? (0, main_agent_compiled_prompt_runner_1.runMainAgentCompiledPrompt)({
            projectRoot: input.projectRoot,
            recordPath,
            sourcePath: normalizeText(runtimeContext?.artifactPath) ||
                normalizeText(runtimeContext?.artifactRoot),
            packetId,
            flow,
            executionHost: host === 'codex' ? 'codex' : host === 'claude' ? 'claude-code' : 'cursor-ide',
    ...
    const inheritedCompiledPromptRef = taskType === 'audit'
        ...
        : null;
    const auditExecution = taskType === 'audit' && inheritedCompiledPromptRef
        ...
        : null;
    const runtimeModeSelection = taskType === 'implement' && compiledRun?.status === 'pass' && compiledRun.compiledPromptRef
        ? (0, host_runtime_mode_1.writeExecutionRuntimeModeSelection)({
            projectRoot: input.projectRoot,
            recordId: normalizeText(activeRecord?.recordId) || sessionId,
            packetId,
            attemptId: packetId,
    ...
    if (runtimeModeSelection && compiledRun?.compiledPromptRef) {
        const nativeGoalBlocker = (0, host_runtime_mode_1.validateNativeGoalReadiness)({
            projectRoot: input.projectRoot,
            recordId: normalizeText(activeRecord?.recordId) || sessionId,
            packetId,
            attemptId: packetId,
        ...
        if (nativeGoalBlocker) {
            (0, host_runtime_mode_1.writeRuntimeBlocker)(input.projectRoot, normalizeText(activeRecord?.recordId) || sessionId, packetId, nativeGoalBlocker);
        }
    }
    const executionStrategy = compiledRun?.status === 'pass'
        ...
        : null;
    const sddArtifactManifestRef = compiledRun?.status === 'pass'
        ...
        : null;
    const packet = currentSurface.orchestrationState?.originalExecutionPacketId != null
        ? (0, orchestration_dispatch_contract_1.createResumePacket)({
            packetId,
            ...
            inputArtifacts: [resolvedContext(runtimeContext)?.recordPath, ...inputArtifacts].filter(Boolean),
            allowedWriteScope,
        ...
        : (0, orchestration_dispatch_contract_1.createExecutionPacket)({
            packetId,
            parentSessionId: sessionId,
            flow,
            phase: input.stage,
            ...
            legacyPromptFallbackReason: taskType === 'audit'
                ? null
                : compiledRun?.status === 'no_confirmed_source' || compiledRun === null
                    ? 'no_confirmed_source'
                    : null,
            compilerBlock: taskType === 'audit' && !inheritedCompiledPromptRef
    ...
    const compilerBlocked = 'compilerBlock' in packet &&
    ...
    let writtenState;
    const canUpdateCurrentState = Boolean(currentSurface.orchestrationState) && currentSurface.sessionId === sessionId;
    if (canUpdateCurrentState) {
        (0, orchestration_state_1.updateOrchestrationState)(input.projectRoot, sessionId, (current) => ({
            ...current,
            host,
            pendingPacket: {
                packetId,
            ...
            nextAction: currentSurface.mainAgentNextAction ??
                current.nextAction,
            gatesLoop: {
        ...
        writtenState = (0, orchestration_state_1.readOrchestrationState)(input.projectRoot, sessionId);
    }
    else {
        writtenState = {
            version: 1,
        ...
        const file = path.join((0, orchestration_state_1.orchestrationStateDirForRecordPath)(input.projectRoot, resolvedContext(runtimeContext)?.recordPath), `${sessionId}.json`);
        fs.mkdirSync(path.dirname(file), { recursive: true });
    ...
    return {
        source: 'orchestration_state',
        sessionId,
        orchestrationStatePath: path.join((0, orchestration_state_1.orchestrationStateDirForRecordPath)(input.projectRoot, resolvedContext(runtimeContext)?.recordPath), `${sessionId}.json`),
...
function buildMainAgentDispatchInstruction(input) {
    ...
    const route = (0, orchestration_dispatch_contract_1.resolveDispatchRoute)(host, taskType);
    return {
        flow: input.flow,
        stage: input.stage,
        host,
        ...
        role: 'role' in surface.pendingPacket
            ? surface.pendingPacket.role
            : (surface.pendingPacket.recommendedRole ?? defaultPacketRole(taskType)),
        expectedDelta: surface.pendingPacket.expectedDelta ??
            surface.pendingPacket.expectedDelta,
    };
}
...
function isMainAgentCliAction(value) {
    return MAIN_AGENT_CLI_ACTIONS.has(value);
}
function defaultRunLoopTaskReportPath(projectRoot, sessionId, packetId) {
    return path.join(projectRoot, '_bmad-output', 'runtime', 'governance', 'task-reports', sessionId, `${packetId}.json`);
}
function dispatchPacketGoalCommandMode(packet) {
    const receiptPath = normalizeText(packet.compiledPromptRef?.auditReceiptPath);
    if (!receiptPath || !fs.existsSync(receiptPath))
        return '';
    try {
        const receipt = JSON.parse(fs.readFileSync(receiptPath, 'utf8'));
        const goalCommand = receipt.goalCommand && typeof receipt.goalCommand === 'object'
            ? receipt.goalCommand
            : {};
...
function isNativeGoalExecutionPacket(instruction, args) {
    if (args.codexSmoke === 'true')
        return false;
    if (instruction.host !== 'codex' && instruction.host !== 'claude')
        return false;
...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        else if ((token === '--implementation-run-kind' || token === '--implementationRunKind') &&
            argv[index + 1]) {
            out.implementationRunKind = argv[++index];
        }
        else if ((token === '--readiness-report-path' || token === '--readinessReportPath') &&
            argv[index + 1]) {
        ...
        else if ((token === '--confirmation-language' || token === '--confirmationLanguage') &&
            argv[index + 1]) {
        ...
        else if ((token === '--critical-auditor-response' || token === '--criticalAuditorResponse') &&
            argv[index + 1]) {
    ...
    if (!out.cwd && cwdCandidate)
        out.cwd = cwdCandidate;
    return out;
}
...
function pushOptionalArg(target, flag, value, root, pathLike = false) {
    const normalized = normalizeText(value);
    if (!normalized) {
        return;
    }
...
function stageSummaryForCommandResult(root, args, delegatedOutput) {
    try {
    ...
    const implementationEntryGate = record.implementationEntryGate &&
        typeof record.implementationEntryGate === 'object' &&
        !Array.isArray(record.implementationEntryGate)
...
function directRequirementRecordPathForCommand(root, args, delegatedOutput) {
    const fromArg = normalizeText(args.requirementRecord);
    if (fromArg) {
        return path.resolve(root, stripWrappingQuotes(fromArg));
    }
    const output = delegatedOutput && typeof delegatedOutput === 'object' && !Array.isArray(delegatedOutput)
        ? delegatedOutput
        : null;
    const fromOutput = normalizeText(output?.requirementRecordPath) ||
        normalizeText(output?.stdout && typeof output.stdout === 'object' && !Array.isArray(output.stdout)
            ? output.stdout.requirementRecordPath
            : null);
    if (fromOutput) {
        return path.resolve(root, stripWrappingQuotes(fromOutput));
    }
    const index = readJsonIfExists(path.join(root, '_bmad-output', 'runtime', 'requirement-records', 'index.json'));
    const active = index?.active && typeof index.active === 'object' && !Array.isArray(index.active)
        ? index.active
        : null;
    const activeRequirementSetId = normalizeText(active?.requirementSetId || args.requirementSetId);
    if (activeRequirementSetId) {
        return path.join(root, '_bmad-output', 'runtime', 'requirement-records', activeRequirementSetId, 'requirement-record.json');
    }
    return null;
}
function runMainAgentConfirmCloseoutAcceptance(root, args) {
    ...
    if (!normalizeText(args.confirmationText) && !normalizeText(args.confirmationTextFile)) {
        throw new Error('confirm-closeout-acceptance requires --confirmation-text <exact closeout confirmation> or --confirmation-text-file <file>');
    }
    const delegatedArgs = [
        '--action',
        'confirm-closeout-acceptance',
    ...
    const step = (0, node_child_process_1.spawnSync)(process.execPath, [entry, ...delegatedArgs], {
        cwd: root,
        encoding: 'utf8',
    });
    let parsedStdout = undefined;
    if (step.stdout.trim()) {
        try {
...
function runMainAgentConfirmScope(root, args) {
    ...
    if (!normalizeText(args.confirmationText) && !normalizeText(args.confirmationTextFile)) {
        throw new Error('confirm-scope requires --confirmation-text <exact chat confirmation> or --confirmation-text-file <file>');
    }
...
function runMainAgentConfirmationBookkeepingRepair(root, args) {
    const entry = resolveSkillScript(root, 'confirm-requirements-scope.js');
    if (!fs.existsSync(entry)) {
...
function loadConfirmationDriftClassifier(root) {
    ...
    return requireCommonJs(classifierPath);
}
function currentConfirmationHashes(sourcePath) {
    const sourceText = fs.readFileSync(sourcePath, 'utf8');
    const extraction = extractImplementationConfirmationBlock(sourceText);
    if (!extraction) {
...
function confirmationTextFromRenderReport(report) {
...
function runMainAgentConfirmationDriftRoute(root, args) {
    const source = normalizeText(args.source);
    if (!source) {
    ...
    if (kind === 'projection_refresh_required') {
        ...
        const priorProjectionHash = normalizeText(classification.latestProjectionHash) ||
            normalizeText(record.latestConfirmationProjectionHash) ||
            normalizeText(record.confirmationPageHash);
    ...
    if (kind === 'semantic_reconfirmation_required') {
        const reconfirmation = (0, reconfirmation_runtime_1.requestSemanticReconfirmation)({
            recordPath,
            classification,
...
async function runMainAgentControlledReadinessAudit(root, args) {
    const loaded = (0, emit_runtime_policy_1.loadPolicyContextFromRegistry)(root, {
        recordId: args.recordId,
        requirementSetId: args.requirementSetId,
        runId: args.runId,
    });
    return (0, controlled_readiness_audit_bridge_1.runControlledReadinessAuditBridge)({
        root,
...
function resolveFlowAndStage(root, args) {
    ...
    try {
        const runtimeContext = (0, emit_runtime_policy_1.loadPolicyContextFromRegistry)(root, {
            recordId: args.recordId,
            requirementSetId: args.requirementSetId,
            runId: args.runId,
        }).runtimeContext;
        ...
        return {
            flow: explicitFlow ? explicitFlow : null,
            stage: explicitStage || null,
            noActiveRequirement: true,
        };
    }
}
function buildNoActiveRequirementRunLoopResult(root, flow, stage) {
...
function parseOrchestrationHost(value) {
    const normalized = normalizeText(value);
...
function resolveSessionAndPacketFromSurface(surface, args) {
    ...
    const packetId = normalizeText(args.packetId) ||
...
function parseTaskReportStatus(value) {
    const normalized = normalizeText(value);
...
function validateTaskReportShape(value) {
    const report = value;
    return (typeof report?.packetId === 'string' &&
...
function readTaskReportFromFile(reportPath, packetId) {
...
function readTaskReportPacketId(reportPath) {
    const normalized = normalizeText(reportPath);
    if (!normalized || !fs.existsSync(normalized))
        return null;
    try {
        const parsed = JSON.parse(fs.readFileSync(path.resolve(normalized), 'utf8'));
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed))
            return null;
        return normalizeText(parsed.packetId) || null;
    }
    catch {
        return null;
    }
...
function controlledIngestCodexWorkerEvidence(input) {
    ...
    if (envelope && validation.ok) {
        ...
        for (const artifact of artifactRefs) {
            ...
            const artifactCommit = (0, requirement_record_control_store_1.appendControlEventAndReplay)({
                recordPath: input.recordPath,
                writerId: 'main-agent-orchestration',
                eventType: 'artifact_indexed',
    ...
    const executionEvent = {
        ...
        architectureConfirmationHash: normalizeText(record.architectureConfirmationState
            ?.currentArchitectureConfirmationHash),
        recordedAt: input.recordedAt,
        recordedBy: 'main-agent-orchestration',
    };
    const iterationCommit = (0, requirement_record_control_store_1.appendControlEventAndReplay)({
        recordPath: input.recordPath,
        writerId: 'main-agent-orchestration',
        eventType: 'execution_iteration_recorded',
...
function writeMainAgentRunLoopTaskReport(projectRoot, instruction, args = {}) {
    ...
    const reportPath = path.resolve(args.taskReportPath ??
        path.join(projectRoot, '_bmad-output', 'runtime', 'governance', 'task-reports', instruction.sessionId, `${instruction.packetId}.json`));
    fs.mkdirSync(path.dirname(reportPath), { recursive: true });
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2) + '\n', 'utf8');
    return reportPath;
}
function latestImplementationReadinessGate(record) {
    const gates = latestGateChecks(record?.gateChecks).filter((gate) => normalizeText(gate.gate) === 'Implementation Readiness Gate');
    return gates.length > 0 ? gates[gates.length - 1] : null;
}
function readReadinessReport(recordPath, args) {
...
function latestReadinessBlockers(record, report) {
    const fromReport = stringsFrom(report?.blockingReasons);
    if (fromReport.length > 0)
        return Array.from(new Set(fromReport));
    return Array.from(new Set(stringsFrom(latestImplementationReadinessGate(record)?.blockingReasons)));
}
...
function sourceAmendmentReadinessAction(blocker, reasonCode = 'source_authority_blocker_requires_amendment', reason = 'blocker affects confirmed execution contract semantics or authority') {
    return {
        blocker,
        schemaVersion: 'readiness-blocker-classification/v1',
...
function userDecisionReadinessAction(blocker, reasonCode = 'multiple_equal_candidates_require_user_decision', reason = 'confirmed source does not select one candidate among multiple equal valid candidates') {
    return {
        blocker,
        schemaVersion: 'readiness-blocker-classification/v1',
...
function blockerHasMultipleEqualCandidates(blocker) {
    return (blocker.includes(':multiple_equal_candidates') ||
        blocker.includes(':ambiguous_candidates') ||
        blocker.includes(':ambiguous') ||
        blocker.startsWith('multiple_equal_candidate:'));
}
function readinessBlockerBaseId(blocker) {
...
function classifyReadinessBlocker(blocker) {
    ...
    if (baseBlocker === 'PACKET_SOURCE_RECONCILIATION_FAILED') {
        return sourceAmendmentReadinessAction(blockerId, 'reconciliation_failed_requires_source_amendment', 'packet/source reconciliation failure indicates source and packet drift');
    }
    ...
    if (baseBlocker === 'ATOMIC_TASK_LINEAGE_REQUIRED' ||
        baseBlocker === 'MANIFEST_SECTION_REQUIRED:atomicImplementationTaskLineage') {
        if (hasMultipleEqualCandidates)
            return userDecisionReadinessAction(blockerId);
        return deriveReadinessAction({
            blocker: blockerId,
    ...
    if (baseBlocker === 'AI_TDD_MANIFEST_REQUIRED') {
        return sourceAmendmentReadinessAction(blockerId, 'ai_tdd_manifest_root_blocker_requires_section_detail', 'AI_TDD_MANIFEST_REQUIRED must be classified by concrete missing manifest sections');
    }
    if ([
        ...
        if (hasMultipleEqualCandidates)
            return userDecisionReadinessAction(blockerId);
        return sourceAmendmentReadinessAction(blockerId, 'authority_or_relation_manifest_section_requires_source_amendment', 'manifest section can affect source authority, target authority, command authority, artifact authority, or relation bindings');
    }
    if (baseBlocker.startsWith('PRE_IMPLEMENTATION_RED_PROOF_PLAN_REQUIRED:') ||
        ...
        if (hasMultipleEqualCandidates)
            return userDecisionReadinessAction(blockerId);
        return sourceAmendmentReadinessAction(blockerId, 'relation_or_red_proof_requires_source_amendment', 'missing relation or expected-red proof plan belongs to confirmed source execution contract');
    }
    if ([
    ...
    return sourceAmendmentReadinessAction(blockerId, 'unknown_blocker_fail_closed', 'blocker has no deterministic non-semantic repair mapping');
}
function classifyReadinessBlockers(blockers) {
    const uniqueBlockers = Array.from(new Set(blockers.map(normalizeText).filter(Boolean)));
    const hasMissingTestRoot = uniqueBlockers.some((blocker) => blocker.startsWith('required_command_file_missing:') ||
        [
    ...
    return uniqueBlockers.map((blocker) => {
        if (hasMissingTestRoot &&
            ...
            return deriveReadinessAction({
                ...
                reason: 'missing expected-red test files are uniquely declared by the confirmed AI-TDD manifest',
...
function toProjectRelativePath(root, candidate) {
...
function artifactSnapshotFromRow(input) {
    ...
    if (!pathValue || !previousHash)
        return null;
    const absolutePath = resolveProjectPath(input.root, pathValue);
    return {
        artifactId: normalizeText(input.row.artifactId) ||
...
function currentCloseoutAttemptId(record) {
    const closeout = record.closeout && typeof record.closeout === 'object' && !Array.isArray(record.closeout)
        ? record.closeout
        : {};
...
function postCloseArtifactSnapshots(input) {
    const snapshots = [];
    const closeoutEvidence = input.record.closeoutEvidence &&
    ...
    const artifactRows = readJsonlObjects(artifactIndexPath).filter((row) => {
        if (normalizeText(row.artifactType) !== 'target_file_snapshot')
            return false;
...
function changedPostCloseTargetArtifacts(snapshots) {
    const changed = [];
    for (const snapshot of snapshots) {
...
function hasCloseoutProofDefectSignal(record, explicitSignal) {
    if ([
...
function explicitPostCloseClassification(signal) {
    if (signal === 'implementation_defect' ||
...
function defaultRunId(recordId) {
...
function postCloseReportPath(input) {
    ...
    if (explicit)
        return resolveProjectPath(input.root, stripWrappingQuotes(explicit));
    return path.join(path.dirname(input.recordPath), 'post-close', `${input.runId}.json`);
}
function gateReportRef(filePath, report) {
    return {
...
function writeGateReport(filePath, report) {
    writeJsonUtf8(filePath, report);
    return gateReportRef(filePath, report);
}
function captureNestedMainOutput(fn) {
    const stdoutChunks = [];
    const stderrChunks = [];
...
function postCloseCarrierControlRecordPath(reportDir) {
...
function runPostCloseGateStack(input) {
    ...
    const commandRun = captureNestedMainOutput(() => (0, run_required_commands_from_ai_tdd_manifest_1.mainRunRequiredCommandsFromAiTddManifest)([
        '--source',
        input.sourcePath,
        '--requirement-record',
        carrierRecordPath,
    ...
    const targetReport = (0, target_artifact_realization_gate_1.evaluateTargetArtifactRealization)({
        sourcePath: input.sourcePath,
    ...
    gateReports.push(writeGateReport(path.join(input.reportDir, 'target-artifact-realization-report.json'), targetReport));
    const strictReport = (0, strict_closeout_proof_gate_1.evaluateStrictCloseoutProof)({
        sourcePath: input.sourcePath,
        record: updatedRecord,
        recordPath: carrierRecordPath,
        attemptId: input.attemptId,
    ...
    return {
        gateReports,
        blockingReasons: uniqueNonEmpty(gateReports.flatMap((report) => normalizeText(report.decision) === 'pass'
            ? []
            : [
...
function runMainAgentPostCloseDefectIntake(root, args) {
    const source = normalizeText(args.source);
    const requirementRecord = normalizeText(args.requirementRecord);
    if (!source)
        throw new Error('post-close-defect-intake requires --source <source-document.md>');
    if (!requirementRecord)
    ...
    if (!record)
        throw new Error(`requirement record missing or invalid: ${recordPath}`);
    const recordId = normalizeText(record.recordId) || path.basename(path.dirname(recordPath));
    const requirementSetId = normalizeText(record.requirementSetId) || recordId;
    const attemptId = normalizeText(args.attemptId) || currentCloseoutAttemptId(record) || `post-close-${recordId}`;
    ...
    const classification = proofDefect
        ? 'closeout_proof_defect'
    ...
    const gateStack = classification === 'post_close_revalidation_required' && !dryRun
    ...
    const decision = classification === 'no_post_close_action_required'
        ? 'no_action'
        : classification === 'post_close_revalidation_required'
            ...
            : 'blocked';
    const nextSafeAction = classification === 'closeout_proof_defect'
        ? 'closure_integrity_incident_required'
        : classification === 'post_close_revalidation_required'
            ? dryRun
    ...
    const carrier = {
        ...
        previousArtifactEvidenceRefs: uniqueNonEmpty(changedTargetArtifacts.flatMap((artifact) => artifact.previousArtifactEvidenceRefs)),
    ...
    if (!dryRun)
        writeJsonUtf8(reportPath, carrier);
    return {
        ok: classification === 'no_post_close_action_required' ||
            classification === 'post_close_revalidation_required',
        ...base,
        classification,
        decision,
...
function createExpectedRedAdapterTest(filePath, blocker) {
    if (fs.existsSync(filePath))
        return;
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
...
function acceptanceRowsFromReport(report) {
    for (const check of objectsFrom(report?.checks)) {
        const manifest = check.contractExecutionManifest;
...
function aiTddPreImplementationReport(input) {
    if (input.implementationRunKind !== 'first-implementation')
        return null;
    try {
        return (0, ai_tdd_contract_gate_1.evaluateAiTddContractGate)({
            sourcePath: input.sourcePath,
            record: input.record,
            recordPath: input.recordPath,
...
function acceptanceRowsFromAiTddReport(report) {
    const manifest = report?.contractExecutionManifest;
    if (!manifest || typeof manifest !== 'object' || Array.isArray(manifest))
        return [];
    return [
        ...objectsFrom(manifest.acceptanceTests),
...
function filePathsFromAcceptanceRows(rows) {
...
function missingAcceptanceFilePaths(root, report) {
    return filePathsFromAcceptanceRows(acceptanceRowsFromAiTddReport(report)).filter((candidate) => !fs.existsSync(resolveProjectPath(root, candidate)));
}
function defaultProofRows(report, aiTddReport) {
    ...
    for (const row of rows) {
        ...
        if (!acceptanceId || uniqueRows.has(acceptanceId))
            continue;
...
function defaultPreImplementationPlan(input) {
    const acceptanceIds = uniqueNonEmpty([
        ...
        ...defaultProofRows(input.report, input.aiTddReport).map((row) => normalizeText(row.acceptanceId)),
...
function appendReadinessRemediationControlEvent(input) {
    const commit = (0, requirement_record_control_store_1.appendControlEventAndReplay)({
        ...
        reduce: (record) => {
            const aiTdd = record.aiTddContractGate &&
            ...
            return {
                ...
                extensionRefs: [
                    ...objectsFrom(record.extensionRefs),
                    {
                        ...
                        purpose: 'records blocker classification, deterministic repair actions, and rerun gate result',
...
function runImplementationReadinessGateForRecord(input) {
    const reportPath = path.join(path.dirname(input.recordPath), 'implementation-readiness-report.json');
    const exitCode = (0, main_agent_implementation_readiness_gate_1.mainImplementationReadinessGate)([
        '--requirement-record',
        input.recordPath,
        '--source',
        input.sourcePath,
...
function runMainAgentReadinessAutoRemediation(input) {
    ...
    const concreteActions = blockerActions.filter((action) => action.blocker !== 'ai_tdd_pre_implementation_readiness_not_ready');
    const blockingActions = concreteActions.filter((action) => !action.autoRemediationAllowed);
    if (blockingActions.length > 0) {
        const hasUserDecision = blockingActions.some((action) => action.classification === 'requires_user_decision');
        return {
            ok: false,
            status: 'blocked',
            blockerActions,
            ...
            evidence: blockingActions.map((action) => `${action.classification}:${action.requiredNextAction}:${action.blocker}`),
            blockingReasons: [
    ...
    const filesChanged = [];
    for (const action of concreteActions.filter((item) => item.action === 'create_expected_red_adapter_test')) {
    ...
    const shouldCreateManifestFiles = concreteActions.some((item) => item.action === 'create_missing_acceptance_files_from_manifest');
    ...
    const proofRows = defaultProofRows(report, aiTddReport).map((row, index) => ({
    ...
    const receiptPath = path.join(path.dirname(input.recordPath), 'implementation-readiness', 'auto-remediation', `readiness-auto-remediation-${Date.now()}.json`);
    const receipt = {
...
function runMainAgentAutomaticLoop(input) {
    const args = input.args ?? {};
    const steps = [];
    const runtimeContext = loadRuntimeContextForMainAgent({
        projectRoot: input.projectRoot,
    ...
    const readinessGateBlocked = initialSurface.mainAgentNextAction === 'dispatch_remediation' &&
    ...
    if (activeRecordPath && readinessGateBlocked) {
        ...
        if (!remediation.ok) {
            const blockedNextAction = remediation.requiredNextAction === 'source_amendment_required'
                ? 'source_amendment_required'
                ...
                finalSurface: {
                    ...
                    mainAgentStageSummary: finalSurface.mainAgentStageSummary
                        ? {
                            ...
                            userFacingMessage: `当前六心智阶段: ${finalSurface.mainAgentStageSummary.currentMentalModel ?? 'unknown'} (${finalSurface.mainAgentStageSummary.currentMentalModelStatus ?? 'unknown'}); 下一步: ${blockedNextAction}.`,
                        }
                        : null,
    ...
    const instruction = buildMainAgentDispatchInstruction({
        ...
        host: input.host,
        hydratePacket: true,
        preferredPacketId: process.env.MAIN_AGENT_ALLOW_EXTERNAL_TASK_REPORT === 'true'
    ...
    (0, orchestration_state_1.updateOrchestrationState)(input.projectRoot, instruction.sessionId, (current) => (0, long_run_runtime_policy_1.applyLongRunPolicyToState)(current, {
    ...
    try {
        ...
        if (!taskReport && isNativeGoalExecutionPacket(instruction, args)) {
            ...
            const nativeTaskReportPath = taskReportPath ||
            ...
            steps.push({
                ...
                summary: `command=${nativeResult.command}, report=${path.relative(input.projectRoot, nativeResult.taskReportPath)}`,
            });
        ...
        if (!taskReport && instruction.host === 'codex') {
            ...
            const adapterReport = (0, main_agent_codex_worker_adapter_1.runCodexWorkerAdapter)({
                projectRoot: input.projectRoot,
            ...
            steps.push({
                ...
                status: adapterReport.exitCode === 0 &&
                    adapterReport.scopePassed &&
                    adapterReport.taskReport.status === 'done'
                    ? 'pass'
                    : 'fail',
                summary: `mode=${adapterReport.mode}, report=${path.relative(input.projectRoot, adapterReport.taskReportPath)}`,
            });
        ...
        return {
            ...
            steps: [
                ...steps,
                {
                    step: 'task-report.load',
                    status: 'fail',
                    summary: 'missing real task report artifact; pass --taskReportPath or provide an executor',
                },
            ],
            dispatchInstruction: instruction,
            taskReport: null,
    ...
    if (codexAdapterReport &&
        ...
        taskReport = {
            ...
            evidence: [
                ...taskReport.evidence,
                ...codexAdapterReport.subagentEvidenceEnvelopeValidation.mismatches.map((mismatch) => `subagentEvidenceEnvelope invalid: ${mismatch}`),
            ],
            driftFlags: [
                ...new Set([
    ...
    const completedState = ingestMainAgentTaskReport(input.projectRoot, instruction.sessionId, taskReport, { currentStage: input.stage });
    steps.push({
...
function findReadinessBaselineActivationDiagnostic(surface, record) {
    if (normalizeText(readinessBaselineActivation(record)?.status) !== 'audit_required') {
        return null;
    }
    return (surface.diagnostics.find((diagnostic) => diagnostic.automaticRepairAvailable &&
        READINESS_BASELINE_ACTIVATION_REPAIR_ACTIONS.has(diagnostic.repairAction)) ?? null);
}
function readinessBaselineActivationTaskReport() {
    return {
...
async function runMainAgentAutomaticLoopAsync(input) {
    const args = input.args ?? {};
    const runtimeContext = loadRuntimeContextForMainAgent({
        projectRoot: input.projectRoot,
        recordId: input.recordId,
            ...
            finalSurface: {
                ...
                mainAgentStageSummary: finalSurface.mainAgentStageSummary
                    ? {
                        ...
                        ready: false,
                        blocked: true,
                        userFacingMessage: `当前六心智阶段: ${finalSurface.mainAgentStageSummary.currentMentalModel ?? 'unknown'} (${finalSurface.mainAgentStageSummary.currentMentalModelStatus ?? 'unknown'}); 下一步: await_user.`,
                    }
                    : null,
            },
            mainAgentStageSummary: finalSurface.mainAgentStageSummary
...
function mainMainAgentOrchestration(argv) {
    ...
    if (action === 'route-intake' || action === 'adaptive-intake') {
        ...
        if (!inputPath && !payload) {
            console.error('main-agent-orchestration: route-intake requires --input <json-file> or --payload <json>');
            return 1;
        }
        const candidate = JSON.parse(payload || fs.readFileSync(path.resolve(root, inputPath), 'utf8'));
        const result = (0, adaptive_intake_governance_gate_1.runAdaptiveIntakeGovernanceGate)(root, candidate, {
    ...
    if (action === 'confirm-scope' || action === 'confirmation-ingest') {
        try {
            ...
            console.error(`main-agent-orchestration confirm-scope: ${error instanceof Error ? error.message : String(error)}`);
            return 1;
        }
    }
    if (action === 'confirm-closeout-acceptance' || action === 'closeout-acceptance-ingest') {
        try {
            ...
            console.error(`main-agent-orchestration confirm-closeout-acceptance: ${error instanceof Error ? error.message : String(error)}`);
            return 1;
        }
    }
    if (action === 'route-confirmation-drift' || action === 'confirmation-drift-route') {
        try {
            ...
            console.error(`main-agent-orchestration route-confirmation-drift: ${error instanceof Error ? error.message : String(error)}`);
            return 1;
        }
    }
    if (action === 'repair-confirmation-bookkeeping' ||
        ...
        try {
            ...
            console.error(`main-agent-orchestration repair-confirmation-bookkeeping: ${error instanceof Error ? error.message : String(error)}`);
            return 1;
        }
    }
    if (action === 'pre-confirmation-drilldown' ||
        ...
        try {
            ...
            const actionLabel = action === 'author-confirmation-ready-source' ||
                action === 'author_confirmation_ready_source'
                ? 'author-confirmation-ready-source'
                : 'pre-confirmation-drilldown';
            console.error(`main-agent-orchestration ${actionLabel}: ${error instanceof Error ? error.message : String(error)}`);
            return 1;
        }
    }
    if (action === 'authoring-repair' || action === 'authoring_repair') {
        try {
            ...
            console.error(`main-agent-orchestration authoring-repair: ${error instanceof Error ? error.message : String(error)}`);
            return 1;
        }
    }
    if (action === 'post-close-defect-intake') {
        try {
            ...
            console.error(`main-agent-orchestration post-close-defect-intake: ${error instanceof Error ? error.message : String(error)}`);
            return 1;
        }
    }
        ...
        case 'run-loop': {
            const result = runMainAgentAutomaticLoop({
                ...
                executor: args.taskReportPath
                    ? ({ instruction, args: runArgs }) => process.env.MAIN_AGENT_ALLOW_EXTERNAL_TASK_REPORT === 'true'
...
async function mainMainAgentOrchestrationAsync(argv) {
    const args = parseArgs(argv);
    const root = pickRoot(args);
    const action = normalizeText(args.action) || 'inspect';
    if (action === 'run-loop') {
    ...
    try {
        ...
        console.error(`main-agent-orchestration controlled-readiness-audit: ${error instanceof Error ? error.message : String(error)}`);
        return 1;
    }
}
