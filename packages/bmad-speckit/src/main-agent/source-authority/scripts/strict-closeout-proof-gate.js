...
exports.evaluateStrictCloseoutProof = evaluateStrictCloseoutProof;
exports.mainStrictCloseoutProofGate = mainStrictCloseoutProofGate;
/* eslint-disable no-console */
const crypto = __importStar(require("node:crypto"));
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
const target_artifact_realization_gate_1 = require("./target-artifact-realization-gate");
function isDirectStrictCloseoutProofGateCli(entry) {
...
function resolveArtifact(recordPath, artifactPath) {
...
function currentAttempt(record, explicit) {
    ...
    return (text(nested(strict?.lastRunRef).closeoutAttemptId) ||
...
function artifactRefComplete(ref) {
    return Boolean(text(ref.path) &&
...
function artifactIndexed(record, artifact) {
    ...
    return (objects(record.artifactIndex).find((item) => normalizePath(text(item.path)) === wantedPath &&
        text(item.contentHash ?? item.hash) === wantedHash &&
        text(item.sourceOfTruthRole) === 'evidence') ?? null);
}
function latestArtifact(record, artifactType) {
    return (objects(record.artifactIndex)
        .filter((item) => text(item.artifactType) === artifactType &&
...
function runtimeDirFromRecord(recordPath) {
    return path.dirname(path.dirname(path.dirname(recordPath)));
}
function defaultDatasetReleaseArtifact(record, recordPath, artifactType) {
    const fileName = artifactType === 'dataset_release_manifest'
...
function latestOrDefaultDatasetReleaseArtifact(record, recordPath, artifactType) {
    return (latestArtifact(record, artifactType) ??
...
function eventForArtifact(events, artifact) {
    ...
    return (events.find((event) => {
        ...
        return refs.some((ref) => normalizePath(text(ref.path)) === artifactPath &&
            text(ref.hash ?? ref.contentHash) === artifactHash);
    }) ?? null);
}
function controlledRebaseline(event) {
    if (!REBASELINE_EVENT_TYPES.has(text(event.eventType)))
        return {};
    ...
    const valid = rebaseline.controlled === true &&
...
function replayProof(record, events) {
    const reasons = [];
    if (events.length === 0)
        reasons.push('control_events_missing');
...
function writerRegistryProof(events, attemptId) {
    ...
    const attemptEvents = events.filter((event) => text(nested(nested(event.payload).packet).closeoutAttemptId) === attemptId ||
...
function receiptProof(recordPath, events) {
...
function schemaEvolutionProof(record) {
...
function commandProof(runs, commandId) {
...
function productionSubsystemProofApplies(input) {
    ...
    if (explicit !== null)
        return explicit;
    return Boolean(latestArtifact(input.record, 'observability_extension') ||
...
function failureCaseProofApplies(input) {
    ...
    if (explicit !== null)
        return explicit;
    return Boolean(latestArtifact(input.record, 'failure_case_coverage') ||
...
function sftProjectionProofApplies(input) {
    ...
    if (explicit !== null)
        return explicit;
    return Boolean(latestOrDefaultDatasetReleaseArtifact(input.record, input.recordPath, 'dataset_release_manifest') ||
...
function sftProjectionProof(record, recordPath) {
    ...
    const reasons = [];
    if (!manifestRef)
        reasons.push('dataset_manifest_artifact_missing');
    if (!releaseRef)
        reasons.push('dataset_release_report_artifact_missing');
            ...
            ? [
                {
                    ...
                    message: 'strict closeout proof requires --source when implementationConfirmationHash is present',
...
function evaluateStrictCloseoutProof(input) {
    ...
    const closeoutAttemptInvalidationPolicy = {
        ok: events.some((event) => text(event.eventType) === 'closeout_recorded' ||
    ...
    const targetArtifactRealization = input.sourcePath
        ? (0, target_artifact_realization_gate_1.evaluateTargetArtifactRealization)({
...
function mainStrictCloseoutProofGate(argv) {
    const args = parseArgs(argv);
    if (args.help) {
        console.log('Usage: strict-closeout-proof-gate --requirement-record <json> --source <requirement.md> [--attempt-id <id>] [--report-path <json>] [--json]');
        return 0;
    }
    ...
    if (!attemptId)
        throw new Error('closeout attempt id missing');
