...
exports.mainEntryFlowTraceabilityCheck = mainEntryFlowTraceabilityCheck;
...
function parseArgs(argv) {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (arg === '--help' || arg === '-h')
            ...
            out.json = true;
        else if (arg === '--check-all-source-traces')
            out.checkAllSourceTraces = true;
...
function checkEntryFlowState(record, sourceConfirmation) {
    const issues = [];
...
function checkGlobalContractTraceabilityPolicy(record) {
    const issues = [];
    const policy = asObject(record.globalContractTraceabilityPolicy);
    if (!policy)
        return { issues: ['globalContractTraceabilityPolicy_missing'], policy: null };
    ...
    for (const flow of REQUIRED_TRACEABILITY_FLOWS) {
        if (!flows.has(flow))
            issues.push(`globalContractTraceabilityPolicy_missing_entryFlow:${flow}`);
    }
...
function checkSourceTaskBinding(sourceConfirmation, activeTraceIds, checkAllSourceTraces) {
    if (!sourceConfirmation) {
        return {
    ...
    const taskBindingRows = tasks.map((task) => {
        ...
        if (!id)
            taskBindingIssues.push('implementationTask_id_missing');
    ...
    const traceBindingRows = traceRows.map((row) => {
        ...
        if (!id)
            taskBindingIssues.push('traceRow_id_missing');
...
function checkTraceBinding(record) {
...
function checkNoStandaloneRuntimeFactArtifacts(record) {
    const entryFlow = text(record.entryFlow);
    const artifacts = objects(record.artifactIndex);
    if (entryFlow !== 'standalone_tasks')
    ...
    const forbidden = artifacts.filter((artifact) => {
        ...
        return (artifactPath.includes('/standalone_tasks/') &&
...
function mainEntryFlowTraceabilityCheck(argv) {
    ...
    const reportPath = path.resolve(args.reportPath ??
        path.join(path.dirname(path.resolve(args.requirementRecord)), 'entry-flow-adaptation-matrix.json'));
