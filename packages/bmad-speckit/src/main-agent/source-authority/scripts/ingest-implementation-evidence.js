...
exports.mainIngestImplementationEvidence = mainIngestImplementationEvidence;
...
function isDirectImplementationEvidenceIngestCli(entry) {
...
function requireArgs(args) {
    const missing = ['evidence', 'requirementRecord'].filter((key) => !args[key]);
    if (missing.length > 0)
...
function containsForbiddenField(value, field) {
    if (!value || typeof value !== 'object')
        return false;
    if (Array.isArray(value))
        return value.some((item) => containsForbiddenField(item, field));
    const obj = value;
    return (Object.prototype.hasOwnProperty.call(obj, field) ||
        Object.values(obj).some((item) => containsForbiddenField(item, field)));
}
function legacyResultToDecision(value) {
...
function normalizeGateChecks(packet) {
...
function normalizeContractChecks(packet) {
...
function closureInputs(packet) {
    const explicitClosures = arrayOfObjects(packet.requirementClosures);
    if (text(packet.status) !== 'done')
        return explicitClosures;
...
function requireHashMatch(packet, record) {
    const mismatches = [];
    for (const field of ['sourceDocumentHash', 'implementationConfirmationHash']) {
        if (text(packet[field]) !== text(record[field]))
            mismatches.push(`${field}_mismatch`);
    }
    const state = record.architectureConfirmationState;
    const architectureRequired = record.architectureConfirmationRequired === true ||
...
function resolvedArchitectureConfirmationHash(packet, record) {
    const state = record.architectureConfirmationState;
    return (text(packet.architectureConfirmationHash) ||
...
function sourceRefs(packet) {
    const refs = [];
    for (const id of arrayOfStrings(packet.traceRows))
        refs.push({ sourceType: 'trace_row', id });
    for (const id of arrayOfStrings(packet.evidenceRefs))
        refs.push({ sourceType: 'evidence', id });
...
function validateCommands(packet) {
    const mismatches = [];
    ...
    if (!runId)
        mismatches.push('run_id_missing');
    if (commandRuns.length === 0)
        mismatches.push('command_runs_missing');
    for (const run of commandRuns) {
        if (!text(run.commandId))
            mismatches.push('command_id_missing');
        if (!text(run.command))
            mismatches.push('command_missing');
        if (typeof run.exitCode !== 'number')
            mismatches.push('command_exit_code_missing');
        if (!text(run.startedAt) || !text(run.completedAt))
            mismatches.push('command_time_missing');
        if (text(run.runId) !== runId)
            mismatches.push('command_run_id_mismatch');
...
function validateArtifacts(packet) {
    const mismatches = [];
    const validateArtifact = (artifact, prefix, passGradeOnly) => {
        ...
        if (!artifactPath)
            mismatches.push(`${prefix}_artifact_path_missing`);
        if (!hash)
            mismatches.push(`${prefix}_artifact_hash_missing`);
        if (!text(artifact.artifactType))
            mismatches.push(`${prefix}_artifact_type_missing`);
        if (!role)
            mismatches.push(`${prefix}_artifact_source_of_truth_role_missing`);
        ...
        if (!text(artifact.producer))
            mismatches.push(`${prefix}_artifact_producer_missing`);
        if (!text(artifact.purpose))
            mismatches.push(`${prefix}_artifact_purpose_missing`);
        ...
        if (!text(artifact.status))
            mismatches.push(`${prefix}_artifact_status_missing`);
        if (!text(artifact.inputVersion))
            mismatches.push(`${prefix}_artifact_input_version_missing`);
        if (!text(artifact.outputVersion))
            mismatches.push(`${prefix}_artifact_output_version_missing`);
    ...
    const deliveryEvidence = packet.deliveryEvidence;
...
function validateImplementationDelta(packet) {
    const mismatches = [];
    const delta = packet.implementationDelta;
    ...
    if (!text(delta.diffSummaryRef))
        mismatches.push('implementation_delta_diff_summary_ref_missing');
...
function validateEntryFlowState(packet) {
    const mismatches = [];
    ...
    if (!ENTRY_FLOWS.has(entryFlow))
        mismatches.push('entry_flow_state_entry_flow_invalid');
...
function validateGlobalContractTraceabilityPolicy(policy, prefix) {
    const mismatches = [];
...
function validateTraceStatusPolicy(policy, prefix) {
    const mismatches = [];
    if (!policy || typeof policy !== 'object' || Array.isArray(policy)) {
    ...
    for (const status of [
        ...
        if (terminal.has(status))
            mismatches.push(`${prefix}_trace_status_policy_user_scoped_status_can_full_closeout:${status}`);
    }
...
function validateRuntimePolicySnapshotRef(packet) {
    ...
    const runtimePolicySnapshotRef = ref;
    ...
    if (!artifactPath)
        mismatches.push('runtime_policy_snapshot_ref_path_missing');
    if (!hash)
        mismatches.push('runtime_policy_snapshot_ref_hash_missing');
    if (role === 'control')
        mismatches.push('runtime_policy_snapshot_ref_must_not_be_control');
...
function validateHookReconciliation(packet) {
    ...
    const hookReconciliation = value;
    const mismatches = [];
    ...
    const sequenceLedger = hookReconciliation.sequenceLedger &&
    ...
    for (const mismatch of hashMismatches) {
        if (!text(mismatch.field))
            mismatches.push('hook_reconciliation_hash_mismatch_field_missing');
        if (!text(mismatch.expected))
            mismatches.push('hook_reconciliation_hash_mismatch_expected_missing');
        if (!text(mismatch.actual))
            mismatches.push('hook_reconciliation_hash_mismatch_actual_missing');
    }
    for (const ref of noHookFallbackRefs) {
        ...
        if (!text(ref.id))
            mismatches.push('hook_reconciliation_fallback_ref_id_missing');
    }
    ...
    if (hookReconciliation.closeoutReconciled === true &&
...
function validateFailureRecords(packet) {
    const mismatches = [];
    for (const failure of arrayOfObjects(packet.failureRecords)) {
        ...
        if (!text(failure.failureId))
            mismatches.push('failure_record_id_missing');
        if (!text(failure.type))
            mismatches.push('failure_record_type_missing');
...
function validateRcaRecords(packet) {
    const mismatches = [];
    for (const rca of arrayOfObjects(packet.rcaRecords)) {
        ...
        if (!text(rca.rcaId))
            mismatches.push('rca_record_id_missing');
        if (!text(rca.type))
            mismatches.push('rca_record_type_missing');
...
function validateRerunLoops(packet) {
    const mismatches = [];
    for (const loop of arrayOfObjects(packet.rerunLoops)) {
        if (!text(loop.rerunLoopId))
            mismatches.push('rerun_loop_id_missing');
        if (![
            'open',
            'in_progress',
            'no_progress',
        ...
        if (rerunSourceRefs.length === 0)
            mismatches.push('rerun_loop_source_refs_missing');
        for (const sourceRef of rerunSourceRefs) {
            ...
            if (!text(sourceRef.id))
                mismatches.push('rerun_loop_source_ref_id_missing');
        }
...
function validateSubagentEvidenceEnvelopePacket(packet, record) {
    const envelope = packet.subagentEvidenceEnvelope;
    if (envelope === undefined || envelope === null)
        return [];
    const validation = (0, subagent_evidence_envelope_1.validateSubagentEvidenceEnvelope)(envelope, {
        record,
...
function validatePacket(packet, record) {
    const entryFlowState = packet.entryFlowState &&
        typeof packet.entryFlowState === 'object' &&
        !Array.isArray(packet.entryFlowState)
        ? packet.entryFlowState
        : undefined;
    const effectiveTraceabilityPolicy = entryFlowState?.globalContractTraceabilityPolicy &&
    ...
    const effectiveTraceStatusPolicy = entryFlowState?.traceStatusPolicy &&
    ...
    if (containsForbiddenField(packet, 'fullDiff'))
        mismatches.push('forbidden_inline_full_diff');
    if (containsForbiddenField(packet, 'diffPatch'))
        mismatches.push('forbidden_inline_diff_patch');
    if (containsForbiddenField(packet, 'inlineDiff'))
        mismatches.push('forbidden_inline_diff');
    if (text(packet.eventType) && text(packet.eventType) !== 'execution_iteration_recorded') {
        mismatches.push('unsupported_event_type');
    }
    if (!EXECUTION_STATUSES.has(text(packet.status)))
        mismatches.push('execution_status_invalid');
    if (!text(packet.executionIterationId))
        mismatches.push('execution_iteration_id_missing');
    if (sourceRefs(packet).length === 0)
        mismatches.push('source_refs_missing');
    for (const closure of arrayOfObjects(packet.requirementClosures)) {
        if (!text(closure.requirementId))
            mismatches.push('closure_requirement_id_missing');
        if (!CLOSURE_STATUSES.has(text(closure.status)))
            mismatches.push('closure_status_invalid');
    }
    for (const gate of normalizeGateChecks(packet)) {
        if (!text(gate.gate))
            mismatches.push('gate_missing');
        if (!GATE_DECISIONS.has(text(gate.decision)))
            mismatches.push('gate_decision_invalid');
    }
    for (const contractCheck of normalizeContractChecks(packet)) {
        if (!text(contractCheck.contract))
            mismatches.push('contract_check_contract_missing');
        if (!CONTRACT_DECISIONS.has(text(contractCheck.decision)))
            mismatches.push('contract_check_decision_invalid');
    }
    const deliveryEvidence = packet.deliveryEvidence;
    for (const command of arrayOfObjects(deliveryEvidence?.requiredCommands)) {
        if (!text(command.commandId))
            mismatches.push('required_command_id_missing');
        if (!text(command.command))
            mismatches.push('required_command_missing');
        if (command.blockingIfMissing !== true)
            mismatches.push('required_command_not_blocking');
        if (arrayOfObjects(command.artifactRefs).length === 0)
            mismatches.push('required_command_artifact_refs_missing');
    }
    return [...new Set(mismatches)];
}
function commandRunRefs(packet) {
...
function artifactEvents(packet, recordId, requirementSetId) {
...
function evidencePacketArtifact(packet, evidencePath, recordId, requirementSetId) {
    return {
        eventType: 'artifact_indexed',
...
function normalizeHistoricalArtifactRef(artifact, recordId, requirementSetId, fallbackRelatedRequirementIds = []) {
...
function normalizeHistoricalCommand(command, recordId, requirementSetId) {
    ...
    return {
        ...command,
        artifactRefs: arrayOfObjects(command.artifactRefs).map((artifact) => normalizeHistoricalArtifactRef(artifact, recordId, requirementSetId, fallbackRelatedRequirementIds)),
    };
}
function updateRecord(record, packet, recordedAt, recordedBy, evidencePath = '') {
    ...
    const packetArtifact = evidencePath
        ...
        : undefined;
    const extensionRefs = artifactEvents({ ...packet, artifactRefs: arrayOfObjects(packet.extensionRefs), extensionRefs: [] }, recordId, requirementSetId);
    ...
    const subagentEnvelope = packet.subagentEvidenceEnvelope &&
        ...
        : undefined;
    const subagentEnvelopeValidation = subagentEnvelope
        ? (0, subagent_evidence_envelope_1.validateSubagentEvidenceEnvelope)(subagentEnvelope, {
            record,
            projectRoot: process.cwd(),
            indexedArtifactRefs: [
                ...arrayOfObjects(packet.artifactRefs),
    ...
    const contractCheckEvents = normalizeContractChecks(packet).map((contractCheck) => ({
        ...
        recordId,
        requirementSetId,
        checkId: text(contractCheck.checkId) ||
    ...
    const existingDeliveryEvidence = record.deliveryEvidence &&
        typeof record.deliveryEvidence === 'object' &&
        !Array.isArray(record.deliveryEvidence)
        ? record.deliveryEvidence
        : {};
    const packetDeliveryEvidence = packet.deliveryEvidence &&
        typeof packet.deliveryEvidence === 'object' &&
        !Array.isArray(packet.deliveryEvidence)
        ? packet.deliveryEvidence
        : {};
    const existingRequiredCommands = arrayOfObjects(existingDeliveryEvidence.requiredCommands).map((command) => normalizeHistoricalCommand(command, recordId, requirementSetId));
    const packetRequiredCommands = arrayOfObjects(packetDeliveryEvidence.requiredCommands);
    const packetRuntimePolicySnapshotRef = packet.runtimePolicySnapshotRef &&
    ...
    return {
        ...
        ...(packet.entryFlowState &&
            ...
            !Array.isArray(packet.entryFlowState)
            ? {
                entryFlow: text(packet.entryFlowState.entryFlow),
                entryFlowClass: text(packet.entryFlowState.entryFlowClass),
                workflowAdapter: text(packet.entryFlowState.workflowAdapter),
                contractAuthoringRequired: packet.entryFlowState.contractAuthoringRequired === true,
                ...(packet.entryFlowState.globalContractTraceabilityPolicy &&
                    typeof packet.entryFlowState.globalContractTraceabilityPolicy ===
                        'object' &&
                    !Array.isArray(packet.entryFlowState.globalContractTraceabilityPolicy)
                    ? {
                        globalContractTraceabilityPolicy: packet.entryFlowState
                            .globalContractTraceabilityPolicy,
                    }
                    : {}),
                ...(packet.entryFlowState.traceStatusPolicy &&
                    typeof packet.entryFlowState.traceStatusPolicy === 'object' &&
                    !Array.isArray(packet.entryFlowState.traceStatusPolicy)
                    ? {
                        traceStatusPolicy: packet.entryFlowState.traceStatusPolicy,
                    }
                    : {}),
            }
            : {}),
        ...
        artifactIndex: [
            ...arrayOfObjects(record.artifactIndex).map((artifact) => normalizeHistoricalArtifactRef(artifact, recordId, requirementSetId)),
            ...(packetArtifact ? [packetArtifact] : []),
            ...artifactRefs,
        ],
        extensionRefs: [
            ...arrayOfObjects(record.extensionRefs).map((artifact) => normalizeHistoricalArtifactRef(artifact, recordId, requirementSetId)),
            ...extensionRefs,
        ],
        deliveryEvidence: {
...
function mainIngestImplementationEvidence(argv) {
    ...
    const eventLog = path.resolve(args.eventLog ?? (0, requirement_record_control_store_1.eventLogPathForRecord)(recordPath));
    const artifactIndex = path.resolve(args.artifactIndex ?? path.join(baseDir, 'artifact-index.jsonl'));
    const globalArtifactIndex = path.resolve(args.globalArtifactIndex ?? path.join(path.dirname(baseDir), 'artifact-index.jsonl'));
    const packetArtifact = evidencePacketArtifact(packet, normalizePathForRecord(evidencePath), text(packet.recordId) || text(record.recordId), text(packet.requirementSetId) || text(record.requirementSetId));
    const commit = (0, requirement_record_control_store_1.appendControlEventAndReplay)({
        ...
        reduce: (currentRecord) => updateRecord(currentRecord, packet, recordedAt, recordedBy, normalizePathForRecord(evidencePath)),
    });
    for (const artifact of [
        packetArtifact,
        ...artifactEvents(packet, text(packet.recordId) || text(record.recordId), text(packet.requirementSetId) || text(record.requirementSetId)),
    ]) {
