...
function tsNodeScript(scriptPath, args = []) {
    return [
        process.execPath,
...
function runStep(id, command, allowFailure = false) {
...
function runStepWithEnv(id, command, env, allowFailure = false) {
    const result = (0, node_child_process_1.spawnSync)(command[0], command.slice(1), {
        cwd: process.cwd(),
        encoding: 'utf8',
        stdio: 'inherit',
...
function main(argv) {
    ...
    const provenance = (0, evidence_provenance_1.buildEvidenceProvenance)({
        root,
        runId: args.runId,
        storyKey,
    ...
    steps.push(runStepWithEnv('release-gate', tsNodeScript('scripts/main-agent-release-gate.ts', commonReleaseArgs), releaseGateEnv, true));
    const truthGate = runStep('delivery-truth-gate', tsNodeScript('scripts/main-agent-delivery-truth-gate.ts'), true);
