...
function realpathOrNull(value) {
  try {
    return fs.realpathSync(value, { encoding: 'utf8' });
  } catch {
    return null;
  }
}
...
const SOURCE_REPO_EQUIVALENT_ROOT = findSourceRepoEquivalentRoot(__dirname);
const PACKAGE_SELF_ROOTS_REAL = new Set(
  [PKG_ROOT, SOURCE_REPO_EQUIVALENT_ROOT].map((candidate) => candidate && realpathOrNull(candidate)).filter(Boolean)
);

function isPackageSelfTarget(targetDir) {
  const targetReal = realpathOrNull(targetDir);
  return Boolean(targetReal && PACKAGE_SELF_ROOTS_REAL.has(targetReal));
}

function isExternalTarget(targetDir) {
  return !isPackageSelfTarget(targetDir);
}
...
function normalizePlatformSkillFrontmatterFile(filePath) {
  ...
  if (hasRequiredSkillFrontmatter(withoutPolicy)) {
    if (withoutPolicy !== raw) {
      fs.writeFileSync(filePath, withoutPolicy, 'utf8');
    }
    return;
  }
...
function deployConsumerRuntimeEmitToHooks(pkgRoot, targetDir) {
  if (isPackageSelfTarget(targetDir)) return;
