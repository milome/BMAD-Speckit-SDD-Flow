...
exports.runControlledReadinessAuditBridge = runControlledReadinessAuditBridge;
...
function recordRoot(recordPath) {
...
function readinessReportContent(record) {
    return [
...
function writeReadinessAuditReport(recordPath, auditRequestId, record) {
...
function sha256File(filePath) {
    ...
    return (0, requirement_record_control_store_1.sha256Text)(fs.readFileSync(filePath, 'utf8'));
}
function writeReadinessAuditManifest(recordPath, manifest) {
    const out = path.join(recordRoot(recordPath), 'readiness-audit', 'manifest.json');
    fs.mkdirSync(path.dirname(out), { recursive: true });
    fs.writeFileSync(out, `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
    return out;
}
function resolveReadinessAuditContext(record, now) {
    ...
    if (metadataStatus === 'current') {
        return {
            activationId: metadataActivationId || `implementation-readiness-result:${requirementSetId}:${now}`,
            baselineId: metadataBaselineId || `readiness-baseline:${requirementSetId}`,
            readinessGateRecipeVersion: text(metadata.readinessGateRecipeVersion) || 'implementation-readiness-gate/v1',
        };
    }
    throw new Error('controlled readiness audit requires readinessBaselineMetadata.status=current or legacy readinessBaselineActivation.status=audit_required');
}
function appendEvent(recordPath, input) {
        ...
        reduce: (currentRecord) => ({
            ...
            [input.appendField]: [
                ...(Array.isArray(currentRecord[input.appendField])
                    ? currentRecord[input.appendField]
                    : []),
                input.payload,
            ],
...
async function runControlledReadinessAuditBridge(input) {
    const root = path.resolve(input.root);
    const recordPath = path.resolve(input.recordPath);
    const initialRecord = (0, requirement_record_control_store_1.readJson)(recordPath);
    ...
    const scoringRunId = input.scoringRunId ??
        `${text(initialRecord.requirementSetId)}-readiness-${now.replace(/[^0-9]/gu, '').slice(0, 14)}`;
    const dataPath = (0, runtime_scoring_data_path_1.resolveRuntimeScoringDataPath)({
        root,
        dataPath: input.dataPath,
    });
    const sourceRequirementRecordHash = (0, requirement_record_control_store_1.sha256Json)(initialRecord);
    ...
    const toolTraceRef = (0, requirement_record_control_store_1.sha256Text)(fs.readFileSync(toolTracePath, 'utf8'));
    ...
    const recordAfterRequest = (0, requirement_record_control_store_1.readJson)(recordPath);
    ...
    const readinessAuditReportHash = (0, requirement_record_control_store_1.sha256Text)(fs.readFileSync(readinessAuditReportPath, 'utf8'));
    const content = fs.readFileSync(readinessAuditReportPath, 'utf8');
    const scoreRecord = await (0, parse_and_write_1.parseAndWriteScore)({
        content,
    ...
    const dimensionScores = (scoreRecord.dimension_scores ?? []);
    ...
    const manifestHash = (0, requirement_record_control_store_1.sha256Text)(fs.readFileSync(manifestPath, 'utf8'));
    appendEvent(recordPath, {
