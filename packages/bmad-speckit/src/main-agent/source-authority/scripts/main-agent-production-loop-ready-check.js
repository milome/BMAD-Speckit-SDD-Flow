...
exports.mainProductionLoopReadyCheck = mainProductionLoopReadyCheck;
...
function extensionRefs(record) {
    return objects(record.extensionRefs).filter((ref) => text(ref.artifactType) === 'observability_extension' ||
...
function resolveArtifactPath(recordPath, artifactPath) {
    return path.isAbsolute(artifactPath)
        ? artifactPath
        : path.resolve(path.dirname(recordPath), '..', '..', '..', '..', artifactPath);
}
function latestActiveExtension(record) {
    const refs = extensionRefs(record).filter((ref) => text(ref.status) === 'active');
    return refs.length > 0 ? refs[refs.length - 1] : null;
}
function hasCompleteArtifactRef(ref) {
    if (!ref)
        return false;
...
function artifactCompletenessIssues(ref) {
    const issues = [];
    if (!text(ref.path))
...
function extensionIdentityIssues(record, extension) {
    const issues = [];
    if (text(extension.recordId) !== text(record.recordId))
        issues.push('extension_record_id_mismatch');
...
function subsystemIssues(extension) {
    const issues = [];
    ...
    for (const subsystemId of REQUIRED_SUBSYSTEM_IDS) {
        ...
        if (!text(subsystem.status))
            issues.push(`subsystem_status_missing:${subsystemId}`);
...
function confirmationAuthorityIssues(record) {
...
function artifactExistsAndMatches(ref) {
    const artifactPath = text(ref.path);
    const hash = text(ref.hash ?? ref.contentHash);
    return Boolean(artifactPath &&
        isSha256(hash) &&
        fs.existsSync(artifactPath) &&
        sha256File(artifactPath) === hash);
}
function datasetManifestIssues(record, manifestPath) {
    const issues = [];
    ...
    const architectureState = nested(record, 'architectureConfirmationState');
    if (text(source.recordId) !== text(record.recordId))
        issues.push('dataset_manifest_record_id_mismatch');
    ...
    if (text(source.architectureConfirmationHash) !==
        text(architectureState.currentArchitectureConfirmationHash)) {
        issues.push('dataset_manifest_architecture_hash_mismatch');
    }
    const counts = nested(manifest, 'counts');
    if (Number(counts.canonicalSamples ?? 0) <= 0)
        issues.push('dataset_manifest_canonical_samples_missing');
    if (Number(counts.sampleRoutes ?? 0) <= 0)
        issues.push('dataset_manifest_sample_routes_missing');
    if (Number(counts.blockedIssues ?? 1) !== 0)
    ...
    refs.forEach((ref, index) => {
        if (!artifactExistsAndMatches(ref))
            issues.push(`dataset_manifest_artifact_hash_mismatch:${index}`);
    });
    return issues;
}
function datasetReleaseIssues(record, recordPath, args) {
    const issues = [];
    ...
    if (!fs.existsSync(reportPath)) {
        ...
        if (text(report.manifestHash) &&
            fs.existsSync(manifestPath) &&
            sha256File(manifestPath) !== text(report.manifestHash)) {
...
function evaluate(record, recordPath, args = {}) {
    const checks = [];
    ...
    if (!extensionRef)
        blockingReasons.push('observability_extension_ref_missing');
    if (extensionRef && !extensionRefComplete)
        blockingReasons.push('observability_extension_ref_incomplete');
    let extension = null;
    if (extensionRef && extensionRefComplete) {
        ...
        checks.push({
            id: 'observability-extension-file-exists',
            passed: exists,
            path: normalizePathForRecord(resolvedPath),
        });
        checks.push({ id: 'observability-extension-hash-current', passed: hashMatches });
        if (!exists)
            blockingReasons.push('observability_extension_file_missing');
        if (exists && !hashMatches)
            blockingReasons.push('observability_extension_hash_mismatch');
        if (exists && hashMatches)
            extension = readJson(resolvedPath);
    }
    if (extension) {
        ...
        checks.push({
            id: 'observability-extension-identity-current',
            passed: identityIssues.length === 0,
            issues: identityIssues,
        });
        checks.push({
            id: 'observability-plan-complete',
            passed: observationIssues.length === 0,
...
function mainProductionLoopReadyCheck(argv) {
