...
function writeScopeIssues(plan) {
  const issues = [];
  const orderIndex = new Map(plan.merge_order.map((nodeId, index) => [nodeId, index]));
  for (let leftIndex = 0; leftIndex < plan.nodes.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < plan.nodes.length; rightIndex += 1) {
      ...
      for (const scope of intersects(left.write_scope, right.write_scope)) {
        const conflict = plan.conflicts.find(
          (candidate) => candidate.scope === scope && candidate.contenders.includes(left.node_id) && candidate.contenders.includes(right.node_id)
        );
        if (!conflict) {
          issues.push(`write_scope_conflict_unresolved:${scope}:${left.node_id},${right.node_id}`);
          continue;
        }
        if (conflict.resolution === 'block') issues.push(`write_scope_conflict_blocked:${scope}:${left.node_id},${right.node_id}`);
...
function topologyIssues(plan, topology) {
  const issues = [];
  ...
  for (const id of planIds) if (!topologyIds.has(id)) issues.push(`pr_topology_required_node_missing:${id}`);
  for (const id of topologyIds) if (!planIds.has(id)) issues.push(`pr_topology_unknown_node:${id}`);
...
function integratedVerificationIssues(input) {
  const verification = input.integratedVerification;
  const issues = [];
  if (!verification) return ['integrated_verification_missing'];
  if (verification.closeoutAttemptId !== input.currentCloseoutAttemptId) issues.push('integrated_verification_closeout_attempt_mismatch');
  if (verification.workspaceRef?.kind !== 'main_workspace') issues.push(`integrated_verification_workspace_not_main:${verification.workspaceRef?.kind || '<missing>'}`);
  ...
  for (const [index, run] of commandRuns.entries()) {
    ...
    if (objects(run.artifactRefs).length === 0) issues.push(`integrated_verification_command_artifact_refs_missing:${text(run.commandId) || index}`);
  }
  return issues;
}

function conflictProof(plan) {
  const orderIndex = new Map(plan.merge_order.map((nodeId, index) => [nodeId, index]));
...
function validateSubagentEnvelope(envelope, node) {
  const issues = [];
  if (!envelope || typeof envelope !== 'object') {
    return { envelopeHash: null, issues: [`node_envelope_missing:${node.node_id}`] };
  }
  ...
  if (!objects(envelope.commandRuns).some((run) => run.exitCode === 0)) issues.push(`node_command_evidence_missing:${node.node_id}`);
  if (envelope.status && envelope.status !== 'accepted') issues.push(`node_envelope_not_accepted:${node.node_id}`);
  return { envelopeHash: sha256Object(envelope), issues };
}

function evaluateParallelMissionEvidenceIntegration(input) {
  ...
  const nodeResults = input.plan.nodes.map((node) => {
    ...
    if (!envelope) {
      ...
      const validation = validateSubagentEnvelope(envelope, node);
...
function buildParallelMissionPlan(input) {
  const nodes = input.nodes.map((node) => ({ ...node, protected_write_paths: DEFAULT_PROTECTED_WRITE_PATHS }));
  const conflicts = [];
  for (let leftIndex = 0; leftIndex < nodes.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < nodes.length; rightIndex += 1) {
      ...
      const protectedShared = intersects([...left.write_scope, ...right.write_scope], DEFAULT_PROTECTED_WRITE_PATHS);
      const hasDependency = left.depends_on.includes(right.node_id) || right.depends_on.includes(left.node_id);
  ...
  const blocked = new Set(conflicts.filter((conflict) => conflict.resolution === 'block').flatMap((conflict) => conflict.contenders));
...
function buildPrTopology(input) {
  ...
  return {
    ...
    required_nodes,
    all_affected_stories_passed: required_nodes.every((node) => ['merged', 'closed_not_needed'].includes(node.state)),
  };
}

function validatePrTopologyForReleaseGate(topology) {
  const notClosed = topology.required_nodes.filter((node) => !['merged', 'closed_not_needed'].includes(node.state));
  if (topology.all_affected_stories_passed && notClosed.length > 0) {
    return {
      passed: false,
      reason: `PR topology claims passed while nodes are still open: ${notClosed.map((node) => node.node_id).join(', ')}`,
    };
  }
