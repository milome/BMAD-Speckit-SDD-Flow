...
function reportPathFor(context) {
  ...
  return path.join(context.cwd, '_bmad-output', 'runtime', 'main-agent', 'soak-runner-report.json');
}
...
function soakRunnerAction(context) {
  const report = {
    reportType: 'main_agent_soak_runner_package_runtime',
    generatedAt: new Date().toISOString(),
    action: 'soak-runner',
    cwd: context.cwd,
    mode: 'package_runtime_module',
    supportedConsumerInvocation: 'bmad-speckit main-agent soak-runner',
    longRunEvidencePolicy: 'optional_observability_evidence',
    ...
    checks: [
      {
        ...
        passed: true,
        summary: 'soak runner command resolved through package runtime',
      },
    ],
  };
  return {
