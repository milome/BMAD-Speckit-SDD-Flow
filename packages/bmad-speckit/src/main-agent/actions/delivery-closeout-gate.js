...
function reportPathFor(context) {
  ...
  return path.join(context.cwd, '_bmad-output', 'runtime', 'main-agent', 'delivery-closeout-gate-report.json');
}
...
function deliveryCloseoutGateAction(context) {
  const report = {
    reportType: 'main_agent_delivery_closeout_gate_package_runtime',
    generatedAt: new Date().toISOString(),
    action: 'delivery-closeout-gate',
    cwd: context.cwd,
    mode: 'package_runtime_module',
    supportedConsumerInvocation: 'bmad-speckit main-agent delivery-closeout-gate',
    ...
    checks: [
      {
        ...
        passed: true,
        summary: 'delivery closeout gate command resolved through package runtime',
      },
    ],
  };
  return {
