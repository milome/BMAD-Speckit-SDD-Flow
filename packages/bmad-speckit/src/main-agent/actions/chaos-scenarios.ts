...
function reportPathFor(context) {
  ...
  return path.join(context.cwd, '_bmad-output', 'runtime', 'main-agent', 'chaos-scenarios-report.json');
}
...
function chaosScenariosAction(context) {
  const report = {
    reportType: 'main_agent_chaos_scenarios_package_runtime',
    generatedAt: new Date().toISOString(),
    action: 'chaos-scenarios',
    cwd: context.cwd,
    mode: 'package_runtime_module',
    supportedConsumerInvocation: 'bmad-speckit main-agent chaos-scenarios',
    ...
    checks: [
      {
        ...
        passed: true,
        summary: 'chaos scenarios command resolved through package runtime',
      },
    ],
  };
  return {
