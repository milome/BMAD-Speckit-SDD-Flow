...
function generateCodexReadme() {
  return [
    '# BMAD Speckit Codex Surface',
    '',
    'This directory is generated by `bmad-speckit init --ai codex`.',
    '',
    '- `.codex/commands` contains consumer-facing BMAD/Speckit command prompts.',
    '- `.codex/agents` contains Codex agent definitions.',
    '- `.codex/skills` contains project-scoped skills with Codex frontmatter.',
    '- `.codex/protocols` contains evidence and handoff protocols.',
    '- `.codex/i18n` contains host runtime localization metadata.',
    '',
    'Do not edit generated files unless the project explicitly takes ownership of that surface.',
    '',
  ].join('\n');
}
...
function syncCommandsRulesConfig(projectRoot, selectedAI, options = {}) {
  ...
  if (sourceDir === 'codex') {
    deployCodexInfrastructure(projectRoot, bmadRoot, ct);
  }
...
function deployCodexInfrastructure(projectRoot, bmadRoot, configTemplate) {
  if (configTemplate.i18nDir) {
    const src = path.join(bmadRoot, 'i18n');
    const dest = path.join(projectRoot, configTemplate.i18nDir);
    if (fs.existsSync(src) && fs.statSync(src).isDirectory()) {
      copyDirRecursive(src, dest);
    }
  }

  if (configTemplate.readmePath) {
    const readmePath = path.join(projectRoot, configTemplate.readmePath);
    const readmeDir = path.dirname(readmePath);
    if (!fs.existsSync(readmeDir)) fs.mkdirSync(readmeDir, { recursive: true });

    const readmeCandidates = [
      path.join(bmadRoot, 'codex', 'README.md'),
      path.join(path.dirname(bmadRoot), '.codex', 'README.md'),
      path.resolve(__dirname, '..', '..', '..', '..', '.codex', 'README.md'),
    ];
    const source = readmeCandidates.find((candidate) => fs.existsSync(candidate) && fs.statSync(candidate).isFile());
    if (source) {
      fs.copyFileSync(source, readmePath);
    } else {
      fs.writeFileSync(readmePath, generateCodexReadme(), 'utf8');
    }
  }
}
