...
function readHashReferencedRecord(input: {
  ...
  schemaName: string;
  hashField: string;
}): { path: string; record: SchemaRecord; hash: string; persistedRecord?: SchemaRecord } {
  ...
  const persistedRecord = readJsonFile(targetPath, 'goal_execution_authority_invalid');
...
function resolveFrozenGoalAuthority(input: { projectRoot: string; goalAuthorityPath: string }): {
  ...
  if (
    ...
    closureRef.record.schemaVersion !== (isTypedGoalExecutionIr(goalExecutionIr.schemaVersion) ? 'GoalExecutionClosure/v2' : 'GoalExecutionClosure/v1')
  ) {
    throw failure('goal_execution_authority_invalid', {
  ...
  if (activeAuthority.profile === 'requirements_backed' && isTypedGoalExecutionIr(goalExecutionIr.schemaVersion)) {
  ...
  if (activeAuthority.profile === 'standalone') {
    ...
    if (
      ...
      standaloneSemanticIr.schemaVersion !== (isTypedGoalExecutionIr(goalExecutionIr.schemaVersion) ? 'StandaloneGoalSemanticIR/v2' : 'StandaloneGoalSemanticIR/v1') ||
    ...
    const internalGateRef = activeAuthority.standaloneInternalSemanticGateRef;
    const internalGate = readHashReferencedRecord({
      outRoot,
      ref: internalGateRef,
      field: 'standaloneInternalSemanticGateRef',
...
export function validateGoalExecutionAdmission(input: {
  ...
  if (resolved.activeAuthority.profile === 'requirements_backed') {
    ...
    const requirementsReadiness = validateRequirementsBackedGoalAdmissionCurrent({
      ...
      expectedGoalExecutionIr: resolved.goalExecutionIr,
    });
    return Object.freeze({
      ...resolved,
      phase: input.phase,
...
function deriveComponentOwnedPaths(
  ...
  for (const artifact of !isTypedGoalExecutionIr(ir.schemaVersion) && Array.isArray(ir.artifacts) ? ir.artifacts.filter(isRecord) : []) {
  ...
  for (const domain of domains) {
    ...
    if (!isTypedGoalExecutionIr(ir.schemaVersion) && (domainComponentCounts.get(domainId) ?? 0) === 1) {
...
function readCompatibleActiveRun(input: {
  ...
  if (input.executionMode === 'partitioned_goal') {
    ...
    const currentPolicy = partitionPolicyIdentity(String(manifestRef.record.schemaVersion));
    if (
...
function resolveDirectExecutionAuthority(input: {
  ...
  return [
    Object.freeze({
      ...
      ...aggregateExecutionMetadata(input.goalExecutionIr),
...
function aggregateExecutionMetadata(authority: SchemaRecord): SchemaRecord {
  ...
  if (aggregates.length !== tasks.length || !['GoalExecutionIR/v2', 'GoalExecutionIR/v3', 'GoalChildExecutionContract/v2'].includes(authority.schemaVersion) ||
  ...
  for (const row of rows) {
    ...
    authorities.set(
      ...
      Object.freeze({
        ...
        partitionId,
        ...aggregateExecutionMetadata(childContract),
        ownedPaths: sortedUniqueText(
...
export function resolveCommittedActiveRun(input: {
  ...
  if (candidateRun.profile === 'requirements_backed') {
    ...
    requirementsReadiness = validateRequirementsBackedGoalAdmissionCurrent({
      ...
      expectedGoalExecutionIr: goalExecutionIrRef.record,
