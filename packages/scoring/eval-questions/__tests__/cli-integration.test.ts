...
describe(
  ...
  () => {
    ...
    beforeEach(() => {
      ...
      fs.mkdirSync(path.join(tmpRoot, 'packages', 'scoring', 'eval-questions', 'v1'), {
        recursive: true,
      });
      fs.mkdirSync(path.join(tmpRoot, 'packages', 'scoring', 'eval-questions', 'v2'), {
        recursive: true,
      });
      fs.writeFileSync(
...
`,
        ...
        const record = JSON.parse(fs.readFileSync(path.join(dataDir, runFile!), 'utf-8'));
