...
describe('Epic 聚合集成 (US-4.2)', () => {
  ...
  it('(5) CLI 无完整 Story 时输出含「Epic N 下无完整 Story」', { timeout: 45000 }, () => {
    ...
    execSync(
      `npx ts-node --project tsconfig.node.json packages/bmad-speckit/src/main-agent/source-authority/scripts/dashboard-generate.ts --dataPath "${FIXTURE_NO_COMPLETE}" --epic 9 --strategy epic_story_window --windowHours 999999 --output "${outPath}"`,
  ...
  it('(6) CLI epic 聚合输出含 Epic 9 聚合视图', { timeout: 45000 }, () => {
    ...
    execSync(
      `npx ts-node --project tsconfig.node.json packages/bmad-speckit/src/main-agent/source-authority/scripts/dashboard-generate.ts --dataPath "${dataPath}" --epic 9 --strategy epic_story_window --windowHours 999999 --output "${outPath}"`,
      { cwd: process.cwd(), encoding: 'utf-8' }
    );
    const content = fs.readFileSync(outPath, 'utf-8');
