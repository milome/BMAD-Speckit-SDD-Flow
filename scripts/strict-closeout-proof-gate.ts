...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--help' || arg === '-h') out.help = true;
      ...
      (out as Record<string, string | boolean | undefined>)[arg.slice(2).replace(/-([a-z])/gu, (_, letter: string) => letter.toUpperCase())] = value;
      index += 1;
    } else {
...
function eventLogPath(recordPath: string, record: JsonObject): string {
  ...
  return fromStore ? resolveArtifact(recordPath, fromStore) : path.join(path.dirname(recordPath), 'events', 'control-events.jsonl');
}
...
function currentAttempt(record: JsonObject, explicit?: string): string {
  ...
  const strict = commands.find((command) => text(command.commandId) === 'CMD-STRICT-CLOSEOUT-PROOF-GATE');
  return text(nested(strict?.lastRunRef).closeoutAttemptId) || text(strict?.closeoutAttemptId) || text(nested(record.closeout).currentAttemptId);
}
...
function latestArtifact(record: JsonObject, artifactType: string): JsonObject | null {
  return (
    objects(record.artifactIndex)
      .filter((item) => text(item.artifactType) === artifactType && text(item.sourceOfTruthRole) === 'evidence' && text(item.status) === 'active')
      .at(-1) ?? null
  );
}
...
function eventForArtifact(events: JsonObject[], artifact: JsonObject): JsonObject | null {
  ...
  return (
    events.find((event) => {
      ...
      return refs.some((ref) => normalizePath(text(ref.path)) === artifactPath && text(ref.hash ?? ref.contentHash) === artifactHash);
    }) ?? null
  );
}
...
function replayProof(record: JsonObject, events: JsonObject[]): JsonObject {
  ...
  if (replayStartIndex > 0 && !rebaseline?.ok) reasons.push(`event_log_rebaseline_invalid:${replayStartIndex}`);
  for (let index = replayStartIndex + 1; index < events.length; index += 1) {
    if (text(events[index].previousEventHash) !== text(events[index - 1].eventHash)) reasons.push(`event_chain_broken:${index}`);
  }
  const last = events.at(-1);
  if (last && text(record.eventChainHead) && text(record.eventChainHead) !== text(last.eventHash)) reasons.push('record_event_chain_head_mismatch');
  if (last && text(record.lastAppliedEventHash) && text(record.lastAppliedEventHash) !== text(last.eventHash)) reasons.push('record_last_applied_event_hash_mismatch');
  if (Number.isInteger(record.eventCount) && record.eventCount !== events.length) reasons.push('record_event_count_mismatch');
  return {
    ok: reasons.length === 0,
...
function writerRegistryProof(events: JsonObject[], attemptId: string): JsonObject {
  const allowed = new Set(['implementation-evidence-ingest', 'delivery-closeout-gate-writer', 'implementation-readiness-gate-writer', 'architecture-confirmation-ingest']);
  const attemptEvents = events.filter((event) => text(nested(nested(event.payload).packet).closeoutAttemptId) === attemptId || text(nested(event.payload).attemptId) === attemptId);
  const unauthorized = attemptEvents.filter((event) => !allowed.has(text(event.writerId))).map((event) => text(event.writerId) || '<missing>');
  return { ok: attemptEvents.length > 0 && unauthorized.length === 0, checkedEvents: attemptEvents.length, unauthorized };
}
...
function schemaEvolutionProof(record: JsonObject): JsonObject {
  ...
  const scriptsExist = ['scripts/requirement-record-event-reducer.ts', 'scripts/controlled-ingest-atomic-committer.ts', 'scripts/requirement-record-schema-evolution.ts'].every((file) =>
    fs.existsSync(file)
  );
  return {
    ok: scriptsExist && text(store.reducer) === 'canonical-requirement-record-reducer/v1' && text(store.atomicCommitter) === 'requirement-record-control-store/v1',
...
function commandProof(runs: JsonObject[], commandId: string): JsonObject | null {
  ...
  return run ? { commandId, runId: text(run.runId), executionIterationRef: text(run.executionIterationId), exitCode: run.exitCode } : null;
}

function proveSubsystems(input: { record: JsonObject; recordPath: string; events: JsonObject[]; attemptRuns: JsonObject[] }): JsonObject[] {
  ...
  if (!extensionRef) return REQUIRED_SUBSYSTEM_IDS.map((subsystemId) => ({ subsystemId, ok: false, issues: ['extension_artifact_missing'] }));
  ...
  return REQUIRED_SUBSYSTEM_IDS.map((subsystemId) => {
    ...
    const issues = [
      ...
      ...(!artifactRefComplete(extensionRef) || !fs.existsSync(extensionPath) || sha256File(extensionPath) !== text(extensionRef.hash ?? extensionRef.contentHash) ? ['artifact_ref_invalid'] : []),
      ...
      ...(item && strings(nested(item.failureHandling).recoveryActions).length === 0 ? ['recovery_action_evidence_missing'] : []),
    ];
    return { subsystemId, ok: issues.length === 0, issues, currentAttemptCommandRunRef: commandRunRef, artifactRef: extensionRef, controlledEventRef: event ? { eventId: text(event.eventId), exists: true } : null };
  });
}

function proveFailureCases(input: { record: JsonObject; recordPath: string; events: JsonObject[]; attemptRuns: JsonObject[] }): JsonObject[] {
  ...
  if (!ref) return [{ failureCaseId: '<missing>', ok: false, issues: ['failure_case_coverage_artifact_missing'] }];
  ...
  return cases.map((item) => {
    ...
    const issues = [
      ...(!commandRunRef ? ['current_attempt_command_run_missing'] : []),
      ...(!artifactRefComplete(ref) || !fs.existsSync(file) || sha256File(file) !== text(ref.hash ?? ref.contentHash) ? ['artifact_ref_invalid'] : []),
      ...(!artifactIndexed(input.record, ref) ? ['artifact_index_ref_missing'] : []),
      ...(!event ? ['controlled_event_ref_missing'] : []),
      ...(strings(item.sourceRefs).length === 0 && objects(item.sourceRefs).length === 0 ? ['source_refs_missing'] : []),
      ...(strings(item.expectedRecoveryActions).length === 0 && objects(item.recoveryActionEvidence).length === 0 ? ['recovery_action_evidence_missing'] : []),
    ];
    return { failureCaseId, ok: issues.length === 0, issues, currentAttemptCommandRunRef: commandRunRef, artifactRef: ref, controlledEventRef: event ? { eventId: text(event.eventId), exists: true } : null };
  });
}
...
function sftProjectionProof(record: JsonObject, recordPath: string): JsonObject {
  ...
  refs.forEach((ref, index) => {
    ...
    if (!file || !fs.existsSync(file) || sha256File(file) !== text(ref.hash ?? ref.contentHash)) reasons.push(`dataset_projection_ref_invalid:${index}`);
  });
  return { ok: reasons.length === 0, manifestPath: normalizePath(manifestPath), reasons };
}

export function evaluateStrictCloseoutProof(input: { record: JsonObject; recordPath: string; attemptId: string; evaluatedAt: string; evaluatedBy: string }): JsonObject {
  ...
  const subsystemEvidence = proveSubsystems({ record: input.record, recordPath: input.recordPath, events, attemptRuns });
  const failureCaseEvidence = proveFailureCases({ record: input.record, recordPath: input.recordPath, events, attemptRuns });
  ...
  const closeoutAttemptInvalidationPolicy = { ok: events.some((event) => text(event.eventType) === 'closeout_recorded' || text(event.eventType) === 'record_closed' || text(event.eventType) === 'implementation_evidence_ingested') };
  ...
  const blockingReasons = [
    ...
    ...(!closeoutAttemptInvalidationPolicy.ok ? ['closeout_attempt_invalidation_policy_missing'] : []),
    ...subsystemEvidence.flatMap((item) => (item.ok ? [] : [`subsystem_join_failed:${text(item.subsystemId)}`])),
    ...failureCaseEvidence.flatMap((item) => (item.ok ? [] : [`failure_case_join_failed:${text(item.failureCaseId)}`])),
...
export function mainStrictCloseoutProofGate(argv: string[]): number {
  ...
  if (args.help) {
    console.log('Usage: strict-closeout-proof-gate --requirement-record <json> [--attempt-id <id>] [--report-path <json>] [--json]');
    return 0;
  }
  ...
  const reportPath = path.resolve(args.reportPath ?? path.join(path.dirname(recordPath), 'strict-closeout-proof-report.json'));
  ...
  const output = { ok: true, reportPath: normalizePath(reportPath), decision: report.decision, blockingReasons: report.blockingReasons };
