...
const NON_CONTROL_SOURCE_TYPES = new Set(['artifact_ref', 'artifact', 'read_model', 'projection', 'dashboard', 'score', 'sft']);
const READ_MODEL_ARTIFACT_TYPES = new Set(['dashboard', 'dashboard_snapshot', 'score', 'score_report', 'sft_dataset', 'sft_export']);
const PROJECTION_ARTIFACT_TYPES = new Set(['bmad_workflow_projection', 'runtime_resume_projection', 'confirmation_view']);
...
function containsForbiddenControlField(value: unknown): boolean {
  ...
  return ['pass', 'complete', 'done', 'closed'].some((field) => Object.prototype.hasOwnProperty.call(obj, field));
}
...
function sourceRefIssues(refs: JsonObject[], owner: string): string[] {
  return refs.flatMap((ref) => {
    ...
    if (NON_CONTROL_SOURCE_TYPES.has(sourceType)) return [`${owner}:non_control_source_ref:${sourceType}`];
    if (!CONTROL_SOURCE_TYPES.has(sourceType)) return [`${owner}:unknown_control_source_ref:${sourceType}`];
...
function artifactIssues(record: JsonObject): string[] {
  ...
  for (const artifact of [...objects(record.artifactIndex), ...objects(record.extensionRefs)]) {
    ...
    if (role === 'control') issues.push(`artifact_role_control_forbidden:${artifactPath || artifactType || '<missing>'}`);
    if (READ_MODEL_ARTIFACT_TYPES.has(artifactType) && role !== 'read_model' && role !== 'evidence') {
      issues.push(`read_model_artifact_bad_role:${artifactType}`);
    }
    if (PROJECTION_ARTIFACT_TYPES.has(artifactType) && role !== 'projection' && role !== 'evidence') {
      issues.push(`projection_artifact_bad_role:${artifactType}`);
    }
    if ((artifactPath.includes('/dashboard/') || artifactPath.includes('/datasets/') || artifactPath.includes('/scores/')) && role === 'control') {
      issues.push(`derived_output_as_control:${artifactPath}`);
    }
    if (containsForbiddenControlField(artifact)) issues.push(`artifact_contains_control_field:${artifactPath || artifactType}`);
  }
  return issues;
}
...
function gateAndContractIssues(record: JsonObject): string[] {
  ...
  for (const [groupName, rows] of [
    ...
    for (const row of rows) {
      if (!text(row.decision)) issues.push(`${groupName}_decision_missing:${text(row.checkId) || text(row.gate) || text(row.contract)}`);
      if (Object.prototype.hasOwnProperty.call(row, 'result')) issues.push(`${groupName}_result_forbidden`);
...
function failureAndRerunIssues(record: JsonObject): string[] {
  ...
  for (const failure of objects(record.failureRecords)) {
    if (Object.prototype.hasOwnProperty.call(failure, 'result')) issues.push('failure_result_forbidden');
    if (Object.prototype.hasOwnProperty.call(failure, 'decision')) issues.push('failure_decision_forbidden');
  ...
  for (const rca of objects(record.rcaRecords)) {
    ...
    if (Object.prototype.hasOwnProperty.call(rca, 'decision')) issues.push('rca_decision_forbidden');
  ...
  for (const loop of objects(record.rerunLoops)) {
    ...
    if (Object.prototype.hasOwnProperty.call(loop, 'decision')) issues.push('rerun_decision_forbidden');
...
function closeoutBoundaryIssues(record: JsonObject): string[] {
  ...
  const orphanArtifacts = artifacts.filter((artifact) => text(artifact.status) === 'orphan' || strings(artifact.relatedRequirementIds).length === 0);
  const closeout =
  ...
  const currentAttempt = objects(closeout.attempts).find((attempt) => text(attempt.closeoutAttemptId) === currentAttemptId);
  ...
  if (!reasons.some((reason) => reason.includes('orphan'))) issues.push('orphan_artifact_missing_closeout_blocking_reason');
  return issues;
}

function rootFieldIssues(record: JsonObject): string[] {
  return ['dashboard', 'score', 'report', 'result'].filter((field) => Object.prototype.hasOwnProperty.call(record, field)).map(
    (field) => `root_control_field_forbidden:${field}`
  );
}

function evaluate(record: JsonObject): { decision: IsolationDecision; blockingReasons: string[]; checks: JsonObject[] } {
  const groups = [
  ...
  const checks = groups.map((group) => ({ id: group.id, passed: group.issues.length === 0, issues: group.issues }));
...
export function mainControlPlaneIsolationCheck(argv: string[]): number {
  ...
  if (args.help) {
    console.log('Usage: main-agent-control-plane-isolation-check --requirement-record <json> [--json]');
    return 0;
  }
  ...
  const reportPath = path.resolve(args.reportPath ?? path.join(path.dirname(recordPath), 'control-plane-isolation-report.json'));
