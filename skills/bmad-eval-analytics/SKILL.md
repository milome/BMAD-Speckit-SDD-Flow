...
description: |
  ...
  Coach：当用户说「帮我看看短板」「最近一轮的 Coach 报告」「诊断一下」「看看评分短板」等时，Agent 应执行 `npx ts-node scripts/coach-diagnose.ts` 获取 Coach 诊断输出。与 /bmad-coach Command 共用该脚本。
  SFT：当用户说「提取微调数据集」「生成 SFT 训练数据」「生成 SFT 数据」等时，Agent 应执行 `npx ts-node scripts/sft-extract.ts` 提取 SFT 训练数据并展示摘要。与 /bmad-sft-extract Command 共用该脚本。
...
2. **Coach 分支**：若匹配 Coach 短语 → 运行 `npx ts-node scripts/coach-diagnose.ts` → 将输出（Markdown 或 JSON）展示给用户。
3. **SFT 分支**：若匹配 SFT 短语 → 运行 `npx ts-node scripts/sft-extract.ts` → 将脚本输出（摘要）展示给用户。
...
**SFT**：
- 执行 `npx ts-node scripts/sft-extract.ts` 可成功运行；输出含「共提取 N 条，覆盖 M 个 Story；跳过 K 条（原因：…）」或等价摘要。
