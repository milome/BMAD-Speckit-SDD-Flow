...
- 与 config/code-reviewer-config.yaml modes.code 对应：code-reviewer 在 code 模式下使用本文件作为 prompt_template
- 可解析块部分明确为 §5.1 四维格式，与 modes.code.dimensions 一致（功能性、代码质量、测试覆盖、安全性）
-->

# Implement 阶段代码审计提示词（code 模式）

本文件为 `config/code-reviewer-config.yaml` 中 modes.code 的 prompt_template，等效于 `audit-prompts.md` §5。用于 speckit-workflow §5.2 执行阶段审计及 bmad-story-assistant 阶段四实施后审计。
